...
public class MSimLockPatternKeyguardView extends LockPatternKeyguardView {
    ...
    protected boolean stuckOnLockScreenBecauseSimMissing() {
        ...
        for (int i = 0; i < MSimTelephonyManager.getDefault().getPhoneCount(); i++) {
            ...
            result = result && (getSimState(i) == IccCardConstants.State.ABSENT ||
            getSimState(i) == IccCardConstants.State.PERM_DISABLED);
            if (!result) break;
        }
        return result;
    }
            ...
            */
            public void updatePinUnlockCancel(int subscription) {
                ...
                if ((isSimPukLocked(otherSub) && mIsPukUnlockCancelled[otherSub])
                        || (getSimState(otherSub) == IccCardConstants.State.ABSENT)) {
            ...
            public void updatePukUnlockCancel(int subscription) {
                ...
                int otherSub = (subscription == MSimConstants.SUB1) ?
                    MSimConstants.SUB2 : MSimConstants.SUB1;
                if (getSimState(otherSub) == IccCardConstants.State.ABSENT) {
    ...
    private IccCardConstants.State getSimState(int subscription) {
        return mUpdateMonitor.getSimState(subscription);
    }

    private boolean isSimPinLocked(int subscription) {
        return (getSimState(subscription) == IccCardConstants.State.PIN_REQUIRED);
    }

    private boolean isSimPukLocked(int subscription) {
        return (getSimState(subscription) == IccCardConstants.State.PUK_REQUIRED);
    }
