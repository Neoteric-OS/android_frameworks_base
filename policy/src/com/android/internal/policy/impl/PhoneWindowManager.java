...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void layoutWindowLw(WindowState win, WindowState attached, boolean statusBarBeImeTarget) {
        ...
        if ((win == mStatusBar && (attrs.privateFlags & PRIVATE_FLAG_KEYGUARD) == 0 && !statusBarBeImeTarget) ||
                win == mNavigationBar) {
            return;
        }
        ...
        } else if (win == mStatusBar && (attrs.privateFlags & PRIVATE_FLAG_KEYGUARD) != 0 || statusBarBeImeTarget) {
            pf.left = df.left = of.left = mUnrestrictedScreenLeft;
            pf.top = df.top = of.top = mUnrestrictedScreenTop;
            pf.right = df.right = of.right = mUnrestrictedScreenWidth + mUnrestrictedScreenLeft;
