...
void CompileApkLayouts(const std::string& filename) {
  cout << "Listing assets in " << filename << "\n";
  ...
  assets->ForEachFile(
      "res/",
      [& assets = std::as_const(assets), &resources](const android::StringPiece& s,
                                                     android::FileType) {
        cout << "  " << s << "\n";
        if (s.contains("layout")) {
          ...
          assets->ForEachFile(
              path,
              [& path = std::as_const(path), &resources](const android::StringPiece& layout_file,
                                                         android::FileType) {
                cout << "    " << layout_file << "\n";
                auto layout_path =
                    StringPrintf("%s%s", path.c_str(), layout_file.to_string().c_str());
                android::ApkAssetsCookie cookie = android::kInvalidCookie;
                auto asset =
                    resources.OpenNonAsset(layout_path, android::Asset::ACCESS_RANDOM, &cookie);
                CHECK(asset);
                ...
                if (CanCompileLayout(&parser)) {
                  cout << "      compilation supported\n";
                } else {
                  cout << "      compilation NOT supported\n";
                }
              });
        }
      });
