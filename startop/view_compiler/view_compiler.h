...
// features, such as imports and merges, we need to pre-validate the layout before we start
// compiling.
...
class LayoutCompilerVisitor {
 public:
  ...
  void VisitStartTag(const std::string& name) { parent_stack_.push_back(ViewEntry{name, {}}); }
  void VisitEndTag() {
...
class DexViewBuilder {
 ...
 private:
  ...
  dex::Value xml_;
  dex::Value attrs_;
  dex::Value classname_tmp_;
  dex::Value layout_params_tmp_;

  dex::MethodDeclData xml_next_;
  dex::MethodDeclData try_create_view_group_;
  dex::MethodDeclData try_create_view_;
  dex::MethodDeclData generate_layout_params_;
  dex::MethodDeclData add_view_;
