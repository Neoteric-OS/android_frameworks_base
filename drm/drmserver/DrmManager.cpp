...
status_t DrmManager::loadPlugIns(
	...
	if (true == mSupportInfoToPlugInIdMap.isEmpty()) {
...
status_t DrmManager::installDrmEngine(
					int uniqueId,
					const String8& absolutePath) {
...
bool DrmManager::canHandle(
					int uniqueId,
					const String8& path,
					const String8& mimeType) {

	const String8 plugInId = getSupportedPlugInId(mimeType);
	bool result = (EMPTY_STRING == plugInId) ? false : true;	
...
DrmInfoStatus* DrmManager::processDrmInfo(
					int uniqueId,
...
DrmInfo* DrmManager::acquireDrmInfo(
					int uniqueId,
...
String8 DrmManager::getOriginalMimeType(
					int uniqueId,
					const String8& path) {
...
int DrmManager::checkRightsStatus(
					int uniqueId,
					const String8& path,
					int action) {
...
int DrmManager::openConvertSession(
					int uniqueId,
					const String8& mimeType) {
...
DrmConvertedStatus* DrmManager::closeConvertSession(
					int uniqueId,
					int convertId) {
...
status_t DrmManager::getAllSupportInfo(
	...
	if (0 < size) {
		...
		for (int i = 0; i < size; ++i) {
			...
			DrmSupportInfo* drmSupportInfo = mPlugInManager.getPlugIn(plugInPath).getSupportInfo(uniqueId);
...
DecryptHandle* DrmManager::openDecryptSession(
	...
	for (unsigned int index = 0; index < plugInIdList.size(); index++) {
		...
		IDrmEngine& rDrmEngine = mPlugInManager.getPlugIn(plugInId);
		handle = rDrmEngine.openDecryptSession(uniqueId, fd, offset, length);
		...
		if (NULL != handle) {			
			...
			handle->decryptId = mDecryptSessionId;
			break;
		}
	}

	if (NULL == handle) {
		LOGE("DrmManager::openDecryptSession: no plug-in returns valid decryptHandle");	
	}

	return handle;
}

void DrmManager::closeDecryptSession(
					int uniqueId,
					DecryptHandle* decryptHandle) {
...
void DrmManager::onInfo(const InfoEvent& event) {
