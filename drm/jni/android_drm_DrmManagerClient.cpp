...
String8 Utility::getStringValue(
					JNIEnv* env,
					jobject object,
					const char* fieldName) {
...
class JNIOnInfoListener : public DrmManagerClient::OnInfoListener {
	...
	public:
    	...
    	void onInfo(const InfoEvent& event);
...
void JNIOnInfoListener::onInfo(const InfoEvent& event) {
	...
	env->CallStaticVoidMethod(mClass, env->GetStaticMethodID(mClass, "notify", "(Ljava/lang/Object;IILjava/lang/String;)V"),
...
static sp<DrmManagerClientImpl> setDrmManagerClientImpl(
					JNIEnv* env,
					jobject thiz,
					const sp<DrmManagerClientImpl>& client) {
...
static sp<DrmManagerClientImpl> getDrmManagerClientImpl(
					JNIEnv* env,
					jobject thiz) {
...
static void android_drm_DrmManagerClient_unloadPlugIns(
					JNIEnv* env,
					jobject thiz,
					jint uniqueId) {
...
static jobject android_drm_DrmManagerClient_getConstraintsFromContent(
	...
	DrmConstraints* pConstraints = getDrmManagerClientImpl(env, thiz)->getConstraints(uniqueId, &pathString, usage);
		...
		while (keyIt.hasNext()) {
			...
			if (DrmConstraints::EXTENDED_METADATA == key) {
				...
				if (NULL != value) {
					...
					env->CallVoidMethod(constraints, env->GetMethodID(localRef, "put", "(Ljava/lang/String;[B)V"), 
										env->NewStringUTF(key.string()), dataArray);
				}
			} else {
				String8 value = pConstraints->get(key);
				env->CallVoidMethod(constraints, env->GetMethodID(localRef, "put", "(Ljava/lang/String;Ljava/lang/String;)V"),
...
static jobjectArray android_drm_DrmManagerClient_getAllSupportInfo(
	...
	for (int i = 0; i < length; i++) {
		...
		jmethodID addMimeTypeId = env->GetMethodID(clazz, "addMimeType", "(Ljava/lang/String;)V");
		jmethodID addFileSuffixId = env->GetMethodID(clazz, "addFileSuffix", "(Ljava/lang/String;)V");

		env->CallVoidMethod(drmSupportInfo, env->GetMethodID(clazz, "setDescription", "(Ljava/lang/String;)V"),
		...
		while (it.hasNext()) {
			...
			env->CallVoidMethod(drmSupportInfo, addFileSuffixId, env->NewStringUTF(value.string()));
		}
...
static void android_drm_DrmManagerClient_installDrmEngine(
	...
	//getDrmManagerClient(env, thiz)->installDrmEngine(uniqueId, Utility::getStringValue(env, engineFilePath));
...
static void android_drm_DrmManagerClient_saveRights(
	...
	if (NULL != mData) {
		DrmRights rights(DrmBuffer(mData, dataLength), Utility::getStringValue(env, drmRights, "mMimeType"),
												Utility::getStringValue(env, drmRights, "mAccountId"),
												Utility::getStringValue(env, drmRights, "mSubscriptionId"));
		getDrmManagerClientImpl(env, thiz)->saveRights(uniqueId, rights, Utility::getStringValue(env, rightsPath), 
...
static jboolean android_drm_DrmManagerClient_canHandle(
	...
	jboolean result = getDrmManagerClientImpl(env, thiz)->canHandle(uniqueId, Utility::getStringValue(env, path),
...
static jobject android_drm_DrmManagerClient_processDrmInfo(
	...
	jobject keyIterator = env->CallObjectMethod(drmInfoObject, env->GetMethodID(clazz, "keyIterator", "()Ljava/util/Iterator;"));
	...
	while (env->CallBooleanMethod(keyIterator, hasNextId)) {
		jstring key = (jstring) env->CallObjectMethod(keyIterator, 
										env->GetMethodID(env->FindClass("java/util/Iterator"), "next", "()Ljava/lang/Object;"));
	...
	DrmInfoStatus* pDrmInfoStatus = getDrmManagerClientImpl(env, thiz)->processDrmInfo(uniqueId, &drmInfo);
	...
	if (NULL != localRef && NULL != pDrmInfoStatus) {
		...
		if (NULL != pDrmInfoStatus->drmBuffer) {
			...
			env->SetByteArrayRegion(dataArray, 0, length, (jbyte*) pDrmInfoStatus->drmBuffer->data);
		...
		jmethodID constructorId = env->GetMethodID(clazz, "<init>", "([BLjava/lang/String;Ljava/lang/String;)V");
		...
		constructorId = env->GetMethodID(localRef, "<init>", "(ILandroid/drm/ProcessedData;Ljava/lang/String;)V");
...
static jobject android_drm_DrmManagerClient_acquireDrmInfo(
	...
	jobject keyIterator = env->CallObjectMethod(drmInfoRequest, env->GetMethodID(clazz, "keyIterator", "()Ljava/util/Iterator;"));
	...
	if (NULL != pDrmInfo) {
		...
		if (NULL != localRef) {
			...
			drmInfoObject = env->NewObject(localRef, env->GetMethodID(localRef, "<init>", "(I[BLjava/lang/String;)V"),
									mInfoType, dataArray, env->NewStringUTF(pDrmInfo->getMimeType().string()));

			DrmInfo::KeyIterator it = pDrmInfo->keyIterator();
			jmethodID putMethodId = env->GetMethodID(localRef, "put", "(Ljava/lang/String;Ljava/lang/Object;)V");

			while(it.hasNext()) {
				String8 key = it.next();
				String8 value = pDrmInfo->get(key);

				env->CallVoidMethod(drmInfoObject, putMethodId, env->NewStringUTF(key.string()), env->NewStringUTF(value.string()));
			}
		}
...
static jint android_drm_DrmManagerClient_getDrmObjectType(
	...
	int drmObjectType = getDrmManagerClientImpl(env, thiz)->getDrmObjectType(uniqueId, Utility::getStringValue(env, path), 
																Utility::getStringValue(env, mimeType));
...
static jstring android_drm_DrmManagerClient_getOriginalMimeType(
	...
	String8 mimeType = getDrmManagerClientImpl(env, thiz)->getOriginalMimeType(uniqueId, Utility::getStringValue(env, path));
...
static jint android_drm_DrmManagerClient_checkRightsStatus(
	...
	LOGV("getOriginalMimeType Enter");
	int rightsStatus = getDrmManagerClientImpl(env, thiz)->checkRightsStatus(uniqueId, Utility::getStringValue(env, path), action);
	LOGV("getOriginalMimeType Exit");
	return rightsStatus;
}
...
static jint android_drm_DrmManagerClient_openConvertSession(
	...
	int convertId = getDrmManagerClientImpl(env, thiz)->openConvertSession(uniqueId,
																Utility::getStringValue(env, mimeType));
...
static jobject android_drm_DrmManagerClient_convertData(
	...
	DrmConvertedStatus* pDrmConvertedStatus = getDrmManagerClientImpl(env, thiz)->convertData(uniqueId, convertId, &buffer);
	...
	if (NULL != localRef && NULL != pDrmConvertedStatus) {
		...
		if (NULL != pDrmConvertedStatus->convertedData) {
			...
			dataArray = env->NewByteArray(length);
			env->SetByteArrayRegion(dataArray, 0, length, (jbyte*) pDrmConvertedStatus->convertedData->data);
		...
		drmConvertedStatus = env->NewObject(localRef, constructorId, statusCode, dataArray, pDrmConvertedStatus->offset);
	}
...
static jobject android_drm_DrmManagerClient_closeConvertSession(
	...
	DrmConvertedStatus* pDrmConvertedStatus = getDrmManagerClientImpl(env, thiz)->closeConvertSession(uniqueId, convertId);
