...
status_t BpDrmManagerService::loadPlugIns(
					int uniqueId,
...
status_t BpDrmManagerService::installDrmEngine(
					int uniqueId,
...
bool BpDrmManagerService::canHandle(
					int uniqueId, 
					const String8& path,
...
DrmInfoStatus* BpDrmManagerService::processDrmInfo(
					int uniqueId, 
...
DrmInfo* BpDrmManagerService::acquireDrmInfo(
					int uniqueId,
...
int BpDrmManagerService::checkRightsStatus(
					int uniqueId,
					const String8& path,
					int action) {
...
void BpDrmManagerService::removeRights(
					int uniqueId,
					const String8& path) {
...
int BpDrmManagerService::openConvertSession(
					int uniqueId,
					const String8& mimeType) {
...
DrmConvertedStatus* BpDrmManagerService::closeConvertSession(
					int uniqueId,
					int convertId) {
...
DecryptHandle* BpDrmManagerService::openDecryptSession(
	...
	LOGV("BpDrmManagerService::openDecryptSession: InterfaceDescriptor name is %s", interfaceDescriptor.string());
...
void BpDrmManagerService::closeDecryptSession(
					int uniqueId,
					DecryptHandle* decryptHandle) {
...
IMPLEMENT_META_INTERFACE(DrmManagerService, "sony.drm.IDrmManagerService");
...
status_t BnDrmManagerService::onTransact(
	...
	case SET_DRM_SERVICE_LISTENER:
	{
		...
		const sp<IDrmServiceListener> drmServiceListener = interface_cast<IDrmServiceListener> (data.readStrongBinder());
	...
	case GET_DRM_OBJECT_TYPE:
	{
		...
		const int drmObjectType = getDrmObjectType(data.readInt32(), data.readString8(), data.readString8());
	...
	case CHECK_RIGHTS_STATUS:
	{
		...
		const int result = checkRightsStatus(data.readInt32(), data.readString8(), data.readInt32());
	...
	case CLOSE_CONVERT_SESSION:
	{
		...
		DrmConvertedStatus*	drmConvertedStatus = closeConvertSession(data.readInt32(), data.readInt32());
	...
	case GET_ALL_SUPPORT_INFO:
	{
		...
		for (int i = 0; i < length; ++i) {
			...
			DrmSupportInfo::FileSuffixIterator fileSuffixIt = drmSupportInfo.getFileSuffixIterator();
	...
	case OPEN_DECRYPT_SESSION:
	{
		...
		DecryptHandle* handle = openDecryptSession(uniqueId, fd, data.readInt32(), data.readInt32());
	...
	case DECRYPT:
	{
		...
		DrmBuffer* encBuffer = new DrmBuffer((char *)data.readInplace(encBufferSize), encBufferSize);
