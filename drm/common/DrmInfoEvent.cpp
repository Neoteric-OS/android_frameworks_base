...
DrmInfoEvent::DrmInfoEvent(int uniqueId, int infoType, const String8 message, const DrmBuffer& drmBuffer)
    : mUniqueId(uniqueId),
      mInfoType(infoType),
      mMessage(message),
      mDrmBuffer(drmBuffer),
      mData(NULL) {
...
void DrmInfoEvent::setData(const DrmBuffer& drmBuffer) {
    delete [] mData;
    mData = new char[drmBuffer.length];
    mDrmBuffer.data = mData;
