...
status_t DrmConstraints::put(const String8* key,
					const char* value) {
	char* charValue = new char[strlen(value) + 1];
	if(NULL != charValue) {
		memset(charValue, 0, strlen(value) + 1);
...
DrmConstraints::KeyIterator& DrmConstraints::KeyIterator::operator=(const DrmConstraints::KeyIterator& keyIterator) {
