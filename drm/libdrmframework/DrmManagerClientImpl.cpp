...
DrmManagerClientImpl* DrmManagerClientImpl::create(int* pUniqueId) {     
	...
	if(0 == *pUniqueId) {
		...
		while(!foundUniqueId) {
			//TODO: The algoritham to gernerate uniqueId can be improvised
...
status_t DrmManagerClientImpl::loadPlugIns(
					int uniqueId,
...
void DrmManagerClientImpl::saveRights(
					int uniqueId,
...
void DrmManagerClientImpl::setPlaybackStatus(
	...
	if (NULL != decryptHandle) {
		getDrmManagerService()->setPlaybackStatus(uniqueId, decryptHandle, playbackStatus, position);
	}
}
...
void DrmManagerClientImpl::removeRights(
					int uniqueId,
					const String8& path) {
...
void DrmManagerClientImpl::closeDecryptSession(
					int uniqueId,
					DecryptHandle* decryptHandle) {
...
void DrmManagerClientImpl::initializeDecryptUnit(
					int uniqueId,
					DecryptHandle* decryptHandle,
					int decryptUnitId,
	...
	if ((NULL != decryptHandle) && (NULL != headerInfo)) {
		getDrmManagerService()->initializeDecryptUnit(uniqueId, decryptHandle, decryptUnitId, headerInfo);
	}
}
...
status_t DrmManagerClientImpl::decrypt(
	...
	status_t status = DRM_ERROR_UNKNOWN;
	if ((NULL != decryptHandle) && (NULL != encBuffer) && (NULL != decBuffer) && (NULL != *decBuffer)) {	
		status = getDrmManagerService()->decrypt(uniqueId, decryptHandle, decryptUnitId, encBuffer, decBuffer);
	}
	return status;
}
...
ssize_t DrmManagerClientImpl::pread(
					int uniqueId,
					DecryptHandle* decryptHandle,
					void* buffer,
					ssize_t numBytes,
...
status_t DrmManagerClientImpl::notify(const InfoEvent& event) {
