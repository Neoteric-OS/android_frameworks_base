...
TEST_F(OmaFLAgentTest, testEngineSupport) {
    ...
    String8 mimeStrList[] = {
        String8(FWDLOCK_MIMETYPE_FL),
        String8(FWDLOCK_MIMETYPE_DM)
    };
    ...
    String8 suffixStrList[] = {
        String8(FWDLOCK_DOTEXTENSION_DM),
        String8(FWDLOCK_DOTEXTENSION_FL)
    };
...
TEST_F(OmaFLAgentTest, testGetConstraints) {
    ...
    String8 pathFL = String8(TESTFILE1_OUTFL);
...
/**
 * TC - testUnusedAPIs: tests all unused APIs
 */
TEST_F(OmaFLAgentTest, testUnusedAPIs) {
    // Validate that error signal is received for setOnInfoListener
    ASSERT_TRUE(DRM_NO_ERROR == flengine->setOnInfoListener(0, NULL));
    DrmInfoStatus* drmInfoStatus = flengine->processDrmInfo(0, NULL);
    // Validate drmInfoStatus is received
    ASSERT_TRUE(NULL != drmInfoStatus);
    // Validate drmInfoStatus has status ok.
    ASSERT_TRUE(DrmInfoStatus::STATUS_OK == drmInfoStatus->statusCode);
    delete drmInfoStatus;
    DrmBuffer drmBuffer;
    drmBuffer.data = NULL;
    drmBuffer.length = NULL;
    // Validate that no crash is received for the following unused APIs.
    flengine->saveRights(0, DrmRights(drmBuffer, String8(""), String8("")),
            String8(""), String8(""));
    flengine->consumeRights(0, NULL, 0, false);
    flengine->removeAllRights(0);
    flengine->setPlaybackStatus(0, NULL, 0, 0);
    // Validate that acquireDrmInfo returnes NULL.
    ASSERT_TRUE(NULL == flengine->acquireDrmInfo(0, NULL));
}
