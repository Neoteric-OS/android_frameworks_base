...
public:
	...
	virtual status_t loadPlugIns(
					int uniqueId,
					const String8& plugInDirPath) = 0;

	virtual status_t setDrmServiceListener(
					int uniqueId,
					const sp<IDrmServiceListener>& infoListener) = 0;

	virtual status_t unloadPlugIns(int uniqueId) = 0;

	virtual status_t installDrmEngine(
					int uniqueId,
					const String8& drmEngineFile) = 0;
	...
	virtual DrmInfoStatus* processDrmInfo(
					int uniqueId, 
					const DrmInfo* drmInfo) = 0;

	virtual DrmInfo* acquireDrmInfo(
					int uniqueId,
					const DrmInfoRequest* drmInforequest) = 0;
	...
	virtual String8 getOriginalMimeType(
					int uniqueId, 
					const String8& path) = 0;
	...
	virtual bool validateAction(
					...
					const String8& path,
					int action,
					const ActionDescription& description) = 0;

	virtual void removeRights(
					int uniqueId,
					const String8& path) = 0;
	
	virtual void removeAllRights(int uniqueId) = 0;

	virtual int openConvertSession(
					int uniqueId,
					const String8& mimeType) = 0;
	...
	virtual DrmConvertedStatus* convertData(
					int uniqueId,
					int convertId,
					const DrmBuffer* inputData) = 0;

	virtual DrmConvertedStatus* closeConvertSession(
					int uniqueId,
					int convertId) = 0;
	...
	virtual status_t getAllSupportInfo(
					int uniqueId,
					int* length,
					DrmSupportInfo** drmSupportInfoArray) = 0;
	...
	virtual void closeDecryptSession(
					int uniqueId,
					DecryptHandle* decryptHandle) = 0;

	virtual void initializeDecryptUnit(
					int uniqueId,
					DecryptHandle* decryptHandle,
					int decryptUnitId,
					const DrmBuffer* headerInfo) = 0;
...
public:
	...
	virtual status_t setDrmServiceListener(
					int uniqueId,
					const sp<IDrmServiceListener>& infoListener);
...
#endif // End __IDRM_MANAGER_SERVICE_H__
