...
nClosureCreate(JNIEnv *_env, jobject _this, jlong con, jlong kernelID,
  ...
  jFieldIDs = _env->GetLongArrayElements(fieldIDArray, nullptr);
  jValues = _env->GetLongArrayElements(valueArray, nullptr);
  jSizes = _env->GetIntArrayElements(sizeArray, nullptr);
  jDepClosures = _env->GetLongArrayElements(depClosureArray, nullptr);
  jDepFieldIDs = _env->GetLongArrayElements(depFieldIDArray, nullptr);

  fieldIDs = (RsScriptFieldID*)jFieldIDs;
  values = (uintptr_t*)jValues;
  depClosures = (RsClosure*)jDepClosures;
  depFieldIDs = (RsScriptFieldID*)jDepFieldIDs;

  if (sizeof(void *) < sizeof(jlong)) {
      // On a 32-bit platform, compact the jlong arrays into arrays of pointer-size elements
...
nInvokeClosureCreate(JNIEnv *_env, jobject _this, jlong con, jlong invokeID,
  ...
  jParams      = _env->GetByteArrayElements(paramArray, nullptr);
  jParamLength = _env->GetArrayLength(paramArray);
  jFieldIDs    = _env->GetLongArrayElements(fieldIDArray, nullptr);
  jValues      = _env->GetLongArrayElements(valueArray, nullptr);
  jSizes       = _env->GetIntArrayElements(sizeArray, nullptr);
...
nScriptGroup2Create(JNIEnv *_env, jobject _this, jlong con, jstring name,
  ...
  jClosures = _env->GetLongArrayElements(closureArray, nullptr);

  closures = (RsClosure*)jClosures;
  if (sizeof(void*) < sizeof(jlong)) {
      // On a 32-bit platform, compact the jlong arrays into arrays of pointer-size elements
