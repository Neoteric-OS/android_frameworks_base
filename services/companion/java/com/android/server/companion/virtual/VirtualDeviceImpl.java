...
final class VirtualDeviceImpl extends IVirtualDevice.Stub implements IBinder.DeathRecipient {
  ...
  /**
   * Do not show a toast on the virtual display when a secure surface is shown after {@link
   * android.os.Build.VERSION_CODES#VANILLA_ICE_CREAM}. VDM clients should use {@link
   * VirtualDeviceManager.ActivityListener#onSecureWindowShown} instead to provide a custom
  ...
  /**
   * Check the {@link android.Manifest.permission.ADD_MIRROR_DISPLAY} permission instead of relying
   * on the app streaming role. VDM clients must declare the new permission after {@link
  ...
  private static final List<Integer> DEVICE_PROFILES_ALLOWING_MIRROR_DISPLAYS =
      List.of(VirtualDevice.DEVICE_PROFILE_APP_STREAMING);
   ...
   *
   * <p>Never call outside this class while holding this lock. A number of other system services
   * like WindowManager, DisplayManager, etc. call into this device to get device-specific
   * information, while holding their own global locks.
   *
   * <p>Making a call to another service while holding this lock creates lock order inversion and
  ...
  @Nullable private final AssociationInfo mAssociationInfo;
  @NonNull private final VirtualDeviceManagerService mService;
  @NonNull private final PendingTrampolineCallback mPendingTrampolineCallback;
  private final int mOwnerUid;
  @NonNull private final VirtualDeviceLog mVirtualDeviceLog;
  @NonNull private final String mOwnerPackageName;
  @NonNull private final AttributionSource mAttributionSource;
  private final int mDeviceId;
  @DeviceProfile private final int mDeviceProfile;
  @Nullable private final String mPersistentDeviceId;
  @NonNull private final InputController mInputController;
  @NonNull private final SensorController mSensorController;
  @Nullable private final CameraAccessController mCameraAccessController;
  ...
  @Nullable private VirtualAudioController mVirtualAudioController;
  ...
  @Nullable private IBinder mAudioFocusEnvToken = null;
  @NonNull private final IBinder mAppToken;
  @NonNull private final VirtualDeviceParams mParams;
  ...
  @NonNull private IVirtualDeviceActivityListener mActivityListener;
  @Nullable private GenericWindowPolicyController.ActivityListener mActivityListenerAdapter = null;
  @Nullable private IVirtualDeviceSoundEffectListener mSoundEffectListener;
  @NonNull private final DisplayManager mDisplayManager;
  @NonNull private final DisplayManagerGlobal mDisplayManagerGlobal;
  @NonNull private final DisplayManagerInternal mDisplayManagerInternal;
  @NonNull private final UiModeManagerInternal mUiModeManagerInternal;
  @NonNull private final ThermalManagerInternal mThermalManagerInternal;
  @NonNull private final PowerManager mPowerManager;
  @NonNull private final IWindowManager mWindowManager;
  ...
  @NonNull private final VirtualDevice mPublicVirtualDeviceObject;
  ...
  private class GwpcActivityListener implements GenericWindowPolicyController.ActivityListener {
    ...
    public void onTopActivityChanged(
        int displayId, @NonNull ComponentName topActivity, @UserIdInt int userId) {
      try {
    ...
    public void onActivityLaunchBlocked(
        int displayId, @NonNull ActivityInfo activityInfo, @Nullable IntentSender intentSender) {
      Intent intent = BlockedAppStreamingActivity.createIntent(activityInfo, getDisplayName());
      if (shouldShowBlockedActivityDialog(activityInfo.getComponentName(), intent.getComponent())) {
    ...
    @Override
    public void onSecureWindowShown(int displayId, @NonNull ComponentName componentName, int uid) {
      try {
        mActivityListener.onSecureWindowShown(
            displayId, componentName, UserHandle.getUserHandleForUid(uid));
      } catch (RemoteException e) {
        Slog.w(TAG, "Unable to call mActivityListener for display: " + displayId, e);
      }

      if (CompatChanges.isChangeEnabled(
          DO_NOT_SHOW_TOAST_WHEN_SECURE_SURFACE_SHOWN,
          mOwnerPackageName,
          UserHandle.getUserHandleForUid(mOwnerUid))) {
        return;
      }
      ...
      if (display != null) {
        if ((display.getFlags() & Display.FLAG_SECURE) == 0) {
          ...
          Counter.logIncrementWithUid(
              "virtual_devices.value_secure_window_blocked_count", mAttributionSource.getUid());
        }
      } else {
        Slog.e(
            TAG, "Calling onSecureWindowShown on a non existent/connected display: " + displayId);
      }
    }
     ...
     * the intent matches any filter notifying the DisplayPolicyController to abort the activity
     * launch to be replaced by the interception.
     */
    @Override
    ...
    @Override
    public void onRunningAppsChanged(
        int displayId, @NonNull ArraySet<Pair<Integer, String>> uidPackagePairs) {
      final ArraySet<Pair<Integer, String>> newAllRunningUidPackagePairs;
      synchronized (mVirtualDeviceLock) {
        if (Objects.equals(uidPackagePairs, mRunningUidPackagePairsPerDisplay.get(displayId))) {
          return;
        }
        ...
        for (int i = 0; i < mRunningUidPackagePairsPerDisplay.size(); i++) {
          newAllRunningUidPackagePairs.addAll(mRunningUidPackagePairsPerDisplay.valueAt(i));
        }
    ...
    @Override
    public void onActivityLaunchRequested(
        int displayId, @NonNull ComponentName componentName, @UserIdInt int userId) {
      try {
        mActivityListener.onActivityLaunchRequested(displayId, componentName, userId);
      } catch (RemoteException e) {
        Slog.w(
            TAG,
            "Unable to call mActivityListener for activity launch request: " + componentName,
            e);
      }
    }
  }
  ...
  VirtualDeviceImpl(
    ...
    this(
        ...
        isVirtualCameraEnabled()
            ? new VirtualCameraController(params.getDevicePolicy(POLICY_TYPE_CAMERA), deviceId)
            : null,
        android.content.res.Flags.rroConstraints() && Flags.viewconfigurationApis()
            ? new ViewConfigurationController(context)
            : null);
  }
  ...
  VirtualDeviceImpl(
    ...
    mPersistentDeviceId =
        associationInfo == null ? null : createPersistentDeviceId(associationInfo.getId());
    mService = service;
    ...
    if (mDevicePolicies.get(POLICY_TYPE_CLIPBOARD, DEVICE_POLICY_DEFAULT)
      ...
      if (mContext.checkCallingOrSelfPermission(ADD_TRUSTED_DISPLAY)
        ...
        throw new SecurityException(
            "Requires ADD_TRUSTED_DISPLAY permission to " + "set a custom clipboard policy.");
      }
    }
    ...
    if (mParams.isLocalDeviceOnly() && Binder.getCallingUid() != Process.SYSTEM_UID) {
      throw new SecurityException(
          "Only system_server can create a local-only " + "virtual device.");
    }
    ...
    mSensorController =
        new SensorController(
            ...
            mDeviceId,
            mAttributionSource,
            mParams.getVirtualSensorCallback(),
            mParams.getVirtualSensorConfigs());
    ...
    if (mParams.getInputMethodComponent() != null) {
      ...
      Slog.d(
          TAG,
          "Setting custom input method " + imeId + " as default for virtual device " + deviceId);
      InputMethodManagerInternal.get().setVirtualDeviceInputMethodForAllUsers(mDeviceId, imeId);
    }
    ...
    synchronized (mPowerLock) {
      if (lockdownActive != mLockdownActive) {
        ...
        if (mLockdownActive) {
          ...
          wakeUpInternal(
              PowerManager.WAKE_REASON_DISPLAY_GROUP_TURNED_ON,
  ...
  void applyViewConfigurationParams(@Nullable ViewConfigurationParams viewConfigurationParams) {
    if (mViewConfigurationController != null) {
      mViewConfigurationController.applyViewConfigurationParams(mDeviceId, viewConfigurationParams);
    }
  }

  void onUserStarting(int userId) {
    if (mViewConfigurationController != null) {
      mViewConfigurationController.applyViewConfigurationParamsForUser(
          userId, mDeviceId, mParams.getViewConfigurationParams());
    }
  }
  ...
  /**
   * Returns the flags that should be added to any virtual displays created on this virtual device.
   */
  ...
  /**
   * Setter for listeners that live in the client process, namely in {@link
   * android.companion.virtual.VirtualDeviceInternal}.
   *
   * <p>This is needed for virtual devices that are created by the system, as the VirtualDeviceImpl
  ...
  public void wakeUp() {
    ...
    try {
      wakeUpInternal(
          PowerManager.WAKE_REASON_POWER_BUTTON, "android.server.companion.virtual:DEVICE_ON");
    } finally {
      Binder.restoreCallingIdentity(ident);
    }
  }
  ...
  public void launchPendingIntent(
    ...
    if (pendingIntent.isActivity()) {
      try {
        ...
        resultReceiver.send(VirtualDeviceManager.LAUNCH_FAILURE_PENDING_INTENT_CANCELED, null);
      }
    } else {
      PendingTrampoline pendingTrampoline =
          new PendingTrampoline(pendingIntent, resultReceiver, displayId);
  ...
  public void close() {
    ...
    try {
      ...
      for (VirtualDisplayWrapper virtualDisplayWrapper : virtualDisplaysToBeReleased) {
        mDisplayManagerGlobal.releaseVirtualDisplay(
            virtualDisplayWrapper.getToken(), virtualDisplayWrapper.getDisplayId());
      ...
      if (mParams.getInputMethodComponent() != null) {
        InputMethodManagerInternal.get().setVirtualDeviceInputMethodForAllUsers(mDeviceId, null);
      }
  ...
  public void setDevicePolicy(
      @VirtualDeviceParams.DynamicPolicyType int policyType,
    ...
    switch (policyType) {
      ...
      default:
        throw new IllegalArgumentException(
            "Device policy " + policyType + " cannot be changed at runtime. ");
    }
  }

  @Override // Binder call
  public void setDevicePolicyForDisplay(
      int displayId,
    ...
    synchronized (mVirtualDeviceLock) {
      ...
      switch (policyType) {
        ...
        default:
          throw new IllegalArgumentException(
              "Device policy " + policyType + " cannot be changed for a specific display. ");
      }
    }
  }

  @Override // Binder call
  public IVirtualDpad createVirtualDpad(
      @NonNull VirtualDpadConfig config, @NonNull IBinder deviceToken) {
    checkCallerIsDeviceOwner();
    Objects.requireNonNull(config);
    Objects.requireNonNull(deviceToken);
    checkVirtualInputDeviceDisplayIdAssociation(config.getAssociatedDisplayId());
    return Binder.withCleanCallingIdentity(() -> mInputController.createDpad(deviceToken, config));
  }

  @Override // Binder call
  public IVirtualKeyboard createVirtualKeyboard(
      @NonNull VirtualKeyboardConfig config, @NonNull IBinder deviceToken) {
    checkCallerIsDeviceOwner();
    Objects.requireNonNull(config);
    Objects.requireNonNull(deviceToken);
    checkVirtualInputDeviceDisplayIdAssociation(config.getAssociatedDisplayId());
    IVirtualKeyboard device =
        Binder.withCleanCallingIdentity(() -> mInputController.createKeyboard(deviceToken, config));
    synchronized (mVirtualDeviceLock) {
  ...
  public IVirtualMouse createVirtualMouse(
      @NonNull VirtualMouseConfig config, @NonNull IBinder deviceToken) {
    checkCallerIsDeviceOwner();
    Objects.requireNonNull(config);
    Objects.requireNonNull(deviceToken);
    checkVirtualInputDeviceDisplayIdAssociation(config.getAssociatedDisplayId());
    return Binder.withCleanCallingIdentity(() -> mInputController.createMouse(deviceToken, config));
  }

  @Override // Binder call
  public IVirtualTouchscreen createVirtualTouchscreen(
      @NonNull VirtualTouchscreenConfig config, @NonNull IBinder deviceToken) {
    checkCallerIsDeviceOwner();
    Objects.requireNonNull(config);
    Objects.requireNonNull(deviceToken);
    checkVirtualInputDeviceDisplayIdAssociation(config.getAssociatedDisplayId());
  ...
  @Override // Binder call
  public IVirtualStylus createVirtualStylus(
      @NonNull VirtualStylusConfig config, @NonNull IBinder deviceToken) {
    checkCallerIsDeviceOwner();
    Objects.requireNonNull(config);
    Objects.requireNonNull(deviceToken);
    checkVirtualInputDeviceDisplayIdAssociation(config.getAssociatedDisplayId());
    ...
    try {
      synchronized (mVirtualDeviceLock) {
        ...
        for (int i = 0; i < mVirtualDisplays.size(); i++) {
          VirtualDisplayWrapper wrapper = mVirtualDisplays.valueAt(i);
          if (wrapper.isTrusted()) {
            mInputController.setShowPointerIcon(mDefaultShowPointerIcon, mVirtualDisplays.keyAt(i));
          }
        }
      }
    } finally {
  ...
  public void setDisplayInTouchMode(int displayId, boolean inTouchMode) {
    ...
    Binder.withCleanCallingIdentity(() -> mWindowManager.setInTouchMode(inTouchMode, displayId));
  }
  ...
  @Override // Binder call
  public boolean sendSensorAdditionalInfo(
      @NonNull IBinder token, @NonNull VirtualSensorAdditionalInfo info) {
    checkCallerIsDeviceOwner();
    final long ident = Binder.clearCallingIdentity();
    try {
  ...
  public boolean canCreateMirrorDisplays() {
    if (Flags.vdmMirrorDisplayPermission()
        && CompatChanges.isChangeEnabled(
            CHECK_ADD_MIRROR_DISPLAY_PERMISSION,
            mOwnerPackageName,
            UserHandle.getUserHandleForUid(mOwnerUid))) {
  ...
  private void createAudioFocusEnvironment(@NonNull IBinder token) {
    if (mAudioManager == null
        || getDevicePolicy(POLICY_TYPE_AUDIO) != DEVICE_POLICY_CUSTOM
    ...
    if (mContext.checkCallingOrSelfPermission(MODIFY_AUDIO_ROUTING)
      ...
      Slog.w(
          TAG,
          "Can't create a separate audio focus environment for the virtual "
              + "device "
              + mDeviceId
              + " without the MODIFY_AUDIO_ROUTING permission.");
      return;
    }
    ...
    if (mAudioManager.createFocusEnvironment(token)) {
      Slog.d(
          TAG,
          "Created a separate audio focus environment for the virtual "
              + "device with id "
              + mDeviceId
              + " using token: "
              + token);
      ...
      Slog.e(
          TAG,
          "Failed to create a separate audio focus environment for the virtual"
              + " device with id "
              + mDeviceId
              + " using token: "
              + token);
  ...
  private void destroyAudioFocusEnvironment() {
    ...
    if (mContext.checkCallingOrSelfPermission(MODIFY_AUDIO_ROUTING)
      ...
      Slog.w(
          TAG,
          "MODIFY_AUDIO_ROUTING missing at closing of the audio focus environment"
    ...
    try (SafeCloseable ignored = ClearCallingIdentityContext.create()) {
      if (mAudioManager.destroyFocusEnvironment(mAudioFocusEnvToken)) {
        Slog.d(
            TAG,
            "Destroyed the separate audio focus environment: "
                + mAudioFocusEnvToken
                + " for the virtual device "
                + mDeviceId);
      } else {
        Slog.e(
            TAG,
            "Failed to destroy the separate audio focus environment: "
                + mAudioFocusEnvToken
                + " for the virtual device "
                + mDeviceId);
      }
  ...
  protected void dump(
    ...
    fout.println(
        indent + "mDeviceProfile: " + VirtualDeviceLog.deviceProfileToString(mDeviceProfile));
    ...
    fout.println(indent + "hasCustomAudioInputSupport: " + hasCustomAudioInputSupportInternal());
  }
  ...
  public int createVirtualDisplay(
    ...
    int displayId =
        mDisplayManagerInternal.createVirtualDisplay(
            virtualDisplayConfig, callback, this, gwpc, mOwnerPackageName, mOwnerUid);
    if (displayId == Display.INVALID_DISPLAY) {
      return displayId;
    }
    ...
    synchronized (mVirtualDeviceLock) {
      if (!mVirtualDisplays.contains(displayId)) {
        throw new IllegalStateException(
            "Virtual device was not notified about the "
                + "creation of display with ID "
                + displayId);
      }
    ...
    Binder.withCleanCallingIdentity(
        () -> {
          ...
          mInputController.setDisplayEligibilityForPointerCapture(
              /* isEligible= */ false, displayId);
          if (displayWrapper.isTrusted()) {
            mInputController.setShowPointerIcon(showPointer, displayId);
            mInputController.setDisplayImePolicy(displayId, WindowManager.DISPLAY_IME_POLICY_LOCAL);
          } else {
    ...
    Counter.logIncrementWithUid(
        "virtual_devices.value_virtual_display_created_count", mAttributionSource.getUid());
    return displayId;
  }

  private boolean shouldShowBlockedActivityDialog(
      ComponentName blockedComponent, ComponentName blockedAppStreamingActivityComponent) {
      ...
      for (UserHandle profile : userManager.getAllProfiles()) {
        if (isInAllowedUsers(profile)
            && isAllowedByNearbyAppStreamingPolicy(
                dpm.getNearbyAppStreamingPolicy(profile.getIdentifier()), profile)) {
          result.add(profile);
        }
      }
    } finally {
  ...
  private boolean isInAllowedUsers(UserHandle profile) {
    return mParams.getAllowedUsers().isEmpty() || mParams.getAllowedUsers().contains(profile);
  }

  /**
   * Returns whether the user is allowed based on {@link
   * VirtualDeviceParams#getUsersWithMatchingAccounts()}.
   */
  ...
  /**
   * DisplayManagerService is notifying this virtual device about the display creation. This should
   * happen before the DisplayManagerInternal#createVirtualDisplay() call above returns. This is
   * called while holding the DisplayManagerService lock, so no heavy-weight work must be done here
   * and especially *** no calls to WindowManager! ***
   */
  public void onVirtualDisplayCreated(
      int displayId, IVirtualDisplayCallback callback, DisplayWindowPolicyController dwpc) {
    ...
    synchronized (mVirtualDeviceLock) {
      ...
      mVirtualDisplays.put(
          displayId, new VirtualDisplayWrapper(callback, displayId, gwpc, isTrustedDisplay));
    }
  }

  /**
   * This is callback invoked by VirtualDeviceManagerService when VirtualDisplay was released by
   * DisplayManager (most probably caused by someone calling VirtualDisplay.close()). At this point,
   * the display is already released, but we still need to clean up.
   *
   * <p>Note that when the display is destroyed during VirtualDeviceImpl.close() call, this callback
   * won't be invoked because the display is removed from VirtualDeviceManagerService before any
  ...
  void onVirtualDisplayRemoved(int displayId) {
    ...
    if (virtualDisplayWrapper == null) {
      Slog.w(
          TAG,
          "Virtual device " + mDeviceId + " doesn't have a virtual display with ID " + displayId);
      return;
    }
   ...
   * belong to the default display group (the API to mirror non-default display is hidden).
   *
   ...
   * group should be owned by this virtual device. However, this is a good enough heuristic for now.
   */
   ...
   *
   * <p>Note that this method won't release the virtual display itself.
   *
  ...
  GenericWindowPolicyController getDisplayWindowPolicyControllerForTest(int displayId) {
    ...
    return virtualDisplayWrapper != null ? virtualDisplayWrapper.getWindowPolicyController() : null;
  }

  /**
   * Returns true if an app with the given {@code uid} is currently running on this virtual device.
   */
  ...
  void showToastWhereUidIsRunning(
      int uid, @StringRes int resId, @Toast.Duration int duration, Looper looper) {
    showToastWhereUidIsRunning(uid, mContext.getString(resId), duration, looper);
  }

  /** Shows a toast on virtual displays owned by this device which have a given uid running. */
  void showToastWhereUidIsRunning(
      int uid, String text, @Toast.Duration int duration, Looper looper) {
  ...
  private void showToastOnDisplay(
      int displayId, String text, @Toast.Duration int duration, Looper looper) {
  ...
  void onAuthenticationPrompt(int displayId, String packageName) {
    ...
    if (mDeviceProfile != VirtualDevice.DEVICE_PROFILE_COMPUTER_CONTROL) {
      showToastOnDisplay(
          displayId,
          mContext.getString(R.string.app_streaming_blocked_message_for_fingerprint_dialog),
          Toast.LENGTH_LONG,
  ...
  interface PendingTrampolineCallback {
     ...
     * Implementations should try to listen for activity starts associated with the given {@code
     * pendingTrampoline}, and launch the activity on the display with {@link
     * PendingTrampoline#mDisplayId}.
     */
  ...
  static class PendingTrampoline {
    ...
    /**
     * The result receiver associated with this pending call. {@link Activity#RESULT_OK} will be
     * sent to the receiver if the trampoline activity was captured successfully. {@link
     * Activity#RESULT_CANCELED} is sent otherwise.
     */
    ...
    private PendingTrampoline(
        PendingIntent pendingIntent, ResultReceiver resultReceiver, int displayId) {
  ...
  private static final class VirtualDisplayWrapper {
    ...
    VirtualDisplayWrapper(
        @NonNull IVirtualDisplayCallback token,
        int displayId,
