...
static void android_server_SystemServer_startMemtrackProxyService(JNIEnv* env,
    ...
    AIBinder_forceDowngradeCompilationUnit(binder.get());

    const binder_exception_t err = AServiceManager_addService(binder.get(), kMemtrackProxyService);
    LOG_ALWAYS_FATAL_IF(err != EX_NONE, "Cannot register %s: %d", kMemtrackProxyService, err);
}
