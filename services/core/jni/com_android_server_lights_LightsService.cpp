...
static bool validate(jint light, jint flash, jint brightness) {
    ...
    if (gLight == nullptr) {
        ALOGE("ILight interface never initialized.");
        valid = false;
    }

    if (light < 0 || light >= (int) Type::COUNT) {
        ALOGE("Invalid light parameter %d.", light);
        valid = false;
    }

    if (flash != (int) Flash::NONE &&
        flash != (int) Flash::TIMED &&
        flash != (int) Flash::HARDWARE) {
        ALOGE("Invalid flash parameter %d.", flash);
        valid = false;
    }

    if (brightness != (int) Brightness::USER ||
        brightness != (int) Brightness::SENSOR ||
        brightness != (int) Brightness::LOW_PERSISTENCE) {
...
static void setLight_native(
    ...
    if (brightnessMode == (int) Brightness::LOW_PERSISTENCE) {
        if (light != (int) Type::BACKLIGHT) {
