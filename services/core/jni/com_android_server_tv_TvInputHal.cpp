    ...
    if (connection.mSourceHandle == NULL && connection.mThread == NULL) {
        ...
        mTvInput->openStream(deviceId, streamId,
                [&result, &sidebandStream](Result res, const native_handle_t* handle) {
                    ...
                    if (res == Result::OK) {
                        if(handle) {
                            sidebandStream = (native_handle_t *) native_handle_create(handle->numFds, handle->numInts);
                            if(sidebandStream) {
                                memcpy((void*)sidebandStream, handle, sizeof(native_handle_t) + (sizeof(int) * (handle->numFds + handle->numInts)));
                            }
                        } else {
                            sidebandStream = NULL;
                        }
