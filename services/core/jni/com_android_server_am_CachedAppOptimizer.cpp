...
static jboolean com_android_server_am_CachedAppOptimizer_enableBinderReport(JNIEnv *env, jobject thiz) {
    if (genl.open() < 0) {
        ALOGE("Failed to open binder genl socket");
        return false;
    }

    if (genl.setReport(0, BINDER_REPORT_ALL) < 0) {
...
static void com_android_server_am_CachedAppOptimizer_handleBinderReport(JNIEnv *env, jobject thiz) {
    ...
    while (true) {
        ...
        if (!genl.recv(&msg, sizeof(msg), BINDER_GENL_CMD_REPORT, BINDER_GENL_ATTR_REPORT,
                       callback)) {
