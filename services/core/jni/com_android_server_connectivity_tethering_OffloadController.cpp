...
static jboolean android_server_connectivity_tethering_OffloadController_configOffload(
    ...
    hidl_handle h1(handleFromFileDescriptor(&fd1)), h2(handleFromFileDescriptor(&fd2));

    // call setHandles or return false (and close)
    bool rval;
    hidl_string msg;
    const IOffloadConfig::setHandles_cb cb =
            [&rval, &msg](bool success, const hidl_string& errMsg) {
                rval = success;
                msg = errMsg;
            };
    configInterface->setHandles(h1, h2, cb);
    if (!rval) {
