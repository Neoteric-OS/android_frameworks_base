...
static jint native_pairing_start(JNIEnv* env,
                                 ...
                                 jstring guid,
                                 jstring password) {
    // Generate the system certificate and private key to initialize the pairing
    // server.
    ...
    memcpy(system_info.info, guidWrapper.data(), guidWrapper.size());

    JStringUTFWrapper passwordWrapper(env, &password);

    // Create the pairing server
    sServer = PairingServerPtr(pairing_server_new(reinterpret_cast<const uint8_t*>(passwordWrapper.data()),
                                                  passwordWrapper.size(),
                                                  system_info,
                                                  reinterpret_cast<const uint8_t*>(cert_pem.data()),
                                                  cert_pem.size(),
                                                  reinterpret_cast<const uint8_t*>(pkey_pem.data()),
                                                  pkey_pem.size(),
                                                  0));
...
static jboolean native_pairing_wait(JNIEnv* env,
    ...
    std::string peer_public_key = sWaiter->peer_info_.info;
