...
void NativeInputManager::getReaderConfiguration(InputReaderConfiguration* outConfig) {
    ...
    jobjectArray excludedDevicesVidPid = jobjectArray(env->CallStaticObjectMethod(
            gServiceClassInfo.clazz, gServiceClassInfo.getExcludedDevicesPidVid));
    if (!checkAndClearExceptionFromCallback(env, "getExcludedDevicesPidVid") && excludedDevicesVidPid) {
        ...
        for (jsize i = 0; i < length / 2; i++) {
            ...
            std::string pidStr = getStringElementFromJavaArray(env, excludedDevicesVidPid, 2 * i + 1);
            uint16_t vid, pid;
