    ...
    void updateSession(MediaSessionRecord record) {
        synchronized (mLock) {
            ...
            if ((record.getFlags() & MediaSession.FLAG_EXCLUSIVE_GLOBAL_PRIORITY) != 0) {
                ...
                user.pushMediaKeyEventSessionChangedLocked();
            } else {
    ...
    final class FullUserRecord implements MediaSessionStack.OnMediaButtonSessionChangedListener {
        ...
        public void onMediaButtonSessionChanged(MediaSessionRecord oldMediaButtonSession,
            ...
            synchronized (mLock) {
                ...
                pushMediaKeyEventSessionChangedLocked();
            }
        }
        ...
        private void pushMediaKeyEventSessionChangedLocked(
                IOnMediaKeyEventSessionChangedListener listener) {
            try {
                MediaSessionRecord mediaButtonSession = getMediaButtonSessionLocked();
                if (mediaButtonSession != null) {
                    listener.onMediaKeyEventSessionChanged(mediaButtonSession.getPackageName(),
                ...
                Log.w(TAG, "Failed to pushMediaKeyEventSessionChangedLocked", e);
            }
        }
        ...
        private void pushMediaKeyEventSessionChangedLocked() {
            for (OnMediaKeyEventSessionChangedListenerRecord cr
                    : mOnMediaKeyEventSessionChangedListeners.values()) {
                pushMediaKeyEventSessionChangedLocked(cr.listener);
            }
        }
        ...
        final class OnMediaKeyEventDispatchedListenerRecord implements IBinder.DeathRecipient {
            public final IOnMediaKeyEventDispatchedListener listener;
            public final int uid;

            OnMediaKeyEventDispatchedListenerRecord(IOnMediaKeyEventDispatchedListener listener,
                    int uid) {
            ...
            public void binderDied() {
                synchronized (mLock) {
                    mOnMediaKeyEventDispatchedListeners.remove(listener.asBinder());
                }
            }
        }

        final class OnMediaKeyEventSessionChangedListenerRecord implements IBinder.DeathRecipient {
            public final IOnMediaKeyEventSessionChangedListener listener;
            public final int uid;
            ...
            public void binderDied() {
                synchronized (mLock) {
                    mOnMediaKeyEventSessionChangedListeners.remove(listener.asBinder());
                }
            }
        }
    }
    ...
    class SessionManagerImpl extends ISessionManager.Stub {
            ...
            try {
                if (!hasMediaControlPermission(pid, uid)) {
                    throw new SecurityException("MEDIA_CONTENT_CONTROL permission is required to"
                            + "  register MediaKeyEventDispatchedListener");
                }
                synchronized (mLock) {
                    FullUserRecord user = getFullUserRecordLocked(userId);
        ...
        public void removeOnMediaKeyEventDispatchedListener(
            ...
            try {
                if (!hasMediaControlPermission(pid, uid)) {
                    throw new SecurityException("MEDIA_CONTENT_CONTROL permission is required to"
                            + "  unregister MediaKeyEventDispatchedListener");
                }
                synchronized (mLock) {
                    FullUserRecord user = getFullUserRecordLocked(userId);
                    if (user == null || user.mFullUserId != userId) {
            ...
            if (session != null) {
                ...
                try {
                    for (FullUserRecord.OnMediaKeyEventDispatchedListenerRecord cr
                        ...
                        cr.listener.onMediaKeyEventDispatched(
                                keyEvent, session.getPackageName(), session.getSessionToken());
                    }
                } catch (RemoteException e) {
                    Log.w(TAG, "Failed to send listener", e);
                }
            } else if (mCurrentFullUserRecord.mLastMediaButtonReceiver != null
                    ...
                    if (mCurrentFullUserRecord.mLastMediaButtonReceiver != null) {
                        ...
                        if (componentName != null) {
                            for (FullUserRecord.OnMediaKeyEventDispatchedListenerRecord cr
                                    ...
                                    .mOnMediaKeyEventDispatchedListeners.values()) {
                                cr.listener.onMediaKeyEventDispatched(keyEvent,
