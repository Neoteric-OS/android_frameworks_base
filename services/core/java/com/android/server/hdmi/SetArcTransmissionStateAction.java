               ...
               RequestSadAction action = new RequestSadAction(
                       ...
                       new RequestSadAction.RequestSadCallback() {
                           ...
                           public void onRequestSadDone(List<byte[]> supportedSads) {
                               // Enable ARC status immediately before sending <Report
			       // Arc Initiated>. If AVR responds with <Feature Abort>,
			       // disable ARC status again. This is different from
			       // spec that says that turns ARC status to "Enabled"
			       // if <Report ARC Initiated> is acknowledged and no
			       // <Feature Abort> is received. But implemented this
			       // way to save the time having to wait for <Feature Abort>.
                               Slog.i(TAG, "Enabling ARC");
                               tv().enableArc(supportedSads);
                               // If succeeds to send <Report ARC Initiated>, wait
			       // general timeout to check whether there is no
			       // <Feature Abort> for <Report ARC Initiated>.
