...
final class RequestShortAudioDescriptorAction extends HdmiCecFeatureAction {
    ...
    */
    interface RequestSADCallback {
         ...
         *
         * @param sadParams a set of SADs which is returned by AVR and
         *         need to pass to audio HAL.
         * @param supportMultiChannels indicates if support multi-channels.
         * @param update indicates if update EDID.
         */
        void updateSAD(byte[] sadParams, boolean supportMultiChannels, boolean update);
        /**
         * Called when SAD parameters are ready to be stored.
         *
         * @param sadParams a set of SADs which is returned by AVR.
         */
        void setSadParams(byte[] sadParams);
        /**
         * Called when RequestShortAudioDescriptorAction need to get sadParams.
         *
         * @return sadParams a set of SADs which is returned by AVR.
         */
        byte[] getSadParams();
    }
    ...
    RequestShortAudioDescriptorAction(HdmiCecLocalDevice source, int avrAddress,
            boolean systemAudioModeEnabled, RequestSADCallback callback) {
        super(source);
    ...
    boolean start() {
        if (mEnabled) {
            ...
            if (mCallback.getSadParams() != null) {
    ...
    private void sendRequestShortAudioDescriptor() {
        HdmiCecMessage command =
                HdmiCecMessageBuilder.buildRequestShortAudioDescriptor(getSourceAddress(),
                    mAvrAddress, new int[] {Constants.CODEC_DD, Constants.CODEC_AAC,
                    Constants.CODEC_DTS, Constants.CODEC_DDP});
    ...
    private void resetShortAudioDescriptor() {
        HdmiLogger.debug("resetShortAudioDescriptor");
        mCallback.updateSAD(null, false, true);
    }

    private boolean isMultiChannelsSupported() {
        byte codec = Constants.CODEC_NONE;
        byte channels = 0;
        byte[] sadParams = mCallback.getSadParams();

        for (int index = 0; index < sadParams.length; index += SAD_LEN) {
            // bit 6~3: Audio Format Code
            codec = (byte) ((sadParams[index] & 0x78) >> 3); //enAudioFormatCode
            // bit 2~0: Max number of channels -1
            channels = (byte) (sadParams[index] & 0x07);
            if ((codec == Constants.CODEC_DDP) || (codec == Constants.CODEC_DD)) {
                if (channels >= 5) {
                    return true;
                }
            }
    ...
    private void setAudioFormat() {
        mCallback.updateSAD(mCallback.getSadParams(), isMultiChannelsSupported(), true);
        finish();
    }
    ...
    boolean processCommand(HdmiCecMessage cmd) {
        ...
        if (opcode == Constants.MESSAGE_FEATURE_ABORT) {
            ...
            if (originalOpcode == Constants.MESSAGE_REQUEST_SHORT_AUDIO_DESCRIPTOR) {
                ...
                mCallback.updateSAD(null, false, false);
                finish();
                return true;
            }
            ...
            if ((params[0] & 0xFF) == Constants.CODEC_NONE) {
                resetShortAudioDescriptor();
                finish();
                return true;
            }
            mCallback.setSadParams(params);
            setAudioFormat();
            return true;
        }
        return false;
