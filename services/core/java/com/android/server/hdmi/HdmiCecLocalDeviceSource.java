...
abstract class HdmiCecLocalDeviceSource extends HdmiCecLocalDevice {
    ...
    protected HdmiCecLocalDeviceSource(HdmiControlService service, int deviceType) {
        ...
        mOneTouchPlayEnabed = service.readBooleanSetting(Global.HDMI_CONTROL_ONE_TOUCH_PLAY_ENABLED, true);
        service.writeBooleanSetting(Global.HDMI_CONTROL_ONE_TOUCH_PLAY_ENABLED, mOneTouchPlayEnabed);
    }
    ...
    protected void setActiveState(HdmiCecMessage message, int physicalAddress) {
        if (mService.isPlaybackDevice()) {
            if (physicalAddress == mService.getPhysicalAddress()) {
                ...
                // The logical address will be updated when the specified source
                // device broadcasts <Active Source>.
                setActiveSource(Constants.ADDR_UNREGISTERED, physicalAddress);
            }
        }
    }
