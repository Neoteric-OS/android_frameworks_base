...
final class HdmiCecController {
    ...
    private static final class NativeWrapperImpl implements NativeWrapper,
        ...
        public int nativeGetPhysicalAddress() {
            try {
                final int[] address = {INVALID_PHYSICAL_ADDRESS};
                mHdmiCec.getPhysicalAddress(
                        new getPhysicalAddressCallback() {
                            @Override
                            public void onValues(int result, short addr) {
                                address[0] = (result == Result.SUCCESS
                                        ? new Short(addr).intValue() : INVALID_PHYSICAL_ADDRESS);
                            }
                        });
                return address[0];
            } catch (RemoteException e) {
