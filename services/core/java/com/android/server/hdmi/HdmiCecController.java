...
final class HdmiCecController {
    ...
    static HdmiCecController create(HdmiControlService service, HdmiCecAtomWriter atomWriter) {
        ...
        controller = createWithNativeWrapper(service, new NativeWrapperImplAidl(), atomWriter);
        if (controller != null) {
            return controller;
        }
     ...
     */
    private void setOption(int flag, boolean enabled) {
        assertRunOnServiceThread();
    ...
    /**
     * Determines whether a TV panel device in standby mode should wake up when it receives an OTP
    ...
    /**
     * Determines which module processes CEC messages - the Android framework or the HAL.
     *
    ...
    private static final class NativeWrapperImplAidl
        ...
        boolean connectToHal() {
            mHdmiCec =
                    IHdmiCec.Stub.asInterface(
                            ServiceManager.waitForService(Context.HDMI_CONTROL_SERVICE));
            if (mHdmiCec == null) {
                return false;
            }
            try {
        ...
        @Override
        public void nativeSetOption(int option, boolean enabled) {
            HdmiLogger.warning("No set option in AIDL, please use the option specific API");
            switch (option) {
                ...
                case OptionKey.SYSTEM_CEC_CONTROL:
                    ...
                    break;
                default:
                    HdmiLogger.error("Incorrect option %d", option);
            }
        }
