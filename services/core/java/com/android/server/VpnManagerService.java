...
public class VpnManagerService extends IVpnManager.Stub {
    ...
    public VpnManagerService(Context context, Dependencies deps) {
        ...
        if (replaceVpnProfileStore()
                && Build.VERSION.DEVICE_INITIAL_SDK_INT <= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
    ...
    // TODO(b/307903113): Remove the usage of LegacyVpnProfileStore after migration is complete.
        ...
        for (String prefix : prefixes) {
            for (String key : LegacyVpnProfileStore.list(prefix)) {
                ...
                if (!mVpnProfileStore.put(name, LegacyVpnProfileStore.get(name))) {
                    Log.wtf(TAG, "Failed to import vpn profile " + name);
                }
            }
        }
    }
