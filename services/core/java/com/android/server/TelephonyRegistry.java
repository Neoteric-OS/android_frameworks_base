        ...
        if (events != PhoneStateListener.LISTEN_NONE) {
            ...
            synchronized (mRecords) {
                ...
                if (notifyNow && validatePhoneId(phoneId)) {
                    ...
                    if ((events & PhoneStateListener.LISTEN_OUTGOING_CALL_EMERGENCY_NUMBER) != 0) {
                        try {
                            r.callback.onOutgoingEmergencyCall(
                                    mOutgoingCallEmergencyNumber[phoneId]);
                        } catch (RemoteException ex) {
                            remove(r.binder);
                        }
                    }
                    if ((events & PhoneStateListener.LISTEN_OUTGOING_SMS_EMERGENCY_NUMBER) != 0) {
                        try {
                            r.callback.onOutgoingEmergencySms(
                                    mOutgoingSmsEmergencyNumber[phoneId]);
                        } catch (RemoteException ex) {
                            remove(r.binder);
                        }
                    }
