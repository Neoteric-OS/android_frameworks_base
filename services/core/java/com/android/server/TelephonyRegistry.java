...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    private boolean[] mCellularDataState;
    ...
    TelephonyRegistry(Context context) {
        ...
        mCellularDataState = new boolean[numPhones];
    ...
    public void notifyCellularDataStateChangedForPhoneId(int phoneId, int subId, boolean state) {
        if (!checkNotifyPermission("notifyCellularDataStateChanged()")) {
            return;
        }
        if (VDBG) {
            log("notifyCellularDataStateChangedForSubscriberPhoneID: subId=" + phoneId
                    + " state=" + state);
        }
        synchronized (mRecords) {
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : mRecords) {
                    if (r.matchPhoneStateListenerEvent(
                            PhoneStateListener.LISTEN_CELLULAR_DATA_STATE) &&
                            idMatch(r.subId, subId, phoneId)) {
                        try {
                            r.callback.onCellularDataStateChanged(state);
                        } catch (RemoteException ex) {
                            mRemoveList.add(r.binder);
                        }
                    }
    ...
    private void checkPossibleMissNotify(Record r, int phoneId) {
        ...
        if ((events & PhoneStateListener.LISTEN_CELLULAR_DATA_STATE) != 0) {
            try {
                if (VDBG) {
                    log("checkPossibleMissNotify: onCellularDataStateChanged phoneId="
                            + phoneId + " cdi=" + mCellularDataState[phoneId]);
                }
                r.callback.onCellularDataStateChanged(mCellularDataState[phoneId]);
            } catch (RemoteException ex) {
                mRemoveList.add(r.binder);
