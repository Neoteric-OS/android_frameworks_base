...
public class AdbService extends IAdbManager.Stub {
    ...
    private class AdbSettingsObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri, @UserIdInt int userId) {
            if (mAdbUsbUri.equals(uri)) {
                ...
                FgThread.getHandler().sendMessage(obtainMessage(
                        AdbService::setAdbEnabled, AdbService.this, enable, AdbTransportType.USB));
            } else if (mAdbWifiUri.equals(uri)) {
                boolean enable = (Settings.Global.getInt(mContentResolver,
                        Settings.Global.ADB_WIFI_ENABLED, 0) > 0);
                FgThread.getHandler().sendMessage(obtainMessage(
                        AdbService::setAdbEnabled, AdbService.this, enable, AdbTransportType.WIFI));
            }
        }
    }
    ...
    public void systemReady() {
        ...
        try {
            ...
            Settings.Global.putInt(mContentResolver,
                    Settings.Global.ADB_WIFI_ENABLED, mAdbWifiEnabled ? 1 : 0);
        ...
        if (transportType == AdbTransportType.USB && enable != mAdbUsbEnabled) {
            ...
            if (mAdbWifiEnabled) {
                if (!AdbProperties.secure().orElse(false) && mDebuggingManager == null) {
                    // Start adbd. If this is secure adb, then we defer starting adbd.
