...
class BLASTSyncEngine {
            ...
            for (int i = mRootMembers.size() - 1; i >= 0; --i) {
                ...
                if (!wc.isSyncFinished(this)) {
                    if(canIgnoreFromRecents(wc)){
                        //if wallpaper has drawn, ignore unfinished container when
                        //window is animating by recents.
                        Slog.w(TAG, "Sync group " + mSyncId + " ignoring " + wc +
                                " when wallpaper has drawn");
                    } else {
    ...
    private boolean canIgnoreFromRecents(WindowContainer wc){
        ...
        if(wc.asActivityRecord() != null && wc.getDisplayContent() != null
            ...
            && mWm.mAtmService.getTransitionController().getCollectingTransitionType()
                == TRANSIT_TO_FRONT
