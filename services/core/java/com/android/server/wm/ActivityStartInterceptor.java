...
/**
 * A class that contains activity intercepting logic for {@link ActivityStarter#execute()} It's
 * initialized via setStates and interception occurs via the intercept method.
 *
 * <p>Note that this class is instantiated when {@link ActivityManagerService} gets created so there
...
class ActivityStartInterceptor {
  ...
  private static final List<String> INTERCEPT_PACKAGES =
      Arrays.asList(
  ...
  ActivityStartInterceptor(
      ActivityTaskManagerService service, ActivityTaskSupervisor supervisor, Context context) {
    mService = service;
  ...
  /**
   * A helper function to obtain the targeted {@link TaskFragment} during {@link #intercept(Intent,
   * ResolveInfo, ActivityInfo, String, Task, TaskFragment, int, int, ActivityOptions,
   * TaskDisplayArea)} if any.
   */
  @Nullable
  ...
  /**
   * Intercept the launch intent based on various signals. If an interception happened the internal
   * variables get assigned and need to be read explicitly by the caller.
   *
  ...
  boolean intercept(
      ...
      TaskDisplayArea presumableLaunchDisplayArea) {
    ...
    for (int i = 0; i < callbacks.size(); i++) {
      ...
      final ActivityInterceptResult interceptResult =
          callback.onInterceptActivityLaunch(interceptorInfo);
      ...
      mRInfo =
          mSupervisor.resolveIntent(mIntent, null, mUserId, 0, mRealCallingUid, mRealCallingPid);
  ...
  /**
   * If the activity option is the {@link ActivityOptions#ANIM_OPEN_CROSS_PROFILE_APPS} one, defer
   * the animation until the original intent is started.
   *
  ...
  private boolean interceptQuietProfileIfNeeded() {
    ...
    Slog.i(
        ...
        "Intent : "
            + mIntent
            + " intercepted for user: "
            + mUserId
            + " because quiet mode is enabled.");

    IntentSender target =
        createIntentSenderForOriginalIntent(mCallingUid, FLAG_CANCEL_CURRENT | FLAG_ONE_SHOT);
    ...
    mRInfo =
        mSupervisor.resolveIntent(
            mIntent, mResolvedType, parent.id, 0, mRealCallingUid, mRealCallingPid);
    mAInfo = mSupervisor.resolveActivity(mIntent, mRInfo, mStartFlags, null /*profilerInfo*/);
    return true;
  }

  private boolean interceptSuspendedByAdminPackage() {
    DevicePolicyManagerInternal devicePolicyManager =
    ...
    if (parent != null) {
      ...
      mRInfo =
          mSupervisor.resolveIntent(
              mIntent, mResolvedType, mUserId, 0, mRealCallingUid, mRealCallingPid);
    }
    mAInfo = mSupervisor.resolveActivity(mIntent, mRInfo, mStartFlags, null /*profilerInfo*/);
    return true;
  }
  ...
  private boolean interceptSuspendedPackageIfNeeded() {
    ...
    final SuspendDialogInfo dialogInfo =
        pmi.getSuspendedDialogInfo(suspendedPackage, suspender, mUserId);
    final Bundle crossProfileOptions =
        hasCrossProfileAnimation()
            ? ActivityOptions.makeOpenCrossProfileAppsAnimation().toBundle()
            : null;
    final IntentSender target = createIntentSenderForOriginalIntent(mCallingUid, FLAG_IMMUTABLE);
    mIntent =
        SuspendedAppActivity.createSuspendedAppInterceptIntent(
            suspendedPackage, suspender, dialogInfo, crossProfileOptions, target, mUserId);
    mCallingPid = mRealCallingPid;
    mCallingUid = mRealCallingUid;
    mResolvedType = null;
    mRInfo =
  ...
  private boolean interceptDigitalWellbeingIfNeeded() {
    ...
    final IntentSender target =
        createIntentSenderForOriginalIntent(
    ...
    final ResolveInfo rInfo =
        mSupervisor.resolveIntent(
            newIntent, null /* resolvedType */, mUserId, 0, callingUid, callingPid);
    final ActivityInfo aInfo =
        mSupervisor.resolveActivity(newIntent, rInfo, mStartFlags, null /* profilerInfo */);
  ...
  private boolean isCallerDigitalWellbeing() {
    if (DIGITAL_WELLBEING_PACKAGE.equals(mCallingPackage)) {
      return true;
    }
    final PackageManagerInternal pmi = mService.getPackageManagerInternalLocked();
    if (pmi != null) {
      return pmi.getPackageUid(DIGITAL_WELLBEING_PACKAGE, 0, mUserId) == mRealCallingUid;
    }
    return false;
  }
  ...
  private Intent interceptWithConfirmCredentialsIfNeeded(ActivityInfo aInfo, int userId) {
    ...
    final IntentSender target =
        createIntentSenderForOriginalIntent(
            mCallingUid, FLAG_CANCEL_CURRENT | FLAG_ONE_SHOT | FLAG_IMMUTABLE);
    final KeyguardManager km = (KeyguardManager) mServiceContext.getSystemService(KEYGUARD_SERVICE);
    final Intent newIntent =
        km.createConfirmDeviceCredentialIntent(
            null, null, userId, true /* disallowBiometricsIfPolicyExists */);
    if (newIntent == null) {
      return null;
    }
    newIntent.setFlags(
        FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS | FLAG_ACTIVITY_TASK_ON_HOME);
  ...
  private boolean interceptHarmfulAppIfNeeded() {
    ...
    try {
      harmfulAppWarning =
          mService.getPackageManager().getHarmfulAppWarning(mAInfo.packageName, mUserId);
    ...
    mIntent =
        HarmfulAppWarningActivity.createHarmfulAppWarningIntent(
            mServiceContext, mAInfo.packageName, target, harmfulAppWarning);
  ...
  private boolean interceptHomeIfNeeded() {
    ...
    Pair<ActivityInfo, Intent> info =
        mService.mRootWindowContainer.resolveSecondaryHomeActivity(
            mUserId, mPresumableLaunchDisplayArea);
    mIntent = info.second;
    ...
    mRInfo =
        mSupervisor.resolveIntent(
            mIntent, mResolvedType, mUserId, /* flags= */ 0, mRealCallingUid, mRealCallingPid);
  ...
  void onActivityLaunched(TaskInfo taskInfo, ActivityRecord r) {
    final SparseArray<ActivityInterceptorCallback> callbacks =
        mService.getActivityInterceptorCallbacks();
    final ActivityInterceptorCallback.ActivityInterceptorInfo info =
        getInterceptorInfo(
            () -> {
