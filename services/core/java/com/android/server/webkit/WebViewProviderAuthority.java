...
public final class WebViewProviderAuthority {
    ...
    /**
     * Return true if the PackageInfo validates OK as any of the configured WebView providers.
     *
     * Wraps {@link #validatePackageAsProvider(PackageInfo, WebViewProviderInfo, long)} in a
     * loop. See that method for more details.
     */
    public boolean packageIsWebView(PackageInfo packageInfo, long minimumVersionCode) {
        for (WebViewProviderInfo providerInfo : mWebViewProviderPackages) {
            if (validatePackageAsProvider(packageInfo, providerInfo, minimumVersionCode)
                    .isValid()) {
                return true;
            }
        }
     ...
     * loop. See that method for more details.
     */
    public boolean packageIsWebView(AndroidPackage androidPackage,
     ...
     * @returns {@link #Validity} enum describing the result of the validation. If there are
     *          multiple reasons why a package fails validation, only a single reason is reported.
     */
