...
public class PermissionMonitor implements PackageManagerInternal.PackageListObserver {
    ...
    public static class UidNetdPermissionInfo {
        ...
        /** Return whether permission is uninstalled or not. */
        public boolean isPermissionUninstalled() {
        ...
        public boolean hasNetdPermissions(final int permissions) {
            if (isPermissionUninstalled()) return false;
    ...
    public synchronized void startMonitoring() {
        ...
        for (int i = 0; i < systemPermission.size(); i++) {
            ...
            netdPermsUids.put(uid, uidPerms.plusNetdPermissions(netdPermission));
        }
     ...
     * @param permissions the permission info of given uid.
     *
     * @hide
     */
    @VisibleForTesting
     ...
     *
     * @param uidNetdPerms permission info array generated from each uid. If the uid permission is
    ...
    void sendPackagePermissionsToNetd(final SparseArray<UidNetdPermissionInfo> uidNetdPerms) {
        ...
        for (int i = 0; i < uidNetdPerms.size(); i++) {
            ...
            } else if (permissions.isPermissionUninstalled()) {
