...
public class PermissionMonitor {
    ...
    private synchronized void onAppRemoved(int appUid) {
        ...
        if (packages != null && packages.length > 0) {
            ...
            if (permission != null) {
                // If other app of this uid have network permissions, the same
                // as before do not update, or update permissions.
                if (permission != mApps.get(appUid)) {
                    mApps.put(appUid, permission);
                    apps.put(appUid, permission);
                    update(mUsers, apps, true);
        ...
        // If all apps of this uid do not have any network permission to remove permissions.
        mApps.remove(appUid);
