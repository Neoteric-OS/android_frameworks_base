...
public class PermissionMonitor implements PackageManagerInternal.PackageListObserver {
    ...
    public static class UidNetdPermissionInfo {
        ...
        public boolean hasNetdPermissions(final int permissions) {
            if (permissions == PERMISSION_NONE || mNetdPermissions == PERMISSION_UNINSTALLED) {
                return permissions == mNetdPermissions;
            }
    ...
    public synchronized void startMonitoring() {
        ...
        SparseArray<UidNetdPermissionInfo> netdPermsUids = new SparseArray<>();
        ...
        for (int i = 0; i < systemPermission.size(); i++) {
            ...
            final UidNetdPermissionInfo uidPerms =
                    netdPermsUids.get(uid, new UidNetdPermissionInfo());
    ...
    void sendPackagePermissionsForUid(int uid, UidNetdPermissionInfo permissions) {
        final SparseArray<UidNetdPermissionInfo> uidNetdPerms = new SparseArray<>();
     ...
     */
    @VisibleForTesting
    void sendPackagePermissionsToNetd(final SparseArray<UidNetdPermissionInfo> uidNetdPerms) {
        if (mNetd == null) {
        ...
        for (int i = 0; i < uidNetdPerms.size(); i++) {
            final int uid = uidNetdPerms.keyAt(i);
            final UidNetdPermissionInfo permissions = uidNetdPerms.valueAt(i);
            ...
            } else if (permissions.hasNetdPermissions(PERMISSION_UNINSTALLED)) {
