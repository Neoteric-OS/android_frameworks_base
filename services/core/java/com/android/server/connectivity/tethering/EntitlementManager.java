...
public class EntitlementManager {
    ...
    private void checkIfPermittedChange() {
        ...
        if (DBG) {
            Log.d(TAG, "checkIfPermittedChange from " + oldPermitted
                    + " to " + mCellularUpstreamPermitted);
        }

        if (mCellularUpstreamPermitted != oldPermitted) {
            mLog.log("Entitlement permitted change: " + mCellularUpstreamPermitted);
    ...
    public void dump(PrintWriter pw) {
        ...
        for (Integer type : mCurrentTethers) {
            ...
            if (mCellularPermitted.indexOfKey(type) > -1) {
                pw.print(", Value: ");
                pw.println(valueString(mCellularPermitted.get(type)));
            } else {
