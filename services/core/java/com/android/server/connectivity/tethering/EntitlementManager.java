...
public class EntitlementManager {
    ...
    private static final int EVENT_RUN_ENTITLEMENT_TIMEOUT = 5;
    ...
    // An intent action that will be broadcast in non-ui cases for checking the provisioning
    // result.
    private @NonNull String mSilentProvisioningResponse = "";
    ...
    public void updateConfiguration(TetheringConfiguration conf) {
        ...
        if (isTetherProvisioningRequired()) {
            if (mConfig.provisionResponse.equals(mSilentProvisioningResponse)) return;

            if (!mSilentProvisioningResponse.isEmpty()) {
    ...
    private void handleStartProvisioningIfNeeded(int type, boolean showProvisioningUi) {
        ...
        if (isTetherProvisioningRequired()) {
            ...
            if (showProvisioningUi) {
                resetTypeEntitlementAndRunUiProvisioning(type);
        ...
        for (Integer downstream : mCurrentTethers) {
            if (mCellularPermitted.indexOfKey(downstream) < 0) {
                if (mNeedReRunProvisioningUi) {
                    mNeedReRunProvisioningUi = false;
                    resetTypeEntitlementAndRunUiProvisioning(downstream);
                } else {
    ...
    public void reevaluateSimCardProvisioning() {
        ...
        if (!mConfig.hasMobileHotspotProvisionApp()
            ...
            mSilentProvisioningList.clear();
            clearEntitlementTimeoutTimer();
    ...
    private void setupEntitlementTimeoutTimer(final long timeoutMillis) {
        mHandler.sendMessageDelayed(mHandler.obtainMessage(EVENT_RUN_ENTITLEMENT_TIMEOUT),
                timeoutMillis);
    }

    private void clearEntitlementTimeoutTimer() {
        mTimeoutCount = 0;
        mHandler.removeMessages(EVENT_RUN_ENTITLEMENT_TIMEOUT);
    }
    ...
    protected void runSilentTetherProvisioning(int type) {
        ...
        mEntitlementCacheValue.put(type, TETHER_ERROR_ENTITLEMENT_UNKONWN);
        ...
        setupEntitlementTimeoutTimer(60000 /* one minute */);
    }
    ...
    private void setEntitlementAppActive(@NonNull final Intent intent) {
        ...
        for (ResolveInfo resolver : resolvers) {
            if (resolver.activityInfo == null || resolver.activityInfo.applicationInfo == null
                    || !resolver.activityInfo.applicationInfo.isSystemApp()) {
                continue;
            }
            ...
            // See b/29514913.
            // After not being used for a few days (as can easily be the case for tethering)
            // the entitlement app might have been put into the "app standby" mode. That would
            // prevent it from accessing the network in the background, making it impossible
            // for it to silently check for provisioning. Therefore, forcibly move the
            // provisioning app out of app standby mode to let it access the network silently.
     ...
     * @param type tethering type from ConnectivityManager.TETHERING_{@code *}
     */
    private void resetTypeEntitlementAndRunUiProvisioning(int type) {
        mCellularPermitted.put(type, TETHER_ERROR_ENTITLEMENT_UNKONWN);
        mEntitlementCacheValue.put(type, TETHER_ERROR_ENTITLEMENT_UNKONWN);
    ...
    // This receiver is registered with permission CONNECTIVITY_INTERNAL.
    // Only trusted applications can send intent to this receiver.
            ...
            switch (msg.what) {
                ...
                case EVENT_GET_ENTITLEMENT_VALUE:
                    handleGetLatestTetheringEntitlementValue(msg.arg1, (ResultReceiver) msg.obj,
                            toBool(msg.arg2));
                    break;
                case EVENT_RUN_ENTITLEMENT_TIMEOUT:
                    if (mSilentProvisioningList.size() > 0 && mTimeoutCount < MAX_RETRY_TIMES) {
                        final Integer downstream = mSilentProvisioningList.get(0);
                        mLog.log("TIMEOUT run  silent provisioning in " + typeString(downstream));
                        runSilentTetherProvisioning(downstream);
                        mTimeoutCount++;
                    }
                    break;
