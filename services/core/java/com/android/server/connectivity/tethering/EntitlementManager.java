...
public class EntitlementManager {
    ...
    // Event to handle startProvisioningIfNeeded method.
    private static final int EVENT_START_PROVISIONING = 0;
    // Event to handle stopProvisioningIfNeeded method.
    private static final int EVENT_STOP_PROVISIONING = 1;
    // Event to handle notifyUpstream method.
    private static final int EVENT_UPSTREAM_CHANGED = 2;
    // Event to handle addDownstreamMapping method.
    private static final int EVENT_ADD_DOWNSTREAM_MAPPING = 3;
    // Event to handle removeDownstreamMapping method.
    ...
    //key: ConnectivityManager.TETHERING_*, value: provisioning result
     ...
     * Launches provisioning app if upstream is cellular.
     *
    ...
    /**
     * Notify EntitlementManager if upstream is mobile or not.
     *
     * @param isMobile mobile would be tethering upstream
     */
    public void notifyUpstream(boolean isMobile) {
        mHandler.sendMessage(mHandler.obtainMessage(
                EVENT_UPSTREAM_CHANGED, encodeBool(isMobile), 0));
    }

    private void handleNotifyUpstream(boolean isMobile) {
        if (DBG) {
            Log.d(TAG, "notifyUpstream: " + isMobile
                    + ", mMobileUpstreamPermitted: " + mMobileUpstreamPermitted
                    + ", mNeedReRunProvisioningUi: " + mNeedReRunProvisioningUi);
        }
        mUsingCellularAsUpstream = isMobile;
        ...
        if (mUsingCellularAsUpstream) {
            for (Integer type : mCurrentTethers) {
                if (mMobilePermitted.indexOfKey(type) < 0) {
    ...
    public void reevaluateSimCardProvisioning() {
        ...
        if (mCurrentTethers.size() == 0 || !mUsingCellularAsUpstream) return;
    ...
    private void checkIfPermittedChange() {
        final boolean prePermitted = mMobileUpstreamPermitted;
    ...
    public void dump(PrintWriter pw) {
        ...
        for (Integer type : mCurrentTethers) {
            ...
            if (mMobilePermitted.indexOfKey(type) > -1) {
                pw.print(", Value: ");
                pw.println(valueString(mMobilePermitted.get(type)));
            } else {
