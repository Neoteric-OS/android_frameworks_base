 ...
 *         triggered in NetworkAgent's NetworkMonitor, via a call to
 ...
 *         delayed CMD_PRIVATE_DNS_SETTINGS_CHANGED message in a simple backoff
 *         schedule.
 ...
 *         (second) validation that expressly does not bypass Private DNS.
 *
 ...
 * [C] Event: An application calls ConnectivityManager#reportBadNetwork().
    ...
    XXX: clean this and make it comprehensible.

    - always program netd immediately, even in strict mode with no resolved IPs
        - event: change in Private DNS settings
          mechanism: ConnectivityService listens for settings changes
                     - handlePrivateDnsSettingsChanged()
                     - handlePerNetworkPrivateDnsConfig()

        - event: network attach
          mechanism: ConnectivityService gets changes from NetworkAgent
                     - updateNetworkInfo()
                     - when going to state CONNECTED:
                        - handlePerNetworkPrivateDnsConfig()

    - start resolve (in strict mode)
        - happens in handlePerNetworkPrivateDnsConfig()
        - NetworkMonitor sends itself CMD_PRIVATE_DNS_SETTINGS_CHANGED
        - does resolution (if it's a network that requires validation)
        - sends ConnectivityServices EVENT_PRIVATE_DNS_CONFIG_RESOLVED

    - reresolve if no IPs found
        - NetworkMonitor retries in the same backoff schedule as validation.

    - program netd with resolved IP addresses (in strict mode)
        - ConnectivityService receives EVENT_PRIVATE_DNS_CONFIG_RESOLVED
        - updateDnses

    - trigger a revalidation
        - without TLS bypass
        - NetworkMonitor, post strict mode resolution should send itself
          a message to reevaluate
        - ConnectivityService, when receiving validation update about
          opportunistic mode should ask NetworkMonitor to revalidate.
