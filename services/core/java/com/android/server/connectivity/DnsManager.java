...
public class DnsManager {
    ...
    /**
     * Save transport types to a given network.
    ...
    public void updateTransportsForNetwork(int netId, @NonNull int[] transportTypes) {
        ...
        if (mLinkPropertiesMap.containsKey(netId)) {
            setDnsConfigurationForNetwork(netId);
        }
    }

    /**
     * Save link properties to a given network.
     * When link properties are changed in a specific network, link properties are
     * always saved to a hashMap before update dns config.
     * When destroying network, the specific network will be removed from the hashMap.
     * The hashMap is always accessed on the same thread.
     */
    public void updateLinkPropertiesForNetwork(int netId, @NonNull LinkProperties lp) {
        mLinkPropertiesMap.put(netId, lp);
        setDnsConfigurationForNetwork(netId);
    }

    /**
     * Set dns configuration parameters to resolver for a given network.
     */
    public void setDnsConfigurationForNetwork(int netId) {
        ...
        paramsParcel.servers =
                NetworkUtils.makeStrings(mLinkPropertiesMap.get(netId).getDnsServers());
        paramsParcel.domains = getDomainStrings(mLinkPropertiesMap.get(netId).getDomains());
        paramsParcel.tlsName = strictMode ? privateDnsCfg.hostname : "";
        paramsParcel.tlsServers =
                strictMode ? NetworkUtils.makeStrings(
                        Arrays.stream(privateDnsCfg.ips)
                              .filter((ip) -> mLinkPropertiesMap.get(netId).isReachable(ip))
                              .collect(Collectors.toList()))
                : useTls ? paramsParcel.servers  // Opportunistic
                : new String[0];            // Off
        paramsParcel.experimentalOptions = new ResolverExperimentalOptionsParcel();
        if (mTransportsMap.containsKey(netId)) {
            paramsParcel.transportTypes = mTransportsMap.get(netId);
        }
    ...
    /**
     * Flush DNS caches and let connectivity events work before boot has completed.
     */
