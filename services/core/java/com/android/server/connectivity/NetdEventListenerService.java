...
public class NetdEventListenerService extends INetdEventListener.Stub {
    ...
    static class NetworkMetricsSnapshot {
        ...
        static NetworkMetricsSnapshot collect(long timeMs, SparseArray<NetworkMetrics> networkMetrics) {
            ...
            snapshot.stats = new NetworkMetrics.Summary[networkMetrics.size()];
            for (int i = 0; i < snapshot.stats.length; i++) {
                snapshot.stats[i] = networkMetrics.valueAt(i).pushPendingStats();
            }
            return snapshot;
        }
        ...
        public String toString() {
            return String.format("%tT.%tL: %s", timeMs, timeMs, Arrays.toString(stats));
        }
    }
}
