...
import android.net.metrics.NetworkMetricsSummary;
...
public class NetdEventListenerService extends INetdEventListener.Stub {
    ...
    private NetworkMetrics getMetricsForNetwork(long nowMs, int netId) {
        collectPendingMetricsSnapshot(nowMs);
    ...
    private NetworkMetricsSnapshot[] getNetworkMetricsSnapshots() {
        ...
        // TODO Do I need to also add the current stats bucket ??
        return mNetworkMetricsSnapshots.toArray();
    }

    private void collectPendingMetricsSnapshot(long nowMs) {
        ...
        if (Math.abs(nowMs - mLastSnapshot) > METRICS_SNAPSHOT_SPAN_MS) {
            ...
            mNetworkMetricsSnapshots.append(
                    new NetworkMetricsSnapshot(mLastSnapshot, mNetworkMetrics));
        }
    }
    ...
    static class NetworkMetricsSnapshot {
        ...
        final long timestampMs;
        final NetworkMetricsSummary[] stats;

        NetworkMetricsSnapshot(long timeMs, SparseArray<NetworkMetrics> networkMetrics) {
            timestampMs = timeMs;
            stats = new NetworkMetricsSummary[networkMetrics.size()];
