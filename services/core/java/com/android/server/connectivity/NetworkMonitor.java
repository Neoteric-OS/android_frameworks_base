...
public class NetworkMonitor extends StateMachine {
    ...
    protected class DnsStallDetector {
        ...
        protected void accumulateConsecutiveDnsTimeoutCount(int code) {
            ...
            // The mDnsEvents works as a ringbuffer here to keep the dns events. Keep the event in
            // mDnsEvents without clearing it so that there are logs to do the simulation and
            // analysis.
            if (result.isTimeout()) {
                mConsecutiveTimeoutCount++;
                mDnsEvents.add(result);
                if (mDnsEvents.size() > mSize) {
        ...
        private boolean isDataStallSuspected(int timeoutCountThreshold, int validTime) {
            ...
            // useful here because but still perform it for safety because the size of mDnsEvents is
            // defined larger than or equal to timeoutCountThreshold
            return (mConsecutiveTimeoutCount >= timeoutCountThreshold
                && (mDnsEvents.size() >= timeoutCountThreshold)
                && (now - mDnsEvents.get(mDnsEvents.size() - timeoutCountThreshold).mTimeStamp)
                    < validTime);
        }
