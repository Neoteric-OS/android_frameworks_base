...
public class NetworkMonitor extends StateMachine {
    ...
    protected class DnsStallDetector {
        ...
        protected void accumulateConsecutiveDnsTimeoutCount(int code) {
            ...
            // Keep only the last mSize event in the mDnsEvents. Remove the oldest one if its size
            // become larger than mSize.
            if (result.isTimeout()) {
        ...
        private boolean isDataStallSuspected(int timeoutCountThreshold, int validTime) {
            ...
            final int firstConsecutiveTimeout = mResultIndices.size() - timeoutCountThreshold;
            return (mConsecutiveTimeoutCount >= timeoutCountThreshold
                && mResultIndices.size() >= timeoutCountThreshold
                && (now - mDnsEvents[mResultIndices.indexOf(firstConsecutiveTimeout)].mTimeStamp)
                    < validTime);
        }
