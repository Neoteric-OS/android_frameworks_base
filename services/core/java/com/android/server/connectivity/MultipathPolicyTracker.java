...
public class MultipathPolicyTracker {
    ...
    class MultipathTracker {
        ...
        int mSubId = DEFAULT_SUBSCRIPTION_ID;
        ...
        public MultipathTracker(Network network, NetworkCapabilities nc) {
            ...
            NetworkSpecifier specifer = nc.getNetworkSpecifier();
            if (specifer instanceof TelephonyNetworkSpecifier) {
                mSubId = ((TelephonyNetworkSpecifier) specifer).subscriptionId();
            } else if (specifer instanceof StringNetworkSpecifier) {
                try {
                    mSubId = Integer.parseInt(
                            ((StringNetworkSpecifier) specifer).toString());
                    // TODO: why call toString(), it should be .specifier
                } catch (ClassCastException | NullPointerException | NumberFormatException e) {
                    throw new IllegalStateException(String.format(
                            "Can't get mSubId from mobile network %s (%s): %s",
            ...
            if (tele == null) {
                throw new IllegalStateException(String.format(
                        "Can't get TelephonyManager for mSubId %d", mSubId));
            }
