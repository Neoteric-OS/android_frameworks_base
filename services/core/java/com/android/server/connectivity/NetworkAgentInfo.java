    ...
    private void printStackTrace(String logString) {
        if (!IS_USER) {
            ...
            for (int i = 2; i < callStack.length; i++) {
                stackTrace = callStack[i].toString();

                if (stackTrace == null || stackTrace.contains("android.os")) {
                    break;
                }

                Log.d(TAG + " : Stack Log", stackTrace);
                    // Ex) D <TAG> : Stack Log: ...connectivity.NetworkAgentInfo.printStackTrace...
                    //     D <TAG> : Stack Log: ...connectivity.NetworkAgentInfo.removeRequest...
                    //     D <TAG> : Stack Log: ...ConnectivityService.rematchNetworkAndRequests...
                    //     D <TAG> : Stack Log: ...ConnectivityService.updateNetworkInfo...
            }
        }
    }
}
