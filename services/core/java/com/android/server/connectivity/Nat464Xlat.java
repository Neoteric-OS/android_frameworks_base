 ...
 * from a consistent and unique thread context.
 *
 * @hide
 */
public class Nat464Xlat {
    ...
    /**
     * @return true if clatd has been started. Or return false if clatd has not been started, has
     * been stopped, or the stacked interface has been removed.
     ...
     * Sets internal state.
     */
    private void setStartedState(String baseIface) {
    ...
    public LinkProperties interfaceLinkStateChanged(String iface, boolean up) {
        if (!isStarting() || !up || !mIface.equals(iface)) {
            return null;
        }
