...
public class Nat464Xlat extends BaseNetworkObserver {
    ...
    private void handleInterfaceLinkStateChanged(String iface, boolean up) {
        if (!up || !Objects.equals(mIface, iface)) {
            return;
        }

        if (isStopping()) {
            // stop() was called early after the stacked interface went up but before the
            // notification arrived: enter idle state and do not send LinkProperties updates.
            enterIdleState();
            return;
        }

        if (!isStarting()) {
            // Ignore interface up notifications in IDLE or RUNNING states.
            return;
        }
    ...
    private void handleInterfaceRemoved(String iface) {
        if (!(isRunning() || isStopping()) || !Objects.equals(mIface, iface)) {
            return;
        }

        Slog.i(TAG, "interface " + iface + " removed");
        // Ensure clatd is stopped if stop() has not be called: this likely means that clatd has
        // crashed.
        enterStoppingState();
        enterIdleState();
        LinkProperties lp = new LinkProperties(mNetwork.linkProperties);
