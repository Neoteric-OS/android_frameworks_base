...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    /**
     * Flag used to indicate whether the legacy files need to be imported, and retry count
     * before giving up.
     */
    ...
    private void maybeUpgradeLegacyStatsLocked() {
        ...
        try {
            ...
            maxEndMillis = maybeApplyImportCollection(mDevRecorder, maxEndMillis);
            maxEndMillis = maybeApplyImportCollection(mXtRecorder, maxEndMillis);
            maxEndMillis = maybeApplyImportCollection(mUidRecorder, maxEndMillis);
            maxEndMillis = maybeApplyImportCollection(mUidTagRecorder, maxEndMillis);
            setDeviceConfigInt(NETSTATS_IMPORT_LEGACY_FILE_NEEDED, 0);
        } catch (IOException | OutOfMemoryError e) {
            mDevRecorder.clearWithInTimeRangeLocked(Long.MIN_VALUE, maxEndMillis);
            mXtRecorder.clearWithInTimeRangeLocked(Long.MIN_VALUE, maxEndMillis);
            mUidRecorder.clearWithInTimeRangeLocked(Long.MIN_VALUE, maxEndMillis);
            mUidTagRecorder.clearWithInTimeRangeLocked(Long.MIN_VALUE, maxEndMillis);
            Log.wtf(TAG, "problem during legacy upgrade", e);
            // For debug build, if fails, retry forever to expose the bug.
            setDeviceConfigInt(NETSTATS_IMPORT_LEGACY_FILE_NEEDED, importNeeded - 1);
        }
    }

    private long maybeApplyImportCollection(NetworkStatsRecorder recorder, long maxEndMillis)
        ...
        if (collection == null || collection.isEmpty()) return maxEndMillis;
