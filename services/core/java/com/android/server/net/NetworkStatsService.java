...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    private void dumpNormal(FileDescriptor fd, PrintWriter rawWriter, String[] args,
        ...
        synchronized (mStatsLock) {
            // --proto will be parsed by PriorityDump which indicates to dump logs in protobuf
            // format. --incident will be passed by incident_report tool and it dumps complete
            // history, otherwise, to limit the size of bugreport zip file, it dumps since-boot
            // history when args only contains --proto.
            if (asProto) {
    ...
    private void dumpProtoLocked(FileDescriptor fd, boolean isIncidentReport) {
        ...
        mDevRecorder.dumpDebugLocked(
                proto, NetworkStatsServiceDumpProto.DEV_STATS, isIncidentReport);
        mXtRecorder.dumpDebugLocked(
                proto, NetworkStatsServiceDumpProto.XT_STATS, isIncidentReport);
        mUidRecorder.dumpDebugLocked(
                proto, NetworkStatsServiceDumpProto.UID_STATS, isIncidentReport);
        mUidTagRecorder.dumpDebugLocked(
                proto, NetworkStatsServiceDumpProto.UID_TAG_STATS, isIncidentReport);
