    ...
    private void updateIfacesLocked(@Nullable Network[] defaultNetworks,
        ...
        for (NetworkState state : states) {
            if (state.networkInfo.isConnected()) {
                ...
                //
                // Additionally for ingress we drop in ip6tables raw prerouting and thus also
                // only account on the stacked interface.
                //
                ...
                // (or non eBPF offloaded) TX they would appear on both, but we explicitly bypass
                // interface counting on egress from the clat uid.
                //
