 ...
 *
 * <p>When there are multiple telephony sources, the one with the lowest ID is used unless the data
 * becomes too stale.
 *
...
public final class TimeDetectorStrategyImpl implements TimeDetectorStrategy {
    ...
    public interface Environment {
        ...
        /**
         * The absolute threshold below which the system clock can be updated. i.e. if the detector
         * receives a high-confidence time and the current system clock is +/- this value from that
         * time, then the device's confidence in the current time can be upgraded.
         */
        int systemClockConfidenceUpgradeThresholdMillis();
        ...
        @CurrentTimeMillisLong long systemClockMillis();
    ...
    public synchronized void dump(@NonNull IndentingPrintWriter ipw, @Nullable String[] args) {
        ...
        ipw.println("mEnvironment.systemClockConfidenceUpgradeThresholdMillis()="
                + mEnvironment.systemClockConfidenceUpgradeThresholdMillis());
    ...
    @GuardedBy("this")
    private boolean upgradeSystemClockConfidenceIfRequired(
        ...
        try {
            ...
            return updateConfidenceRequired;
        } finally {
            mEnvironment.releaseWakeLock();
        }
    }
