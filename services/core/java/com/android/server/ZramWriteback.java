...
public final class ZramWriteback extends JobService {
    ...
    private static final String PERIODIC_WB_DELAY_PROP = "ro.zram.periodic_wb_delay_mins";
    ...
    private void flushIdlePages() {
        Slog.d(TAG, "Start writing back idle pages to disk");
        ...
        Slog.d(TAG, "Finished writeback back idle pages");
    }

    private int getWbPageCount() {
    ...
    private void markAndFlushPages() {
        int pageCount = getWbPageCount();

        flushIdlePages();
        markPagesAsIdle();

        if (pageCount != -1) {
            Slog.i(TAG, "Total pages written to disk is " + (getWbPageCount() - pageCount));
        }
    ...
    private static boolean isWritebackEnabled() {
        for (int i = 0; i < MAX_ZRAM_DEVICES; i++) {
            String zramPath = String.format("/sys/block/zram%d", i);
            File file = new File(zramPath);
            if (!file.exists()) {
                break;
            }
            ...
            try {
                String backingDev = FileUtils
                        .readTextFile(new File(zramPath + "/backing_dev"), 128, "");
    ...
    public static void scheduleZramWriteback(Context context) {
        ...
        // After the initial writeback, subsequent writebacks are done at 24 hour intervals
        js.schedule(new JobInfo.Builder(WRITEBACK_IDLE_JOB_ID, sZramWriteback)
