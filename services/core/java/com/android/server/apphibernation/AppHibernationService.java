...
public final class AppHibernationService extends SystemService {
    ...
    public boolean isHibernating(String packageName, int userId) {
        ...
        try {
            userId = mIActivityManager.handleIncomingUser(Binder.getCallingPid(),
                    callingUid, userId, false, true, "isHibernating", null);
    ...
    public void setHibernating(String packageName, int userId, boolean isHibernating) {
        ...
        try {
            userId = mIActivityManager.handleIncomingUser(Binder.getCallingPid(),
                    callingUid, userId, false, true, "setHibernationState", null);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
        synchronized (mLock) {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_PACKAGE_ADDED.equals(action) || ACTION_PACKAGE_REMOVED.equals(action)) {
                final Uri pkgData = intent.getData();
                if (pkgData == null) {
                    return;
                }
                final String packageName = pkgData.getSchemeSpecificPart();
                if (packageName == null) {
                    return;
                }
     ...
     */
    private static class UserPackageState {
