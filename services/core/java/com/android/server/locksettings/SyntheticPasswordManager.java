    ...
    public Pair<VerifyCredentialResponse, byte[]> verifySpecialUserCredential(
        ...
        if (persistentData.type == PersistentData.TYPE_SP_GATEKEEPER) {
            ...
            VerifyCredentialResponse response =
                    weaverVerify(weaver, data.weaverSlotData, stretchedLskf).stripPayload();
            byte[] frpSecret = null;
            if (response.getResponseCode() == VerifyCredentialResponse.RESPONSE_OK) {
                frpSecret =
                        mStorage.deactivateFrpSecretProtection(data.weaverSlotData.authBoundKey);
            }
            return Pair.create(response, frpSecret);
        } else {
            Slog.e(
                    TAG,
