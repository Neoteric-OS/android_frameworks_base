 ...
 * LockscreenCredential.  The LSKF may be empty (none).  There may be escrow token-based protectors
 * as well, only for specific use cases such as enterprise-managed users.
 *
 * - The user's credential-encrypted storage is always protected by the SP.
 *
 * - The user's Keystore superencryption keys are always protected by the SP.  These in turn protect
 * the Keystore keys that require user authentication, an unlocked device, or both.
 *
 ...
 *
 * Files stored on disk for each user: For the SP itself, stored under NULL_PROTECTOR_ID:
 ...
 *
 * For each protector, stored under the corresponding protector ID: SP_BLOB_NAME: The encrypted SP
 * secret (the SP itself or the P0 value).  Always exists. PASSWORD_DATA_NAME: Data used for LSKF
...
class SyntheticPasswordManager {
    ...
    private static VerifyCredentialResponse responseFromTimeout(GateWeaverVerifyResponse response) {
        int timeout =
            response.timeout > Integer.MAX_VALUE || response.timeout < 0 ? Integer.MAX_VALUE
    ...
    private @Nullable IGateWeaver getWeaverAidlService() {
        ...
        try {
            aidlWeaver = IGateWeaver.Stub.asInterface(
                ServiceManager.waitForDeclaredService(IGateWeaver.DESCRIPTOR + "/default"));
...
*/
        Slog.i(TAG, "=*=*=*=*= Found AIDL gateweaver service v" + aidlVersion);
        return aidlWeaver;
    }
    ...
    private synchronized @Nullable IGateWeaver getWeaverService() {
        ...
        if (weaverConfig == null || weaverConfig.slots <= 0 || weaverConfig.timeRequired) {
     ...
     *
     * @return the protector secret or null if the operation fails
     */
    private WeaverSlotData weaverEnroll(IGateWeaver weaver, int slot, byte[] key) {
        ...
        try {
            ...
            Slog.e(TAG, "gateweaver create arc call failed with SecurityException, slot: " + slot,
                e);
            return null;
        }
    }
     ...
     * returned.
     */
    private VerifyCredentialResponse weaverVerify(IGateWeaver weaver, WeaverSlotData data,
        byte[] key) {
        int slot = data.slot;
        if (slot == INVALID_WEAVER_SLOT || slot >= mWeaverConfig.slots) {
            throw new IllegalArgumentException("Invalid slot for weaver");
        ...
        try {
            verifyResponse = gateWeaverVerify(data, key);
        } catch (RemoteException e) {
        ...
        switch (verifyResponse.status) {
            case GateWeaverVerifyResponse.OK:
                return new VerifyCredentialResponse.Builder().setGatekeeperHAT(
                    verifyResponse.authBoundKey).build();
            ...
            default:
                Slog.e(TAG,
                    "weaver read unknown status " + readResponse.status + ", slot: " + slot);
                return VerifyCredentialResponse.ERROR;
        }
    }
     ...
     * <p>
     * Also saves the escrow information necessary to re-generate the synthetic password under an
     ...
     * initial LSKF-based protector, are efficiently created with only a single
     * {@link syncState()}.
     */
    ...
    public void newSidForUser(IGateKeeperService gatekeeper, SyntheticPassword sp, int userId) {
        ...
        if (response.getResponseCode() != GateKeeperResponse.RESPONSE_OK) {
            throw new IllegalStateException(
                "Fail to create new SID for user " + userId + " response: "
    ...
    public boolean hasEscrowData(int userId) {
        return hasState(SP_E0_NAME, NULL_PROTECTOR_ID, userId) && hasState(SP_P1_NAME,
            NULL_PROTECTOR_ID, userId);
    }
     ...
     * for calling {@link syncState()} afterwards, once all the protector's files have been
     * created.
     */
    private void saveWeaverSlot(WeaverSlotData slot, long protectorId, int userId) {
        saveState(WEAVER_SLOT_NAME, slot.toBytes(), protectorId, userId);
    }
    ...
    private void destroyWeaverSlot(long protectorId, int userId) {
        ...
        if (weaverSlot.slot != INVALID_WEAVER_SLOT) {
            ...
            if (!usedSlots.contains(weaverSlot.slot)) {
                Slogf.i(TAG, "Erasing Weaver slot %d", weaverSlot.slot);
                weaverInvalidate(weaver, weaverSlot);
                mPasswordSlotManager.markSlotDeleted(weaverSlot.slot);
            } else {
                Slogf.i(TAG, "Weaver slot %d was already reused; not erasing it", weaverSlot.slot);
            }
        }
    }
     ...
     * {@link UserManager#getUsers}). As a result their weaver slots will not be considered taken
     * and can be reused by new users. Care should be taken when cleaning up the deleted user in
     * {@link #removeUser}, to prevent a reused slot from being erased unintentionally.
     */
    private Set<Integer> getUsedWeaverSlots() {
        Map<Integer, List<Long>> protectorIds = mStorage.listSyntheticPasswordProtectorsForAllUsers(
            WEAVER_SLOT_NAME);
    ...
    public long createLskfBasedProtector(IGateKeeperService gatekeeper,
        ...
        if (weaver != null) {
            ...
            protectorSecret = weaverSlotData.weaverSecret;
        } else {
            ...
            if (!credential.isNone()) {
                ...
                try {
                    ...
                    throw new IllegalStateException(
                        "Failed to enroll LSKF for new SP protector" + " for user " + userId, e);
                }
                if (response.getResponseCode() != GateKeeperResponse.RESPONSE_OK) {
                    throw new IllegalStateException(
    ...
    private int derivePinLength(int sizeOfCredential, boolean isPinCredential, int userId) {
        if (!isPinCredential || !mStorage.isAutoPinConfirmSettingEnabled(userId)
    ...
    public void migrateFrpPasswordLocked(long protectorId, UserInfo userInfo,
        ...
        if (mStorage.getPersistentDataBlockManager() != null
            ...
            PasswordData pwd = PasswordData.fromBytes(
                loadState(PASSWORD_DATA_NAME, protectorId, userInfo.id));
    ...
    private void synchronizeWeaverFrpPassword(@Nullable PasswordData pwd, int requestedQuality,
        int userId, WeaverSlotData weaverSlot) {
        if (shouldSynchronizeFrpCredential(pwd, userId)) {
            ...
            if (!isNoneCredential(pwd)) {
                ...
                mStorage.writePersistentDataBlock(PersistentData.TYPE_SP_WEAVER, weaverSlot.slot,
                    requestedQuality, data.toBytes());
            } else {
    ...
    public boolean writeRepairModeCredentialLocked(long protectorId, int userId) {
        ...
        if (weaverSlot.slot != INVALID_WEAVER_SLOT) {
            ...
            data.passwordData = pwd;
            data.weaverSlotData = weaverSlot;
            mStorage.writeRepairModePersistentData(PersistentData.TYPE_SP_WEAVER, weaverSlot.slot,
                data.toBytes());
        } else {
            // write gatekeeper password
            mStorage.writeRepairModePersistentData(PersistentData.TYPE_SP_GATEKEEPER, userId,
                pwd.toBytes());
        }
        return true;
    }
    ...
    public long addPendingToken(byte[] token, @TokenType int type, int userId,
        ...
        if (getWeaverService() != null) {
            ...
            tokenData.weaverSecret = SyntheticPasswordCrypto.personalizeHash(
                PERSONALIZATION_WEAVER_TOKEN, token);
        } else {
    ...
    public boolean createTokenBasedProtector(long tokenHandle, SyntheticPassword sp, int userId) {
        ...
        if (weaver != null) {
            ...
            tokenData.secdiscardableOnDisk = SyntheticPasswordCrypto.encrypt(protectorSecret,
                PERSONALIZATION_WEAVER_TOKEN, tokenData.secdiscardableOnDisk);
            tokenData.aggregatedSecret = transformUnderSecdiscardable(token, secdiscardable);
    ...
    private void createSyntheticPasswordBlob(long protectorId, byte protectorType,
         ...
         */
        byte version = sp.mVersion == SYNTHETIC_PASSWORD_VERSION_V3 ? SYNTHETIC_PASSWORD_VERSION_V3
    ...
    public AuthenticationResult unlockLskfBasedProtector(IGateKeeperService gatekeeper,
        ...
        if (result.syntheticPassword != null && !credential.isNone() && !hasPasswordMetrics(
            protectorId, userId)) {
     ...
     */
    public boolean refreshPinLengthOnDisk(PasswordMetrics passwordMetrics, long protectorId,
        int userId) {
        if (!isAutoPinConfirmationFeatureAvailable()) {
            return false;
        }
     ...
     * verification to refresh the SID and HardwareAuthToken maintained by the system.
     */
    public @NonNull AuthenticationResult unlockTokenBasedProtector(IGateKeeperService gatekeeper,
        long protectorId, byte[] token, int userId) {
    ...
    private @NonNull AuthenticationResult unlockTokenBasedProtectorInternal(
        IGateKeeperService gatekeeper, long protectorId, byte expectedProtectorType, byte[] token,
        int userId) {
        AuthenticationResult result = new AuthenticationResult();
        byte[] secdiscardable = loadSecdiscardable(protectorId, userId);
        if (secdiscardable == null) {
        ...
        if (weaverSlot.slot != INVALID_WEAVER_SLOT) {
            ...
            byte[] weaverToken = SyntheticPasswordCrypto.personalizedHash(
                PERSONALIZATION_WEAVER_TOKEN, token);
    ...
    protected @Nullable VerifyCredentialResponse verifyChallengeInternal(
        ...
        int responseCode = response.getResponseCode();
        if (responseCode == GateKeeperResponse.RESPONSE_OK) {
            VerifyCredentialResponse result = new VerifyCredentialResponse.Builder().setGatekeeperHAT(
                response.getPayload()).build();
            if (response.getShouldReEnroll()) {
                try {
    ...
    public void destroyTokenBasedProtector(long protectorId, int userId) {
        ...
        SyntheticPasswordBlob blob = SyntheticPasswordBlob.fromBytes(
            loadState(SP_BLOB_NAME, protectorId, userId));
    ...
    public void destroyAllWeakTokenBasedProtectors(int userId) {
        List<Long> protectorIds = mStorage.listSyntheticPasswordProtectorsForUser(SP_BLOB_NAME,
            userId);
     ...
     * is needed, call {@link syncState()} afterwards, preferably after batching up related
     * updates.
     */
    ...
    public boolean migrateKeyNamespace() {
        ...
        final Map<Integer, List<Long>> allProtectors = mStorage.listSyntheticPasswordProtectorsForAllUsers(
            SP_BLOB_NAME);
    ...
    public void writeVendorAuthSecret(@NonNull final byte[] vendorAuthSecret,
        @NonNull final SyntheticPassword sp, @UserIdInt final int userId) {
        final byte[] encrypted = SyntheticPasswordCrypto.encrypt(
    ...
    public @Nullable byte[] readVendorAuthSecret(@NonNull final SyntheticPassword sp,
        @UserIdInt final int userId) {
        final byte[] encrypted = loadState(VENDOR_AUTH_SECRET_NAME, NULL_PROTECTOR_ID, userId);
        if (encrypted == null) {
            return null;
        }
        return SyntheticPasswordCrypto.decrypt(sp.deriveVendorAuthSecretEncryptionKey(),
    ...
    static class SyntheticPassword {
        ...
        static SyntheticPassword create() {
            ...
            byte[] escrowSplit0 = SecureRandomUtils.randomBytes(
                SYNTHETIC_PASSWORD_SECURITY_STRENGTH);
            byte[] escrowSplit1 = SecureRandomUtils.randomBytes(
                SYNTHETIC_PASSWORD_SECURITY_STRENGTH);
        ...
        private byte[] deriveSubkey(byte[] personalization) {
            if (mVersion == SYNTHETIC_PASSWORD_VERSION_V3) {
                return (new SP800Derive(mSyntheticPassword)).withContext(personalization,
         ...
         * {@link SyntheticPassword#getEscrowSecret}). Escrow data needs to be loaded by
         * {@link #setEscrowData} before calling this.
         */
        ...
        private void recreate(byte[] escrowSplit0, byte[] escrowSplit1) {
            mSyntheticPassword = bytesToHex(
                SyntheticPasswordCrypto.personalizedHash(PERSONALIZATION_SP_SPLIT, escrowSplit0,
                    escrowSplit1));
        }
        ...
        /**
         * Returns the raw synthetic password, for later use with
         * {@link #recreateDirectly(byte[])}.
         */
    ...
    static class PasswordData {
        ...
        public byte[] toBytes() {
            ...
            ByteBuffer buffer = ByteBuffer.allocate(
                Integer.BYTES + 3 * Byte.BYTES + Integer.BYTES + salt.length + Integer.BYTES + (
