...
public class LockSettingsService extends ILockSettings.Stub {
    ...
    private void removeUserState(@UserIdInt int userId) {
        ...
        mStrongAuth.clearUserState(userId);
    ...
    public void userPresent(int userId) {
        if (!android.security.Flags.clearStrongAuthOnSuccessfulCredentialVerification()) {
            checkWritePermission();
        ...
        if (isCredentialSharableWithParent(userId)) {
            if (getSeparateProfileChallengeEnabledInternal(userId)) {
                ...
                if (!android.security.Flags.clearStrongAuthOnSuccessfulCredentialVerification()) {
                    ...
                    // is cleared after the user successfully confirms the separate challenge to
                    // enter
    ...
    private long setLockCredentialWithSpLocked(LockscreenCredential credential,
        ...
        if (!credential.isNone()) {
            ...
            if (!mSpManager.hasSidForUser(userId)) {
                ...
                if (android.security.Flags.resetStrongAuthOnAddAndClearPrimaryCredential()) {
                    mStrongAuth.reportUnlock(userId);
                }
            }
        } else {
            ...
            removeBiometricsForUser(userId);
            if (android.security.Flags.resetStrongAuthOnAddAndClearPrimaryCredential()) {
                // Users with no credentials have no strong auth value. Clear the user state rather
                // than set it to STRONG_AUTH_NOT_REQUIRED, resulting in the default value.
                mStrongAuth.clearUserState(userId);
            }
        }
