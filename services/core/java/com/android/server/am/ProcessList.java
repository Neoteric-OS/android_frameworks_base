...
public final class ProcessList {
  ...
  /**
   * Apps have no access to the private data directories of any other app, even if the other app has
   * made them world-readable.
   */
  @ChangeId
  ...
  /**
   * A global counter for generating sequence numbers. This value will be used when incrementing
   * sequence numbers in individual uidRecords.
   *
   * Having a global counter ensures that seq numbers are monotonically increasing for a particular
   * uid even when the uidRecord is re-created.
   */
  @VisibleForTesting
  ...
  /**
   * List of running applications, sorted by recent usage. The first entry in the list is the least
   * recently used.
   */
  ...
  /**
   * Where in mLruProcesses that the processes hosting services start. This is after (lower index)
   * than mLruProcessesActivityStart.
   */
  @CompositeRWLock({"mService", "mProcLock"})
  ...
  /**
   * The server socket in system_server, zygote will connect to it in order to send unsolicited
   * messages to system_server.
   */
  private LocalSocket mSystemServerSocketForZygote;

  /**
   * Maximum number of bytes that an incoming unsolicited zygote message could be. To be updated if
   * new message type needs to be supported.
   */
  ...
  private final String PROPERTY_SDK_SANDBOX_ENFORCE_RESTRICTIONS_NEXT = "enforce_restrictions_next";
  private static final boolean DEFAULT_SDK_SANDBOX_ENFORCE_RESTRICTIONS_NEXT = false;
  ...
  @VisibleForTesting(visibility = VisibleForTesting.Visibility.PRIVATE)
  void setProcessListSettingsListener(ProcessListSettingsListener listener) {
    synchronized (mService) {
      mProcessListSettingsListener = listener;
    }
  }

  class ProcessListSettingsListener implements DeviceConfig.OnPropertiesChangedListener {

    private final Context mContext;
    private final Object mLock = new Object();

    @GuardedBy("mLock")
    private boolean mSdkSandboxEnforceRestrictionsNext =
        DeviceConfig.getBoolean(
            DeviceConfig.NAMESPACE_ADSERVICES,
            PROPERTY_SDK_SANDBOX_ENFORCE_RESTRICTIONS_NEXT,
            DEFAULT_SDK_SANDBOX_ENFORCE_RESTRICTIONS_NEXT);
    ...
    boolean enforceSdkSandboxRestrictionsNext() {
      synchronized (mLock) {
        return mSdkSandboxEnforceRestrictionsNext;
      }
    }
    ...
    public void onPropertiesChanged(@NonNull DeviceConfig.Properties properties) {
      synchronized (mLock) {
        ...
        for (String name : properties.getKeyset()) {
          ...
          switch (name) {
            case PROPERTY_SDK_SANDBOX_ENFORCE_RESTRICTIONS_NEXT:
              // TODO(b/2527286): Restart existing sdk sandbox processes if this changes.
              mSdkSandboxEnforceRestrictionsNext =
  ...
  final class IsolatedUidRangeAllocator {
    ...
    /**
     * We map the uid range [mFirstUid, mFirstUid + mNumUidRanges * mNumUidsPerRange) back to an
     * underlying bitset of [0, mNumUidRanges) and allocate out of that.
     */
    @GuardedBy("ProcessList.this.mService")
  ...
  /**
   * All of the applications we currently have running organized by name. The keys are strings of
   * the application package name (as returned by the package manager), and the keys are
  ...
  /**
   * Return the maximum pss size in kb that we consider a process acceptable to restore from its
   * cached state for running in the background when RAM is low.
   */
  ...
  /**
   * Set the out-of-memory badness adjustment for a process. If {@code pid <= 0}, this method will
   * be a no-op.
   *
  ...
  boolean startProcessLocked(ProcessRecord app, HostingRecord hostingRecord,
    ...
    try {
      ...
      android.util.Log.i("TAG", "the seinfo is set to : " + seInfo);
      if (mProcessListSettingsListener.enforceSdkSandboxRestrictionsNext()
          && Process.isSdkSandboxUid(app.info.uid)) {
        android.util.Log.i("TAG",
            "enforceSdkSandboxRestrictionsNext is set to true and we're considering a sandbox UID."
                + " Does it contain domain= "
                + seInfo.contains("domain="));
        int domainStart = seInfo.indexOf("domain=") + 7;
        int domainEnd = seInfo.indexOf(",", domainStart);
        final String updatedSeInfo =
            seInfo.substring(0, domainStart) + "sdk_sandbox_next" + seInfo.substring(domainEnd);
        android.util.Log.i("TAG", "updated seInfo is: " + updatedSeInfo);
        //seInfo = newSeInfo;
      }
  ...
  ProcessRecord startProcessLocked(String processName, ApplicationInfo info,
    ...
    //     already running.
    if (DEBUG_PROCESSES) {
      Slog.v(TAG_PROCESSES, "startProcess: name=" + processName
  ...
  private int updateLruProcessInternalLSP(ProcessRecord app, long now, int index,
    ...
    if (DEBUG_LRU) {
      Slog.d(TAG_LRU, "Moving dep from " + lrui + " to " + index
          + " in LRU list: " + app);
    }
  ...
  /**
   * Handle the case where we are inserting a process hosting client activities: Make sure any
   * groups have their order match their importance, and take care of distributing old clients
   * across other activity processes so they can't spam the LRU list.  Processing of the list will
   * be restricted by the indices provided, and not extend out of them.
   *
   ...
   *     of whichever section of the LRU list we are in.  Nothing past it will be touched.
  ...
  private void updateLruProcessLSP(ProcessRecord app, ProcessRecord client,
    ...
    if (hasActivity) {
      ...
      // Process has services, put it at the top of the service list.
      if (DEBUG_LRU) {
        Slog.d(TAG_LRU, "Adding to top of LRU service list: " + app);
      }
      ...
      if (client != null) {
        ...
        int clientIndex = mLruProcesses.lastIndexOf(client);
        if (DEBUG_LRU && clientIndex < 0) {
          Slog.d(TAG_LRU, "Unknown client " + client
              + " when updating " + app);
        }
        if (clientIndex <= lrui) {
  ...
  /**
   * Iterate the whole LRU list, invoke the given {@code callback} with each of the ProcessRecord in
   * that list.
   *
   * @param iterateForward If {@code true}, iterate the LRU list from the least recent used to
   *     most recent used ProcessRecord.
  ...
  /**
   * Search in the LRU list, invoke the given {@code callback} with each of the ProcessRecord in
   * that list; if the callback returns a non-null object, halt the search, return that object as
   * the return value of this search function.
   *
   * @param iterateForward If {@code true}, iterate the LRU list from the least recent used to
   *     most recent used ProcessRecord.
   * @param callback The callback interface to accept the current ProcessRecord; if it returns a
   *     non-null object, the search will be halted and this object will be used as the return value
   *     of this search function.
   */
  @GuardedBy(anyOf = {"mService", "mProfileLock"})
  ...
  /**
   * Returns the uid's process state or {@link ActivityManager#PROCESS_STATE_NONEXISTENT} if not
   * running
   */
  @GuardedBy(anyOf = {"mService", "mProcLock"})
  ...
  /**
   * Returns the uid's process capability or {@link ActivityManager#PROCESS_CAPABILITY_NONE} if not
   * running
   */
  @GuardedBy(anyOf = {"mService", "mProcLock"})
  @ProcessCapability
  ...
  /**
   * Call {@link ActivityManagerService#doStopUidLocked} (which will also stop background services)
   * for all idle UIDs.
   */
  @GuardedBy("mService")
  ...
  /**
   * Checks if the uid is coming from background to foreground or vice versa and returns appropriate
   * block state based on this.
   *
   * @return blockState based on whether the uid is coming from background to foreground or vice
   *     versa. If bg->fg or fg->bg, then {@link #NETWORK_STATE_BLOCK} or
   *     {@link #NETWORK_STATE_UNBLOCK} respectively, otherwise {@link #NETWORK_STATE_NO_CHANGE}.
   */
  @VisibleForTesting
  @GuardedBy(anyOf = {"mService", "mProcLock"})
   ...
   * {@link ProcessList#mProcStateSeqCounter} and checks if any uid is coming from background to
   * foreground or vice versa and if so, notifies the app if it needs to block.
   */
  @VisibleForTesting
  @GuardedBy(anyOf = {"mService", "mProcLock"})
  ...
  /**
   * Create a server socket in system_server, zygote will connect to it in order to send unsolicited
   * messages to system_server.
   */
   ...
   *
   * @return A future timestamp when the app should be killed at, or a 0 if it shouldn't be killed
  ...
  /**
   * Called by {@link ActivityManagerService#enqueueUidChangeLocked} only, it doesn't schedule the
   * standy killing checks because it should have been scheduled before enqueueing UID idle
   * changed.
   */
  @GuardedBy("mService")
  ...
  private final class ImperceptibleKillRunner extends IUidObserver.Stub {
     ...
     *
     * @return True if the process is killed, or it's gone already, or we are not allowed to kill it
     *     (one of the packages in this process is being exempted).
     */
    @GuardedBy("mService")
