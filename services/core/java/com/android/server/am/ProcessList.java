...
public final class ProcessList {
    ...
    private int decideTaggingLevel(ProcessRecord app) {
        ...
        if (Zygote.nativeSupportsTaggedPointers() && level != Zygote.MEMORY_TAG_LEVEL_NONE) {
            return Zygote.MEMORY_TAG_LEVEL_TBI;
        }
    ...
    private boolean enableNativeHeapZeroInit(ProcessRecord app) {
        // Look at the process attribute first.
        if (app.processInfo != null && app.processInfo.nativeHeapZeroInit) {
            return true;
        }
        // Then at the application attribute.
        if (app.info.getNativeHeapZeroInit()) {
            return true;
        }
        // Compat feature last.
