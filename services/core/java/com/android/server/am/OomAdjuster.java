        ...
        for (int is = psr.numberOfRunningServices() - 1;
            ...
            for (int conni = serviceConnections.size() - 1;
                ...
                for (int i = 0;
                    ...
                    if (computeClients) {
                        final int clientCachedAdj = cstate.getCurRawAdj() >= ProcessList.CACHED_APP_MIN_ADJ
                                ? cstate.getCurRawAdj() : cachedAdj;
                        computeOomAdjLSP(client, clientCachedAdj, topApp, doingAll, now,
                                cycleReEval, true);
                    } else {
        ...
        for (int provi = ppr.numberOfProviders() - 1;
            ...
            for (int i = cpr.connections.size() - 1;
                ...
                if (computeClients) {
                    final int clientCachedAdj = cstate.getCurRawAdj() >= ProcessList.CACHED_APP_MIN_ADJ
                            ? cstate.getCurRawAdj() : cachedAdj;
                    computeOomAdjLSP(client, clientCachedAdj, topApp, doingAll, now, cycleReEval, true);
                } else {
                    cstate.setCurRawAdj(cstate.getCurAdj());
                    cstate.setCurRawProcState(cstate.getCurProcState());
                }
