...
//                    options != null ? options.toBundle() : null, mUserId);
    ...
    private int runCompat(PrintWriter pw) {
        ...
        if (opt != null) {
            if (opt.equals("--by-name")) {
                changeId = config.lookupChangeId(getNextArgRequired());
            } else {
            ...
            try {
                changeId = Long.parseLong(nextArg);
            } catch (NumberFormatException e) {
                pw.println("Invalid change id " + nextArg);
                return -1;
            }
        }
        if (changeId == -1) {
            pw.println("Unknown or invalid change!");
            return -1;
        }
        String packageName = getNextArgRequired();
        switch(toggleValue) {
    ...
    static void dumpHelp(PrintWriter pw, boolean dumping) {
        if (dumping) {
            ...
            pw.println("  compat enable|disable|set-default <CHANGE_ID|--by-name CHANGE_NAME> <PACKAGE_NAME>");
