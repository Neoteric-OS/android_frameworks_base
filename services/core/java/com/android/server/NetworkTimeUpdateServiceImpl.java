...
public class NetworkTimeUpdateServiceImpl extends Binder implements NetworkTimeUpdateService {
    ...
    private void onPollNetworkTimeUnderWakeLock(int event) {
        ...
        NtpTrustedTime.NtpResult cachedNtpResult = mTime.getCachedNtpResult();
        if (cachedNtpResult == null || cachedNtpResult.getAgeMillis() >= mPollingIntervalMs) {
            if (DBG) Log.d(TAG, "Stale NTP fix; forcing refresh");
            mTime.forceRefresh();
            cachedNtpResult = mTime.getCachedNtpResult();
        }
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        NtpTrustedTime.NtpResult ntpResult = mTime.getCachedNtpResult();
