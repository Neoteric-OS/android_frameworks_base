...
final class WifiDisplayController implements DumpUtils.Dump {
    ...
    private void setWfdInfo(boolean enabled) {
        ...
        mWifiP2pManager.setWfdInfo(mWifiP2pChannel, wfdInfo, new ActionListener() {
            ...
            public void onSuccess() {
                if (DEBUG) {
                    Slog.d(TAG, "Successfully set WFD info to " + (enabled ? "enabled" : "disabled"));
                }
            }
            ...
            if (mWfdEnabled || mWfdEnabling) {
                if (mWifiP2pManager != null && mWifiP2pChannel != null) {
                    ...
                    mWifiP2pManager.setWfdInfo(mWifiP2pChannel, wfdInfo, new ActionListener() {
                        ...
                        @Override
                        public void onFailure(int reason) {
                            if (DEBUG) Slog.d(TAG, "Failed to set WFD info to disabled with reason " + reason + ".");
                        }
                    });
                }
            }
    ...
    private void stopPeerDiscovery() {
        if (mWifiP2pManager != null) {
            mWifiP2pManager.stopPeerDiscovery(mWifiP2pChannel, new ActionListener() {
                ...
                @Override
                public void onFailure(int reason) {
                    if (DEBUG) Slog.d(TAG, "Stop peer discovery failed with reason " + reason + ".");
                }
            });
        }
    }
    ...
    private void updateConnection() {
        ...
        if (mDesiredDevice == null) {
            if (mWifiDisplayCertMode) mListener.onDisplaySessionInfo(getSessionInfo(mConnectedDeviceGroupInfo, 0));
            unadvertiseDisplay();
            destroyWifiP2pChannel();
            return; // done
        }
            ...
            public void onGroupInfoAvailable(WifiP2pGroup info) {
                ...
                if (contains(info, device)) {
                    ...
                    mWifiP2pManager.removeGroup(mWifiP2pChannel, new ActionListener() {
                        ...
                        @Override
                        public void onFailure(int reason) { Slog.i(TAG, "Failed to disconnect the old device: reason=" + reason); }
                    });
                }
            }
        });
