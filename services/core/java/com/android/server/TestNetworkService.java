...
class TestNetworkService extends ITestNetworkManager.Stub {
    ...
    public class TestNetworkAgent extends NetworkAgent implements IBinder.DeathRecipient {
        ...
        // Cache NetworkCapabilities for this Network for this Network for VCN Network Policy
        // updates.
        ...
        @NonNull private LinkProperties mLinkProperties;
    ...
    private TestNetworkAgent registerTestNetworkAgent(
        ...
        final long token = Binder.clearCallingIdentity();
        final NetworkCapabilities vcnNetworkPolicyCapabilities;
        try {
            ...
            vcnNetworkPolicyCapabilities = getVcnNetworkPolicyCapabilities(nc, vcnNetworkPolicy);
        } finally {
            Binder.restoreCallingIdentity(token);
        }

        final TestNetworkAgent agent = new TestNetworkAgent(context, looper,
                vcnNetworkPolicyCapabilities, lp, new NetworkAgentConfig.Builder().build(),
                callingUid, binder, mNetworkProvider);
        agent.register();
        agent.markConnected();
        return agent;
    }

    private static NetworkCapabilities getVcnNetworkPolicyCapabilities(
            @NonNull NetworkCapabilities nc, @NonNull VcnNetworkPolicyResult vcnNetworkPolicy) {
        final NetworkCapabilities.Builder ncBuilder = new NetworkCapabilities.Builder(nc);
        if (!vcnNetworkPolicy
                .getNetworkCapabilities()
                .hasCapability(NET_CAPABILITY_NOT_VCN_MANAGED)) {
            ncBuilder.removeCapability(NET_CAPABILITY_NOT_VCN_MANAGED);
        }
    ...
    private void applyVcnNetworkPolicyLocked(@NonNull TestNetworkAgent networkAgent) {
        ...
        // VcnManagementService will only remove NOT_VCN_MANAGED (not add it), so explicitly add it
        // in case this Network regained NOT_VCN_MANAGED
        final NetworkCapabilities nc =
                new NetworkCapabilities.Builder(networkAgent.getNetworkCapabilities())
        ...
        if (vcnNetworkPolicy.isTeardownRequested()) {
            ...
            final NetworkCapabilities vcnNetworkPolicyCapabilities =
                    getVcnNetworkPolicyCapabilities(nc, vcnNetworkPolicy);
            networkAgent.updateNetworkCapabilities(vcnNetworkPolicyCapabilities);
        }
    }
}
