...
public class PackageTracker implements IntentHelper.Listener {
    ...
    private void throwIfDeviceSettingsOrAppsAreBad() {
        failIfNullOrEmpty(mUpdateAppPackageName, "Update app package name missing.");
        failIfNullOrEmpty(mDataAppPackageName, "Data app package name missing.");
        if (mFailedCheckRetryCount < 1) {
            throw failWithException("mFailedRetryCount=" + mFailedCheckRetryCount, null);
        }
        if (mCheckTimeAllowedMillis < 1000) {
            throw failWithException("mCheckTimeAllowedMillis=" + mCheckTimeAllowedMillis, null);
        }

        // Validate the updater application package.
        // TODO Permissions checks?
        // TODO An app stops being a priv-app after it is replaced by one in data. http://b/35995024
//        try {
...
//        }
    ...
    public synchronized void triggerUpdateIfNeeded(boolean packageChanged) {
        ...
        if (!updaterAppManifestValid || !dataAppManifestValid) {
            Slog.w(TAG, "No update triggered due to invalid application manifest entries."
    ...
    private void setCheckInProgress() {
        mLastTriggerTimestamp = mClockHelper.currentTimeMillis();
    }
    ...
    private boolean isCheckResponseOverdue() {
        ...
        return mClockHelper.currentTimeMillis() > mLastTriggerTimestamp + mCheckTimeAllowedMillis;
    }
    ...
    private static void failIfNullOrEmpty(String value, String message) {
        if (value == null || value.trim().isEmpty()) {
            throw failWithException(message, null);
        }
    }

    private static RuntimeException failWithException(String message, Throwable cause) {
