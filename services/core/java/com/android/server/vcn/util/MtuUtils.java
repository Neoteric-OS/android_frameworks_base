...
public class MtuUtils {
     ...
     *       algorithms).
     *   <li>The maximum MTU allowed by the configuration.
     * </ul>
     */
    public static int getMtu(List<ChildSaProposal> childProposals, int maxMtu, int underlyingMtu) {
        if (underlyingMtu <= 0) {
            return IPV6_MIN_MTU;
        }
        ...
        for (ChildSaProposal proposal : childProposals) {
            for (Pair<Integer, Integer> encryptionAlgoPair : proposal.getEncryptionAlgorithms()) {
                ...
                if (CRYPT_ALGORITHM_OVERHEAD.containsKey(algo)) {
                    maxCryptOverhead = max(maxCryptOverhead, CRYPT_ALGORITHM_OVERHEAD.get(algo));
                    continue;
                }

                hasUnknownAlgorithm = true;
            }

            for (Integer algo : proposal.getIntegrityAlgorithms()) {
        ...
        // If no algorithm found, default to min MTU
        if (hasUnknownAlgorithm) {
            Slog.wtf(TAG, "Unknown algorithm requested");
            return IPV6_MIN_MTU;
        }
