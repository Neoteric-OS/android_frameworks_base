 ...
 * @hide
 */
public class Vcn extends Handler implements VcnNetworkProvider.NetworkRequestListener {
    ...
    private Vcn(
        ...
        mVcnContext.getVcnNetworkProvider().registerListener(this);
    }
    ...
    public void teardownAsynchronously() {
        sendMessage(obtainMessage(MSG_CMD_TEARDOWN));
    }
    ...
    private void handleTeardown() {
        mVcnContext.getVcnNetworkProvider().unregisterListener(this);

        for (VcnGatewayConnection gatewayConnection : mVcnGatewayConnections.values()) {
            gatewayConnection.teardown();
        }
    }
    ...
    private boolean requestSatisfiedByGatewayConnectionConfig(
        ...
        for (int cap : config.getAllTunnelCapabilities()) {
            configCaps.addCapability(cap);
        }
        if (!config.isMetered()) {
            configCaps.addCapability(NET_CAPABILITY_NOT_METERED);
        }
        if (!config.isRoaming()) {
            configCaps.addCapability(NET_CAPABILITY_NOT_ROAMING);
        }
