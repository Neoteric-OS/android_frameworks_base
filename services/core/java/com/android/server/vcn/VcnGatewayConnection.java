...
public class VcnGatewayConnection extends StateMachine implements UnderlyingNetworkTrackerCallback {
    ...
    /**
     * Sent when underlying networks change, either to a new network or upon property changes
     *
     ...
     * @param obj @NonNull UnderlyingNetworkRecord instance. If all underlying networks are lost,
     *     EVENT_DISCONNECT_REQUESTED should be fired with a timeout.
     */
     ...
     *
     * @param arg1 the "current" token, used to prevent out-of-date signals from propagating
     * @param obj @NonNull The List<InetAddress> of the resolved host
     */
     ...
     *
     * <p>Only relevant in the Retry-timeout state.
     *
     ...
     *
     * @param arg1 the "current" token, used to prevent out-of-date signals from propagating
     * @param obj @Nullable The Exception that triggered the loss of the session, if any.
     */
     ...
     *
     * @param arg1 the "current" token, used to prevent out-of-date signals from propagating
     * @param arg2 the {@link IpSecManager.Direction} constant denoting the transform's direction.
     * @param obj @NonNull The created IpSecTransform. References do NOT have to be held in order
     *     for the transform to be kept alive.
     */
     ...
     *
     * @param arg1 the "current" token, used to prevent out-of-date signals from propagating
     * @param obj @NonNull The ChildSessionConfiguration representing the configured Child Session.
     */
    private static final int EVENT_SETUP_COMPLETE = 6;
     ...
     *
     * <p>This signal is often fired after a timeout in order to prevent disconnecting during
     * transient conditions, such as network switches. Upon new network availability, the signal is
     ...
     *
     * @param arg1 the "any" token, used to prevent out-of-date signals from propagating
     * @param obj @Nullable The String reason why a disconnect was requested, for debugging purposes
     */
     ...
     *   <li>Switching to a IKE session as the primary session
     * </ul>
     *
    ...
    public void onSelectedUnderlyingNetworkChanged(@Nullable UnderlyingNetworkRecord underlying) {
        ...
        if (getHandler() != null) {
            getHandler()
                    .removeMessages(
                            EVENT_DISCONNECT_REQUESTED, DISCONNECT_REASON_UNDERLYING_NETWORK_LOST);
        }
        sendMessage(EVENT_UNDERLYING_NETWORK_CHANGED, TOKEN_ANY, underlying);
    }

    private void sendMessage(int what, int token, Object obj) {
        super.sendMessage(what, token, ARG_NOT_PRESENT, obj);
    }

    private void sendMessageDelayed(int what, int token, Object obj, long timeout) {
    ...
    private void childTransformCreated(
        ...
        sendMessage(EVENT_TRANSFORM_CREATED, token, direction, transform);
    }

    private void childOpened(int token, @NonNull ChildSessionConfiguration childConfig) {
        sendMessage(EVENT_SETUP_COMPLETE, token, childConfig);
    }
    ...
    static NetworkCapabilities buildNetworkCapabilities(
        ...
        for (int cap : gatewayConnectionConfig.getAllTunnelCapabilities()) {
            caps.addCapability(cap);
        }

        if (!gatewayConnectionConfig.isMetered()) {
            caps.addCapability(NET_CAPABILITY_NOT_METERED);
        }
        if (!gatewayConnectionConfig.isRoaming()) {
