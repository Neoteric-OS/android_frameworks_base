 ...
 *               |
 *       Underlying Network
 *            changed                   +---Retriable error---+
 *               |                      |      occurred       |
 *               v                      |                     v
 * +----------------------------+       ^       +----------------------------+
 * |     DnsResolutionState     |<------|---+   |     RetryTimeoutState      |
 * |                            |       |   |   |                            |
 * |    Transitive state for    |       |   |   |    Transitive state for    |
 * | performing DNS resolution. |-------+   ^   | handling retriable errors. |
 * +----------------------------+       |   |   +----------------------------+
 *               |                      |   |                 |
 *         DNS resolution               |   +--------<--------+
 *           completed                  |
 ...
 * |      ConnectingState       |       |
 * |                            |       |
 * |    Transitive state for    |       |
 * |  starting IKE negotiation. |-------+
 * +----------------------------+       |
 *         |            +-----<-----+   |
 *    IKE session    Migration      |   |
 *     negotiated    complete       |   |
 *         |            |           |   |
 *         v            v           |   |
 * +----------------------------+   ^   ^
...
public class VcnGatewayConnection extends StateMachine implements UnderlyingNetworkTrackerCallback {
     ...
     * @param obj @NonNull An EventUnderlyingNetworkChangedData instance with relevant data.
     */
    private static final int EVENT_UNDERLYING_NETWORK_CHANGED = 1;

    private static class EventUnderlyingNetworkChangedData implements EventData {
        @NonNull public final UnderlyingNetworkRecord newUnderlying;

        EventUnderlyingNetworkChangedData(@NonNull UnderlyingNetworkRecord newUnderlying) {
            this.newUnderlying = Objects.requireNonNull(newUnderlying);
        }
        ...
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventUnderlyingNetworkChangedData)) {
                return false;
            }

            final EventUnderlyingNetworkChangedData rhs = (EventUnderlyingNetworkChangedData) other;
            return Objects.equals(newUnderlying, rhs.newUnderlying);
        }
    }

    /**
     * Sent when DNS resolution completes.
     *
     * <p>Only relevant in DNS resolution state.
     *
     * @param arg1 The session token for the DNS resolution, to prevent out-of-date signals from
     *     propagating.
     * @param obj @NonNull An EventDnsResolvedData instance with relevant data.
     */
    private static final int EVENT_DNS_RESOLVED = 2;

    private static class EventDnsResolvedData implements EventData {
        @NonNull public final List<InetAddress> resolvedAddrs;

        EventDnsResolvedData(@NonNull List<InetAddress> resolvedAddrs) {
            this.resolvedAddrs = Objects.requireNonNull(resolvedAddrs);
        }

        @Override
        public int hashCode() {
            return Objects.hash(resolvedAddrs);
        }

        @Override
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventDnsResolvedData)) {
                return false;
            }

            final EventDnsResolvedData rhs = (EventDnsResolvedData) other;
            return Objects.equals(resolvedAddrs, rhs.resolvedAddrs);
        }
    }
     ...
     * state to the DNS resolution state.
     *
     * @param arg1 The "any" token; no sessions are active in the RetryTimeoutState.
     */
    private static final int EVENT_RETRY_TIMEOUT_EXPIRED = 3;

    /**
     * Sent when an IKE session has been lost, due to remote action, network failure, or otherwise.
     *
     ...
     * expected) transition to the Retry-timeout state, and reset the retry count.
     *
     * @param arg1 The session token for the IKE Session that was lost, used to prevent out-of-date
     *     signals from propagating.
     * @param obj @NonNull An EventSessionLostData instance with relevant data.
     */
    private static final int EVENT_SESSION_LOST = 4;

    private static class EventSessionLostData implements EventData {
        @Nullable public final Exception exception;

        EventSessionLostData(@NonNull Exception exception) {
        ...
        @Override
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventSessionLostData)) {
                return false;
            }

            final EventSessionLostData rhs = (EventSessionLostData) other;
     ...
     * @param obj @NonNull An EventTransformCreatedData instance with relevant data.
     */
    private static final int EVENT_TRANSFORM_CREATED = 5;

    private static class EventTransformCreatedData implements EventData {
        @PolicyDirection public final int direction;
        @NonNull public final IpSecTransform transform;

        EventTransformCreatedData(
        ...
        @Override
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventTransformCreatedData)) {
                return false;
            }

            final EventTransformCreatedData rhs = (EventTransformCreatedData) other;
     ...
     * @param obj @NonNull An EventSetupCompletedData instance with relevant data.
     */
    private static final int EVENT_SETUP_COMPLETED = 6;

    private static class EventSetupCompletedData implements EventData {
        @NonNull public final ChildSessionConfiguration childSessionConfig;

        EventSetupCompletedData(@NonNull ChildSessionConfiguration childSessionConfig) {
        ...
        @Override
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventSetupCompletedData)) {
                return false;
            }

            final EventSetupCompletedData rhs = (EventSetupCompletedData) other;
     ...
     * @param obj @NonNull An EventDisconnectRequestedData instance with relevant data.
     */
    private static final int EVENT_DISCONNECT_REQUESTED = 7;

    private static class EventDisconnectRequestedData implements EventData {
        /** The reason why the disconnect was requested. */
        @NonNull public final String reason;

        EventDisconnectRequestedData(@NonNull String reason) {
        ...
        @Override
        public boolean equals(@Nullable Object other) {
            if (!(other instanceof EventDisconnectRequestedData)) {
                return false;
            }

            final EventDisconnectRequestedData rhs = (EventDisconnectRequestedData) other;
     ...
     *
     * <ul>
     *   <li>Starting (and awaiting) a DNS resolution
     ...
     *
     * <p>A new token MUST be used for all new stateful/async sessions:
     *
     * <ul>
     *   <li>New DNS resolution attempts
     *   <li>New IKE sessions
     * </ul>
     */
    ...
    /**
     * The resolved address of mobility anchor.
     *
     * <p>Set in DnsResolutionState, always @NonNull in all states except Disconnected and
     * DnsResolution states, @Nullable otherwise.
     */
    private InetAddress mResolvedAddr;
    ...
    VcnGatewayConnection(
        ...
        addState(mDnsResolutionState);
        addState(mConnectingState);
        addState(mConnectedState);
        addState(mMigratingState);
    ...
    public void teardownAsynchronously() {
        ...
        sendMessage(
                ...
                TOKEN_ANY,
                new EventDisconnectRequestedData(DISCONNECT_REASON_TEARDOWN));
    ...
    public void onSelectedUnderlyingNetworkChanged(@Nullable UnderlyingNetworkRecord underlying) {
        ...
        if (underlying == null) {
            sendMessageDelayed(
                    EVENT_DISCONNECT_REQUESTED,
                    TOKEN_ANY,
                    new EventDisconnectRequestedData(DISCONNECT_REASON_UNDERLYING_NETWORK_LOST),
        ...
        if (getHandler() != null) {
            getHandler()
                    .removeEqualMessages(
                            EVENT_DISCONNECT_REQUESTED,
                            new EventDisconnectRequestedData(
                                    DISCONNECT_REASON_UNDERLYING_NETWORK_LOST));
        }
        sendMessage(
                EVENT_UNDERLYING_NETWORK_CHANGED,
                TOKEN_ANY,
                new EventUnderlyingNetworkChangedData(underlying));
    }

    private void sendMessage(int what, int token, EventData data) {
        super.sendMessage(what, token, ARG_NOT_PRESENT, data);
    }

    private void sendMessage(int what, int token, int arg2, EventData data) {
        super.sendMessage(what, token, arg2, data);
    }

    private void sendMessageDelayed(int what, int token, EventData data, long timeout) {
        super.sendMessageDelayed(what, token, ARG_NOT_PRESENT, data, timeout);
    }

    private void sendMessageDelayed(int what, int token, int arg2, EventData data, long timeout) {
        super.sendMessageDelayed(what, token, arg2, data, timeout);
    }

    @VisibleForTesting(visibility = Visibility.PRIVATE)
    void sessionLost(int token, @Nullable Exception exception) {
        sendMessage(EVENT_SESSION_LOST, token, new EventSessionLostData(exception));
    }
    ...
    private void childTransformCreated(
        ...
        sendMessage(
                ...
                token,
                new EventTransformCreatedData(direction, transform));
    }

    private void childOpened(int token, @NonNull ChildSessionConfiguration childConfig) {
        sendMessage(EVENT_SETUP_COMPLETED, token, new EventSetupCompletedData(childConfig));
    }
        ...
        protected void processStateMsg(Message msg) {
            switch (msg.what) {
                case EVENT_UNDERLYING_NETWORK_CHANGED:
                    ...
                    mUnderlying = ((EventUnderlyingNetworkChangedData) msg.obj).newUnderlying;
     ...
     *
     * <p>The result of the DNS query will be used for all IKE connections/migrations until this
     * state is re-entered.
     */
        ...
        protected void processStateMsg(Message msg) {
            switch (msg.what) {
                ...
                case EVENT_DNS_RESOLVED:
                    ...
                    final List<InetAddress> list = ((EventDnsResolvedData) msg.obj).resolvedAddrs;
    ...
    /**
     * Transitive state representing a VCN tunnel that is in the process of migration
     *
     * <p>This state may create a secondary connection and switch over to it once it is established,
     * or directly migrate the existing tunnel, based on available kernel support.
     */
    class MigratingState extends ConnectedStateBase {
        @Override
        protected void processStateMsg(Message msg) {}
    }
    ...
    private class IkeSessionCallbackImpl implements IkeSessionCallback {
        ...
        public void onClosed() {
            ...
            sessionLost(mToken, null); // Server requested session closure. Retry?
        }
