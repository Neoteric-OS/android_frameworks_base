...
public class UnderlyingNetworkRecord {
    ...
    public int hashCode() {
        return Objects.hash(
                network, networkCapabilities, linkProperties, isBlocked, isSelected, priorityClass);
    }

    /**
     * Returns if two records represent the same Network with same Network states.
     *
     * <p>This comparison method will ignore isSelected and priorityClass because they are only for
     * use in network prioritization and differences in those two fields do not necessarily mean
     * changes in Networks or Network's states.
     */
    public static boolean areSame(UnderlyingNetworkRecord left, UnderlyingNetworkRecord right) {
        if (left != null && right != null) {
            return left.network.equals(right.network)
                    && left.networkCapabilities.equals(right.networkCapabilities)
                    && left.linkProperties.equals(right.linkProperties)
    ...
    private static boolean isSelected(
        ...
        if (currentlySelected.network == networkToCheck) {
            return true;
        }
        return false;
    }
