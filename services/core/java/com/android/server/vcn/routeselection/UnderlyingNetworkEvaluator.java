...
class UnderlyingNetworkEvaluator {
    ...
    UnderlyingNetworkEvaluator(
        ...
        Log.d(TAG, getLogPrefix(network) + "Initializing");
    ...
    private class MyMetricMonitorCallback
        ...
        public void onValidationResultChanged() {
            logD("MyMetricMonitorCallback#onValidationResultChanged");
    ...
    private void handleValidationResultChanged() {
        ...
        logD(
                "handleValidationResultChanged: wasPenalized "
                        + wasPenalized
                        + " mIsPenalized "
                        + mIsPenalized);
        ...
        logD("handleValidationResultChanged: fire onEvaluationResultChanged");
    ...
    private class ExitPenaltyBoxRunnable implements Runnable {
        ...
        public void run() {
            ...
            mIsPenalized = false;
            logD("ExitPenaltyBoxRunnable: fire onEvaluationResultChanged");
            mEvaluatorCallback.onEvaluationResultChanged();
        }
    }
    ...
    void setSelectedNetwork(@Nullable UnderlyingNetworkRecord currentlySelected) {
        final boolean wasSelected = mIsSelected;
        ...
        if (wasSelected == mIsSelected) {
            return;
        }

        logD("setSelectedNetwork " + currentlySelected + " mIsSelected changed to " + mIsSelected);
        updatePriorityClass();

        if (mIpSecPacketLossDetector != null) {
            mIpSecPacketLossDetector.setSelected(mIsSelected);
        }
    }
    ...
    private static String getLogPrefix(Network network) {
        return "[Network " + network + "] ";
    }

    private String getLogPrefix() {
        return getLogPrefix(mNetworkRecordBuilder.getNetwork());
    }

    void logD(String details) {
        Log.i(TAG, getLogPrefix() + details);
    }

    void logE(String details) {
        Log.e(TAG, getLogPrefix() + details);
    }

    void logWtf(String details) {
        Log.wtf(TAG, getLogPrefix() + details);
    }
}
