...
abstract class NetworkMetricMonitor implements AutoCloseable {
    ...
    interface NetworkMetricMonitorCallback {
         ...
         *
         * <p>When receiving this call, UnderlyingNetworkEvaluator should reevaluate the network
    ...
    void logV(String details) {
        if (VDBG) {
            Log.i(getTag(), getLogPrefix() + details);
        }
    }
    ...
    void logE(String details) {
        Log.e(getTag(), getLogPrefix() + details);
    }

    void logWtf(String details) {
        Log.wtf(getTag(), getLogPrefix() + details);
    }
}
