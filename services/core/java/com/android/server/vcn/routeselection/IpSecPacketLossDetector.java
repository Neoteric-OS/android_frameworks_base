...
public class IpSecPacketLossDetector extends NetworkMetricMonitor {
    ...
    // A conservative bar to declare validation fail
    private static final int IPSEC_PACKET_LOSS_PERCENT_THRESHOLD_DEFAULT = 10;
    ...
    public IpSecPacketLossDetector(
        ...
        if (!vcnContext.getCoreNetFeatureFlags().ipsecTransformState()) {
            logWtf(TAG, "ipsecTransformState flag disabled");
        }
    ...
    public static class PacketLossCalculator {
        ...
        public int getPacketLossRatePercentage(
            ...
            if (oldSeqHi == newSeqHi) {
                logV(TAG, logPrefix + "Estimation unsupported: replay window did not proceed");
            ...
            final long newExpectedPktCnt =
                    newSeqLow + getRxPacketInWindow(newState.getReplayBitmap());
            final long oldExpectedPktCnt =
                    oldSeqLow + getRxPacketInWindow(oldState.getReplayBitmap());
            ...
            if (expectedPktCntDiff < 0 || expectedPktCntDiff == 0 || actualPktCntDiff < 0) {
                logWtf(TAG, "Impossible data expectedPktCntDiff or" + " actualPktCntDiff");
                return PACKET_LOSS_UNAVALAIBLE;
            }

            return (int) (100 - (actualPktCntDiff * 100) / expectedPktCntDiff);
        }
    }
