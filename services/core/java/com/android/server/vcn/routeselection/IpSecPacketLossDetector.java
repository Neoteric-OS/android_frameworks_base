...
public class IpSecPacketLossDetector extends NetworkMetricMonitor {
    ...
    // "ICTP-SDU: About PingER"). Thus 12% will be used as a conservative bar to declare validation
    // fail.
    ...
    public IpSecPacketLossDetector(
        ...
        if (!vcnContext.isFlagIpSecTransformStateEnabled()) {
            // Caller error
            throw new IllegalStateException("ipsecTransformState flag disabled");
        }
    ...
    @Override
    protected void onIsUnderlyingNetworkSelectedChanged() {
        if (!isUnderlyingNetworkSelected()) {
            stop();
        }

        // No action when the underlying network got selected. Monitoring will not start until the
        // inbound transform that has applied to the underlying network is provided.
    }

    /**
     * Called when the inbound transform becomes available or gets updated
     *
     * <p>When multiple parallel transforms are created, this method will be called on an already
     * started monitor
     */
    @Override
    protected void onInboundTransformChanged() {
        start();
    }

    @Override
    public void setCarrierConfig(@Nullable PersistableBundleWrapper carrierConfig) {
        // The already scheduled event will not be affected. The next event will use the new
        // interval
        mPollIpSecStateIntervalMs = getPollIpSecStateIntervalMs(carrierConfig);
    }
    ...
    private void onIpSecTransformStateReceived(@NonNull IpSecTransformState state) {
        ...
        final String logMsg =
                TextUtils.formatSimple(
                        "packetLossRate: %d %% in the past %d ms",
                        packetLossRate,
    ...
    public static class PacketLossCalculator {
        ...
        public int getPacketLossRatePercentage(
            ...
            if (oldSeqHi == newSeqHi || newSeqHi < replayWindowSize) {
                // The replay window did not proceed and all packets can be out-of-order delivered
            ...
            logV(
                    TAG,
                    TextUtils.formatSimple(
                            logPrefix + " expectedPktCntDiff: %d;" + " actualPktCntDiff %d",
                            expectedPktCntDiff,
                            actualPktCntDiff));
