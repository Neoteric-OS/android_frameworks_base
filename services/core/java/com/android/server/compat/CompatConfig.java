...
final class CompatConfig {
    ...
    /**
     * Removes overrides whose change ID is specified in {@code overridesToRemove}, or all overrides
     * if no change ID is specified, that were previously added via
     * {@link #addOverride(long, String, boolean)} or {@link
     * #addOverrides(CompatibilityOverrideConfig, String)} for a certain package.
     *
    ...
    void removePackageOverrides(CompatibilityOverridesToRemoveConfig overridesToRemove,
            String packageName) {
        if (overridesToRemove.changeIds.isEmpty()) {
            removePackageOverrides(packageName);
        }
        synchronized (mChanges) {
