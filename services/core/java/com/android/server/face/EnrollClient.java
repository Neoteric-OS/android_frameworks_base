...
public abstract class EnrollClient extends ClientMonitor {
    ...
    public EnrollClient(Context context, long halDeviceId, IBinder token,
            IFaceServiceReceiver receiver, int userId, byte [] cryptoToken,
    ...
    public boolean onEnrollResult(int remaining) {
        if (remaining == 0) {
            FaceUtils.getInstance().setFaceForUser(getContext(), getTargetUserId());
        }
        return sendEnrollResult(remaining);
    }

    /*
     * @return true if we're done.
     */
    private boolean sendEnrollResult(int remaining) {
        ...
        try {
            receiver.onEnrollResult(getHalDeviceId(), remaining);
            return remaining == 0;
    ...
    public int start() {
        ...
        try {
            final int result = daemon.enroll(mCryptoToken, timeout);
            if (result != 0) {
    ...
    public int stop(boolean initiatedByClient) {
        ...
        if (initiatedByClient) {
            onError(FaceAuthenticationManager.FACE_ERROR_CANCELED, 0 /* vendorCode */);
        }
