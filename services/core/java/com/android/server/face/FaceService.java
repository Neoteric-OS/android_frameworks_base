...
import static android.Manifest.permission.MANAGE_FACE_AUTHENTICATION;
import static android.Manifest.permission.RESET_FACE_LOCKOUT;
import static android.Manifest.permission.USE_FACE_AUTHENTICATION;
...
import android.hardware.biometrics.face_authentication.V1_0.FaceEnroll;
import android.hardware.biometrics.face_authentication.V1_0.IBiometricsFace;
import android.hardware.biometrics.face_authentication.V1_0.IBiometricsFaceClientCallback;
import android.hardware.face.Face;
import android.hardware.face.FaceAuthenticationManager;
import android.hardware.face.IFaceClientActiveCallback;
...
public class FaceService extends SystemService implements IHwBinder.DeathRecipient {
    ...
    private final ArrayList<FaceServiceLockoutResetMonitor> mLockoutMonitors =
            new ArrayList<>();
    private final CopyOnWriteArrayList<IFaceClientActiveCallback> mClientActiveCallbacks =
    ...
    public void serviceDied(long cookie) {
        ...
        handleError(mHalDeviceId, FaceAuthenticationManager.FACE_ERROR_HW_UNAVAILABLE,
                0 /*vendorCode */);
    }
    ...
    public synchronized IBiometricsFace getFaceDaemon() {
        if (mDaemon == null) {
            ...
            try {
                mHalDeviceId = mDaemon.setCallback(mDaemonCallback);
            } catch (RemoteException e) {
    ...
    protected void handleError(long deviceId, int error, int vendorCode) {
        ...
        if (error == FaceAuthenticationManager.FACE_ERROR_CANCELED) {
        ...
        } else if (error == FaceAuthenticationManager.FACE_ERROR_HW_UNAVAILABLE) {
    ...
    protected void handleRemoved(long deviceId) {
        if (DEBUG) Slog.w(TAG, "Removed: face, dev=" + deviceId);

        ClientMonitor client = mCurrentClient;
        if (client != null && client.onRemoved()) {
            removeClient(client);
        }
    }

    protected void handleAuthenticated(long deviceId, boolean success, ArrayList<Byte> token) {
        ClientMonitor client = mCurrentClient;
        if (success) {
            // Ugh...
        ...
        if (client != null && client.onAuthenticated(success)) {
            removeClient(client);
        }
        if (success) {
    ...
    protected void handleEnrollResult(long deviceId, FaceEnroll enrollResult) {
        ClientMonitor client = mCurrentClient;
        if (client != null && client.onEnrollResult(enrollResult.samplesRemaining, enrollResult.msg)) {
            removeClient(client);
        }
    }
     ...
     * ({@link FaceAuthenticationManager#FACE_ERROR_CANCELED}).
    ...
    private boolean canUseFace(String opPackageName, boolean requireForeground, int uid,
            int pid, int userId) {
        checkPermission(USE_FACE_AUTHENTICATION);
        ...
        if (mAppOps.noteOp(AppOpsManager.OP_USE_FACE_AUTHENTICATION, uid, opPackageName)
    ...
    private void notifyClientActiveCallbacks(boolean isActive) {
        List<IFaceClientActiveCallback> callbacks = mClientActiveCallbacks;
    ...
    private void startAuthentication(IBinder token, long opId, int callingUserId,
        ...
        if (lockoutMode != AuthenticationClient.LOCKOUT_NONE) {
            ...
            int errorCode = lockoutMode == AuthenticationClient.LOCKOUT_TIMED ?
                    FaceAuthenticationManager.FACE_ERROR_LOCKOUT :
                    FaceAuthenticationManager.FACE_ERROR_LOCKOUT_PERMANENT;
    ...
    private IBiometricsFaceClientCallback mDaemonCallback =
        ...
        @Override
        public void onEnrollResult(final long deviceId, FaceEnroll enrollResult) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    handleEnrollResult(deviceId, enrollResult);
                }
            });
        }
        ...
        @Override
        public void onAuthenticated(final long deviceId, final boolean success, ArrayList<Byte> token) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    handleAuthenticated(deviceId, success, token);
                }
            });
        }
        ...
        @Override
        public void onRemoved(final long deviceId) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
    ...
    private final class FaceServiceWrapper extends IFaceService.Stub {
        ...
        public long preEnroll(IBinder token) {
            checkPermission(MANAGE_FACE_AUTHENTICATION);
        ...
        private boolean isRestricted() {
            ...
            final boolean restricted = !hasPermission(MANAGE_FACE_AUTHENTICATION);
            return restricted;
        }
        ...
        public void remove(final IBinder token, final int userId,
            ...
            checkPermission(MANAGE_FACE_AUTHENTICATION); // TODO: Maybe have another permission
            final boolean restricted = isRestricted();
            mHandler.post(new Runnable() {
                @Override
                public void run() {
        ...
        public boolean isClientActive() {
            checkPermission(MANAGE_FACE_AUTHENTICATION);
            synchronized(FaceService.this) {
                return (mCurrentClient != null) || (mPendingClient != null);
            }
        }

        @Override
        public void addClientActiveCallback(IFaceClientActiveCallback callback) {
            checkPermission(MANAGE_FACE_AUTHENTICATION);
            mClientActiveCallbacks.add(callback);
        }

        @Override
        public void removeClientActiveCallback(IFaceClientActiveCallback callback) {
            checkPermission(MANAGE_FACE_AUTHENTICATION);
        ...
        if (daemon != null) {
            try {
                ...
                mAuthenticatorIds.put(userId,
                        hasEnrolledFace(userId) ? daemon.getAuthenticatorId() : 0L);
            } catch (RemoteException e) {
