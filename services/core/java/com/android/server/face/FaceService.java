...
public class FaceService extends SystemService implements IHwBinder.DeathRecipient {
    ...
    protected void handleEnrollResult(long deviceId, int remaining) {
        ClientMonitor client = mCurrentClient;
        if (client != null && client.onEnrollResult(remaining)) {
            removeClient(client);
        }
    }
    ...
    private IBiometricsFaceClientCallback mDaemonCallback =
        ...
        @Override
        public void onEnrollResult(final long deviceId, final int remaining) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    handleEnrollResult(deviceId, remaining);
                }
            });
        }
