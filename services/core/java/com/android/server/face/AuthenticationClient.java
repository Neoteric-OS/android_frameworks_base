...
public abstract class AuthenticationClient extends ClientMonitor {
    ...
    @Override
    public boolean onAuthenticated(boolean authenticated) {
            ...
            try {
                ...
                if (!authenticated) {
                    ...
                    Face face = !getIsRestricted()
                            ? new Face(getHalDeviceId())
                            : null;
                    receiver.onAuthenticationSucceeded(getHalDeviceId(), face, getTargetUserId());
                }
    ...
    public int start() {
        ...
        try {
            final int result = daemon.authenticate(mOpId, getTargetUserId());
            if (result != 0) {
                Slog.w(TAG, "startAuthentication failed, result=" + result);
                MetricsLogger.histogram(getContext(), "faced_auth_start_error", result);
                onError(FaceAuthenticationManager.FACE_ERROR_HW_UNAVAILABLE, 0 /* vendorCode */);
                return result;
            }
    ...
    @Override
    public boolean onEnrollResult(int remaining, long vendorMsg) {
