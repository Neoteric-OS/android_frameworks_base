...
final class Result {
    ...
    private static final Result OK = new Result(TYPE_OK, null, 0);

    private Result(int type, @Nullable String stringArg, int intArg) {
        this.type = type;
    ...
    public static Result ok(@NonNull String targetPackageName, int userId) {
        return new Result(TYPE_OK_WITH_DATA, targetPackageName, userId);
    }
}
