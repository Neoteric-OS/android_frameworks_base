 ...
 * @hide
 *
 * <p>Locking in IpSecService is done on all entrypoints due to potential race conditions at the
 * kernel/xfrm level. Further, this allows the simplifying assumption to be made that only one
...
public class IpSecService extends IIpSecService.Stub {
     ...
     *
     * <p>This interface must be implemented for a resource be reference counted.
     */
    @VisibleForTesting
    public interface IResource {
        /**
         * Removes tracking object from resource tracking arrays
         *
         ...
         * the resource is considered invalid for user access, allocation or use in other resources.
         * References to the IResource object may be held by other RefcountedResource objects, and
         * as such, the kernel resources and quota may not be cleaned up.
         *
         * <p>Design note: While this implies that the IResource "removes itself" from the tracking
         * resource arrays, there appears to be no way around it due to the binder's death handler
         * being in the RefcountedResource (which has no idea which flavor of resource cleanup
         * method to call).
         */
        void cleanupResourceArrays() throws RemoteException;

        /**
         * Releases kernel resources and related quotas.
         *
         ...
         * called after userRefCleanup().
         */
        void releaseKernelResources() throws RemoteException;
    }

    @VisibleForTesting
    public class RefcountedResource<T extends IResource> implements IBinder.DeathRecipient {
        private final T mResource;
        private final List<RefcountedResource> mChildren = new LinkedList<>();
        private int mRefCount = 1; // starts at 1 for userRef.
        IBinder mBinder;
         ...
         * be null, and the method should return without performing the cleanup a second time.
         *
        ...
        public void cleanupAndRelease() throws RemoteException {
            ...
            // cleanupAndRelease() multiple times.
            if (mBinder == null) {
                return;
            }
            ...
            mResource.cleanupResourceArrays();
        ...
        /**
         * Triggers recursive release of the underlying resources quota and references to children
         * (dependencies). This method may trigger cleanup of unreferenced children.
         *
         * <p>Calls to this function while the resource is still referenced will have no effect.
         *
    ...
    static final class UserRecord {
        ...
        final ResourceTracker spiQuotaTracker = new ResourceTracker(MAX_NUM_SPIS);

        final RefcountedResourceArray<TransformRecord> mTransformRecords =
                new RefcountedResourceArray<>(TYPENAME_TRANSFORM);
        final ResourceTracker transformQuotaTracker = new ResourceTracker(MAX_NUM_TRANSFORMS);

        final RefcountedResourceArray<EncapSocketRecord> mEncapSocketRecords =
                new RefcountedResourceArray<>(TYPENAME_ENCAP_SOCKET);
        final ResourceTracker socketQuotaTracker = new ResourceTracker(MAX_NUM_ENCAP_SOCKETS);
        ...
        public String toString() {
            return new StringBuilder()
                    .append("{spiQuotaTracker=")
                    .append(spiQuotaTracker)
                    .append(", transformQuotaTracker=")
                    .append(transformQuotaTracker)
                    .append(", socketQuotaTracker=")
                    .append(socketQuotaTracker)
                    .append(", mSpiRecords=")
                    .append(mSpiRecords)
     ...
     * resource. To use this class, the user should implement the cleanupResourceArrays() and
     * releaseKernelResources() method that are responsible for cleaning up IpSecService resource
    ...
    private abstract class KernelResourceRecord implements IResource {
        ...
        /** Cleans up IpSecService internal resource tracking objects */
        public abstract void cleanupResourceArrays() throws RemoteException;
         ...
         * clean up all system resources. Kernel resource cleanup is delegated to subclasses
         */
        public abstract void releaseKernelResources() throws RemoteException;
    ...
    // TODO: Complete stylistic changes separately from switch to using RefcountedResources;
    ...
    private final class TransformRecord extends KernelResourceRecord {
        ...
        @Override
        public void releaseKernelResources() {
        ...
        @Override
        public void cleanupResourceArrays() throws RemoteException {
            getUserRecord().removeTransformRecord(mResourceId);
        }

        @Override
        protected ResourceTracker getResourceTracker() {
            return getUserRecord().transformQuotaTracker;
        }
    ...
    private final class SpiRecord extends KernelResourceRecord {
        ...
        @Override
        protected ResourceTracker getResourceTracker() {
            return getUserRecord().spiQuotaTracker;
        }
    ...
    private final class EncapSocketRecord extends KernelResourceRecord {
        ...
        @Override
        protected ResourceTracker getResourceTracker() {
            return getUserRecord().socketQuotaTracker;
        }

        @Override
        public void cleanupResourceArrays() {
    ...
    public synchronized IpSecSpiResponse reserveSecurityParameterIndex(
        ...
        try {
            if (!userRecord.spiQuotaTracker.isAvailable()) {
    ...
    private void releaseResource(RefcountedResourceArray resArray, int resourceId)
        ...
        resArray.getRefcountedResourceOrThrow(resourceId).cleanupAndRelease();
    }
    ...
    public synchronized IpSecUdpEncapResponse openUdpEncapsulationSocket(int port, IBinder binder)
        ...
        try {
            if (!userRecord.socketQuotaTracker.isAvailable()) {
    ...
    public synchronized IpSecTransformResponse createTransportModeTransform(
        ...
        UserRecord userRecord = mUserResourceTracker.getUserRecord(Binder.getCallingUid());
        List<RefcountedResource> dependencies = new LinkedList<>();

        if (!userRecord.transformQuotaTracker.isAvailable()) {
