...
public class IpSecService extends IIpSecService.Stub {
    ...
    private abstract class ManagedResource implements IBinder.DeathRecipient {
        ...
        public final synchronized void nullify() {
            nullifyRecord();
            mBinder = null;
    ...
    private final class TransformRecord extends ManagedResource {
        ...
        TransformRecord(int resourceId, IBinder binder, IpSecConfig config) {
        ...
        protected void releaseResources() {
            ...
            for (int direction : DIRECTIONS) {
                synchronized (mSpiRecords) {
                    spi = mSpiRecords.get(mConfig.getSpiResourceId(direction)).getSpi();
                }
                try {
                    getNetdInstance()
    ...
    // TODO: read from /proc/sys/net/ipv4/ip_local_port_range
    ...
    public Bundle openUdpEncapsulationSocket(int port, IBinder binder) throws RemoteException {
        ...
        try {
            ...
            synchronized (mUdpSocketRecords) {
    ...
    public Bundle createTransportModeTransform(IpSecConfig c, IBinder binder)
        ...
        int resourceId = mNextResourceId.getAndIncrement();
        IpSecAlgorithm auth, crypt;
        int encapType, encapLocalPort, encapRemotePort;
        int spi;
        for (int direction : DIRECTIONS) {
            auth = c.getAuthentication(direction);
            crypt = c.getEncryption(direction);
            encapType = encapLocalPort = encapRemotePort = 0;
            if (direction == IpSecTransform.DIRECTION_OUT) {
                encapType = c.getEncapType();
                if (encapType != IpSecTransform.ENCAP_NONE) {
                    synchronized (mUdpSocketRecords) {
                        encapLocalPort =
                                mUdpSocketRecords.get(c.getEncapLocalResourceId()).getPort();
                    }
                    encapRemotePort = c.getEncapRemotePort();
                }
            }
            synchronized (mSpiRecords) {
                spi = mSpiRecords.get(c.getSpiResourceId(direction)).getSpi();
            }
            try {
                int result =
                        getNetdInstance()
        ...
        synchronized (mTransformRecords) {
            mTransformRecords.put(resourceId, new TransformRecord(resourceId, binder, c));
        }
