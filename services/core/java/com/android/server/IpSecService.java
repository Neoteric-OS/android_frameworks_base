...
public class IpSecService extends IIpSecService.Stub {
    ...
    public interface IResource {
         ...
         * resources.
         *
         * <p>References to the IResource object may be held by other RefcountedResource objects, and
     ...
     *
     * RefcountedResource supports both explicit (From creation to deletion by a user) and implicit
     * (released by user, but still referenced by other RefcountedResource objects). In cases where
     * the user has released the resource, but it is still in use, the releasing of underlying
     * resources cannot yet happen, and as such require the tracking object (RefcountedResource and
     * IResource objects) to be implicitly tracked by the objects that depend on them. Final
     * dereferencing happens upon the last reference being released (whether user calling
     * userRelease(), or the last object depending on it calling the recursiveRelease() function.
     */
    @VisibleForTesting
