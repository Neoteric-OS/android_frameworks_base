...
public class IpSecService extends IIpSecService.Stub {
    ...
    private void checkIpSecConfig(IpSecConfig config) {
        ...
        int sourceFamily = getFamily(config.getSourceAddress());
        int destinationFamily = getFamily(config.getDestinationAddress());
        if (sourceFamily != destinationFamily) {
            throw new IllegalArgumentException(
                    "Source and destination have different address families.");
        }
        ...
        if (config.getEncapType() != IpSecTransform.ENCAP_NONE && sourceFamily != AF_INET) {
            throw new IllegalArgumentException(
                    "UDP Encapsulation is not supported in this environment.");
        }
