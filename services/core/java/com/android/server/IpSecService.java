...
public class IpSecService extends IIpSecService.Stub {
    ...
    private static final int TUN_INTF_NETID_MIN = 0xFC00;
    private static final int TUN_INTF_NETID_MAX = 0xFFFF;
    private final SparseBooleanArray mTunnelNetIds = new SparseBooleanArray();
    private int mNextTunnelNetId = TUN_INTF_NETID_MIN;
    ...
    protected int reserveNetId() {
        synchronized (mTunnelNetIds) {
            for (int i = TUN_INTF_NETID_MIN; i <= TUN_INTF_NETID_MAX; i++) {
                int netId = mNextTunnelNetId;
                int index = netId - TUN_INTF_NETID_MIN;
                if (++mNextTunnelNetId > TUN_INTF_NETID_MAX) mNextTunnelNetId = TUN_INTF_NETID_MIN;
                if (!mTunnelNetIds.get(index)) {
                    mTunnelNetIds.put(index, true);
                    return netId;
                }
            }
        }
    ...
    @VisibleForTesting
    protected void releaseNetId(int netId) {
        synchronized (mNetworkForNetId) {
            mNetIdInUse.delete(netId - TUN_INTF_NETID_MIN);
        }
    }
