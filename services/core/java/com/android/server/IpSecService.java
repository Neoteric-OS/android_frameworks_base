...
public class IpSecService extends IIpSecService.Stub {
    ...
    private final class SpiRecord extends ManagedResource {
        ...
        /** always guarded by mLock */
        @Override
        protected void releaseResources() throws RemoteException {
            if (mLockedByTransform) {
                ...
                // Thus, we should let the user free them without penalty once they are applied in
                // a Transform object.
                return;
            }
        ...
        public void setLockedByTransform() {
            if (mLockedByTransform) {
                // Programming error
                Log.wtf(TAG, "Cannot lock an SPI twice!");
            }
    ...
    private final class UdpSocketRecord extends ManagedResource {
        ...
        @Override
        protected void releaseResources() throws RemoteException {
            if (mTransformLocks > 0) {
                throw new RemoteException(
                        "Cannot release Port " + mPort + ": Currently locked by a Transform");
            }
            try {
        ...
        public void addTransformLock() {
            mTransformLocks++;
        }

        public void removeTransformLock() {
            mTransformLocks--;
            if (mTransformLocks < 0) {
                Log.wtf(TAG, "Programming error: There can never be less than zero Transforms");
            }
        }

        public boolean isLockedByTransform() {
            return (mTransformLocks != 0);
        }
    }
    ...
    public IpSecSpiResponse reserveSecurityParameterIndex(
    ...
    private <T extends ManagedResource> void releaseManagedResource(
            SparseArray<T> resArray, int resourceId, String typeName) throws RemoteException {
        synchronized (mLock) {
    ...
    /**
     * This function finds and forcibly binds to a random system port, ensuring that the port
     * cannot be unbound.
     *
     ...
     * else. The odds are ludicriously small of this ever happening, so retries should probably
     * never happen, and multiple retries is an absurd case.
     */
    private void bindToRandomPort(FileDescriptor sockFd) throws ErrnoException, IOException {
        ...
        throw new IOException("Failed " + MAX_PORT_BIND_ATTEMPTS + " to bind to a port");
    }
     ...
     */
    @Override
    public IpSecUdpEncapResponse openUdpEncapsulationSocket(int port, IBinder binder)
            throws RemoteException {
        int resourceId = mNextResourceId.getAndIncrement();
        ...
        try {
            ...
            try {
                // In case something goes wrong above, then we abort and close the user socket
                // if it was open. The exception blocks here are effectively noop boilerplate
        ...
        try {
            ...
            /*
             * More boilerplate because in this case we don't actually have the possibility of an
             * IOException from the underlying class, but we can't return another
             * IpSecudpEncapResponse because that would generate another potential IOException. To
             * break the loop of insanity, instead we just return null, which as best I can tell,
             * should never actually happen.
             */
            Log.wtf(
                    TAG,
                    "Couldn't construct an IpSecUdpEncapResponse"
                            + " return value... this shouldn't happen");
            return null;
        }
    }
     ...
     */
    @Override
    public IpSecTransformResponse createTransportModeTransform(IpSecConfig c, IBinder binder)
            throws RemoteException {
        int resourceId = mNextResourceId.getAndIncrement();
