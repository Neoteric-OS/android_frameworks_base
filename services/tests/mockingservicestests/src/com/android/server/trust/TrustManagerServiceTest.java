...
public class TrustManagerServiceTest {
    ...
    public void testLockDeviceForManagedProfileWithUnifiedChallenge_usesParentBiometricSids()
        ...
        verify(mKeystoreAuthorization, times(1)).onDeviceUnlocked(PARENT_USER_ID, null);
        verify(mKeystoreAuthorization, times(1)).onDeviceUnlocked(PROFILE_USER_ID, null);

        when(mWindowManager.isKeyguardLocked()).thenReturn(true);
        mTrustManager.reportKeyguardShowingChanged();
        verify(mKeystoreAuthorization, times(1))
    ...
    private void setupMocksForProfile(boolean unifiedChallenge) {
        ...
        when(mUserManager.getUserInfo(anyInt())).thenReturn(parent);
