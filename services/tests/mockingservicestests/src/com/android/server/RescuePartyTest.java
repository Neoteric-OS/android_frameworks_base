...
public class RescuePartyTest {
    ...
    private void mockCrashRecoveryProperties(PackageWatchdog watchdog) {
        ...
        try {
            ...
            doAnswer((Answer<Integer>) invocationOnMock -> {
                String storedValue = mCrashRecoveryPropertiesMap
                        .getOrDefault("crashrecovery.rescue_party_boot_mitigation_count", "0");
                return Integer.parseInt(storedValue);
            }).when(() -> RescueParty.getRescuePartyBootMitigationCount());
            doAnswer((Answer<Void>) invocationOnMock -> {
                int value = invocationOnMock.getArgument(0);
                mCrashRecoveryPropertiesMap.put("crashrecovery.rescue_party_boot_mitigation_count",
                        Integer.toString(value));
                return null;
            }).when(() -> RescueParty.setRescuePartyBootMitigationCount(anyInt()));
        ...
        try {
            ...
            doAnswer((Answer<Void>) invocationOnMock -> {
                ...
                mCrashRecoveryPropertiesMap.put("crashrecovery.rescue_party_boot_mitigation_count",
                        Integer.toString(count));
                return null;
            }).when(mSpyBootThreshold).setRescuePartyMitigationCount(anyInt());
