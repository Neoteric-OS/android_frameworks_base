...
public class PackageDynamicCodeLoadingTests {
    ...
    @Test
    public void testRecord_changeUserForFile_rejected() {
        ...
        assertThrows(IllegalArgumentException.class, () -> record(info, entry2));
    ...
    private Set<Entry> entriesFrom(PackageDynamicCodeLoading info) {
        ...
        for (String owningPackage : info.getAllPackagesWithDynamicCodeLoading()) {
            ...
            for (Map.Entry<String, DynamicCodeFile> fileEntry : packageInfo.mFileUsageMap.entrySet()) {
    ...
    private PackageDynamicCodeLoading makePackageDcl(Entry... entries) {
        return makePackageDcl(Arrays.asList(entries));

    }

    private PackageDynamicCodeLoading makePackageDcl(Iterable<Entry> entries) {
