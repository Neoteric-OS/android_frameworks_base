 ...
 * adb install -r out/target/product/mako/data/app/FrameworksServicesTests/FrameworksServicesTests.apk
...
public class SELinuxMMACTests extends AndroidTestCase {
    ...
    private PackageParser.Package parsePackage(Uri packageURI) {
        try {
            ...
            PackageParser.Package pkg = packageParser.parsePackage(sourceFile, 0);
            assertNotNull(pkg);
            assertNotNull(pkg.packageName);

            // Collect the certs for this package
            packageParser.collectCertificates(pkg, 0);
            assertNotNull(pkg.mSignatures);

            return pkg;
        } catch (PackageParserException ex) {
            fail("Error parsing package " + packageURI.getPath() + ". " + ex);
    ...
    // signer stanza (seinfo, package), default stanza : match signer
    public void test_CSPS_DS_3() {
        // mmac_csps_ds_2.xml contains an inner package stanza with a name
        // different than the signed_platform package name, so the app
        // should catch on the seinfo tag attached to the signer tag
        checkSeinfo(R.raw.mmac_csps_ds_2, R.raw.signed_platform, "signer");
    }
