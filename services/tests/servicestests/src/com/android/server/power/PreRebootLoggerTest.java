...
public class PreRebootLoggerTest {
    ...
    @Test
    public void logLocked_adbEnabled_dumpsInformationProperly() {
        Settings.Global.putInt(mContentResolver, Settings.Global.ADB_ENABLED, 1);

        PreRebootLogger.logLocked(mContext, mDumpDir);

        List<String> fileNameList = Arrays.asList(mDumpDir.list());
        assertThat(fileNameList).containsExactly("system_logs", "package", "rollback");
    }

    @Test
    public void logLocked_adbDisabled_wipesDumpedInformation() {
        Settings.Global.putInt(mContentResolver, Settings.Global.ADB_ENABLED, 1);
        PreRebootLogger.logLocked(mContext, mDumpDir);
