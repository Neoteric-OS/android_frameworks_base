...
public class UsageStatsXml {
    ...
    public static void read(AtomicFile file, IntervalStats statsOut, int statsType)
            throws IOException {
        try {
            FileInputStream in = file.openRead();
            try {
                statsOut.beginTime = parseBeginTime(file);
                read(in, statsOut, statsType);
    ...
    static void read(InputStream in, IntervalStats statsOut, int statsType) throws IOException {
        ...
        try {
            ...
            try {
                switch (Integer.parseInt(versionStr)) {
                    case 1:
                        UsageStatsXmlV1.read(parser, statsOut, statsType);
                        break;
