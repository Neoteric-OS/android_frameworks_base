...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    public void notifyDataConnection(int state, boolean isDataConnectivityPossible, String reason,
            String apnType, ApnTypeInfo ipv4Info, ApnTypeInfo ipv6Info, int networkType) {
        if (!checkNotifyPermission("notifyDataConnection()" )) {
            return;
        }
        synchronized (mRecords) {

            if (mAnyDataConnectionState != state || mDataConnectionNetworkType != networkType) {
                mAnyDataConnectionState = state;
        ...
        broadcastDataConnectionStateChanged(state, isDataConnectivityPossible, reason, apnType,
                ipv4Info, ipv6Info);
    }
