...
public class NetworkStackClient {
    ...
    public void start(Context context) {
        ...
        Intent intent = getNetworkStackIntent(pm, true /* inProcess */);

        // Otherwise use the updatable module version
        if (intent == null) {
            intent = getNetworkStackIntent(pm, false /* inProcess */);
    ...
    @Nullable
    private Intent getNetworkStackIntent(@NonNull PackageManager pm, boolean inProcess) {
        final String baseAction = INetworkStackConnector.class.getName();
        final Intent intent = new Intent(inProcess ? baseAction + IN_PROCESS_SUFFIX : baseAction);
        ...
        final int expectedUid = inProcess ? Process.SYSTEM_UID : Process.NETWORK_STACK_UID;
        if (uid != expectedUid) {
