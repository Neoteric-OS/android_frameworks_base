...
public class DhcpServer {
    ...
    // Lease time to transmit to client instead of a negative time in case a lease expired before we
    // could send it (if the server process is suspended for example).
    ...
    public static class MalformedPacketException extends Exception {
        public MalformedPacketException(String message, Throwable t) {
            super(message, t);
        }
    }
    ...
    public DhcpServer(@NonNull Looper looper, @NonNull InterfaceParams iface,
            @NonNull DhcpServingParams params, @NonNull SharedLog log, @NonNull Dependencies deps) {
        if (deps == null) {
    ...
    private void processDiscover(@NonNull DhcpDiscoverPacket packet)
        ...
        try {
            lease = mLeaseRepo.getOffer(getClientId(packet), clientMac,
    ...
    private void processRequest(@NonNull DhcpRequestPacket packet) throws MalformedPacketException {
        // If set, server ID matches with this server's ID as checked in processPacket().
        final boolean sidSet = packet.mServerIdentifier != null;
        final DhcpLease lease;
        final MacAddress clientMac = getMacAddr(packet);
        try {
            lease = mLeaseRepo.requestLease(getClientId(packet), getMacAddr(packet),
    ...
    private Inet4Address getAckOrOfferDst(@NonNull DhcpPacket request, @NonNull DhcpLease lease,
        ...
        // the lease address if not yet configured
    ...
    /**
     * Determine whether the broadcast flag should be set in the BOOTP packet flags.
     */
    private boolean getBroadcastFlag(@NonNull DhcpPacket request, @NonNull DhcpLease lease) {
        // No broadcast flag if the client already has a configured IP to unicast to. Broadcast
        // if requested by the client or neither clientIp nor lease address can be used to send
        // a unicast reply.
    ...
    @Nullable
    private static byte[] getClientId(@NonNull DhcpPacket packet) {
    ...
    private MacAddress getMacAddr(@NonNull DhcpPacket packet) throws MalformedPacketException {
        try {
            ...
            // Treat invalid addresses as all-zeros
