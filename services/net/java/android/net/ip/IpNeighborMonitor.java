 ...
 * NeighborEvents describing each event. Callers can provide a callback to both
 * filter (e.g. by interface index and/or IP address) and handle the generated
 * NeighborEvents.
 *
 * @hide
 */
public class IpNeighborMonitor extends BlockingSocketReader {
    ...
    public IpNeighborMonitor(Handler h, SharedLog log, Callback cb) {
        super(h, NetlinkSocket.DEFAULT_RECV_BUFSIZE);
        mHandler = h;
        mLog = log.forSubComponent(TAG);
        mCallback = (cb != null) ? cb : new Callback();
    }
    ...
    protected FileDescriptor createFd() {
        ...
        try {
            fd = NetlinkSocket.forProtocol(OsConstants.NETLINK_ROUTE);
            final NetlinkSocketAddress listenAddr =
                    new NetlinkSocketAddress(0, OsConstants.RTMGRP_NEIGH);
            Os.bind(fd, (SocketAddress) listenAddr);
    ...
    private void evaluateRtNetlinkNeighborMessage(
        ...
        if (!mCallback.watchingInterfaceIndex(ifindex)) return;

        final InetAddress destination = neighMsg.getDestination();
        if (!mCallback.watchingInetAddress(destination)) return;
        ...
        mCallback.handleNeighborEvent(event);
    }
}
