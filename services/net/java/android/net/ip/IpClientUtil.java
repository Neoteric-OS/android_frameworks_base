...
public class IpClientUtil {
     ...
     */
    public static DhcpInfo fromStableParcelable(DhcpResultsParcelable p) {
        ...
        if (cfg.getIpAddress() != null
                && cfg.getIpAddress().getAddress() instanceof Inet4Address) {
        ...
        if (p.serverAddress == null) {
            ...
            serverAddress = (Inet4Address) InetAddresses.parseNumericAddress(p.serverAddress);
    ...
    /**  */
    @VisibleForTesting
    public static DhcpInfo fromDhcpResultsToDhcpInfo(DhcpResults results) {
        if (results == null) return null;

        DhcpInfo info = new DhcpInfo();
        final DhcpResultsParcelable parcelResults = toStableParcelable(results);

        info = fromStableParcelable(parcelResults);
