...
public class RavenwoodRuntimeEnvironmentController {
    ...
    public static void setSystemProperties(@Nullable RavenwoodTestProperties systemProperties) {
        if (systemProperties == null) {
            ...
            sActiveProperties.stream()
                    .flatMap(prop -> prop.getValues().keySet().stream())
                    .forEach(RavenwoodRuntimeNative::removeSystemProperty);
    ...
    private static void checkSystemPropertyAccess(String key, boolean write) {
        ...
        if (write && RavenwoodSystemProperties.sDefaultValues.containsKey(key)) {
            ...
            result = RavenwoodSystemProperties.isKeyAccessible(key, write)
                    || sActiveProperties.stream().anyMatch(p -> p.isKeyAccessible(key, write));
        }
