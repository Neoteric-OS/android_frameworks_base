...
public class SubscriptionData {
    public Subscription[] subscription;  //TODO: use ArrayList ?
    ...
    public String getIccId() {
        if (subscription.length > 0 && subscription[0] != null) {
            return subscription[0].getIccid();
        }
        return null;
    }
    ...
    public Subscription getMatchedSubscription(Subscription sub){
        for (int i = 0; i < subscription.length; i++) {
            if (subscription[i].isSame(sub)) {
