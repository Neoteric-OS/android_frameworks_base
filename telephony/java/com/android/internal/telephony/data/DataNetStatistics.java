...
import com.android.internal.telephony.data.DataConnectionTracker.Activity;
...
class DataNetStatistics implements Runnable {
    ...
    public DataNetStatistics(DataConnectionTracker dataConnectionTracker) {
        this.dct = dataConnectionTracker;
    }
    ...
    public void run() {
        ...
        if (enablePoll && (preTxPkts > 0 || preRxPkts > 0)) {
            ...
            if (getActivity() != newActivity && mIsScreenOn) {
                ...
                dct.mPhone.notifyDataActivity();
            }
        }

        if (mIsScreenOn) {
            netStatPollPeriod = Settings.Secure.getInt(dct.mPhone.getContext().getContentResolver(),
        ...
        if (enablePoll) {
            dct.postDelayed(this, netStatPollPeriod);
        }
    }
};
