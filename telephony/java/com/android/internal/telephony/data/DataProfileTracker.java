...
import com.android.internal.telephony.data.DataConnectionTracker.State;
...
public class DataProfileTracker extends Handler {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case EVENT_DATA_PROFILE_DB_CHANGED:
                reloadAllDataProfiles(DataInterface.REASON_DATA_PROFILE_LIST_CHANGED);
                break;
            default:
        }
    }
    ...
    private synchronized boolean reloadAllDataProfiles(String reason) {
        ...
        /*
         * For supporting CDMA, for now, we just create Data profile of TYPE NAI that supports
         * all default service types and add it to all DataProfiles. Also create a Data profile for
         * handling DUN calls.
         * TODO: this information can be read from apns-conf.xml / carriers db.
         */
        DataServiceType t1[] = new DataServiceType[] {
                DataServiceType.SERVICE_TYPE_DEFAULT,
                DataServiceType.SERVICE_TYPE_MMS,
                DataServiceType.SERVICE_TYPE_HIPRI,
                DataServiceType.SERVICE_TYPE_SUPL};
        CdmaNAI cdmaDefaultProfile = new CdmaNAI(0, t1, BearerType.IP.toString());
        allDataProfiles.add(cdmaDefaultProfile);

        DataServiceType t2[] = new DataServiceType[] {
                DataServiceType.SERVICE_TYPE_DUN};
        CdmaNAI cdmaDunProfile = new CdmaNAI(1, t2, BearerType.IP.toString());
        allDataProfiles.add(cdmaDunProfile);
    ...
    private ArrayList<DataProfile> createDataProfileList(Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            do {
                DataServiceType serviceTypes[] = parseServiceTypes(cursor.getString(cursor
                        .getColumnIndexOrThrow(Telephony.Carriers.TYPE)));
                /* carriers db just stores only APNs now - it can change */
                ApnSetting apn = new ApnSetting(
                        ...
                        serviceTypes,
                        cursor.getString(cursor.getColumnIndexOrThrow(Telephony.Carriers.BEARER_TYPE)));
                result.add(apn);
    ...
    DataProfile getNextWorkingDataProfile(DataServiceType ds, DataProfileType dpt, IPVersion ipv) {
        /*
         * For default service type, return preferred APN if it is known to
         * work.
         */
        if (dpt == DataProfileType.PROFILE_TYPE_3GPP_APN
                && ds == DataServiceType.SERVICE_TYPE_DEFAULT) {
            if (mPreferredDefaultApn != null && mPreferredDefaultApn.isWorking(ipv)
                    && mPreferredDefaultApn.getBearerType().supportsIpVersion(ipv)) {
    ...
    /*
     * The following is relevant only for the default profile Type + INET.
     */
