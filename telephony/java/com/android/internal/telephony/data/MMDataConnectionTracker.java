...
public class MMDataConnectionTracker extends DataConnectionTracker {
     ...
     */
    private static final boolean SUPPORT_PRIORITY_ARBITRATION = SystemProperties.getBoolean(
    ...
    protected void onConnectDone(AsyncResult ar) {
        ...
        if (ar.exception == null) { /* connection is up!! */
            ...
            if (c.dc.getBearerType().supportsIpVersion(IPVersion.INET)) {
                mDpt.setServiceTypeAsActive(c.ds, c.dc, IPVersion.INET);
            }
            if (c.dc.getBearerType().supportsIpVersion(IPVersion.INET6)) {
                mDpt.setServiceTypeAsActive(c.ds, c.dc, IPVersion.INET6);
            }
        ...
        if (cause.ipVersionNotSupported()) {
             ...
             */
            if (SUPPORT_PRIORITY_ARBITRATION && disconnectOneLowPriorityDataCall(c.ds, c.reason)) {
    ...
    synchronized protected void onUpdateDataConnections(String reason, int context) {
        ...
        /*
         * Phase 1: Walk through list of all data connections
         * 1a. If data connection went inactive, then update the state of all
         * service types that are dependent on it.
         * 1b. If data connection is active, and if no service type is dependent on it
         * then bring it down.
         */

        boolean wasAnyDcDisconnected = false;
        ...
        for (DataConnection dc : mDataConnectionList) {
            if (dc.isInactive()) {
                 ...
                 */
                for (DataServiceType ds : DataServiceType.values()) {
                    boolean notifyServiceType = false;
                    if (mDpt.getActiveDataConnection(ds, IPVersion.INET) == dc) {
                        mDpt.setServiceTypeAsInactive(ds, IPVersion.INET);
                        notifyServiceType = true;
                    }
                    if (mDpt.getActiveDataConnection(ds, IPVersion.INET6) == dc) {
                        mDpt.setServiceTypeAsInactive(ds, IPVersion.INET6);
                        notifyServiceType = true;
                    }
                    if (notifyServiceType) {
                        /*
                         * state of this service type has changed. notify().
                         */
                        notifyDataConnection(ds, reason);
                    }
                }
            } else if (dc.isActive() && mDataCallSetupPending == false) {
                /*
                 * 1b. If data connection is active, and if no service type is dependent on it
                 * then bring it down. If data call setup is pending, then do this later.
                 */
                boolean needsTearDown = true;
                for (DataServiceType ds : DataServiceType.values()) {
                    if (mDpt.isServiceTypeEnabled(ds)
                            && (mDpt.getActiveDataConnection(ds, IPVersion.INET) == dc || mDpt
                                    .getActiveDataConnection(ds, IPVersion.INET6) == dc)) {
                        needsTearDown = false;
                        break;
                    }
                }
                if (needsTearDown  == true || mDisconnectAllDataCalls == true) {
                    wasAnyDcDisconnected = wasAnyDcDisconnected | tryDisconnectDataCall(dc, reason);
                }
            } else if (dc.isInactive() == false && mDisconnectAllDataCalls == true) {
                /*
                 * 1c. data connection setup is pending and we were asked to
        ...
        if (wasAnyDcDisconnected == true) {
            /*
             * If something was disconnected, then wait for at least one
             * disconnect to complete, before setting up data calls again. This
             * will ensure that all possible data connections are freed up
         ...
         * trying any new calls. This is an optimization that can be removed. Serializing data
         * call setup requests make life easier for debugging etc.
         */
         ...
         * call as required in order of decreasing service priority - highest
         * priority service gets data call setup first!
         */
        for (DataServiceType ds : DataServiceType.getPrioritySortedValues()) {
             ...
             */
            for (DataConnection dc : mDataConnectionList) {
                if (dc.isActive() && dc.getDataProfile().canHandleServiceType(ds)) {

                    if (mDpt.getActiveDataConnection(ds, IPVersion.INET) == null
                            && dc.getBearerType().supportsIpVersion(IPVersion.INET)) {
                        mDpt.setServiceTypeAsActive(ds, dc, IPVersion.INET);
                        notifyServiceType = true;
                    }
                    if (mDpt.getActiveDataConnection(ds, IPVersion.INET6) == null
                            && dc.getBearerType().supportsIpVersion(IPVersion.INET6)) {
                        mDpt.setServiceTypeAsActive(ds, dc, IPVersion.INET6);
                        notifyServiceType = true;
                    }

                    if (mDpt.isServiceTypeEnabled(ds) && notifyServiceType) {
                        notifyDataConnection(ds, reason);
                    }
                }
            }
            ...
            if (mDpt.isServiceTypeEnabled(ds) == true) {
                ...
                // IPV4
                if (SUPPORT_IPV4 && mDpt.isServiceTypeActive(ds, IPVersion.INET) == false) {
                ...
                // IPV6
                if (SUPPORT_IPV6 && mDpt.isServiceTypeActive(ds, IPVersion.INET6) == false) {
    ...
    public synchronized void setDataConnectionAsDesired(boolean desiredPowerState,
        ...
        if (mDesiredPowerState == false &&
            ...
            disconnectAllConnections(Phone.REASON_RADIO_OFF);
            return;
        }
    ...
    private boolean trySetupDataCall(DataServiceType ds, IPVersion ipv, String reason) {
        ...
        if (dc == null) {
            ...
            // big enough!
            boolean ret = SUPPORT_PRIORITY_ARBITRATION &&
