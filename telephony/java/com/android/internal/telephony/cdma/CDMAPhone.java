...
public class CDMAPhone extends PhoneBase {
    ...
    protected void finalize() {
        if(LOCAL_DEBUG) Log.d(LOG_TAG, "...setting radioTechnologyChangeCdmaToGsm to false.");
        PhoneProxy.setRadioTechnologyChangeCdmaToGsm(false);
    }
    ...
    public void dispose() {
        synchronized(PhoneProxy.lockForRadioTechnologyChange) {
            ...
            mSST.dispose();
            mSMS.dispose();
            ((RuimFileHandler)mIccFileHandler).dispose();
            ...
            h.removeMessages(EVENT_RADIO_AVAILABLE);
            h.removeMessages(EVENT_GET_BASEBAND_VERSION_DONE);
            h.removeMessages(EVENT_GET_DEVICE_IDENTITY_DONE);
            h.removeMessages(EVENT_RUIM_RECORDS_LOADED);
            h.removeMessages(EVENT_RADIO_OFF_OR_NOT_AVAILABLE);
            h.removeMessages(EVENT_RADIO_ON);
            h.removeMessages(EVENT_SSN);
            h.removeMessages(EVENT_CALL_RING);
            h.removeMessages(EVENT_REGISTERED_TO_NETWORK);
            h.removeMessages(EVENT_NV_READY);
            h.removeMessages(EVENT_GET_CALL_FORWARD_DONE);
            h.removeMessages(EVENT_SET_CALL_FORWARD_DONE);
            h.removeMessages(EVENT_SET_CLIR_COMPLETE);

            this.mCM = null; //CommandsInterface
            this.mNvLoadedRegistrants = null; //RegistrantList;
            this.mCT = null; //CallTracker
            this.mDataConnection = null; // CdmaDataConnectionTracker
            this.mPostDialHandler = null; //Registrant
            this.mRuimCard = null; //RuimCard
            this.mRuimPhoneBookInterfaceManager = null; //RuimPhoneBookInterfaceManager
            this.mRuimRecords = null; //RuimRecords
            this.mRuimSmsInterfaceManager = null; //RuimSmsInterfaceManager
            this.mSMS = null; //SMSDispatcher
            this.mSST = null; //ServiceStateTracker
            this.mSubInfo = null; //PhoneSubInfo
            this.mIccFileHandler = null; //RuimFileHandler
            this.h = null; //Handler
        }
    }
...
//      throw new RuntimeException(); 
    ...
    /*package*/ void notifyDataActivity() {
    ...
    public void unregisterForNvLoaded(Handler h) {
      ...
      * {@inheritDoc}
      */
     protected final void setSystemProperty(String property, String value) {
