...
public class IccUtils {
    ...
    public static byte[] stringToAdnStringField(@NonNull String alphaTag) {
        ...
        byte[] ret = null;
        int dstOffset = 0;
        short temp = 0, min = (short) 0x7fff, max = 0;
        ...
        if (alphaTagBytes.length > 2) {
            for (int index = 0; index < alphaTagBytes.length; index += 2) {
                ...
                if (alphaTagBytes[index] != 0) {
                    ...
                    // If alphaTagBytes have both UCS-2 format characters and GSM7bit characters,
                    // go through to use '0x80' format.
                    max = (short) (min + 130);
                    break;
                }
            }
        ...
        if (max >= min && (max - min) < 129) {
            ...
            for (int index = 0; index < alphaTagBytes.length; index += 2) {
                temp = (short) (((short) (((alphaTagBytes[index] << 8) & 0xff00)
                        | (alphaTagBytes[index + 1]) & 0xff)) - min);
