...
public final class LteVopsSupportInfo implements Parcelable {
    ...
    @IntDef(
            value = {LTE_VOPS_STATUS_NOT_AVAILABLE, LTE_VOPS_STATUS_SUPPORTED,
                    LTE_VOPS_STATUS_NOT_SUPPORTED})
    public @interface LteVopsStatus {}
    /**
     * Indicates information not available from modem.
     */
    public static final int LTE_VOPS_STATUS_NOT_AVAILABLE = 0;

    /**
     * Indicates network support the feature.
     */
    public static final int LTE_VOPS_STATUS_SUPPORTED = 1;

    /**
     * Indicates network does not support the feature.
     */
    public static final int LTE_VOPS_STATUS_NOT_SUPPORTED = 2;
    ...
    public LteVopsSupportInfo() {
        mVopsSupport = LTE_VOPS_STATUS_NOT_AVAILABLE;
        mEmcBearerSupport = LTE_VOPS_STATUS_NOT_AVAILABLE;
    }

    public LteVopsSupportInfo(boolean vops, boolean emergency) {
        if (vops) {
            mVopsSupport = LTE_VOPS_STATUS_SUPPORTED;
        } else {
            mVopsSupport = LTE_VOPS_STATUS_NOT_SUPPORTED;
        }

        if (emergency) {
            mEmcBearerSupport = LTE_VOPS_STATUS_SUPPORTED;
        } else {
            mEmcBearerSupport = LTE_VOPS_STATUS_NOT_SUPPORTED;
        }
    }
