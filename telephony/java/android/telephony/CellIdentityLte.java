...
public final class CellIdentityLte implements Parcelable {
    ...
    public CellIdentityLte (int ci, int pci, int tac, int earfcn, String mccStr,
        ...
        if (mccStr == null || mccStr.matches("^[0-9]{3}$")) {
            ...
            // If the mccStr parsed from Parcel is empty or unknown, set it as null.
            mMccStr = null;
        } else {
        ...
        if (mncStr == null || mncStr.matches("^[0-9]{2,3}$")) {
            mMncStr = mncStr;
        } else if (mncStr.isEmpty() || mccStr.equals(String.valueOf(Integer.MAX_VALUE))) {
            // If the mncStr parsed from Parcel is empty or unknown, set it as null.
            mMncStr = null;
        } else {
