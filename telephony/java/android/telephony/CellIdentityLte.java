...
public final class CellIdentityLte implements Parcelable {
    ...
    public CellIdentityLte (int ci, int pci, int tac, int earfcn, String mccStr,
        ...
        if (mccStr == null) {
            mMccStr = String.valueOf(Integer.MAX_VALUE);
        } else {
            if (mccStr.matches("^[0-9]{3}$")) {
                mMccStr = mccStr;
            } else {
        ...
        if (mncStr == null) {
            mMncStr = String.valueOf(Integer.MAX_VALUE);
        } else {
            if (mncStr.matches("^[0-9]{2,3}$")) {
                mMncStr = mncStr;
            } else {
    ...
    /**
     * @return a 5 or 6 character string (MCC+MNC), null if unknown
     */
    ...
    @Override
    public int hashCode() {
        return Objects.hash(mMccStr, mMncStr, mCi, mPci, mTac);
    }
