    ...
    public static int getNetworkTypeBitmaskFromString(String networkTypeList) {
        ...
        for (String networkType : splitter) {
            ...
            try {
                ...
                Rlog.e(LOG_TAG, "[ServiceState] Unexpected networkType=" + networkType);
                continue;
            }
            networkTypeInt = getBitmaskForTech(networkTypeInt);
            if (networkTypeInt == 0) {
                if (DBG) {
                    Rlog.d(LOG_TAG, "[ServiceState] networkType=NETWORK_TYPE_UNKNOWN");
                }
                continue;
            }
            networkTypeBitmask |= networkTypeInt;
        }
