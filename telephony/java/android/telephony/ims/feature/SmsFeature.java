...
public class SmsFeature extends ImsFeature {
  ...
  private final IImsSmsFeature mIImsFeature = new IImsSmsFeature.Stub() {
    @Override
    ...
    public void acknowledgeSms(int result) {
      synchronized (mLock) {
        SmsFeature.this.acknowledgeSms(result);
      }
    }
  ...
  public final void deliverSms(int format, byte[] pdu) throws IllegalStateException {
    // TODO(abdalkader): Guard against NPE/ Check if feature is ready and thrown an exception
    // otherwise.
