...
public class CapabilityChangeRequest implements Parcelable {
    ...
    public static class CapabilityPair {
        ...
        public final int radioTech;

        public CapabilityPair(long capability, int radioTech) {
        ...
        public boolean equals(Object o) {
            ...
            return radioTech == that.radioTech;
        }

        @Override
        public int hashCode() {
            int result = (int) (capability ^ (capability >>> 32));
            result = 31 * result + radioTech;
            return result;
        }
    }

    // Pair contains <radio tech, capability>
    ...
    public void addCapabilitiesToEnableForTech(
            @MmTelFeature.MmTelCapabilities.MmTelCapability long capabilities,
    ...
    /**
     * @return an {@link ArraySet} of {@link CapabilityPair}s that are requesting to be enabled.
     */
    public ArraySet<CapabilityPair> getCapabilitiesToEnable() {
        return new ArraySet<>(mCapabilitiesToEnable);
    }

    /**
     * @return an {@link ArraySet} of {@link CapabilityPair}s that are requesting to be disabled.
     */
    public ArraySet<CapabilityPair> getCapabilitiesToDisable() {
        return new ArraySet<>(mCapabilitiesToDisable);
    }

    // Iterate through capabilities bitfield and add each one as a pair associated with the radio
    // technology
    private void addAllCapabilities(Set<CapabilityPair> set, long capabilities, int tech) {
    ...
    protected CapabilityChangeRequest(Parcel in) {
        ...
        for (int i = 0; i < enableSize; i++) {
            mCapabilitiesToEnable.add(new CapabilityPair(/*capability*/ in.readLong(),
                    /*radioTech*/ in.readInt()));
        }
        int disableSize = in.readInt();
        mCapabilitiesToDisable = new ArraySet<>(disableSize);
        for (int i = 0; i < disableSize; i++) {
            mCapabilitiesToDisable.add(new CapabilityPair(/*capability*/ in.readLong(),
                    /*radioTech*/ in.readInt()));
        }
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        for (CapabilityPair pair : mCapabilitiesToEnable) {
            dest.writeLong(pair.capability);
            dest.writeInt(pair.radioTech);
        }
