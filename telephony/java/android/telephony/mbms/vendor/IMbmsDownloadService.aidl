...
interface IMbmsDownloadService
{
     ...
     */
    void getFileServices(String appName, int subId, in List<String> serviceClasses);

    /**
     * should move the params into a DownloadRequest parcelable
     */
    int download(String appName, int subId, String serviceId, in Uri source, in Uri destination,
            in PendingIntent cleanUpPI, in PendingIntent fileDescriptorPI,
            in PendingIntent resultPI, IDownloadListener listener);

    List<DownloadRequest> listPendingDownloads();

    boolean cancelDownload(int downloadId);

    DownloadStatus getDownloadStatus(int downloadId);
     ...
     */
    void resetDownloadKnowledge(int downloadId);
