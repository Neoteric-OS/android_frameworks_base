...
public class SignalStrength implements Parcelable {
    ...
    private CellSignalStrength getPrimary() {
        ...
        // newer faster RATs for default/for display purposes. The logic works on the assumption
        // that the only time 2 RATs will have valid info in SignalStrength is in LTE+CDMA mode.
        // There is also support for "tunneling" values through WCDMA/GSM in case an older radio
        // does not support TDSCDMA. In the future, newer/faster/higher priority RATs should
        // continue being added at the top. CDMA is the outlier in the prioritization order, but
        // in practice it doesn't matter because CDMA is mutually exclusive with TDSCDMA/WCDMA/GSM.
        // If no SignalStrength info is valid for any RAT, then return LTE, which will ensure
        // that isGsm() returns "true" by default, keeping with legacy logic that returns true
        // for isGsm() unless we are camped *exclusively* on CDMA.
    ...
    /**
     * Returns a List of CellSignalStrengths Used to Calculate this SignalStrength.
     *
    ...
    /** @hide */
    public void calculateLevel(PersistableBundle cc, ServiceState ss) {
        mLteRsrpBoost = ss.getLteEarfcnRsrpBoost();
        mCdma.calculateLevel(cc, ss);
        mGsm.calculateLevel(cc, ss);
        mWcdma.calculateLevel(cc, ss);
        mTdscdma.calculateLevel(cc, ss);
        mLte.calculateLevel(cc, ss);
    }
