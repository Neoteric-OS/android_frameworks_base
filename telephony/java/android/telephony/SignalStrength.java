...
public class SignalStrength implements Parcelable {
    ...
    /**
     * Gets WCDMA signal strength as a dbm value between -120 and -24.
     *
     * @hide
     */
    ...
    public int getWcdmaLevel() {
        ...
        switch (mWcdmaDefaultSignalMeasurement) {
            ...
            default:
                ...
                if (mWcdmaSignalStrength < 0 || mWcdmaRscp > 31) {
                    if (mWcdmaSignalStrength != 99) {
                        Log.wtf(LOG_TAG, "getWcdmaLevel - invalid WCDMA RSSI: mWcdmaSignalStrength="
                                + mWcdmaSignalStrength);
                    }
                } else if (mWcdmaRscp >= 12) {
                    level = SIGNAL_STRENGTH_GREAT;
                } else if (mWcdmaRscp >= 8) {
                    level = SIGNAL_STRENGTH_GOOD;
                } else if (mWcdmaRscp >= 5) {
                    level = SIGNAL_STRENGTH_MODERATE;
                } else if (mWcdmaRscp >= 0) {
                    level = SIGNAL_STRENGTH_POOR;
                } else {
                    level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
                }
   ...
   /**
    ...
    public int hashCode() {
        ...
        return ((mGsmSignalStrength * primeNum)
                ...
                + (mWcdmaSignalStrength * primeNum) + (mWcdmaRscp * primeNum)
