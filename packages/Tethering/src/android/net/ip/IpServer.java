...
public class IpServer extends StateMachine {
    ...
    public IpServer(
            Context context, String ifaceName, Looper looper, int interfaceType,
            SharedLog log, Callback callback, boolean usingLegacyDhcp, Dependencies deps) {
        ...
        // TODO: Replace this before IpServer don't run on system server process.
        mStatsService = INetworkStatsService.Stub.asInterface(
                (IBinder) context.getSystemService(Context.NETWORK_STATS_SERVICE));
    ...
    private boolean configureIPv4(boolean enabled) {
        ...
        try {
            final InterfaceConfigurationParcel ifcg = mNetd.interfaceGetCfg(mIfaceName);
            if (ifcg == null) {
                mLog.e("Received null interface config");
                return false;
            }

            ifcg.ifName = mIfaceName;
            ifcg.ipv4Addr = ipAsString;
            ifcg.prefixLength = prefixLen;
            ArraySet<String> interfaceFlags = new ArraySet<>();
            for (String flag : ifcg.flags) interfaceFlags.add(flag);
            if (mInterfaceType == ConnectivityManager.TETHERING_WIFI) {
                // The WiFi stack has ownership of the interface up/down state.
                // It is unclear whether the Bluetooth or USB stacks will manage their own
                // state.
                interfaceFlags.remove("up");
                interfaceFlags.remove("down");
            } else {
                if (enabled) {
                    interfaceFlags.remove("down");
                    interfaceFlags.add("up");
                } else {
                    interfaceFlags.remove("up");
                    interfaceFlags.add("down");
                }
            }
            interfaceFlags.remove("running");
            ifcg.flags = interfaceFlags.toArray(new String[0]);

            // TODO: this may throw if the interface is already gone. Do proper handling and
            // simplify the DHCP server start/stop.
            mNetd.interfaceSetCfg(ifcg);
    ...
    private void configureLocalIPv6Routes(
        ...
        if (!deprecatedPrefixes.isEmpty()) {
            ...
            final int removalFailures = modifyRouteToLocalNetwork(false/* remove */, toBeRemoved);
            if (removalFailures > 0) {
                mLog.w(String.format("Failed to remove %d IPv6 routes from local table.",
                        removalFailures));
            }
        ...
        if (newPrefixes != null && !newPrefixes.isEmpty()) {
            ...
            if (!addedPrefixes.isEmpty()) {
                ...
                // Add routes from local network. Note also that adding routes that
                // already exist does not cause an error (EEXIST is silently ignored).
                final int addFailures = modifyRouteToLocalNetwork(true/* add */, toBeAdded);
                if (addFailures > 0) {
                    mLog.w(String.format("Failed to add %d IPv6 routes from local table.",
                            addFailures));
                }
    ...
    private int modifyRouteToLocalNetwork(final boolean add,
            final ArrayList<RouteInfo> toBeModified) {
        int failures = 0;
        for (RouteInfo route : toBeModified) {
            // Don't add default route in local table.
            if (add && route.isDefaultRoute()) continue;

            if (!modifyRoute(add, INetd.LOCAL_NET_ID, route)) {
                failures++;
            }
        }

        // When adding routes, IPv6 link local should be activated always.
        if (add && !modifyRoute(true/** add*/, INetd.LOCAL_NET_ID,
    ...
    private boolean modifyRoute(final boolean add, final int netId, final RouteInfo route) {
        ...
        try {
            if (add) {
                mNetd.networkAddRoute(netId, ifName, dst, nextHop);
            } else {
                mNetd.networkRemoveRoute(netId, ifName, dst, nextHop);
            }
    ...
    class BaseServingState extends State {
        ...
        public void enter() {
            ...
            try {
                ...
                final InterfaceConfigurationParcel ifcg = mNetd.interfaceGetCfg(mIfaceName);
                final InetAddress addr = (Inet4Address) parseNumericAddress(ifcg.ipv4Addr);
                final LinkAddress linkAddr = new LinkAddress(addr, ifcg.prefixLength);
                ...
                routes.add(new RouteInfo(linkAddr, null, mIfaceName));
            } catch (Exception e) {
        ...
        public void exit() {
            ...
            try {
                ...
                mNetd.networkRemoveInterface(INetd.LOCAL_NET_ID, mIfaceName);
            } catch (Exception e) {
    ...
    class TetheredState extends BaseServingState {
        ...
        private void cleanupUpstreamInterface(String upstreamIface) {
            ...
            try {
                ...
                if (VDBG) Log.e(TAG, "Exception in ipfwdRemoveInterfaceForward: " + e.toString());
            }
            try {
