...
public class PrivateAddressCoordinator {
    ...
    private ArrayList<IpPrefix> mLastConflictPrefixes;

    public PrivateAddressCoordinator() {
        mDownstreams = new ArraySet<>();
        mUpstreamPrefixMap = new ArrayMap<>();
        mLastConflictPrefixes = new ArrayList<>(0);
     ...
     */
    public void updateUpstreamPrefix(final Network network,
        ...
        if (handleMaybePrefixConflict(ipv4Prefixes)) mLastConflictPrefixes = ipv4Prefixes;
    }
    ...
    private boolean handleMaybePrefixConflict(final List<IpPrefix> prefixes) {
        ...
        for (IpServer downstream : mDownstreams) {
            ...
            for (IpPrefix source : prefixes) {
                if (isConflictPrefix(source, target)) {
                    downstream.sendMessage(IpServer.CMD_RESTART_TETHERING);
                    hasConflict = true;
                    break;
                }
            }
    ...
    public LinkAddress requestDownstreamAddress(final IpServer ipServer) {
        ...
        for (int i = 0; i < MAX_UBYTE; i++) {
            ...
            // Downstream address may be requested before getting current upstream notification,
            // ensure prefix do not conflict with last conflict upstreams snapshot. Especitially
            // for the case that restart tethering when ip conflict.
            if (isConflictWithLastUpstreamPrefixes(prefix)) continue;
    ...
    public void releaseDownstreamPrefix(final IpServer ipServer) {
    ...
    private boolean isConflictWithLastUpstreamPrefixes(final IpPrefix source) {
        for (IpPrefix target: mLastConflictPrefixes) {
            if (isConflictPrefix(source, target)) return true;
        }
        return false;
    }
    ...
    void dump(final IndentingPrintWriter pw) {
        pw.println("mLastConflictPrefixes:");
        pw.increaseIndent();
        for (IpPrefix prefix : mLastConflictPrefixes) {
            pw.println(prefix);
        }
