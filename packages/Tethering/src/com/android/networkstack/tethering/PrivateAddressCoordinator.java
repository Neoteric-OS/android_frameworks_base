...
public class PrivateAddressCoordinator {
    ...
    // Get the next non-conflict sub prefix. E.g: To get next sub prefix from 10.0.0.0/8, the
    // previous selecting prefix is 10.20.42.0/24(subPrefix: 0.20.42.0). The conflictPrefix is
    // 10.16.0.0/20 (10.16.0.0 ~ 10.16.15.255) that max address under subPrefix is 0.16.15.255,
    // the next subPrefix is 0.16.16.255/24 (0.16.15.255 + 0.0.1.0).
    private int getNextSubPrefix(final IpPrefix conflictPrefix, final int prefixRangeMask) {
        ...
        // Pick an sub prefix a full prefix (1 << (32 - PREFIX_LENGTH) addresses) greater than
    ...
    private LinkAddress chooseDownstreamAddress(final IpPrefix prefixRange) {
        // The netmask of the prefix assignment block (e.g., 0xfff for 172.16.0.0/12).
        ...
        final int rootAddress = getPrefixBaseAddress(prefixRange);
        ...
        // - If all prefixes within prefixRange are in use, the method will return null.
        ...
        IpPrefix downstreamPrefix = findAvailablePrefixFromRange(
                 randomPrefixStart, (~prefixRangeMask) + 1, rootAddress, prefixRangeMask);
        ...
        downstreamPrefix = findAvailablePrefixFromRange(
                0, randomPrefixStart, rootAddress, prefixRangeMask);
    ...
    private IpPrefix findAvailablePrefixFromRange(final int start, final int end,
        ...
        while (newSubPrefix < end) {
            final InetAddress address = intToInet4AddressHTH(rootAddress | newSubPrefix);
