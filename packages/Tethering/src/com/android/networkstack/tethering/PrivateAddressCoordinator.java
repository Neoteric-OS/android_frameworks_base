...
public class PrivateAddressCoordinator {
    ...
    // next subAddress is 0.16.16.255 (0.16.15.255 + 0.0.1.0).
    ...
    private LinkAddress chooseDownstreamAddress(final IpPrefix prefixRange) {
        ...
        downstreamPrefix = findAvailablePrefixFromRange(
                0, subAddress, rootAddress, prefixRangeMask);

        return  getLinkAddress(downstreamPrefix, subAddress);
    }

    private LinkAddress getLinkAddress(final IpPrefix prefix, final int randomAddress) {
        if (prefix == null) return null;

        final int suffixAddress = randomAddress & ~prefixLengthToV4NetmaskIntHTH(PREFIX_LENGTH);
        final int prefixAddress = getPrefixAddress(prefix);
        final InetAddress address = intToInet4AddressHTH(prefixAddress | suffixAddress);
