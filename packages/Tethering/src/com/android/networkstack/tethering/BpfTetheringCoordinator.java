...
public class BpfTetheringCoordinator {
    ...
    BpfTetheringCoordinator(@NonNull Handler handler, @NonNull INetd netd,
            @NonNull NetworkStatsManager nsm, @NonNull SharedLog log) {
        mHandler = handler;
        mNetd = netd;
    ...
    public void removeForwardingRule(@NonNull Ipv6ForwardingRule rule) {
        ...
        // If there are no more downstream on the upstream, clean up the data limit, tether stats
        // map and remove the entry automatically.
        if (clients.isEmpty()) {
            // Remove the entry to notify clearDataLimit() that there has no more client on a
            // given upstream and it is okay to clear its data limit.
            mClientAddresses.remove(upstreamIfindex);

            // Clean up the data limit on the given upstream.
    ...
    /** Add upstream name lookup table. The lookup table is used for tether stats interface name
    ...
    private class BpfTetherStatsProvider extends NetworkStatsProvider {
        ...
        private void updateNetworkStatsDelta(Integer ifIndex, ForwardedStats diff) {
            ...
            // Accumulate the tether stats delta for the service tether stats query.
    ...
    private void updateTetherStats() {
        ...
        for (TetherStatsParcel tetherStats : tetherStatsList) {
            try {
                ...
                if (mStatsProvider != null) mStatsProvider.updateNetworkStatsDelta(ifIndex, diff);
