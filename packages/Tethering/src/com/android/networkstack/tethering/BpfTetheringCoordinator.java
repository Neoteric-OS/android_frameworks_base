...
public class BpfTetheringCoordinator {
    ...
    public void removeForwardingRule(@NonNull Ipv6ForwardingRule rule) {
        ...
        if (clients.isEmpty()) {
            ...
            updateForwardedStatsFromNetd();
            try {
                mNetd.tetherOffloadClearStats(upstreamIfindex);
    ...
    class BpfTetherStatsProvider extends NetworkStatsProvider {
        ...
        private void accumulateStatsIface(@NonNull NetworkStats diff) {
            mIfaceStats = mIfaceStats.add(diff);
        }

        private void accumulateStatsUid(@NonNull NetworkStats diff) {
            mUidStats = mUidStats.add(diff);
        }
    }
    ...
    private NetworkStats buildNetworkStats(StatsType type, @NonNull Integer ifIndex,
        ...
        stats = stats.addEntry(new Entry(iface, uid, SET_DEFAULT, TAG_NONE, METERED_NO,
                ROAMING_NO, DEFAULT_NETWORK_NO, diff.rxBytes, diff.rxPackets,
                diff.txBytes, diff.txPackets, 0L /* operations */));
        return stats;
    }

    private void accumulateDiffToStatsProvider(@NonNull Integer ifIndex,
            @NonNull ForwardedStats diff) {
        if (mStatsProvider == null) return;

        mStatsProvider.accumulateStatsIface(
                buildNetworkStats(StatsType.STATS_PER_IFACE, ifIndex, diff));
        mStatsProvider.accumulateStatsUid(
    ...
    private void updateForwardedStatsFromNetd() {
        ...
        for (TetherStatsParcel tetherStats : tetherStatsList) {
            try {
                ...
                // querying.
                accumulateDiffToStatsProvider(ifIndex, diff);
