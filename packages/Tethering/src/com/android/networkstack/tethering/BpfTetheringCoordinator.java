...
public class BpfTetheringCoordinator {
    ...
    /** Add upstream name to lookup table. The lookup table is used for tether stats interface name
      ...
      * Note that this can be only called on handler thread.
      */
    public void addUpstreamNameToLookupTable(Integer upstreamIfindex, String upstreamIface) {
        if (upstreamIfindex == 0 || upstreamIface == null) return;
    ...
    private class BpfTetherStatsProvider extends NetworkStatsProvider {
        ...
        private HashMap<String, ForwardedStats> mUnreportedStats = new HashMap<>();
        ...
        private void pushTetherStats() {
            ...
            try {
                ...
                // Clear the unreported network stats cache after reported.
                mUnreportedStats.clear();
            } catch (RuntimeException e) {
                mLog.e("Cannot report network stats: ", e);
            }
        }

        private NetworkStats buildNetworkStatsFromUnreportedStats(int uid) {
            NetworkStats stats = new NetworkStats(0L, 0);

            for (final Map.Entry<String, ForwardedStats> kv : mUnreportedStats.entrySet()) {
        ...
        private void accumulateDiff(@NonNull Integer ifIndex, @NonNull ForwardedStats statsDiff) {
            ...
            ForwardedStats diff = new ForwardedStats(statsDiff);
            ForwardedStats base = mUnreportedStats.get(iface);
            if (base != null) {
                diff.add(base);
            }
            mUnreportedStats.put(iface, diff);
        }
    }
    ...
    private boolean pushDataLimit(Integer ifindex, Long quotaBytes) {
        ...
        try {
            ...
            if (e.errorCode != OsConstants.ENOENT & quotaBytes == QUOTA_UNLIMITED) {
    ...
    private void updateForwadedStatsFromNetd() {
        ...
        for (TetherStatsParcel tetherStats : tetherStatsList) {
            try {
                ...
                // Update the accumulated tether stats delta for the service query.
                if (mStatsProvider != null) mStatsProvider.accumulateDiff(ifIndex, diff);
