...
public class BpfCoordinator {
    ...
    // interface index while calling function to clear all rules. For rule adding, removing,
    // updating cases may be not a problem because the downstream interface index has been there
    // already.
    // TODO: Does this need to be a LinkedHashMap or can it just be a HashMap?
    private final HashMap<IpServer, LinkedHashMap<Inet6Address, Ipv6ForwardingRule>>
            mDownstreamRules = new LinkedHashMap<>();
    ...
    public void tetherOffloadRuleAdd(
        ...
        if (!mDownstreamRules.containsKey(ipServer)) {
            mDownstreamRules.put(ipServer,
                    new LinkedHashMap<Inet6Address, Ipv6ForwardingRule>());
        }
        LinkedHashMap<Inet6Address, Ipv6ForwardingRule> rules = mDownstreamRules.get(ipServer);

        // Setup the data limit on the given upstream if the first rule is added.
        final int upstreamIfindex = rule.upstreamIfindex;
        if (!isAnyRulesOnUpstream(upstreamIfindex)) {
        ...
        // Add rule to the map. Note that must update the rules after calling
        // #isAnyRulesOnUpstream because we would like to check the rules first in that function.
    ...
    public void tetherOffloadRuleRemove(
            @NonNull final IpServer ipServer, @NonNull final Ipv6ForwardingRule rule) {
        try {
            // TODO: Perhaps avoid to remove a non-existent rules.
        ...
        // Remove the rule from the map. Note that must remove rules before calling
        // #isAnyRulesOnUpstream because we would like to check the updated rules in
        // that function.
        Ipv6ForwardingRule existing = rules.remove(rule.address);

        // Avoid unnecessary work on a non-existent rule which may have never been added or removed
        // already.
        if (existing == null) return;

        // Remove the downstream entry if it has no more rule.
        if (rules.isEmpty()) {
            mDownstreamRules.remove(ipServer);
        }
    ...
    public void tetherOffloadRuleClear(@NonNull final IpServer ipServer) {
        final LinkedHashMap<Inet6Address, Ipv6ForwardingRule> rules = mDownstreamRules.get(
                ipServer);
        if (rules == null) return;

        // Build the rule handling list. Can't iterate the rules map from |mDownstreamRules|
        // because it may be modified in #tetherOffloadRuleRemove.
        final List<Ipv6ForwardingRule> ruleList = rules.values().stream().collect(
                Collectors.toList());
        for (Ipv6ForwardingRule rule : ruleList) {
    ...
    public void tetherOffloadRuleUpdate(@NonNull final IpServer ipServer, int newUpstreamIfindex) {
        ...
        // Build the rule handling list. Can't iterate the rules map from |mDownstreamRules|
        // because it may be modified in #tetherOffloadRuleAdd and #tetherOffloadRuleRemove.
        final List<Ipv6ForwardingRule> ruleList = rules.values().stream().collect(
                Collectors.toList());
        for (Ipv6ForwardingRule rule : ruleList) {
            // Remove the old rule before adding the new one because the rule address is a key of
            // the hash map. Reversing the processing order will remove the new rule as unexpected.
    ...
    private int getInterfaceIndexFromRules(@NonNull String ifName) {
        for (LinkedHashMap<Inet6Address, Ipv6ForwardingRule> rules : mDownstreamRules.values()) {
            Optional<Ipv6ForwardingRule> match = rules.values().stream()
                    .filter(r -> TextUtils.equals(ifName, mInterfaceNames.get(r.upstreamIfindex)))
                    .findAny();
            if (match.isPresent()) return match.get().upstreamIfindex;
        }
        return 0;
    }
    ...
    private boolean setDataLimitToNetd(int ifIndex, long quotaBytes) {
        if (ifIndex == 0) {
    ...
    private boolean isAnyRulesOnUpstream(int upstreamIfindex) {
        return mDownstreamRules.values().stream().anyMatch(rules ->
                rules.values().stream().anyMatch(r -> r.upstreamIfindex == upstreamIfindex));
    }
