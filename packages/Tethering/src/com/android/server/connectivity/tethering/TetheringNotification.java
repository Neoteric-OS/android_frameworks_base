 ...
 * @hide
 */
// All callbacks of TetheringStateListener are guaranteed to be called on the same thread.
public class TetheringNotification {
    private final Context mContext;
    private final NotificationManager mNotificationManager;
    private static final int NOTIF_ID = SystemMessage.NOTE_TETHER_GENERAL;
    ...
    private int getActiveDownstreamSize() {
        int size = 0;
        for (Downstream d : mCurrentDownstreams) {
            // Downstream is one of ConnectivityManager.TETHERING_* constants, 0 1 or 2.
    ...
    private Resources getResources(final Context c, final int subId) {
    ...
    private void setupNotification() {
        ...
        if (!tetheringActive) {
            mNotificationManager.cancelAsUser(null, NOTIF_ID, UserHandle.ALL);
            return;
        }
    ...
    private void showNotification(final int iconId, final String title, final String text,
        ...
        NotificationChannel channel = new NotificationChannel(
                "tether", "Tethering Notification", NotificationManager.IMPORTANCE_DEFAULT);
        ...
        mNotificationManager.notifyAsUser(null, NOTIF_ID, notification, UserHandle.ALL);
    }
