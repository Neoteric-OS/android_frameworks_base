...
public class TetheringNotificationUpdater {
    ...
    /** Check whether MCC/MNC is specific carrier */
    @VisibleForTesting
    boolean isSpecificCarrierMccMnc() {
        ...
        // Check GID1 value further if the MCC/MNC value is 20404 because multiple operators use
        // this and only need that GID1 value is BAE0000000000000.
        return "20404".equals(mccmnc) && "BAE0000000000000".equals(gid1);
    }

    @NonNull
    @VisibleForTesting
    Resources getResourcesForSubId(@NonNull final Context c, final int subId) {
        final Resources res = SubscriptionManager.getResourcesForSubId(c, subId);
        if (isSpecificCarrierMccMnc()) {
