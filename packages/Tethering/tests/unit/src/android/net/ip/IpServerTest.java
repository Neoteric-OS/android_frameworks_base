...
public class IpServerTest {
    ...
    public void startsDhcpServerOnNcm() throws Exception {
        initTetheredStateMachine(TETHERING_NCM, UPSTREAM_IFACE);
        dispatchTetherConnectionChanged(UPSTREAM_IFACE);
    ...
    public void testOnNewPrefixRequest() throws Exception {
        ...
        final IDhcpEventCallbacks mEventCallbacks;
        final ArgumentCaptor<IDhcpEventCallbacks> dhcpEventCbsCaptor =
                 ArgumentCaptor.forClass(IDhcpEventCallbacks.class);
        verify(mDhcpServer, timeout(MAKE_DHCPSERVER_TIMEOUT_MS).times(1)).startWithCallbacks(
                any(), dhcpEventCbsCaptor.capture());
        mEventCallbacks = dhcpEventCbsCaptor.getValue();
        assertDhcpStarted(new IpPrefix("192.168.42.0/24"));

        // Simulate the DHCP server receives DHCPDECLINE on MirrorLink and then signals
        // onNewPrefixRequest callback.
        mEventCallbacks.onNewPrefixRequest(new IpPrefix("192.168.42.0/24"));
        ...
        final LinkProperties mLinkProperties = lpCaptor.getValue();
        final List<LinkAddress> mLinkAddresses = mLinkProperties.getLinkAddresses();
        assertEquals(1, mLinkProperties.getLinkAddresses().size());
        assertEquals(1, mLinkProperties.getRoutes().size());
        final IpPrefix mPrefix = new IpPrefix(mLinkAddresses.get(0).getAddress(),
                mLinkAddresses.get(0).getPrefixLength());
        assertNotEquals(mPrefix, new IpPrefix("192.168.42.0/24"));

        verify(mDhcpServer).updateParams(mDhcpParamsCaptor.capture(), any());
        assertDhcpServingParams(mDhcpParamsCaptor, mPrefix);
    }
    ...
    private void assertDhcpServingParams(final ArgumentCaptor<DhcpServingParamsParcel> captor,
            final IpPrefix prefix) {
        final DhcpServingParamsParcel params = captor.getValue();
    ...
    private void assertDhcpStarted(IpPrefix expectedPrefix) throws Exception {
        ...
        verify(mDhcpServer, timeout(MAKE_DHCPSERVER_TIMEOUT_MS).times(1)).startWithCallbacks(
                any(), any());
        assertDhcpServingParams(mDhcpParamsCaptor, expectedPrefix);
    }
