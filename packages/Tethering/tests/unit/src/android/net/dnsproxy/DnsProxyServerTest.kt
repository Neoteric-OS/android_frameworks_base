...
class DnsProxyServerTest {
    ...
    private val mockTestTCPSocket = mock(FileDescriptor::class.java)
    private val mockTestUDPSocket = mock(FileDescriptor::class.java)
    ...
    private val mockServerDeps = mock(ServerDependencies::class.java).apply {
        doReturn(mockTestUDPSocket).`when`(this).makeUDPSocket()
        doReturn(mockTestTCPSocket).`when`(this).makeTCPSocket()
    ...
    fun tearDown() {
        ...
        verify(mockQueue, times(1)).removeOnFileDescriptorEventListener(eq(mockTestUDPSocket))
        verify(mockQueue, times(1)).removeOnFileDescriptorEventListener(eq(mockTestTCPSocket))
        assertTrue(!mockTestUDPSocket.valid())
        assertTrue(!mockTestTCPSocket.valid())
        verify(mockHandlerThread, times(1)).quitSafely()
    }

    @Test
    fun testStart() {
        verify(mockServerDeps, times(1)).listenDnsUDP(eq(mockTestUDPSocket), any())
        verify(mockServerDeps, times(1)).listenDnsTCP(eq(mockTestTCPSocket), any())
        verify(mockQueue, times(1))
                .addOnFileDescriptorEventListener(eq(mockTestUDPSocket), anyInt(), any())
        verify(mockQueue, times(1))
                .addOnFileDescriptorEventListener(eq(mockTestTCPSocket), anyInt(), any())
    }
    ...
    fun testRequestResponder() {
        val request = spy(object : DnsProxyServer.RequestResponder(
                mockNetwork, mockTestUDPSocket, mockLog,
        ...
        verify(request, times(1)).cleanup(eq(mockTestUDPSocket))

        // handleRequest, normal case
        reset(request)
        request.handleRequest()
        verify(request, never()).processDnsPacket(eq(mockNetwork), eq(mockTestUDPSocket),
        ...
        request.processDnsPacket(mockNetwork, mockTestUDPSocket, packetBuf,
        ...
        verify(request, times(1))
                .transmitAnswer(eq(mockTestUDPSocket),
    ...
    fun testUDPRequestResponder() {
        ...
        request.transmitAnswer(mockTestUDPSocket, answerBuf, TEST_CLIENT_ADDR, TEST_CLIENT_PORT)
        verify(mockResponderDeps, times(1))
                .sendPacket(eq(mockTestUDPSocket), aryEq(answerBuf), eq(TEST_CLIENT_ADDR),
                        eq(TEST_CLIENT_PORT))

        val buf = request.readPacket(mockTestUDPSocket)
        verify(mockResponderDeps, times(1))
                .recvfrom(eq(mockTestUDPSocket), any(),
    ...
    fun testTCPRequestResponder() {
        ...
        val request = spy(DnsProxyServer.TCPRequestResponder(
                mockNetwork, mockTestTCPSocket,
                mockInetSocketAddress, mockLog, mockResponderDeps,
                mockDnsResolver, mockTestExecutor))
        assertTrue(request is DnsProxyServer.RequestResponder)

        request.transmitAnswer(mockTestTCPSocket, answerBuf, TEST_CLIENT_ADDR, TEST_CLIENT_PORT)
        verify(mockResponderDeps, times(1))
                .sendPacket(eq(mockTestTCPSocket), aryEq(addPacketLength(answerBuf)),
                        eq(TEST_CLIENT_ADDR),
                        eq(TEST_CLIENT_PORT))
        verify(request, times(1)).cleanup(eq(mockTestTCPSocket))

        val buf = request.readPacket(mockTestTCPSocket)
        verify(request, times(1)).setSocketTimeout(eq(mockTestTCPSocket))
        verify(mockResponderDeps, times(1)).read(eq(mockTestTCPSocket), any(), eq(0), eq(2))
        verify(mockResponderDeps, times(1))
                .read(eq(mockTestTCPSocket), any(), eq(0), eq(answerBuf.size))
        assertTrue(Arrays.equals(buf, answerBuf))
    }
}
