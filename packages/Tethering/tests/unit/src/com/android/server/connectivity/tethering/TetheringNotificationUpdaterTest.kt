...
import com.android.server.connectivity.tethering.TetheringNotificationUpdater.EVENT_SHOW_NO_UPSTREAM
...
class TetheringNotificationUpdaterTest {
    ...
    private val activeNotifications = listOf(NO_UPSTREAM_NOTIFICATION_ID, ENABLE_NOTIFICATION_ID)

    private fun verifyCancelAllActiveNotifications() {
    ...
    private fun verifyOnlyNotification(notifyId: Int, iconId: Int, title: String, text: String) {
        activeNotifications.forEach {
            when (it) {
    ...
    fun testNotificationWithDownstreamChanged() {
        ...
        verifyCancelAllActiveNotifications()
        ...
        verifyOnlyNotification(ENABLE_NOTIFICATION_ID, GENERAL_ICON_ID, TITLE, MESSAGE)

        // Usb downstream. Still show enable notification.
        notificationUpdater.onDownstreamChanged(USB_MASK)
        verifyOnlyNotification(ENABLE_NOTIFICATION_ID, USB_ICON_ID, TITLE, MESSAGE)
    ...
    fun testNotificationWithActiveDataSubscriptionIdChanged() {
        ...
        notificationUpdater.onDownstreamChanged(WIFI_MASK)
        verifyOnlyNotification(ENABLE_NOTIFICATION_ID, WIFI_ICON_ID, TEST_TITLE, TEST_MESSAGE)
    ...
    private fun Handler.waitForDelayMessage(what: Int, delayMs: Long, timeoutMs: Long) {
        var countdown = delayMs
        while (hasMessages(what) && countdown-- > 0) {
            Thread.sleep(1)
        }
    ...
    fun testNotificationWithUpstreamNetworkChanged() {
        ...
        notificationUpdater.handler.waitForDelayMessage(EVENT_SHOW_NO_UPSTREAM, 10, 500)
