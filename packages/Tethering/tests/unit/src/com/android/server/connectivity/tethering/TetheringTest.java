...
public class TetheringTest {
    ...
    private void verifyInterfaceServingModeStarted(String ifname) throws Exception {
        verify(mNetd, times(2)).interfaceGetCfg(ifname);
    ...
    public void workingLocalOnlyHotspotEnrichedApBroadcast(
        ...
        // interfaceGetCfg() called twice for enabling and once for disabling IPv4.
    ...
    private void runUserRestrictionsChange(
        ...
        Tethering.TetheringUserRestriction turl =
                new Tethering.TetheringUserRestriction(mUserManager, mockTethering);
        turl.mDisallowTethering = currentDisallow;

        turl.onUserRestrictionsChanged();
    ...
    private void workingWifiP2pGroupOwner(
        ...
        // interfaceGetCfg() called twice for enabling and once for disabling IPv4.
        verify(mNetd, times(3)).interfaceGetCfg(TEST_P2P_IFNAME);
        // interfaceSetCfg() called once for enabling and twice for disabling IPv4.
        verify(mNetd, times(3)).interfaceSetCfg(any(InterfaceConfigurationParcel.class));
        verify(mNetd, times(1)).tetherStop();
        verify(mNetd, times(1)).ipfwdDisableForwarding("tethering");
    ...
    private void workingWifiP2pGroupClient(
        ...
        verify(mNetd, never()).interfaceGetCfg(TEST_P2P_IFNAME);
        ...
        verify(mNetd, never()).ipfwdEnableForwarding("tethering");
        verify(mNetd, never()).tetherStartWithConfiguration(anyBoolean(), any(String[].class));
        ...
        verify(mNetd, never()).ipfwdDisableForwarding("tethering");
        verifyNoMoreInteractions(mNetd);
        // Asking for the last error after the per-interface state machine
        // has been reaped yields an unknown interface error.
        assertEquals(TETHER_ERROR_UNKNOWN_IFACE, mTethering.getLastTetherError(TEST_P2P_IFNAME));
