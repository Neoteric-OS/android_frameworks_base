...
public class DadProxyTest {
    ...
    private static ByteBuffer createIcmpV6Packet(int type) {
        ...
        length += (type == NeighborPacketForwarder.ICMPV6_NEIGHBOR_ADVERTISEMENT ?
        ...
        byte[] target = {(byte) 0xfe, (byte) 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x22,
                         0x33, 0x44, 0x55, 0x66, 0x77, (byte) 0x88};
        buf.put(target);
    ...
    // Doesn't work since sendTo() pipes packet to output chain, and the NeighborPacketForwarder
    // grabs packets from input chain.
    @Test
    public void testUpstreamNaPacket() {
        ...
        try {
            mUpstreamPacketReader.sendResponse(createIcmpV6Packet(
    ...
    public void testUpstreamNsPacket() {
        ...
        ByteBuffer ns_packet = createIcmpV6Packet(
