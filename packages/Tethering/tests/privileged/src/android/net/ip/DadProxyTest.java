...
public class DadProxyTest {
    ...
    public void tearDown() throws Exception {
        ...
        if (mTetheredParams != null) {
            try {
                mNetd.networkRemoveInterface(INetd.LOCAL_NET_ID, mTetheredParams.name);
            } catch (Exception e) { }
        }
        if (mUpstreamParams != null) {
            try {
                mNetd.networkRemoveInterface(INetd.LOCAL_NET_ID, mUpstreamParams.name);
            } catch (Exception e) { }
        }
    ...
    private static ByteBuffer createIcmpV6Packet(int type) {
        ...
        buf.put(src);                                               // Src
        final byte[] dst = InetAddresses.parseNumericAddress("ff02::1:ff66:7788").getAddress();
        buf.put(dst);                                               // Dst
    ...
    private void disableIpv6ProvisioningDelays() throws Exception {
        ...
        mNetd.setProcSysNet(INetd.IPV6, INetd.CONF, mUpstreamParams.name, "dad_transmits", "0");
        mNetd.setProcSysNet(INetd.IPV6, INetd.CONF, mTetheredParams.name, "dad_transmits", "0");
    }

    @Test
    public void testNaForwardingTetherToUpstream() throws Exception {
        ...
        mNetd.networkAddInterface(INetd.LOCAL_NET_ID, mUpstreamParams.name);
        mNetd.networkAddInterface(INetd.LOCAL_NET_ID, mTetheredParams.name);

        // Need to wait before sending any packets out newly created sockets.
        disableIpv6ProvisioningDelays();

        ByteBuffer na = createIcmpV6Packet(NeighborPacketForwarder.ICMPV6_NEIGHBOR_ADVERTISEMENT);

        mUpstreamPacketReader.sendResponse(na);
        // The packet sent by the proxy is different than the one it received.
        na.position(ETHER_SRC_ADDR_OFFSET);
        byte[] srcMac = mTetheredParams.macAddr.toByteArray();
        na.put(srcMac, 0, srcMac.length);
        na.rewind();
        assertFalse(waitForPacket(na, mTetheredPacketReader));
    }

    @Test
    public void testNsForwardingTetherToUpstream() throws Exception {
        DadProxy proxy = setupProxy();
        ...
        ByteBuffer ns = createIcmpV6Packet(NeighborPacketForwarder.ICMPV6_NEIGHBOR_SOLICITATION);

        mTetheredPacketReader.sendResponse(ns);
        // The packet sent by the proxy is different than the one it received.
        ns.position(ETHER_SRC_ADDR_OFFSET);
        byte[] srcMac = mUpstreamParams.macAddr.toByteArray();
        ns.put(srcMac, 0, srcMac.length);
        ns.rewind();
        assertTrue(waitForPacket(ns, mUpstreamPacketReader));
    }

    @Test
    public void testNsForwardingUpstreamToTether() throws Exception {
        DadProxy proxy = setupProxy();
        ...
        ns.put(srcMac, 0, srcMac.length);
        ns.rewind();
        assertFalse(waitForPacket(ns, mUpstreamPacketReader));
    }
}
