...
public class TetheringModuleTest {
    ...
    private void addressConflictTest(final boolean switchBaseRange) throws Exception {
        ...
        try {
            ...
            final IpPrefix testPrefix = getPrefixRange(hotspotAddr, switchBaseRange);
            ...
            final IpPrefix newTestPrefix = getPrefixRange(newHotspotAddr, switchBaseRange);
    ...
    private IpPrefix getPrefixRange(final LinkAddress address, final boolean baseRange) {
        if (!baseRange) return new IpPrefix(address.getAddress(), address.getPrefixLength());
