...
/**
 * Default carrier app allows carrier customization. Carriers could configure a list
 * of supported carrier actions to act upon certain signal or even different args of the same
 * signal. This allows different interpretations of the signal between carriers
 * and could easily alter the app's behavior in a configurable way.
 * This helper class loads and parses the carrier configs and return a list of
...
public class CustomConfigLoader {
    ...
    // map from intent to arg pair. used for config matching for intents which allow different
    // actions on different values/args of the same signal type.
    private static final HashMap<String, Pair<String, String>> mIntentToArgMap;

    // delimiters for parsing carrier configs
    ...
    // Example: passing Intent TelephonyIntent.ACTION_CARRIER_SIGNAL_REDIRECTED
    // Step 1. read config key from mIntentToConfigKeyMap
    // => KEY_CARRIER_DEFAULT_ACTIONS_ON_REDIRECTION_STRING_ARRAY
    // Step 2. read args pair from mIntentToArgMap => null
    // Step 3. load configKey from carrier config => string[] configs
    // Step 4. match each entry in the configs till found the matching entry without configured args
    // [1, 4,..] => returning action idx list, for example.
    public static List<Integer> loadCarrierActionList(Context context, Intent intent) {
        ...
        if (b != null) {
            ...
            Pair<String, String> args = mIntentToArgMap.get(intent);
            String arg1 = (args != null) ? intent.getStringExtra(args.first) : null;
            String arg2 = (args != null) ? intent.getStringExtra(args.second) : null;
            if (!ArrayUtils.isEmpty(configs)) {
                for (String config : configs) {
                    // parse each entry until find the entry with matching signal & args
                    List<Integer> actionList = matchConfig(config, arg1, arg2);
                    if (!actionList.isEmpty()) {
                        if (ret == null) {
                            if (VDBG) Rlog.d(TAG, "found match action list: " +
                                    actionList.toString());
                            ret = actionList;
                        } else {
                            // there should be only one matched entry, otherwise bad config
                            Rlog.e(TAG, "bad config with multiple matched entries: "
                                    + actionList.toString());
                        }
                        return ret;
                    }
    ...
    private static List<Integer> matchConfig(String config, String arg1, String arg2) {
