...
public class PackageWatchdog {
    ...
    private static final String METADATA_OBSERVER_FILE =
            "/metadata/watchdog/%s_mitigation_count.txt";
    ...
    private static class ObserverInternal {
        ...
        public void saveBootMitigationCountToMetadata() {
            String fileName = METADATA_OBSERVER_FILE.formatted(name);
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
                writer.write(String.valueOf(getBootMitigationCount()));
            } catch (Exception e) {
                Slog.e(TAG, "Could not save observer metadata to file: " + e);
            }
        }

        public void readBootMitigationCountFromMetadataIfNecessary() {
            String fileName = METADATA_OBSERVER_FILE.formatted(name);
            File metadataFile = new File(fileName);
            if (metadataFile.exists()) {
                try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
                    String mitigationCount = reader.readLine();
                    setBootMitigationCount(Integer.parseInt(mitigationCount));
                    metadataFile.delete();
                } catch (Exception e) {
    ...
    private void readAllObserversBootMitigationCountIfNecessary() {
        for (int i = 0; i < mAllObservers.size(); i++) {
            final ObserverInternal observer = mAllObservers.valueAt(i);
            observer.readBootMitigationCountFromMetadataIfNecessary();
        }
    }

    private void saveAllObserversBootMitigationCountToMetadata() {
        for (int i = 0; i < mAllObservers.size(); i++) {
            final ObserverInternal observer = mAllObservers.valueAt(i);
            observer.saveBootMitigationCountToMetadata();
        }
    }
