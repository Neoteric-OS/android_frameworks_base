...
final class RollbackPackageHealthObserver implements PackageHealthObserver {
    ...
    public int onHealthCheckFailed(@Nullable VersionedPackage failedPackage,
        ...
        if (Flags.recoverabilityDetection()) {
            ...
            List<RollbackInfo> lowImpactRollbacks = getRollbackAvailableForImpactLevel(
                    availableRollbacks, PackageManager.ROLLBACK_USER_IMPACT_LOW);
            if (failureReason == PackageWatchdog.FAILURE_REASON_NATIVE_CRASH
                    && !lowImpactRollbacks.isEmpty()) {
                ...
                impact = PackageHealthObserverImpact.USER_IMPACT_LEVEL_30;
            } else if (!availableRollbacks.isEmpty()) {
                impact = proposeUserImpactBasedOnRollbackImpactLevel(availableRollbacks);
            }
        } else {
    ...
    public boolean execute(@Nullable VersionedPackage failedPackage,
        ...
        if (Flags.recoverabilityDetection()) {
            ...
            if (rollback != null) {
                ...
                rollbackBasedOnImpactLevel(availableRollbacks, rollbackReason);
            }
        } else {
            if (rollbackReason == PackageWatchdog.FAILURE_REASON_NATIVE_CRASH) {
    ...
    public boolean executeBootLoopMitigation(int mitigationCount) {
        if (Flags.recoverabilityDetection()) {
            ...
            rollbackBasedOnImpactLevel(availableRollbacks,
     ...
     */
    private void rollbackBasedOnImpactLevel(List<RollbackInfo> availableRollbacks,
            @FailureReasons int rollbackReason) {
    ...
    private void rollbackHighImpact(List<RollbackInfo> availableRollbacks,
        ...
        List<RollbackInfo> highImpactRollbacks =
                getRollbackAvailableForImpactLevel(
        ...
        highImpactRollbacks.sort(
                Comparator.comparing(a -> a.getPackages().get(0).getPackageName()));
        VersionedPackage firstRollback =
                highImpactRollbacks
                        .get(0)
                        .getPackages()
        ...
        mHandler.post(() ->
                rollbackPackage(highImpactRollbacks.get(0), firstRollback, rollbackReason));
    }
    ...
    private List<RollbackInfo> getRollbackAvailableForImpactLevel(
    ...
    private int getMinRollbackImpactLevel(List<RollbackInfo> availableRollbacks) {
        return availableRollbacks.stream()
                ...
                .min()
                .orElse(PackageManager.ROLLBACK_USER_IMPACT_ONLY_MANUAL);
    }

    private int proposeUserImpactBasedOnRollbackImpactLevel(List<RollbackInfo> availableRollbacks) {
        int impact = PackageHealthObserverImpact.USER_IMPACT_LEVEL_0;
