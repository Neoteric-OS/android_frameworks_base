...
public class EthernetManager {
    ...
    private static final class NetworkInterfaceOutcomeReceiver
        ...
        @NonNull
        private final OutcomeReceiver<String, EthernetNetworkManagementException> mReceiver;
        ...
        NetworkInterfaceOutcomeReceiver(
            ...
            Objects.requireNonNull(receiver, "Pass a non-null receiver");
            mExecutor = executor;
            mReceiver = receiver;
        }
        ...
        public void onResult(@NonNull String iface) {
            mExecutor.execute(() -> mReceiver.onResult(iface));
        }

        @Override
        public void onError(@NonNull EthernetNetworkManagementException e) {
            mExecutor.execute(() -> mReceiver.onError(e));
        }
    }

    private NetworkInterfaceOutcomeReceiver getNetworkInterfaceOutcomeReceiver(
            @Nullable final Executor executor,
            @Nullable final OutcomeReceiver<String, EthernetNetworkManagementException> receiver) {
        if (null != receiver) {
            Objects.requireNonNull(executor, "Pass a non-null executor, or a null receiver");
        }
        final NetworkInterfaceOutcomeReceiver proxy;
        if (null == receiver) {
            proxy = null;
        } else {
            proxy = new NetworkInterfaceOutcomeReceiver(executor, receiver);
        }
        return proxy;
    }
     ...
     *
     * If non-null, the receiver will be called exactly once after this is called, unless
    ...
    public void updateConfiguration(
        ...
        final NetworkInterfaceOutcomeReceiver proxy = getNetworkInterfaceOutcomeReceiver(
                executor, receiver);
        try {
