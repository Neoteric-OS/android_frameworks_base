...
public class EthernetManager {
    ...
    private final ArrayList<ListenerInfo<InterfaceStateListener>> mIfaceListeners =
            new ArrayList<>();
    @GuardedBy("mListenerLock")
    private final ArrayList<ListenerInfo<Consumer<Integer>>> mEthernetStateListeners =
            new ArrayList<>();
            ...
            new IEthernetServiceListener.Stub() {
                ...
                public void onEthernetStateChanged(int state) {
                    synchronized (mListenerLock) {
                        for (ListenerInfo<Consumer<Integer>> li : mEthernetStateListeners) {
                            li.executor.execute(() -> {
    ...
    public void addEthernetStateListener(@NonNull Executor executor,
        ...
        synchronized (mListenerLock) {
            maybeAddServiceListener();
            mEthernetStateListeners.add(new ListenerInfo<Consumer<Integer>>(executor, listener));
        }
    }
    ...
    @RequiresPermission(android.Manifest.permission.ACCESS_NETWORK_STATE)
    @SystemApi(client = MODULE_LIBRARIES)
    public void removeEthernetStateListener(@NonNull Consumer<Integer> listener) {
        Objects.requireNonNull(listener);
        synchronized (mListenerLock) {
