...
public class NetworkStatsManager {
    ...
    /**
     * Query network usage statistics details for a given {@link NetworkTemplate}.
     * Result is not aggregated over time. This means buckets' start and
     * end timestamps are going to be between 'startTime' and 'endTime' parameters.
    ...
    public NetworkStats queryDetailsForNetwork(@NonNull NetworkTemplate template,
        ...
        try {
            final NetworkStats result =
                    new NetworkStats(mContext, template, mFlags, startTime, endTime, mService);
            result.startHistoryNetworkEnumeration();
            return result;
        } catch (RemoteException e) {
            e.rethrowFromSystemServer();
        }
