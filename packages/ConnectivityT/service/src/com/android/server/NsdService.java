...
import android.net.MdnsServiceManager;
import android.net.netd.aidl.IMdns;
import android.net.netd.aidl.IMdnsUnsolicitedEventListener;
import android.net.netd.aidl.MdnsServiceEvent;
...
public class NsdService extends INsdManager.Stub {
    ...
    private class NsdStateMachine extends StateMachine {
            ...
            public boolean processMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case NsdManager.DAEMON_STARTUP:
                        ...
                        break;
                    case NsdManager.MDNS_SERVICE_EVENT:
                    default:
        ...
        class EnabledState extends State {
            ...
            public boolean processMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case NsdManager.RESOLVE_SERVICE:
                        ...
                        break;
                    case NsdManager.MDNS_SERVICE_EVENT:
                        final MdnsServiceEvent mdnsServiceEvent = (MdnsServiceEvent) msg.obj;
                        if (!handleMdnsServiceEvent(mdnsServiceEvent)) {
                            return NOT_HANDLED;
                        }
                        break;
                    default:
            ...
            private boolean handleMdnsServiceEvent(MdnsServiceEvent event) {
                NsdServiceInfo servInfo;
                final int id = event.id;
                final int responseCode = event.responseCode;
                Log.d(TAG, "handleMdnsServiceEvent: id=" + id
                        + ", responseCode=" + NativeResponseCode.nameOf(responseCode)
                        + ", name=" + event.name
                        + ", type=" + event.type
                ...
                if (clientInfo == null) {
                    Log.e(TAG, String.format("id %d for responseCode %d has no client mapping",
                            id, responseCode));
                    return false;
                }
                ...
                if (clientId < 0) {
                    ...
                    Log.d(TAG, String.format(
                            "ResponseCode %d for listener id %d that is no longer active",
                            responseCode, id));
                    return false;
                }

                switch (responseCode) {
                    case IMdnsUnsolicitedEventListener.SERVICE_FOUND:
                        servInfo = new NsdServiceInfo(event.name, event.type);
                        clientInfo.onServiceFound(clientId, servInfo);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_LOST:
                        servInfo = new NsdServiceInfo(event.name, event.type);
                        clientInfo.onServiceLost(clientId, servInfo);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_DISCOVERY_FAILED:
                        clientInfo.onDiscoverServicesFailed(
                                clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_REGISTERED:
                        servInfo = new NsdServiceInfo(event.name, null);
                        clientInfo.onRegisterServiceSucceeded(clientId, servInfo);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_REGISTRATION_FAILED:
                        clientInfo.onRegisterServiceFailed(
                                clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_RESOLVED:
                        int index = 0;
                        final String fullName = event.name;
                        ...
                        clientInfo.mResolvedService.setTxtRecords(event.data);

                        stopResolveService(id);
                        removeRequestMap(clientId, id, clientInfo);

                        final int id2 = getUniqueId();
                        if (getAddrInfo(id2, event.hostName)) {
                        ...
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_RESOLUTION_FAILED:
                        stopResolveService(id);
                        removeRequestMap(clientId, id, clientInfo);
                        clientInfo.mResolvedService = null;
                        clientInfo.onResolveServiceFailed(
                                clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_GET_ADDR_FAILED:
                        stopGetAddrInfo(id);
                        removeRequestMap(clientId, id, clientInfo);
                        clientInfo.mResolvedService = null;
                        clientInfo.onResolveServiceFailed(
                                clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        break;
                    case IMdnsUnsolicitedEventListener.SERVICE_GET_ADDR_SUCCESS:
                        try {
                            clientInfo.mResolvedService.setHost(InetAddress.getByName(event.data));
    ...
    private static String removeQuatationMarkFromString(final String target) {
        return TextUtils.isEmpty(target) ? target : target.replaceAll("\"", "");
    }

    class MdnsUnsolicitedEventCallback extends IMdnsUnsolicitedEventListener.Stub {
        @Override
        public void onMdnsServiceEvent(final MdnsServiceEvent rawEvent) {
            Log.d(TAG, "onMdnsServiceEvent: id=" + rawEvent.id
                    + ", responseCode=" + NativeResponseCode.nameOf(rawEvent.responseCode));
            final MdnsServiceEvent cookedEvent = new MdnsServiceEvent(
                    rawEvent.id,
                    rawEvent.responseCode,
                    removeQuatationMarkFromString(rawEvent.name),
                    removeQuatationMarkFromString(rawEvent.type),
                    removeQuatationMarkFromString(rawEvent.hostName),
                    rawEvent.port,
                    removeQuatationMarkFromString(rawEvent.data));
            mNsdStateMachine.sendMessage(NsdManager.MDNS_SERVICE_EVENT, cookedEvent);
        }
    ...
    protected final MdnsUnsolicitedEventCallback
            mMdnsUnsolicitedEventCallback = new MdnsUnsolicitedEventCallback();

    private void registerUnsolicitedEventListener() {
        if (!mIsListenerRegistered) {
            try {
                mMdns.registerUnsolicitedEventListener(mMdnsUnsolicitedEventCallback);
    ...
    private boolean registerService(int regId, NsdServiceInfo service) {
        ...
        try {
            mMdns.registerServiceNameToNetwork(regId, name, type, port, record);
        } catch (RemoteException e) {
    ...
    private boolean unregisterService(int regId) {
        try {
            mMdns.unregisterServiceNameFromNetwork(regId);
        } catch (RemoteException e) {
    ...
    private boolean resolveService(int resolveId, NsdServiceInfo service) {
        ...
        String type = service.getServiceType();
        try {
            mMdns.resolve(resolveId, name, type, "local.");
        } catch (RemoteException e) {
            Log.e(TAG, "stopServiceDiscovery failed. e=" + e);
            return false;
        }
