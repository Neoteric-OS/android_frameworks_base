...
public class SamsungRecommendationPlugin implements PrintServicePlugin {
    ...
    public SamsungRecommendationPlugin(@NonNull Context context) {
        mContext = context;
        mMDNSFilteredDiscovery = new MDNSFilteredDiscovery(context, ALL_MDNS_SERVICES, new MDNSFilteredDiscovery.PrinterFilter() {
            @Override
            public boolean matchesCriteria(NsdServiceInfo nsdServiceInfo) {
                return  mPrinterFilterSamsung.matchesCriteria(nsdServiceInfo) ||
