...
public class ClockPreference extends DropDownPreference implements TunerService.Tunable {
    ...
    private ArraySet<String> mBlocklist;
    ...
    public void onTuningChanged(String key, String newValue) {
        if (StatusBarIconController.ICON_BLOCKLIST.equals(key)) {
            ...
            mClockEnabled = !mBlocklist.contains(mClock);
    ...
    protected boolean persistString(String value) {
        ...
        if (DISABLED.equals(value)) {
            mBlocklist.add(mClock);
        } else {
            mBlocklist.remove(mClock);
        }
        Dependency.get(TunerService.class).setValue(StatusBarIconController.ICON_BLOCKLIST,
                TextUtils.join(",", mBlocklist));
        return true;
    }
}
