...
public class NetworkMonitorTest {
    ...
    public void testIsCaptivePortal_HttpProbeIsPortal() throws IOException {
        ...
        // Expect to trigger FALLBACK and evaluation result notification to CS.
    ...
    public void testIsCaptivePortal_FallbackProbeIsPortal() throws IOException {
        ...
        // Expect to trigger FALLBACK and network test notifications to CS.
        runPortalNetworkTest(VALIDATION_RESULT_INVALID, 2);
    }
    ...
    public void testIsCaptivePortal_OtherFallbackProbeIsPortal() throws IOException {
        ...
        // Expect to receive HTTPS, HTTP, FALLBACK and evaluation result to CS.
        final NetworkMonitor monitor = runNetworkTest(VALIDATION_RESULT_INVALID, 4);
    ...
    public void testIsCaptivePortal_InvalidUrlSkipped() throws IOException {
        ...
        setStatus(mHttpConnection, 500);
        setPortal302(mOtherFallbackConnection);
        // Expect to trigger FALLBACK and evaluation result notifications to CS.
        runPortalNetworkTest(VALIDATION_RESULT_INVALID, 2);
        verify(mOtherFallbackConnection, times(1)).getResponseCode();
    ...
    public void testIsCaptivePortal_FallbackSpecIsPartial() throws IOException {
        ...
        // Expect to send HTTP, HTTPS, FALLBACK and evaluation result network notifications to CS.
        runPartialConnectivityNetworkTest(VALIDATION_RESULT_FALLBACK_PARTIAL, 4);
        verify(mOtherFallbackConnection, times(1)).getResponseCode();
        verify(mFallbackConnection, never()).getResponseCode();
    }
    ...
    public void testIsCaptivePortal_FallbackSpecIsPortal() throws IOException {
        ...
        // Expect to trigger FALLBACK and evaluation network notifications to CS.
        runPortalNetworkTest(VALIDATION_RESULT_INVALID, 2);
    }
    ...
    public void testLaunchCaptivePortalApp() throws Exception {
        ...
        // Expect to trigger HTTPS and evaluation result notifications and verify the last one with
        ...
        assertEquals(Integer.valueOf(NETWORK_VALIDATION_PROBE_DNS | NETWORK_VALIDATION_PROBE_HTTP
                | NETWORK_VALIDATION_RESULT_VALID), intCaptor.getValue());
    ...
    public void testPrivateDnsResolutionRetryUpdate() throws Exception {
        ...
        wnm.notifyPrivateDnsSettingsChanged(new PrivateDnsConfig("dns.google", new InetAddress[0]));
        wnm.notifyNetworkConnected(TEST_LINK_PROPERTIES, NOT_METERED_CAPABILITIES);
        // Expect to trigger HTTPS probe and evaluation result. Verify the last one with the final
        // evaluation result.
        verify(mCallbacks, timeout(HANDLER_TIMEOUT_MS).times(2))
                .notifyNetworkTested(intCaptor.capture(), eq(null));
        assertEquals(Integer.valueOf(NETWORK_VALIDATION_PROBE_DNS | NETWORK_VALIDATION_PROBE_HTTPS),
                intCaptor.getValue());
        ...
        // Expect to trigger HTTPS, PRIVDNS and evaluation result notification. Verify the last one
        // with the final evaluation result.
        wnm.forceReevaluation(Process.myUid());
        verify(mCallbacks, timeout(HANDLER_TIMEOUT_MS).times(3))
                .notifyNetworkTested(intCaptor.capture(), eq(null));
        assertEquals(
                Integer.valueOf(VALIDATION_RESULT_VALID | NETWORK_VALIDATION_PROBE_PRIVDNS),
                intCaptor.getValue());
        ...
        verify(mCallbacks, timeout(HANDLER_TIMEOUT_MS).times(1))
                .notifyNetworkTested(intCaptor.capture(), eq(null));
        assertEquals(Integer.valueOf(NETWORK_VALIDATION_PROBE_DNS | NETWORK_VALIDATION_PROBE_HTTPS
                | NETWORK_VALIDATION_PROBE_PRIVDNS), intCaptor.getValue());
    ...
    public void testIgnoreHttpsProbe() throws Exception {
        ...
        final NetworkMonitor nm = runNetworkTest(VALIDATION_RESULT_PARTIAL, 4);
    ...
    public void testIsPartialConnectivity() throws IOException {
        ...
        setStatus(mHttpConnection, 204);
        setStatus(mFallbackConnection, 500);
        // Expect to trigger 4 network test notification for HTTP probe, HTTPS probe, FALLBACK probe
        // and final evaluation result.
        runPartialConnectivityNetworkTest(VALIDATION_RESULT_PARTIAL, 4);
    ...
    public void testNotifyNetwork_WithforceReevaluation() throws Exception {
        ...
        assertTrue((intArgs.get(0) & NETWORK_VALIDATION_PROBE_HTTPS) == 0);
    ...
    public void testEvaluationState_reportProbeResult() throws Exception {
        ...
        nm.reportHttpProbeResult(
                NETWORK_VALIDATION_PROBE_DNS | NETWORK_VALIDATION_PROBE_HTTP,
                CaptivePortalProbeResult.FAILED);
    ...
    public void testEvaluationState_reportEvaluationResult() throws Exception {
        ...
        nm.getEvaluationState().reportEvaluationResult(VALIDATION_RESULT_INVALID,
                TEST_REDIRECT_URL);
        verify(mCallbacks, timeout(HANDLER_TIMEOUT_MS).times(1)).notifyNetworkTested(
                eq(VALIDATION_RESULT_VALID & ~NETWORK_VALIDATION_RESULT_VALID),
    ...
    private void runFailedNetworkTest() {
        runNetworkTest(VALIDATION_RESULT_INVALID, 4);
        assertEquals(0, mRegisteredReceivers.size());
        assertNull(mNetworkTestedRedirectUrlCaptor.getValue());
    }

    private void runPartialConnectivityNetworkTest(int result, int expectedTimes) {
        runNetworkTest(result, expectedTimes);
        assertEquals(0, mRegisteredReceivers.size());
        assertNull(mNetworkTestedRedirectUrlCaptor.getValue());
    }
    ...
    private NetworkMonitor runValidatedNetworkTest() throws Exception {
        ...
        return runNetworkTest(VALIDATION_RESULT_VALID, 2);
    }
    ...
    private NetworkMonitor runNetworkTest(NetworkCapabilities nc, int testResult,
            int expectedTimes) {
        final NetworkMonitor monitor = makeMonitor(METERED_CAPABILITIES);
