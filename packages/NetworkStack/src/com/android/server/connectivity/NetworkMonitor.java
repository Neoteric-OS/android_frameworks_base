...
public class NetworkMonitor extends StateMachine {
    ...
    /**
     * ConnectivityService notified that network has connected.
    ...
    /**
     * ConnectivityService notified that network has disconnected.
     */
    ...
    /**
     * ConnectivityServices notified NetworkMonitor that the LinkProperties that can be obtained
     * via {@link ConnectivityManager#getLinkProperties(Network)} have changed.
     */
    public static final int CMD_LINK_PROPERTIES_CHANGED = BASE + 18;

    /**
     * ConnectivityServices notified NetworkMonitor that the NetworkCapabilities that can be
     * obtained via {@link ConnectivityManager#getNetworkCapabilities(Network)} have changed.
     */
    public static final int CMD_NETWORK_CAPABILITIES_CHANGED = BASE + 19;
    ...
    protected NetworkMonitor(Context context, INetworkMonitorCallbacks cb, Network network,
        ...
        super(TAG + "network_" + network.netId);
    ...
    public void notifySystemReady() {
        // No need to run on the handler thread: systemReady is volatile and read only once on the
    ...
    public void notifyLinkPropertiesChanged() {
        getHandler().post(() -> {
            mLinkProperties = mCm.getLinkProperties(mNetwork);
        });
    }

    public void notifyNetworkCapabilitiesChanged() {
        getHandler().post(() -> {
            mNetworkCapabilities = mCm.getNetworkCapabilities(mNetwork);
        });
    }
    ...
    protected void log(String s) {
        if (DBG) Log.d(TAG + "/network_" + mNetwork.netId, s);
    }
    ...
    // DefaultState is the parent of all started States.  It exists only to handle CMD_* messages
    // but does not entail any real state (hence no enter() or exit() routines).
    ...
    private class ValidatedState extends State {
        ...
        public void enter() {
            ...
            notifyNetworkTested(NetworkMonitorUtils.NETWORK_TEST_RESULT_VALID, null);
            mValidations++;
        }
    ...
    private static String getCaptivePortalServerHttpUrl(Dependencies deps, Context context) {
        return deps.getCaptivePortalServerHttpUrl(context);
    }
    ...
    private void sendNetworkConditionsBroadcast(boolean responseReceived, boolean isCaptivePortal,
        ...
        if (mNetworkCapabilities.hasTransport(TRANSPORT_WIFI)) {
            ...
            latencyBroadcast.putExtra(NetworkMonitorUtils.EXTRA_CONNECTIVITY_TYPE, TYPE_NONE);
        }
