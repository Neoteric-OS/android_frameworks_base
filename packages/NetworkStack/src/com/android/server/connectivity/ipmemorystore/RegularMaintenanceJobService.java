...
public final class RegularMaintenanceJobService extends JobService {
    ...
    private static ArrayList<InterruptMaintenance> sInterruptList = new ArrayList<>();
    ...
    public boolean onStartJob(JobParameters params) {
        if (sIpMemoryStoreService == null) {
            Log.wtf("RegularMaintenanceJobService",
                    "Can not start job due to sIpMemoryStoreService is null.");
            return false;
        }
    ...
    public boolean onStopJob(JobParameters params) {
        final InterruptMaintenance im = sInterruptList.stream()
                .filter(interrupt -> params.getJobId() == interrupt.getJobId())
                .findAny()
                .orElse(null);
        if (im != null) {
