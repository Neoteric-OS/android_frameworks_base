...
public class DnsUtils {
    ...
    public static InetAddress[] getAllByName(@NonNull final DnsResolver dnsResolver,
        ...
        logger.log("OK: " + privateDnsConfig);
    ...
    public static InetAddress[] getAllByName(@NonNull final DnsResolver dnsResolver,
        ...
        final boolean succeed = !((result == null || result.size() == 0));
        final String msg = (errorMsg.get() == null || errorMsg.get().length() == 0)
                ? "Timeout" : errorMsg.toString();
        logDnsResult(result, watch, succeed, logger, msg, type);
        if (!succeed) {
    ...
    private static void logDnsResult(final List<InetAddress> result, @NonNull final Stopwatch watch,
            boolean succeed, @Nullable final DnsLogFunc logger, @NonNull final String errorMsg,
            int type) {
        if (logger == null) {
            return;
        }
        ...
        if (succeed) {
            ...
            logger.log(String.format("%dms OK %s", latency, buffer.substring(1)));
        } else {
