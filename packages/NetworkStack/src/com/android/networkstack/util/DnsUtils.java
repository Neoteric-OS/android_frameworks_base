...
public class DnsUtils {
    ...
    private DnsResolver mDnsResolver = null;
    /**
     * Return both A and AAAA query results regardless the ip address type of the giving network.
     */
    @NonNull
    public static InetAddress[] getAllTypesAddressesByName(@NonNull final Network network,
            @NonNull String host, int timeout) throws UnknownHostException {
        final List<InetAddress> result = new ArrayList<InetAddress>();

        result.addAll(Arrays.asList(
                getAddressByNameWithType(null, network, host, TYPE_AAAA, FLAG_NO_CACHE_LOOKUP,
                timeout)));
        result.addAll(Arrays.asList(
                getAddressByNameWithType(null, network, host, TYPE_A, FLAG_NO_CACHE_LOOKUP,
                timeout)));
        return result.toArray(new InetAddress[0]);
    }

    /**
     * Return dns query result based on the given QueryType(TYPE_A, TYPE_AAAA or TYPE_UNSPECIFIED).
     */
    @NonNull
    public static InetAddress[] getAddressByNameWithType(final @Nullable DnsResolver dnsResolver,
        ...
        if (type == TYPE_UNSPECIFIED) {
            getDnsResolver(dnsResolver).query(network, host, flag, r -> r.run(),
                    null /* cancellationSignal */, callback);
        } else {
            getDnsResolver(dnsResolver).query(network, host, type, flag, r -> r.run(),
                    null /* cancellationSignal */, callback);
        }
    ...
    private static DnsResolver getDnsResolver(DnsResolver resolver) {
        if (resolver == null) {
