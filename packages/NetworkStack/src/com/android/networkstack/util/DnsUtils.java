...
public class DnsUtils {
    ...
    public static InetAddress[] getAllByName(@NonNull final DnsResolver dnsResolver,
        ...
        final DnsResolver.Callback<List<InetAddress>> callback =
            ...
            public void onError(@NonNull DnsResolver.DnsException e) {
                ...
                if (DBG) {
                    Log.d(TAG, "DNS error resolving " + errorMsg.toString(), e);
                }
                latch.countDown();
            }
        };
        ...
        final boolean success = result != null && result.size() != 0;
        final String msg = TextUtils.isEmpty(errorMsg.toString()) ? "Timeout" : errorMsg.toString();
        final long latency = watch.stop();
        logDnsResult(result, latency, success, logger, msg, type);
        if (!success) {
    ...
    private static void logDnsResult(@Nullable final List<InetAddress> results, final long latency,
            boolean success, @Nullable final DnsLogFunc logger,
            @NonNull final String errorMsg, int type) {
        if (logger == null) {
            return;
        }
