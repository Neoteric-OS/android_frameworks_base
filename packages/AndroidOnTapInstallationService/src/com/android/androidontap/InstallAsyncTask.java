...
class InstallAsyncTask extends AsyncTask<String, Long, Integer> {
    ...
    private final AndroidOnTap mAot;
    ...
    InstallAsyncTask(String url, long systemSize, long userdataSize, AndroidOnTap aot,
    ...
    protected Integer doInBackground(String... voids) {
        ...
        try {
            ...
            mAot.start(mSystemSize, mUserdataSize);
            ...
            while ((numBytesRead = mStream.read(bytes, 0, READ_BUFFER_SIZE)) != -1) {
                ...
                if (!mAot.write(writeBuffer)) {
                    throw new IOException("Failed write() to AndroidOnTap");
                }
