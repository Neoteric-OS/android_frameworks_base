...
public class AudioSystem
{
    ...
    public static int setStreamVolumeIndexAS(int stream, int uid, int index, int device) {
        if (DEBUG_VOLUME) {
            Log.i(TAG, "setStreamVolumeIndex: " + STREAM_NAMES[stream]
                    + " dev=" + Integer.toHexString(device) + " idx=" + index);
        }
        return setStreamVolumeIndex(stream, uid, index, device);
    }
    ...
    @UnsupportedAppUsage
    private static native int setStreamVolumeIndex(int stream, int uid, int index, int device);
    /** @hide */
