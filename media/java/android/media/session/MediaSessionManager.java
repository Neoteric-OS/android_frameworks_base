...
public final class MediaSessionManager {
    ...
    private final Map<OnMediaKeyEventSessionChangedListener, Executor>
            mMediaKeyEventSessionChangedListeners = new HashMap<>();
    @GuardedBy("mLock")
     ...
     * Add a {@link OnMediaKeyEventDispatchedListener}.
     * <p>
     * Listener will be called immediately after added, if the key event session is set.
     *
     * @param executor The executor on which the listener should be invoked
    ...
    public void addOnMediaKeyEventSessionChangedListener(
        ...
        synchronized (mLock) {
            try {
                mMediaKeyEventSessionChangedListeners.put(listener, executor);
                executor.execute(
                        () -> listener.onMediaKeyEventSessionChanged(
                                mCurMediaKeyEventSessionPackage, mCurMediaKeyEventSession));
                if (mMediaKeyEventSessionChangedListeners.size() == 1) {
    ...
    public void removeOnMediaKeyEventSessionChangedListener(
        ...
        synchronized (mLock) {
            try {
                mMediaKeyEventSessionChangedListeners.remove(listener);
                if (mMediaKeyEventSessionChangedListeners.size() == 0) {
    ...
    /**
     * Listener to receive when the media session service dispatched a media key event.
     * @hide
     */
    @SystemApi
    public interface OnMediaKeyEventDispatchedListener {
        /**
         * Called when a media key event is dispatched through the media session service.
         * <p>
         * The session token can be {@link null} if the framework has sent the media key event to
         * the package's media button receiver to revive the media app's playback. The media app
         * would create the media session after the key event is sent.
         *
        ...
        public void onMediaKeyEventSessionChanged(String packageName,
            ...
            synchronized (mLock) {
                ...
                for (Map.Entry<OnMediaKeyEventSessionChangedListener, Executor> e
                        : mMediaKeyEventSessionChangedListeners.entrySet()) {
