...
// static jobject constructJavaAudioCapabilities(JNIEnv* env, jint index, jstring type) {

// }

// static jobject constructJavaCodecCapabilities(JNIEnv* env, jint index, jstring type) {
//     if (type == NULL) {
//         jniThrowException(env, "java/lang/IllegalArgumentException", NULL);
//         return NULL;
//     }
...
//     return constructJavaCodecCapabilitiesFromNative(env, codecCaps);
// }
...
static jobject android_media_MediaCodecList_getCodecCapabilities(
    ...
    bool flag = true;
    jobject caps;
    if (/* com::android::media::codec::flags::native_capabilities() */ flag) {
        std::shared_ptr<CodecCapabilities> codecCaps = info.info->getCodecCapsFor(typeStr);
        caps = constructJavaCodecCapabilitiesFromNative(env, codecCaps);
    } else {
        ...
        caps = env->NewObject(capsClazz, capsConstructID,
