...
void saveDrmException(JNIEnv *env, const char *className) {
    jclass clazz;
    jmethodID init;
    FIND_CLASS(clazz, className);
    GET_METHOD_ID(init, clazz, "<init>", "(Ljava/lang/String;III)V");
    gFields.exceptionMethods[std::string(className)] = {
        .init = init,
...
void drmThrowException(JNIEnv* env, const char *className, const DrmStatus &err, const char *msg) {
    ...
    if (gFields.exceptionMethods.count(std::string(className)) == 0) {
...
static void android_media_MediaDrm_native_init(JNIEnv *env) {
    ...
    saveDrmException(env, "android/media/NotProvisionedException");
    saveDrmException(env, "android/media/ResourceBusyException");
    saveDrmException(env, "android/media/DeniedByServerException");
}
