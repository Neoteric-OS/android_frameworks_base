...
public class CodecDecoderTest extends CodecDecoderTestBase {
    ...
    private short[] setUpAudioReference() throws IOException {
        File refFile = new File(INPUT_PREFIX + mRefFile);
        short[] refData;
    ...
    private void verify() throws IOException {
        if (mRmsError >= 0) {
            ...
            float lowerBound = mRmsError * (1 - RMS_ERROR_TOLERANCE);
            float upperBound = mRmsError * (1 + RMS_ERROR_TOLERANCE);
            assertTrue(String.format("%s rms error not in bounds %f/%f/%f", mRefFile, lowerBound,
                    currError, upperBound), currError >= lowerBound && currError <= upperBound);
        } else if (mRefCRC >= 0) {
    ...
    public static void isDecoderRunPass(CodecDecoderTest cdt, String decoder, boolean isAsync,
