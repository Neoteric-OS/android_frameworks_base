...
def file_to_class(filename):
  # Drop `.class` suffix
  #
  # A class present in a jar file, e.g. com.example.SomeClass$Inner is
  # represented by the file com/example/SomeClass$Inner.class and then shows up
  # in the api-versions.xml as com/example/SomeClass$Inner
  return filename[:-6]
  ...
  with zipfile.ZipFile(stubs) as z:
    for info in z.infolist():
      if (not info.is_dir()
        ...
        classes.add(file_to_class(info.filename))
  return classes


def filter_method_signature(method, classes_to_remove):
  ...
  Args:
    ...
    classes_to_remove: set of classes you want to remove
  """
  signature = method.get("name")
  start = signature.index("(")
  end = signature.index(")")
  classes = set(signature[start+1:end].split(";"))
  for c in classes:
    # c[1:] in order to drop the 'L' from the signature
    if c[1:] in classes_to_remove:
      signature = signature.replace(c+";", "Ljava/lang/Object;")
  method.set("name", signature)
...
def filter_lint_database(database, classes_to_remove, output):
  ...
  for c in xml.findall("class"):
    ...
    if cname in classes_to_remove:
      ...
      super_classes = c.findall("implements")+c.findall("extends")
      ...
      for method in c.findall("method"):
        filter_method_signature(method, classes_to_remove)
  xml.write(output)
