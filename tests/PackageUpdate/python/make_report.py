...
class ReportWriter (object):
    ...
    def __init__(self):
        ...
        self.actions = []
        self.template_d = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../resources/report_template")
    ...
    def read_one_teststep(self, fname):
        ...
        try:
            ...
            for l in lines:
                ...
                start = RawTestOutput.RE_START_PACKAGE_ENTRY.match(l)
                end = RawTestOutput.RE_END_PACKAGE_ENTRY.match(l)
                ...
                if start:
                    ...
                    if fname in self.testresult_matrix[reading_pkg]:
    ...
    def _create_main_table(self, results, filepathes, testcase_definition):
        ...
        sorted_packages = sorted(results.keys(), key=lambda x: int(x.replace(Constant.BASE_PACKAGE_NAME, "")))
        ...
        for (i, pkg) in enumerate(sorted_packages):
            ...
            for filepath in filepathes:
                # create a column of main table
                test_action = "undefined"
                try:
                    test_action = testcase_definition[i + 1][idx_action]
                except:
                    log.warn("unable to get test action from test definition.")
                idx_action += 1
    ...
    def _create_sub_table(self, lines, test_stage, test_operation, pkgname):
        """
        Create a cel that corresponds a launch of test application.
        ...
        rows = ['<tr class="%s"><td>%s</td><td>%s</td></tr>' % (
            r[2], r[0], r[1]) for r in
            sorted(rows, key=lambda x: self.display_keys.index(x[0]))]
...
if __name__ == "__main__":
    ...
    parser.add_option("--out-html", dest="out_html", action="store", type="string", default="out.html")
    ...
    try:
        ...
        if not os.path.isdir(root_dir):
            raise ValueError("%s is not directory." % root_dir)
        if not os.path.isfile(testcase):
            raise ValueError("%s is not file." % testcase)
