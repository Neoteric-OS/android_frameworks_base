...
class ReferenceReader(ResultReader):
    ...
    PRIVATE_FLAG_MASK = PRIVATE_FLAG_OEM | PRIVATE_FLAG_VENDOR | PRIVATE_FLAG_PRODUCT
    PRIVATE_FLAG_PATITION_MAP = {"oem": PRIVATE_FLAG_OEM,
                                 "vendor": PRIVATE_FLAG_VENDOR,
                                 "product": PRIVATE_FLAG_PRODUCT}
    ...
    def __init__(self, location=None, update_pattern=None):
        ...
        if location:
            self._partition_map = PreInstallPartitionHandler().create_action_to_partition_map(location)
        if update_pattern:
            self._sdk_version_map = OsHandler().create_action_to_sdk_version_map(update_pattern)

        self._partition_in_current_teststep = None
        self._sdk_version_in_current_teststep = None
        self._parsing_system_app_entry = False

    def _start_test_step(self, test_step):
        if not (self._partition_map and self._sdk_version_map):
            return
        ...
        self._partition_in_current_teststep = self._partition_map[action]
    ...
    def _line_filter(self, line):
        ...
        if not (self._partition_in_current_teststep and self._sdk_version_in_current_teststep):
            return line

        m = RawTestOutput.RE_PREINSTALL_PATH.search(line)
        if m:
            return line.replace(m.group(1), self._partition_in_current_teststep)

        # Update expected result for pre-installed package
        if RawTestOutput.RE_FLAG_SYSTEM.match(line):
        ...
        if m:
            ...
            private_flags = (private_flags & ~ReferenceReader.PRIVATE_FLAG_MASK)

            if self._sdk_version_in_current_teststep >= OsHandler.SDK_VERSION_PMR0:
                if self._partition_in_current_teststep in ReferenceReader.PRIVATE_FLAG_PATITION_MAP:
                    private_flags |= ReferenceReader.PRIVATE_FLAG_PATITION_MAP[self._partition_in_current_teststep]
            else:
                pass

            return line.replace(m.group(1), format(private_flags, '#010x'))
        ...
        if m:
            flg_value = "false"
            if self._sdk_version_in_current_teststep >= OsHandler.SDK_VERSION_PMR0:
                if self._partition_in_current_teststep == m.group(1).lower():
                    flg_value = "true"
            else:
                pass
