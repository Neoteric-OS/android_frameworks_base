...
function usage_exit() {
    ...
    echo "          [-c compare strictly with reference]" 1>&2
    echo "          [-l location to pre-install test applications: \"sys2sys\", \"sys2vendor\", \"vendor2sys\", \"vendor2vendor\", ...]" 1>&2
    echo "          [-o os versions of ROM1 and ROM2: \"o2o\", \"o2p\", \"p2p\",  ...]" 1>&2
...
do
    case $OPT in
        ...
        "l")  PRE_INSTALL_LOCATION=$OPTARG
            ...
            ;;
        "o")  UPDATE_PATTERN=$OPTARG
              echo "[getopts] set UPDATE_PATTERN=$UPDATE_PATTERN"
            ;;
        "c")  STRICT_COMPARE="true"
              echo "[getopts] set STRICT_COMPARE=true"
            ;;
        \?) usage_exit
            ;;
    esac
...
# Constants
# ================================================
REPORT_TOOL="${SCRIPT_D}/python/make_report.py"
DIFF_TOOL="${SCRIPT_D}/python/make_diff.py"
FILE_CHECKER="${SCRIPT_D}/get_component_pathes_by_pkg.sh"
TESTCASE_PARSER="${SCRIPT_D}/python/testcase_parser.py"
...
if [ -d "$REPORT_D" ]; then
    logi "removing existing reports directory..."
    rm -rf "$REPORT_D"
fi
mkdir -p "$REPORT_D"
LOG_D="${REPORT_D}/logs"
...
function run_autoflash () {
    ...
    bash $SCRIPT_D/flash/autoflash.bash $flash_options $device_id $rom_d || \
...
function compare_with_reference () {
    ...
    if [ "$STRICT_COMPARE" == "false" ]; then
        options="$options --location=$PRE_INSTALL_LOCATION"
        options="$options --update-pattern=$UPDATE_PATTERN"
    fi
    mkdir_if_not_found "$output_d"
    python $DIFF_TOOL $reference_d $target_d $options || \
        { throw "Failed to compare with reference"; }
}
...
TEST_PROPERTIES="$REPORT_D/packageupdate.properties"
...
for install_script in "${INSTALL_SCRIPTS[@]}"; do
    chmod 775 "$REPORT_D/$install_script"
done
...
bash $SCRIPT_D/build-image/run_build.bash "${PREINSTALL_FILELISTS[0]}" "$TESTAPKS_D" "$IMAGES_TMP_D" "$ROM1_IMAGES_D" || \
...
bash $SCRIPT_D/build-image/run_build.bash "${PREINSTALL_FILELISTS[1]}" "$TESTAPKS_D" "$IMAGES_TMP_D" "$ROM2_IMAGES_D" || \
...
report_opts="$report_opts --out-html=$HTML_RESULT_D/analysis.html"

python "$REPORT_TOOL" "$RAW_RESULT_D" "$TESTCASE_FILE" $report_opts || \
    { throw "Failed to make test results report"; }
