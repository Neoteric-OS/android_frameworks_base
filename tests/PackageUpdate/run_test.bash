...
function usage_exit() {
    ...
    echo "- OPTION: [-s ANDROID_SERIAL][-r REFERENCE_D: path to reference_directory]" 1>&2
...
do
    case $OPT in
        ...
        "h")  usage_exit
            ;;
        "r")  REFERENCE_D=$(readlink -f $OPTARG)
...
TESTCASE_FILE=$(readlink -f $1); shift
TESTAPKS_ZIP=$(readlink -f $1); shift
ROM1_BASE_IMAGES_ZIP=$(readlink -f $1); shift
ROM2_BASE_IMAGES_ZIP=$(readlink -f $1); shift
...
function logger_start(){
    ...
    $ADB logcat -v time -b main > $log_path &
    PID_LOGGER=$!
}
...
function reboot_and_unlock_if_need() {
    local log_prefix=$1
    clear_logbuffer
    $ADB reboot
    wait_for_device
    logger_start "$BOOTLOG_D/${log_prefix}_main.log"
    sleep 5
    logger_stop
}
...
# generate install script and ROM configuration
# from the testcase.
# Example of testcase_parser_out.txt:
# ----
# PREINSTALL_FILELISTS=ROM1_priv_app_cl.txt,ROM2_priv_app_cl.txt
...
PARSER_OUT="testcase_parser_out.txt"
test -f $PARSER_OUT || { throw '$PARSER_OUT does not exists'; }

PREINSTALL_FILELISTS=()
for i in $(seq 1 2); do
    files=($(cat $PARSER_OUT | grep -oP '(?<=PREINSTALL_FILELISTS=)\S+' | cut -d, -f$i | tr -s ':' ' '))
    filelist=""
...
INSTALL_SCRIPTS=( $(cat $PARSER_OUT | grep -oP '(?<=INSTALL_SCRIPTS=)\S+' | tr -s ',' ' ') )
for install_script in "${INSTALL_SCRIPTS[@]}"; do
    chmod 775 $install_script
    mv $install_script $REPORT_D
done
TESTCASE_COUNT=$(cat $PARSER_OUT | grep -oP '(?<=TESTCASE_COUNT=)[0-9]+')
mv expected_version.json $REPORT_D
mv $PARSER_OUT $REPORT_D

logi "[$0] Finished successfully. <br />Waiting for rom build completed."
