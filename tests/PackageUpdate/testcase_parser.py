...
class Parser(object):
    ...
    def find_testlib_in_apkpath (self, pkg, apk_path, abi):

        suffix = pkg.replace(self.pkg_basename, "")
        fpath = "lib%s%s.so" % (self.app_name_base, suffix)
        ...
        for slot in self.test_format.get_header():
            if slot == "id":
                ...
                if action in BuildAction.GROUP_PRIVILEGE_APPLICATION:
                    ...
                    libpath = self.find_testlib_in_apkpath(pkg, path, build_path_abi)
