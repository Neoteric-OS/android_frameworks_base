...
public class PackageUpdateSetup implements ITargetPreparer {
    ...
    public void setUp(ITestDevice device, IBuildInfo buildInfo) {
        ...
        if (packageUpdateBuildInfo.getReference() != null) {
            command.add("-r");
            command.add(packageUpdateBuildInfo.getReference().getAbsolutePath());
        }
        command.add(packageUpdateBuildInfo.getTestCase().getAbsolutePath());
        command.add(packageUpdateBuildInfo.getTestApksZip().getAbsolutePath());
        command.add(packageUpdateBuildInfo.getRom1Zip().getAbsolutePath());
        command.add(packageUpdateBuildInfo.getRom2Zip().getAbsolutePath());
