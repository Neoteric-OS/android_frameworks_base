...
public class PackageUpdateBuildProvider implements IBuildProvider {
    ...
    public IBuildInfo getBuild() throws BuildRetrievalError {
        if (mReference != null && (!mReference.exists() || !mReference.isDirectory())) {
            throw new BuildRetrievalError(String.format("reference '%s' does not exist.",
                    mReference.getAbsolutePath()));
        }
        if (mTestCase == null) {
