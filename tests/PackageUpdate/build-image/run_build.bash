...
SIMG2IMG=${SCRIPT_D}/../tools/simg2img
EXT2SIMG=${SCRIPT_D}/../tools/ext2simg
...
function add_apk(){
    ...
    sudo cp "$src_path" "$dest_root/$dest_path"
    find "$dest_root/${dest_path%%/*}" -type d | xargs sudo chmod 755
    sudo chmod 644 "$dest_root/$dest_path/$(basename ${src_path})"
    local ref_dir=$(ls -F ${dest_root} | grep / | head -1)
    sudo chcon -R --reference="${dest_root}/${ref_dir}" "$dest_root/${dest_path%%/*}"
}
...
function add_all_apks(){
    ...
    do
        ...
        if [ ! -f "$src_path" ]; then
            { loge "[$tag] apk not found: $src_path"; return 1; }
        fi
        add_apk "$src_path" "${MOUNT_D}${dest_dir}" "$2"
        if [ $? -eq  0 ]; then
            logi "add $1 to ${dest_dir}/$2"
        else
            return 1
        fi
...
test $(echo $SYSTEM_IMG | wc -l) -eq 1 || \
...
# add apk
PREINSTALL_FILES=( $(echo "$PREINSTALL_FILELIST" | tr -s ':' ' ') )
for preinstall_file in "${PREINSTALL_FILES[@]}"; do
    ...
    if echo $(basename $preinstall_file) | grep "priv_app" > /dev/null; then
        add_all_apks $preinstall_file $APK_D /system/priv-app || { exit 1; }
    else
        add_all_apks $preinstall_file $APK_D /system/app || { exit 1; }
    fi
done
...
if [ "$OUT_ALL_IMAGE" == "true" ]; then
    ...
    DEST_SYSTEM_IMG=$DEST_D/$(basename $UPDATE_IMG_DIR)/system.img
else
    DEST_SYSTEM_IMG=$DEST_D/system.img
fi
if [ -f $DEST_SYSTEM_IMG ]; then
    rm $DEST_SYSTEM_IMG
fi

$EXT2SIMG $SYSTEM_EXT4 $DEST_SYSTEM_IMG || \
    { throw "failed to ext2simg"; }
