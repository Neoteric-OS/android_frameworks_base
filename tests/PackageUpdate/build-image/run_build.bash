...
function add_apk () {
    ...
    $ADB shell "chcon -R $secontext ${dest_path_on_device}"
}
...
function mount_ext4() {
    ...
    logi "[$tag] trying to mount $ext4_on_device to $mountd_on_device (/dev/block/loop${LOOP_NO})"
    $ADB shell "losetup /dev/block/loop${LOOP_NO} $ext4_on_device" || \
        { throw "[$tag] losetup failed"; }
    $ADB shell "mount -o loop /dev/block/loop${LOOP_NO} $mountd_on_device" || \
...
if [ "$UPDATE_FINGERPRINT" == "true" ]; then
    bash ${SCRIPT_D}/update_fingerprint.bash $MOUNT_ROOT_ON_DEVICE/system/build.prop
fi
