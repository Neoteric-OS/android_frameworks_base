...
$ADB pull "$BUILD_PROP" "$LOCAL_BUILD_PROP" || \
    { throw "failed to copy $(basename $BUILD_PROP) from device"; }
if [ -f "$MODIFIED_BUILD_PROP" ]; then
    rm -f "$MODIFIED_BUILD_PROP"
fi
...
$ADB push "$MODIFIED_BUILD_PROP" /sdcard/$(basename $BUILD_PROP) || \
    { throw "failed to copy $(basename $MODIFIED_BUILD_PROP) to device"; }
$ADB shell cp /sdcard/$(basename $BUILD_PROP) "$BUILD_PROP"
$ADB shell rm /sdcard/$(basename $BUILD_PROP)
