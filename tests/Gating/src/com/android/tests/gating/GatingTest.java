...
public class GatingTest {
    ...
    public void testDummyGatingPositiveSystemServer() {
        ...
        assertThat(accountManager.testFunc()).isEqualTo("True");
    }

    @Test
    @DisableCompatChanges({ DummyApi.CHANGE_SYSTEM_SERVER})
    public void testDummyGatingNegativeSystemServer() {
        InstrumentationRegistry.getInstrumentation().setCompatChanges();
        Context ctx = InstrumentationRegistry.getTargetContext();
        AccountManager accountManager = (AccountManager)ctx.getSystemService(Context.ACCOUNT_SERVICE);
        assertThat(accountManager.testFunc()).isEqualTo("False");
    }
}
