...
public class CrashRecoveryTest {
    ...
    public void testBootLoopWithRollbackPackageHealthObserver() throws Exception {
        ...
        VersionedPackage noRollbackPackage = new VersionedPackage(APP_D, VERSION_CODE);
        int rollbackIdLow = 1;
        RollbackInfo rollbackinfoLow = getRollbackInfo(APP_A, VERSION_CODE, rollbackIdLow,
                PackageManager.ROLLBACK_USER_IMPACT_LOW);
        int rollbackIdHigh = 2;
        RollbackInfo rollbackinfoHigh = getRollbackInfo(APP_B, VERSION_CODE, rollbackIdHigh,
                PackageManager.ROLLBACK_USER_IMPACT_HIGH);
        int rollbackIdManual = 3;
        RollbackInfo rollbackinfoManual = getRollbackInfo(APP_C, VERSION_CODE, rollbackIdManual,
                PackageManager.ROLLBACK_USER_IMPACT_ONLY_MANUAL);
        when(mSpyContext.getSystemService(RollbackManager.class)).thenReturn(mRollbackManager);
        when(mRollbackManager.getAvailableRollbacks()).thenReturn(List.of(rollbackinfoLow,
                rollbackinfoHigh, rollbackinfoManual));
        when(mSpyContext.getPackageManager()).thenReturn(mMockPackageManager);
        ...
        when(mRollbackManager.getAvailableRollbacks()).thenReturn(List.of(rollbackinfoHigh,
                rollbackinfoManual));
        ...
        // Update the list of available rollbacks after executing bootloop mitigation once
        when(mRollbackManager.getAvailableRollbacks()).thenReturn(List.of(rollbackinfoManual));
    ...
    public void testBootLoopWithRescuePartyAndRollbackPackageHealthObserver() throws Exception {
        ...
        RollbackInfo rollbackinfoLow = getRollbackInfo(APP_A, VERSION_CODE, rollbackIdLow,
                PackageManager.ROLLBACK_USER_IMPACT_LOW);
        RollbackInfo rollbackinfoHigh = getRollbackInfo(APP_B, VERSION_CODE, 2,
                PackageManager.ROLLBACK_USER_IMPACT_HIGH);
        RollbackInfo rollbackinfoManual = getRollbackInfo(APP_C, VERSION_CODE, 3,
                PackageManager.ROLLBACK_USER_IMPACT_ONLY_MANUAL);
        when(mSpyContext.getSystemService(RollbackManager.class)).thenReturn(mRollbackManager);
        when(mRollbackManager.getAvailableRollbacks()).thenReturn(List.of(rollbackinfoLow,
                rollbackinfoHigh, rollbackinfoManual));
        ...
        verify(rescuePartyObserver, never()).executeBootLoopMitigation(3);
        verify(rollbackObserver).executeBootLoopMitigation(3);
        verify(rollbackObserver, never()).executeBootLoopMitigation(4);
        // Update the list of available rollbacks after executing bootloop mitigation once
        when(mRollbackManager.getAvailableRollbacks()).thenReturn(List.of(rollbackinfoHigh,
                rollbackinfoManual));
        ...
        verify(rescuePartyObserver).executeBootLoopMitigation(5);
        verify(rescuePartyObserver, never()).executeBootLoopMitigation(6);
        verify(rollbackObserver, never()).executeBootLoopMitigation(5);
        ...
        verify(rescuePartyObserver).executeBootLoopMitigation(6);
        verify(rescuePartyObserver, never()).executeBootLoopMitigation(7);
        verify(rollbackObserver, never()).executeBootLoopMitigation(6);

        for (int i = 0; i < PackageWatchdog.DEFAULT_BOOT_LOOP_MITIGATION_INCREMENT; i++) {
            watchdog.noteBoot();
        }
        verify(rescuePartyObserver, never()).executeBootLoopMitigation(7);
        verify(rollbackObserver).executeBootLoopMitigation(7);
        verify(rollbackObserver, never()).executeBootLoopMitigation(8);
        ...
        verify(rescuePartyObserver).executeBootLoopMitigation(8);
        verify(rescuePartyObserver, never()).executeBootLoopMitigation(9);
        verify(rollbackObserver, never()).executeBootLoopMitigation(9);
    }

    private RollbackInfo getRollbackInfo(String packageName, long versionCode, int rollbackId,
            int rollbackUserImpact) {
    ...
    private void mockCrashRecoveryProperties(PackageWatchdog watchdog) {
        ...
        try {
            ...
            doAnswer((Answer<Void>) invocationOnMock -> {
                int value = invocationOnMock.getArgument(0);
                setCrashRecoveryRescueMitigationCount(value);
                return null;
        ...
        try {
            ...
            doAnswer((Answer<Void>) invocationOnMock -> {
                int count = invocationOnMock.getArgument(0);
                setCrashRecoveryRescueMitigationCount(count);
                return null;
    ...
    private void setCrashRecoveryRescueMitigationCount(int count) {
