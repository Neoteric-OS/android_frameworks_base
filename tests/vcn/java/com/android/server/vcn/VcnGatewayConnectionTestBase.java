...
public class VcnGatewayConnectionTestBase {
    ...
    public VcnGatewayConnectionTestBase() {
        ...
        doReturn(mTeardownTimeoutAlarm)
                .when(mDeps)
                .newWakeupMessage(
                        eq(mVcnContext),
                        any(),
                        eq(VcnGatewayConnection.TEARDOWN_TIMEOUT_ALARM),
                        any());
        doReturn(mDisconnectRequestAlarm)
                .when(mDeps)
                .newWakeupMessage(
                        eq(mVcnContext),
                        any(),
    ...
    protected void verifyWakeLockSetUp() {
        verify(mDeps).newWakeLock(eq(mVcnContext), eq(PowerManager.PARTIAL_WAKE_LOCK), any());
        verify(mWakeLock).setReferenceCounted(false);
    }
