...
public class VcnTest {
    ...
    private static final Set<Integer> TEST_SUBIDS_IN_GROUP = new ArraySet<>(Arrays.asList(1, 2, 3));
    ...
    public void setUp() {
        ...
        // Setup mobile data toggle states
        doReturn(TEST_SUBIDS_IN_GROUP).when(mSubscriptionSnapshot).getAllSubIdsInGroup(any());
        doReturn(mTelephonyManager).when(mTelephonyManager).createForSubscriptionId(anyInt());
        doReturn(MOBILE_DATA_ENABLED).when(mTelephonyManager).isDataEnabled();
    ...
    private void verifyMobileDataToggled(boolean startingToggleState, boolean endingToggleState) {
        ...
        for (Entry<VcnGatewayConnectionConfig, VcnGatewayConnection> entry : gateways.entrySet()) {
            if (startingToggleState != endingToggleState
                    && (entry.getKey().getAllExposedCapabilities().contains(NET_CAPABILITY_INTERNET)
                            || entry.getKey()
                                    .getAllExposedCapabilities()
                                    .contains(NET_CAPABILITY_DUN))) {
