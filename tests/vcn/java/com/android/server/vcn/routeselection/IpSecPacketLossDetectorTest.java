...
public class IpSecPacketLossDetectorTest {
    ...
    private static final int IPSEC_PACKET_LOSS_PERCENT_THRESHOLD = 60;
    private static final long POLL_IPSEC_STATE_INTERVAL_MS = TimeUnit.SECONDS.toMillis(30L);

    private static final int IPSEC_PACKET_LOSS_PERCENT = 0;

    private static final byte[] REPLAY_BITMAP_FULL = new byte[REPLAY_BITMAP_LEN];
    private static final byte[] REPLAY_BITMAP_MISSING_BITS = new byte[REPLAY_BITMAP_LEN];

    static {
        for (int i = 0; i < REPLAY_BITMAP_FULL.length; i++) {
            REPLAY_BITMAP_FULL[i] = (byte) 0xff;
        }

        for (int i = 0; i < REPLAY_BITMAP_FULL.length / 2; i++) {
            REPLAY_BITMAP_FULL[i] = (byte) 0xff;
        }
    }

    @Mock private Network mNetwork;
    @Mock private VcnContext mVcnContext;
    ...
    public void setUp() {
        ...
        when(mVcnContext.getLooper()).thenReturn(mTestLooper.getLooper());
    ...
    public void testHandleLossRate_validationPass() throws Exception {
        checkHandleLossRate(
                5, true /* isLastStateExpectedToUpdate */, false /* isCallbackExpected */);
    }

    @Test
    public void testHandleLossRate_validationFail() throws Exception {
        checkHandleLossRate(
                95, true /* isLastStateExpectedToUpdate */, true /* isCallbackExpected */);
    }
    ...
    public void testCalculatePacketLoss_fail() throws Exception {
        ...
        checkCalculatePacketLoss(mTransformStateInitial, stateLossy, 84);
    }
    ...
    public void testCalculatePacketLoss_windowsOverlapped() throws Exception {
        ...
        checkCalculatePacketLoss(
                oldState, newState, IpSecPacketLossDetector.PACKET_LOSS_UNAVALAIBLE);
    }
}
