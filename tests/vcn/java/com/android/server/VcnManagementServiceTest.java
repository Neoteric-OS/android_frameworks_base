...
public class VcnManagementServiceTest {
    ...
    @Test
    public void testGetSubscriptionGroupsWithConfiguredVcnsRequiresSystemUser() throws Exception {
        doReturn(UserHandle.getUid(UserHandle.MIN_SECONDARY_USER_ID, TEST_UID))
                .when(mMockDeps)
                .getBinderCallingUid();

        try {
            mVcnMgmtSvc.getSubscriptionGroupsWithConfiguredVcns(TEST_PACKAGE_NAME);
            fail("Expected security exception for non system user");
        } catch (SecurityException expected) {
        }
    }
    ...
    @Test
    public void testGetSubscriptionGroupsWithConfiguredVcnsMismatchedPackages() throws Exception {
        final String badPackage = "IncorrectPackage";
        doThrow(new SecurityException()).when(mAppOpsMgr).checkPackage(TEST_UID, badPackage);

        try {
            mVcnMgmtSvc.getSubscriptionGroupsWithConfiguredVcns(badPackage);
            fail("Expected security exception due to mismatched packages");
        } catch (SecurityException expected) {
        }
    }
    ...
    @Test
    public void testGetSubscriptionGroupsWithConfiguredVcns() throws Exception {
        mVcnMgmtSvc.setVcnConfig(TEST_UUID_2, TEST_VCN_CONFIG, TEST_PACKAGE_NAME);
