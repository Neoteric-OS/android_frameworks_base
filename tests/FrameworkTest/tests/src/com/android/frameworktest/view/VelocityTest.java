/*
visibility.xml * Copyright (C) 2007 The Android Open Source Project
 *
...
public class VelocityTest extends InstrumentationTestCase {
     ...
     */
    private void drag(VelocityTracker vt, int startX, int endX, int startY, int endY, int steps, long startime, int duration, Interpolator interpolator) {
    ...
    private void assertEqualFuzzy(float expected, float actual, float threshold) {
        ...
        Assert.assertTrue("Expected: <"+expected+"> but was: <"+actual+"> while accepting a variation of: <"+threshold+">", fuzzyEqual);
    }
