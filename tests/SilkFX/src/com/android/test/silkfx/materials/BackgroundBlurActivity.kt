...
class BackgroundBlurActivity : Activity(), SeekBar.OnSeekBarChangeListener  {
    ...
    fun increaseBackgroundColorAlpha(v: View) {
        var currentAlpha = if (getWindowManager().isCrossWindowBlurEnabled()) mAlphaWithBlur
                           else mAlphaNoBlur
        setBackgroundColorAlpha(Math.min(currentAlpha + 0.05f, 1f))
    }

    fun decreaseBackgroundColorAlpha(v: View) {
        var currentAlpha = if (getWindowManager().isCrossWindowBlurEnabled()) mAlphaWithBlur
                           else mAlphaNoBlur
        setBackgroundColorAlpha(Math.max(currentAlpha - 0.05f, 0f))
    }
}
