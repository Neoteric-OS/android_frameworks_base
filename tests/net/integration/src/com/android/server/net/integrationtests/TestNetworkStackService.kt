...
class TestNetworkStackService : Service() {
    ...
    override fun onBind(intent: Intent): IBinder? {
        if (INSTRUMENTATION_ACTION == intent.action) {
            return instrumentationConnector.asBinder()
        }
        return TestNetworkStackConnector(makeTestContext())
    }

    private fun makeTestContext(): Context {
        val context = spy(applicationContext)
        doReturn(mock(IBinder::class.java)).`when`(context).getSystemService(Context.NETD_SERVICE)
        return context
    }

    private class ConnectorDeps : NetworkStackService.ConnectorDependencies() {
