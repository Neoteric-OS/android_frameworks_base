...
open class TestNetworkStackClient(private val context: Context) : NetworkStackClient() {
    // TODO: consider switching to TrackRecord for mor expressive checks
    private val lastCallbacks = HashMap<Network, INetworkMonitorCallbacks>()

    override fun getNetworkStackIntent(pm: PackageManager, inSystemProcess: Boolean): Intent? {
        if (inSystemProcess) return null // Only support out-of-system-process
    ...
    // Use a known open class that delegates to the original instance.
