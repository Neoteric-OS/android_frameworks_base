...
class TcpKeepalivePacketDataTest {
    ...
    fun testParcelUnparcel() {
        val source = makeData()
        val unparceled = parcelingRoundTrip(source)
        assertEquals(source, unparceled)
        // .equals() does not verify .packet
        assertArrayEquals(source.packet, unparceled.packet)
    }
    ...
    fun testToString() {
        ...
        assertTrue(str.contains(data.tcpWnd.toString()))
        assertTrue(str.contains(data.tcpWndScale.toString()))
