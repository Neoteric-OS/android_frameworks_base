...
class NetworkStatsApiTest {
    companion object {
        ...
        val TEST_STATS_EMPTY = NetworkStats(0L, 0)
        ...
        private val TEST_STATS1 = NetworkStats(0L, 0)
                ...
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_DEFAULT, TAG_NONE, METERED_YES,
                        ROAMING_NO, DEFAULT_NETWORK_NO, 25, 3, 47, 8, 2))
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_FOREGROUND, TAG_NONE, METERED_NO,
                ...
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_DEFAULT, 0x80, METERED_NO,
                        ROAMING_NO, DEFAULT_NETWORK_NO, 17, 2, 11, 1, 0))
                .addEntry(Entry(TEST_IFACE, TEST_UID2, SET_DEFAULT, TAG_NONE, METERED_NO,
                        ROAMING_NO, DEFAULT_NETWORK_NO, 40, 1, 0, 0, 8))
                .addEntry(Entry(IFACE_VT, TEST_UID1, SET_DEFAULT, TAG_NONE, METERED_NO,
                        ROAMING_NO, DEFAULT_NETWORK_NO, 3, 1, 6, 2, 0))

        private val TEST_STATS2 = NetworkStats(0L, 0)
                // Entries which are common for set1 and set2.
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_DEFAULT, 0x80, METERED_NO, ROAMING_NO,
                        DEFAULT_NETWORK_NO, 3, 15, 2, 31, 1))
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_FOREGROUND, TAG_NONE, METERED_NO,
                ...
                .addEntry(Entry(IFACE_VT, TEST_UID2, SET_DEFAULT, TAG_NONE, METERED_NO,
        ...
        // and stats2 above.
        private val TEST_STATS3 = NetworkStats(0L, 9)
                // Entries which are unique either in stats1 or stats2.
                .addEntry(Entry(TEST_IFACE, TEST_UID1, SET_DEFAULT, TAG_NONE, METERED_NO,
    ...
    fun testAddEntry() {
        val expectedEntriesInStats2 = arrayOf(
                Entry(TEST_IFACE, TEST_UID1, SET_DEFAULT, 0x80, METERED_NO, ROAMING_NO,
                        DEFAULT_NETWORK_NO, 3, 15, 2, 31, 1),
        ...
        // While TEST_STATS* are already initialized with addEntry, verify content added
        // matches expectation.
        for (i in expectedEntriesInStats2.indices) {
            val entry = TEST_STATS2.getValues(i, null)
            assertEquals(expectedEntriesInStats2[i], entry)
        }

        // Verify entry updated with addEntry.
        val stats = TEST_STATS2.addEntry(Entry(IFACE_VT, TEST_UID1, SET_DEFAULT, TAG_NONE,
                METERED_NO, ROAMING_NO, DEFAULT_NETWORK_NO, 12, -5, 7, 0, 9))
        assertEquals(Entry(IFACE_VT, TEST_UID1, SET_DEFAULT, TAG_NONE, METERED_NO, ROAMING_NO,
                DEFAULT_NETWORK_NO, 16, -2, 9, 1, 9), stats.getValues(3, null))
    }

    @Test
    fun testAdd() {
        var stats = NetworkStats(0L, 0)
        assertNetworkStatsEquals(TEST_STATS_EMPTY, stats)
        stats = stats.add(TEST_STATS2)
        assertNetworkStatsEquals(TEST_STATS2, stats)
        stats = stats.add(TEST_STATS1)
        // EMPTY + STATS2 + STATS1 = STATS3
        assertNetworkStatsEquals(TEST_STATS3, stats)
    }

    @Test
    fun testParcelUnparcel() {
        assertParcelingIsLossless(TEST_STATS_EMPTY)
        assertParcelingIsLossless(TEST_STATS1)
        assertParcelingIsLossless(TEST_STATS2)
    ...
    fun testSubtract() {
        ...
        assertNetworkStatsEquals(TEST_STATS1, TEST_STATS3.subtract(TEST_STATS2))
        // STATS3 - STATS1 = STATS2
        assertNetworkStatsEquals(TEST_STATS2, TEST_STATS3.subtract(TEST_STATS1))
    }
}