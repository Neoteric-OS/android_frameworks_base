...
public class IpSecAlgorithmTest {
    private static final int ALGO_MANDATORY_FOR_ALL_CNT = 7;
    private static final int ALGO_MANDATORY_SINCE_S_CNT = 3;
    ...
    private void checkKeyAndTruncLenValidation(String algoName, int keyLen, int trucLen)
            throws Exception {
        new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8), trucLen);

        try {
            new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8));
            fail("Expected exception on unprovided auth trunclen");
        } catch (IllegalArgumentException pass) {
        }

        try {
            new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8 + 1), trucLen);
            fail("Invalid key length not validated");
        } catch (IllegalArgumentException pass) {
        }

        try {
            new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8), trucLen + 1);
            fail("Invalid truncation length not validated");
        } catch (IllegalArgumentException pass) {
        }
    }

    private void checkKeyLenValidation(String algoName, int keyLen) throws Exception {
        new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8));

        try {
            new IpSecAlgorithm(algoName, Arrays.copyOf(KEY_MATERIAL, keyLen / 8 + 1));
            fail("Invalid key length not validated");
        } catch (IllegalArgumentException pass) {
        }
    }
    ...
    public void testValidationForAlgosAddedInS() throws Exception {
        ...
        for (int len : new int[] {160, 224, 288}) {
            checkKeyLenValidation(IpSecAlgorithm.CRYPT_AES_CTR, len);
        }
        checkKeyAndTruncLenValidation(IpSecAlgorithm.AUTH_AES_XCBC, 128, 96);
        checkKeyAndTruncLenValidation(IpSecAlgorithm.AUTH_CRYPT_CHACHA20_POLY1305, 288, 128);
    }
    ...
    public void testGetSupportedAlgorithms() throws Exception {
        int mandatoryAlgoCnt = ALGO_MANDATORY_FOR_ALL_CNT;

        if (Build.VERSION.FIRST_SDK_INT > Build.VERSION_CODES.R) {
            mandatoryAlgoCnt += ALGO_MANDATORY_SINCE_S_CNT;
        }

        assertTrue(
                "Missing mandatory algorithm(s) in supported algorithm list",
                IpSecAlgorithm.getSupportedAlgorithms().size() >= mandatoryAlgoCnt);
    }
}
