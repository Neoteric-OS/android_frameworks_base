...
public class IpSecAlgorithmTest {
    ...
    private static Set<String> getMandatoryAlgos() {
        ...
        for (Entry<String, Integer> entry : ALGO_TO_REQUIRED_FIRST_SDK.entrySet()) {
            if (Build.VERSION.FIRST_SDK_INT >= entry.getValue()) {
                algos.add(entry.getKey());
            }
        }
        return algos;
    }

    private static Set<String> getAllowOptionalAlgos() {
        final Set<String> algos = new HashSet<>();
        for (Entry<String, Integer> entry : ALGO_TO_REQUIRED_FIRST_SDK.entrySet()) {
            if (Build.VERSION.FIRST_SDK_INT < entry.getValue()) {
                algos.add(entry.getKey());
    ...
    public void testLoadAlgos() throws Exception {
        final Set<String> allowOptionalAlgos = getAllowOptionalAlgos();
        final String[] optionalAlgos =
                allowOptionalAlgos.toArray(new String[allowOptionalAlgos.size()]);
    ...
    @Test
    public void testLoadAlgosWithInvalidAlgoName() throws Exception {
        doReturn(new String[] {"BestAlgorithm"}).when(mMockResources)
                .getStringArray(com.android.internal.R.array.config_optionalIpSecAlgorithms);

        try {
            IpSecAlgorithm.loadAlgos(mMockResources);
