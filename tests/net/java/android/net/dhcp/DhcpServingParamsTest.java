...
public class DhcpServingParamsTest {
    ...
    public void setUp() {
        mBuilder = new DhcpServingParams.Builder()
                ...
                .setInet4Addr(TEST_SERVER_ADDR)
                .setOnlinkPrefix(TEST_PREFIX)
                .setUpstreamMtu(TEST_MTU)
                .setExcludedAddrs(TEST_EXCLUDED_ADDRS);
    }

    @Test
    public void testBuild() throws InvalidParameterException {
        ...
        assertEquals(TEST_SERVER_ADDR, params.inet4Addr);
        assertEquals(TEST_PREFIX, params.onlinkPrefix);
        assertEquals(TEST_MTU, params.upstreamMtu);
    ...
    @Test(expected = InvalidParameterException.class)
    public void testBuild_IPv6Prefix() throws InvalidParameterException {
        mBuilder.setOnlinkPrefix(new IpPrefix(parseNumericAddress("fe80::1111"), 120)).build();
    }
    ...
    public void testBuild_PrefixTooSmall() throws InvalidParameterException {
        mBuilder.setDefaultRouters(Collections.singleton(parseAddr("192.168.0.254")))
                .setOnlinkPrefix(new IpPrefix(TEST_SERVER_ADDR, 31))
                .build();
    }
