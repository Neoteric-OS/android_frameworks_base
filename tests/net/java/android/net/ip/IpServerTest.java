...
import static android.net.ip.IpServer.Callback.STATE_AVAILABLE;
import static android.net.ip.IpServer.Callback.STATE_TETHERED;
import static android.net.ip.IpServer.Callback.STATE_UNAVAILABLE;
...
public class IpServerTest {
    ...
    public void startsOutAvailable() {
        mIpSrv = new IpServer(IFACE_NAME, mLooper.getLooper(),
                TETHERING_BLUETOOTH, mSharedLog, mNMService, mStatsService, mCallback,
                false /* usingLegacyDhcp */, mDependencies);
        mIpSrv.start();
        mLooper.dispatchAll();
        verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_AVAILABLE, TETHER_ERROR_NO_ERROR);
        verify(mCallback).updateLinkProperties(eq(mIpSrv), any(LinkProperties.class));
    ...
    public void handlesImmediateInterfaceDown() throws Exception {
        ...
        verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_UNAVAILABLE, TETHER_ERROR_NO_ERROR);
        verify(mCallback).updateLinkProperties(eq(mIpSrv), any(LinkProperties.class));
        verifyNoMoreInteractions(mNMService, mStatsService, mCallback);
    }
    ...
    public void canBeTethered() throws Exception {
        ...
        inOrder.verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_TETHERED, TETHER_ERROR_NO_ERROR);
        inOrder.verify(mCallback).updateLinkProperties(
                eq(mIpSrv), any(LinkProperties.class));
        verifyNoMoreInteractions(mNMService, mStatsService, mCallback);
    }
    ...
    public void canBeTetheredAsUsb() throws Exception {
        ...
        inOrder.verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_TETHERED, TETHER_ERROR_NO_ERROR);
        inOrder.verify(mCallback).updateLinkProperties(
                eq(mIpSrv), mLinkPropertiesCaptor.capture());
    ...
    public void usbShouldBeTornDownOnTetherError() throws Exception {
        ...
        usbTeardownOrder.verify(mNMService).setInterfaceConfig(
                IFACE_NAME, mInterfaceConfiguration);
        usbTeardownOrder.verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_AVAILABLE, TETHER_ERROR_TETHER_IFACE_ERROR);
        usbTeardownOrder.verify(mCallback).updateLinkProperties(
                eq(mIpSrv), mLinkPropertiesCaptor.capture());
        assertNoAddressesNorRoutes(mLinkPropertiesCaptor.getValue());
    }
    ...
    public void shouldTearDownUsbOnUpstreamError() throws Exception {
        ...
        usbTeardownOrder.verify(mCallback).updateInterfaceState(
                mIpSrv, STATE_AVAILABLE, TETHER_ERROR_ENABLE_NAT_ERROR);
        usbTeardownOrder.verify(mCallback).updateLinkProperties(
                eq(mIpSrv), mLinkPropertiesCaptor.capture());
        assertNoAddressesNorRoutes(mLinkPropertiesCaptor.getValue());
    }
    ...
    private void dispatchCommand(int command, int arg1) {
        mIpSrv.sendMessage(command, arg1);
        mLooper.dispatchAll();
    }
    ...
    private void dispatchTetherConnectionChanged(String upstreamIface) {
        mIpSrv.sendMessage(IpServer.CMD_TETHER_CONNECTION_CHANGED,
