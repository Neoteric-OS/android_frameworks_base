...
public class DnsManagerTest {
    ...
    static final int[] TEST_TRANSPORT_TYPES = {TRANSPORT_TEST};
    ...
    private void assertContainsString(String[] actual, String... expected) {
    ...
    private void assertResolverParamsEqual(@NonNull ResolverParamsParcel actual,
        ...
        assertEquals(actual.servers.length, expected.servers.length);
        for (int i = 0; i < actual.servers.length; i++) {
            assertStringContains(actual.servers[i], expected.servers[i]);
        }
        assertEquals(actual.domains.length, expected.domains.length);
        assertEquals(actual.tlsName, expected.tlsName);
        assertEquals(actual.servers.length, expected.servers.length);
        assertEquals(actual.tlsServers.length, expected.tlsServers.length);
        for (int i = 0; i < actual.tlsServers.length; i++) {
            assertStringContains(actual.servers[i], expected.servers[i]);
        }
        assertEquals(actual.tlsFingerprints.length, expected.tlsFingerprints.length);
        ...
        assertEquals(actual.experimentalOptions.hosts.length,
                expected.experimentalOptions.hosts.length);
        assertEquals(actual.experimentalOptions.tcMode, expected.experimentalOptions.tcMode);
    ...
    public void testTrackedValidationUpdates() throws Exception {
        ...
        mDnsManager.updateLinkPropertiesForNetwork(TEST_NETID, lp);
        if (IS_DEFAULT) mDnsManager.setDefaultDnsSystemProperties(lp.getDnsServers());
        mDnsManager.flushVmDnsCache();
        mDnsManager.updateLinkPropertiesForNetwork(TEST_NETID_ALTERNATE, lp);
        if (NOT_DEFAULT) mDnsManager.setDefaultDnsSystemProperties(lp.getDnsServers());
        mDnsManager.flushVmDnsCache();
    ...
    public void testSetDnsConfiguration() throws Exception {
        ...
        expectedParams.experimentalOptions = new ResolverExperimentalOptionsParcel();
        assertResolverParamsEqual(actualParams, expectedParams);
    }
}
