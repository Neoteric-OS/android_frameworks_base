...
public class NetworkNotificationManagerTest {
    ...
    public void testClearNotificationByType() {
        ...
        // Previous notification is CONNECTED and given type is CONNECTED too. The notification
        // should be cleared.
        mManager.clearNotification(id, CONNECTED);
        verify(mNotificationManager, times(1))
                .cancelAsUser(eq(tag), eq(CONNECTED.eventId), any());

        mManager.showNotification(id, CONNECTED, mWifiNai, mCellNai, null, false);
        verify(mNotificationManager, times(2))
                .notifyAsUser(eq(tag), eq(CONNECTED.eventId), any(), any());

        // LOST_INTERNET notification popup after CONNECTED notification.
        mManager.showNotification(id, LOST_INTERNET, mWifiNai, mCellNai, null, false);
        verify(mNotificationManager, times(1))
                .notifyAsUser(eq(tag), eq(LOST_INTERNET.eventId), any(), any());

        // Previous notification is LOST_INTERNET and given type is CONNECTED. The notification
