...
public class NetworkNotificationManagerTest {
    ...
    private void verifyTitleByNetwork(final int id, final NetworkAgentInfo nai, final int title) {
        ...
        if (transportType == NetworkCapabilities.TRANSPORT_WIFI) {
            verify(mResources).getString(eq(title), eq(TEST_EXTRA_INFO));
        } else {
            verify(mResources).getString(title);
        }
        verify(mResources).getString(eq(R.string.private_dns_broken_detailed));
    }
