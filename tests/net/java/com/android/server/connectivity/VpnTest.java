...
public class VpnTest {
    ...
    public void testLockdownWhitelist() throws Exception {
        ...
        assertTrue(vpn.setAlwaysOnPackage(PKGS[1], true, Collections.singletonList(PKGS[2]), mKeyStore));
        verify(mNetService).setAllowOnlyVpnForUids(eq(true), aryEq(new UidRange[] {
                new UidRange(user.start, user.start + PKG_UIDS[1] - 1),
                new UidRange(user.start + PKG_UIDS[2] + 1, user.stop)
        }));
        ...
        assertTrue(vpn.setAlwaysOnPackage(PKGS[1], true, Collections.singletonList(PKGS[3]), mKeyStore));
        verify(mNetService).setAllowOnlyVpnForUids(eq(false), aryEq(new UidRange[] {
                new UidRange(user.start + PKG_UIDS[2] + 1, user.stop)
        }));
        verify(mNetService).setAllowOnlyVpnForUids(eq(true), aryEq(new UidRange[] {
        ...
        // Change the VPN app.
        assertTrue(vpn.setAlwaysOnPackage(PKGS[0], true, Collections.singletonList(PKGS[3]), mKeyStore));
        verify(mNetService).setAllowOnlyVpnForUids(eq(false), aryEq(new UidRange[] {
                new UidRange(user.start, user.start + PKG_UIDS[1] - 1),
                new UidRange(user.start + PKG_UIDS[1] + 1, user.start + PKG_UIDS[3] - 1)
        }));
        ...
        // Add the whitelist.
        assertTrue(vpn.setAlwaysOnPackage(PKGS[0], true, Collections.singletonList(PKGS[1]), mKeyStore));
        verify(mNetService).setAllowOnlyVpnForUids(eq(false), aryEq(new UidRange[] {
                new UidRange(user.start + PKG_UIDS[0] + 1, user.stop)
        }));
        verify(mNetService).setAllowOnlyVpnForUids(eq(true), aryEq(new UidRange[] {
        ...
        assertFalse(vpn.setAlwaysOnPackage(PKGS[0], true, Collections.singletonList("a.b,c.d"), mKeyStore));
    ...
    private Vpn createVpnAndSetupUidChecks() throws Exception {
        final Vpn vpn = createVpn(primaryUser.id);
        setMockedUsers(primaryUser);
    ...
    private void checkProvisionVpnProfile(
            Vpn vpn, int grantedOp, boolean expectedResult, int... checkedOps) {
        if (grantedOp != AppOpsManager.OP_NONE) {
            when(mAppOps.noteOpNoThrow(grantedOp, Process.myUid(), TEST_VPN_PKG))
    ...
    public void testProvisionVpnProfilePreconsented() throws Exception {
        ...
        checkProvisionVpnProfile(vpn, AppOpsManager.OP_ACTIVATE_PLATFORM_VPN,
                true /* expectedResult */, AppOpsManager.OP_ACTIVATE_PLATFORM_VPN);
    }
    ...
    public void testProvisionVpnProfileNotPreconsented() throws Exception {
        ...
        checkProvisionVpnProfile(vpn, AppOpsManager.OP_NONE, false /* expectedResult */,
    ...
    public void testProvisionVpnProfileVpnServicePreconsented() throws Exception {
        ...
        checkProvisionVpnProfile(vpn, AppOpsManager.OP_ACTIVATE_VPN, true /* expectedResult */,
    ...
    public void testStartVpnProfileVpnServicePreconsented() throws Exception {
        ...
        when(mAppOps.noteOpNoThrow(AppOpsManager.OP_ACTIVATE_VPN, Process.myUid(), TEST_VPN_PKG))
                .thenReturn(AppOpsManager.MODE_ALLOWED);
        when(mKeyStore.get(vpn.getProfileNameForPackage(TEST_VPN_PKG)))
                .thenReturn(mVpnProfile.encode());
    ...
    @Test
    public void testIkev2VpnRunner() throws Exception {
        ...
        IpSecTunnelInterfaceResponse dummyResponse =
                new IpSecTunnelInterfaceResponse(IpSecManager.Status.OK, 0x2345, "TEST_INTF");
        ...
        ArgumentCaptor<ConnectivityManager.NetworkCallback> cbCaptor =
                ArgumentCaptor.forClass(ConnectivityManager.NetworkCallback.class);
        verify(mConnectivityManager).requestNetwork(any(), cbCaptor.capture());

        ConnectivityManager.NetworkCallback networkCb = cbCaptor.getValue();
        assertNotNull(networkCb);

        // Trigger the onAvailable callback
        final Network network = new Network(1);
        networkCb.onAvailable(network);

        // Verify and capture the IKE session creation
        ArgumentCaptor<IkeSessionCallback> ikeCbCaptor =
                ArgumentCaptor.forClass(IkeSessionCallback.class);
        ArgumentCaptor<ChildSessionCallback> childCbCaptor =
                ArgumentCaptor.forClass(ChildSessionCallback.class);
        verify(mIkev2SessionCreator).createIkeSession(
                any(), any(), any(), any(), ikeCbCaptor.capture(), childCbCaptor.capture());

        IkeSessionCallback ikeCb = ikeCbCaptor.getValue();
        ChildSessionCallback childCb = childCbCaptor.getValue();
        assertNotNull(ikeCb);
    }
