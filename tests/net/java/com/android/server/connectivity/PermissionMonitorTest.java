...
public class PermissionMonitorTest {
    ...
    private class NMSMonitor {
        ...
        NMSMonitor(INetworkManagementService mockNMS) throws Exception {
            ...
            doAnswer((InvocationOnMock invocation) -> {
                ...
                final Boolean value = args[0].equals("SYSTEM");
                for (final int uid : (int []) args[1]) {
                    if (mApps.containsKey(uid) && mApps.get(uid) == value) {
                        fail("uid " + uid + " is already set to " + value);
                    }
                    mApps.put(uid, value);
                }
                return null;
        ...
        public void expectNoPermission(int [] users, int [] apps) {
            for (final int user : users) {
                for (final int app : apps) {
                    final int uid = UserHandle.getUid(user, app);
                    if (mApps.containsKey(uid)) fail("uid " + uid + " is existing.");
                }
            }
        }
    }
    ...
    public void testUserAndPackageAddRemove() throws Exception {
        ...
        doReturn(Boolean.TRUE).when(mPermissionMonitor).highestPermissionForUid(eq(Boolean.TRUE),
                anyString());
        doReturn(Boolean.TRUE).when(mPermissionMonitor).highestPermissionForUid(or(isNull(),
                eq(Boolean.FALSE)) , eq(SYSTEM_PACKAGE1));
        doReturn(Boolean.FALSE).when(mPermissionMonitor).highestPermissionForUid(or(isNull(),
                eq(Boolean.FALSE)) , eq(SYSTEM_PACKAGE2));
        doReturn(Boolean.FALSE).when(mPermissionMonitor).highestPermissionForUid(or(isNull(),
                eq(Boolean.FALSE)), eq(MOCK_PACKAGE1));

        // Add SYSTEM_PACKAGE2, expect only have network permission.
        mPermissionMonitor.onUserAdded(MOCK_USER1);
        mPermissionMonitor.onAppAdded(SYSTEM_PACKAGE2, SYSTEM_UID);
        mNMSMonitor.expectPermission(NETWORK, new int[]{MOCK_USER1}, new int[]{SYSTEM_UID});

        // Add SYSTEM_PACKAGE1, expect permission escalate.
        mPermissionMonitor.onAppAdded(SYSTEM_PACKAGE1, SYSTEM_UID);
        ...
        mPermissionMonitor.onAppAdded(MOCK_PACKAGE1, MOCK_UID1);
        mNMSMonitor.expectPermission(SYSTEM, new int[]{MOCK_USER1, MOCK_USER2},
                new int[]{SYSTEM_UID});
        ...
        mPermissionMonitor.onAppRemoved(MOCK_UID1);
        ...
        mPermissionMonitor.onAppRemoved(SYSTEM_UID);
        mNMSMonitor.expectPermission(NETWORK, new int[]{MOCK_USER1, MOCK_USER2},
                new int[]{SYSTEM_UID});
