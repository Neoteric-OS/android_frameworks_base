...
import android.net.NetworkScore.DONT_FORCE_KEEPUP
...
class FullScoreTest {
    ...
    fun testGetLegacyInt() {
        val ns = FullScore(50, 0L /* policy */, DONT_FORCE_KEEPUP)
        assertEquals(10, ns.legacyInt) // -40 penalty for not being validated
        assertEquals(50, ns.legacyIntAsValidated)

        val vpnNs = FullScore(101, 0L /* policy */, DONT_FORCE_KEEPUP).withPolicies(vpn = true)
    ...
    @Test
    fun testToString() {
        val string = FullScore(10, 0L /* policy */, DONT_FORCE_KEEPUP)
