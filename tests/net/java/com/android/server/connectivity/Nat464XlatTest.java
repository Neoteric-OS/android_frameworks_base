...
public class Nat464XlatTest {
    ...
    static final boolean SHOULD_NOTIFY = true;
    static final boolean SHOULD_NOT_NOTIFY = false;
    ...
    public void testNormalStartAndStop() throws Exception {
        ...
        // ConnectivityService stops clat (Network disconnects).
        nat.stop(SHOULD_NOT_NOTIFY);

        verify(mNms).unregisterObserver(eq(nat));
        verify(mNms).stopClatd(eq(BASE_IFACE));
        verify(mNms).setInterfaceIpv6NdOffload(eq(BASE_IFACE), eq(true));

        // Stacked interface removed notification arrives: no-op.
    ...
    public void testStopBeforeClatdStarts() throws Exception {
        ...
        // ConnectivityService immediately stops clat (Network disconnects).
        nat.stop(SHOULD_NOT_NOTIFY);
    ...
    @Test
    public void testIPv4AppearsBeforeClatdStarts() throws Exception {
        Nat464Xlat nat = makeNat464Xlat();
        ...
        // An IPv4 address appears and ConnectivityService immediately stops clat.
        nat.stop(SHOULD_NOTIFY);
        verify(mNms).unregisterObserver(eq(nat));
        verify(mNms).stopClatd(eq(BASE_IFACE));
        assertNull(mLooper.nextMessage());

        // In-flight interface up notification arrives: no-op.
        nat.interfaceLinkStateChanged(STACKED_IFACE, true);
        mLooper.dispatchNext();

        // Interface removed notification arrives: no-op.
        nat.interfaceRemoved(STACKED_IFACE);
        mLooper.dispatchNext();
    ...
    @Test
    public void testIPv4AppearsAfterClatdStarts() throws Exception {
        Nat464Xlat nat = makeNat464Xlat();
        ArgumentCaptor<LinkProperties> c = ArgumentCaptor.forClass(LinkProperties.class);
        ...
        // An IPv4 address appears and ConnectivityService stops clat.
        nat.stop(SHOULD_NOTIFY);
        mLooper.dispatchNext();
