...
class LegacyTypeTrackerTest {
    ...
    private val mMockService = mock(ConnectivityService::class.java)
    private var mTracker: LegacyTypeTracker = LegacyTypeTracker(mMockService)

    @Before
    fun setup() {
        // Install sane defaults
        doReturn(false).`when`(mMockService).isDefaultNetwork(any())
        mTracker = LegacyTypeTracker(mMockService)
        supportedTypes.forEach {
            mTracker.addSupportedType(it)
        }
    }
