...
public class ConnectivityServiceTest extends AndroidTestCase {
    ...
    private class MockNetworkAgent {
        ...
        public void setNetworkSpecifier(String specifier) {
            if (TextUtils.isEmpty(specifier)) {
                mNetworkCapabilities.setNetworkSpecifier(null);
            } else {
                mNetworkCapabilities.setNetworkSpecifier(new StringNetworkSpecifier(specifier));
            }
            mNetworkAgent.sendNetworkCapabilities(mNetworkCapabilities);
        }
    ...
    public void testNetworkSpecifier() {
        ...
        NetworkRequest rEmpty4 = newWifiRequestBuilder().setNetworkSpecifierObject(null).build();
    ...
    public void testInvalidNetworkSpecifier() {
        try {
            NetworkRequest.Builder builder = new NetworkRequest.Builder();
            builder.setNetworkSpecifierObject(new MatchAllNetworkSpecifier());
        ...
        try {
            builder.setNetworkSpecifierObject(new NonParcelableSpecifier());
        ...
        builder = new NetworkRequest.Builder().addTransportType(TRANSPORT_ETHERNET);
        NetworkRequest nr = builder.setNetworkSpecifierObject(new ParcelableSpecifier()).build();
        assertNotNull(nr);

        try {
            Parcel parcelW = Parcel.obtain();
            nr.writeToParcel(parcelW, 0);
            fail("Non-whitelisted specifier did not throw exception");
        } catch (IllegalArgumentException expected) {
            // expected
        }
    }
