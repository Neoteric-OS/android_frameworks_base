...
public class IpSecServiceTest {
  ...
  public void testOpenUdpEncapsulationSocket()
    ...
    IpSecUdpEncapResponse udpEncapResp = ipSecSrv.openUdpEncapsulationSocket(localport,
                                                    new Binder());
  ...
  public void testDumpMessage()
    ...
    IpSecTransform.Builder builder =
        new IpSecTransform.Builder(mockContext)
            ...
            .setAuthentication(
                IpSecTransform.DIRECTION_IN,
                new IpSecAlgorithm(IpSecAlgorithm.AUTH_HMAC_SHA256, AUTH_KEY, CRYPT_KEY.length * 8))
    ...
    assertTrue(result.contains("encryption=(mName=(cbc(aes)) mKey={key length=32"));
    assertTrue(result.contains("authentication=(mName=(hmac(sha256)) mKey={key length=32"));
