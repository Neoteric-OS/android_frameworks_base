...
public class SEService {
    ...
    public SEService(Context context, SEService.CallBack listener) {
        ...
        _context = context;
        _callerCallback = listener;

        _connection = new ServiceConnection() {

            public synchronized void onServiceConnected(ComponentName className, IBinder service) {

                _smartcardService = ISmartcardService.Stub.asInterface(service);
                if (_callerCallback != null)
                    _callerCallback.serviceConnected(SEService.this);
            ...
            public void onServiceDisconnected(ComponentName className) {
                ...
                _smartcardService = null;
                Log.v(SERVICE_TAG, "Service onServiceDisconnected");
            }
        };

        boolean bindingSuccessful = _context.bindService(new Intent(ISmartcardService.class
                .getName()), _connection, Context.BIND_AUTO_CREATE);
    ...
    public Reader[] getReaders() {
        ...
        String[] readerNames;
        try {
            readerNames = _smartcardService.getReaders(error);
        } catch (Exception e) {
            throw new IllegalStateException(e);
        }
        checkForException(error);

        _readers = new Reader[readerNames.length];
        int i = 0;
        for (String readerName : readerNames) {
            _readers[i] = new Reader(readerName, this);
            i++;
        }
        return _readers;
    }
    ...
    boolean isSecureElementPresent(Reader reader) {
        ...
        boolean isPresent;
        try {
            isPresent = _smartcardService.isCardPresent(reader.getName(), error);
            checkForException(error);
        } catch (Exception e) {
            return false;
        }
    ...
    Channel openBasicChannel(Session session, byte[] aid) throws IOException {
        ...
        long hChannel;
        try {
            hChannel = _smartcardService.openBasicChannelAid(session.getReader().getName(), aid,
                    callback, error);
        } catch (Exception e) {
    ...
    Channel openLogicalChannel(Session session, byte[] aid) throws IOException {
        ...
        long hChannel;
        try {
            hChannel = _smartcardService.openLogicalChannel(session.getReader().getName(), aid,
                    callback, error);
        } catch (Exception e) {
            throw new IOException(e.getMessage());
        }
    ...
    void closeChannel(Channel channel) throws IOException {
        ...
        SmartcardError error = new SmartcardError();
        try {
            _smartcardService.closeChannel(channel.getHandle(), error);
        } catch (Exception e) {
            throw new IOException(e.getMessage());
        }
        checkForException(error);
    ...
    byte[] transmit(Channel channel, byte[] command) throws IOException {
        ...
        byte[] response;
        try {
            response = _smartcardService.transmit(channel.getHandle(), command, error);
        } catch (Exception e) {
            throw new IOException(e.getMessage());
        }
        checkForException(error);
    ...
    byte[] getAtr(Reader reader) {
        ...
        byte[] atr;
        try {
            atr = _smartcardService.getAtr(reader.getName(), error);
            checkForException(error);
        } catch (Exception e) {
            return null;
        }
