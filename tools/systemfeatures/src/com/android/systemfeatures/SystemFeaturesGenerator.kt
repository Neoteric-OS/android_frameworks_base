...
object SystemFeaturesGenerator {
     ...
     *
     * This should probably be implemented in terms of a proper Builder-type input argument, but
     * it's primarily used for unit testing as opposed to direct production usage.
     */
    @JvmStatic
                ...
                val featureVersion =
                    featureArgs[1].toIntOrNull()
                        ?: throw IllegalArgumentException(
                            "Invalid feature version input: E$featureArgs[1]"
                        )
    ...
    private fun parseFeatureName(name: String): String =
        when {
            name.startsWith("android") ->
                throw IllegalArgumentException(
                    "Invalid feature name input: android-namespaced features must be provided as " +
                    "PackageManager.FEATURE_* suffixes, not raw feature strings."
                )
