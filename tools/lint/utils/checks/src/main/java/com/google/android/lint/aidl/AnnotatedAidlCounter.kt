...
class AnnotatedAidlCounter : AidlImplementationDetector() {
    ...
    override fun visitAidlMethod(
        ...
        if (node.hasAnnotation(ANNOTATION_ENFORCE_PERMISSION)) {
            packageStat.enforced += 1
        } else if (node.hasAnnotation(ANNOTATION_REQUIRES_NO_PERMISSION)) {
            packageStat.notRequired += 1
        } else {
            packageStat.unannotated += 1
        }
