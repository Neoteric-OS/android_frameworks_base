  ...
  companion object {
    ...
    val EXPLANATION =
      ...
      **Problem:**
      ...
      Calling `PendingIntent.getActivity` often accidentally uses the user 0 context, especially in `system_server`.  Since there's no explicit user parameter in the `getActivity` method, it can be hard to tell at a glance which user the `PendingIntent` activity is associated with, making the code less readable.
      ...
      * You've confirmed that the `PendingIntent` activity you're targeting is the correct one and is **rightly** associated with the context parameter passed into the `PendingIntent.getActivity` method.
      ...
      If you are unsure about the user context, it's best to err on the side of caution and explicitly specify the user using the method specified above.
