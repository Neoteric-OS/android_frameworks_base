 ...
 *      However, a switch statement will benefit from the future of compiler improvement.
 */''')
    print('constexpr const uint64_t REPRESENTATIVE_LOCALES[] = {')
    sorted_uint64 = sorted([pack_to_uint64(l) for l in representative_locales])
    for locale in sorted_uint64:
        print('        0x%08XLLU,' % locale)
    print('};')
    print('constexpr const size_t REPRESENTATIVE_LOCALES_SIZE = sizeof(REPRESENTATIVE_LOCALES) / sizeof(REPRESENTATIVE_LOCALES[0]);')
    print('static_assert(std::is_sorted(REPRESENTATIVE_LOCALES, REPRESENTATIVE_LOCALES + REPRESENTATIVE_LOCALES_SIZE - 1));')
    ...
    print('    return std::binary_search(REPRESENTATIVE_LOCALES, REPRESENTATIVE_LOCALES + REPRESENTATIVE_LOCALES_SIZE - 1, packed_locale);')
    print('}')
