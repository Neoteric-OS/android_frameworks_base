...
bool ParseFeatureFlagsParameter(
    StringPiece arg, android::IDiagnostics* diag,
    std::unordered_map<std::string, std::optional<bool>>* out_feature_flag_values) {
  if (arg.empty()) {
    return true;
  }
  ...
  for (StringPiece flag_and_value : util::Tokenize(arg, ',')) {
    ...
    if (!out_feature_flag_values->try_emplace(std::string(flag_name), flag_value).second) {
      diag->Error(android::DiagMessage()
