...
static int run(int argc, char const* const* argv) {
    ...
    if (minApiLevel > compileApiLevel) {
        // Cannot support a newer API level while compiling against older API level.
        fprintf(stderr,
                "Cannot support a newer API level while compiling against older API level.\n");
        return 1;
    }

    if (minApiLevel != API_LEVEL_CURRENT && minApiLevel >= compileApiLevel) {
        // If specified, minApiLevel should always be lower than compileApiLevel.
        fprintf(stderr, "Invalid minApiLevel or compileApiLevel. If minApiLevel or compileApiLevel"
                " is specified, minApiLevel should be lower than compileApiLevel.\n");
        return 1;
    }
