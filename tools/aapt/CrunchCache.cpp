...
CrunchCache::CrunchCache(String8 sourcePath, String8 destPath, FileFinder* ff)
    : mSourcePath(sourcePath), mDestPath(destPath), mSourceFiles(0), mDestFiles(0), mFF(ff)
{
...
void CrunchCache::loadFiles()
{
    ...
    mFF->findFiles(mSourcePath, mExtensions, mSourceFiles,dw);

    // Load files in the destination directory
    mFF->findFiles(mDestPath,mExtensions,mDestFiles,dw);
