    ...
    while (argc && argv[0][0] == '-') {
        ...
        while (*cp != '\0') {
            ...
            case 'm':
                ...
                break;
            case 'o':
                // FIXME: since the only information extracted from the
                // original package is the package id (0x01, 0x7f), should the
                // -o flag be changed to instead accept an integer argument
                // directly?  Use -o for 0x7f and -O for 0x01? Or do we still
                // wish a strong coupling between overlay and original package,
                // to add checks for certificates, etc?
                argc--;
                argv++;
                if (!argc) {
