...
static void load_maps(int pid, stats_t* stats, bool* foundSwapPss)
{
    *foundSwapPss = false;
    ...
    UniqueFile fp = MakeUniqueFile(smaps_path.c_str(), "r");
...
static long get_allocated_vmalloc_memory() {
    ...
    UniqueFile fp = MakeUniqueFile("/proc/vmallocinfo", "r");
    if (fp == nullptr) {
        return 0;
    }
...
#define ZRAM_SYSFS "/sys/block/zram0/"
    UniqueFile mm_stat_file = MakeUniqueFile(ZRAM_SYSFS "mm_stat", "r");
    if (mm_stat_file) {
    ...
    UniqueFile mem_used_total_file = MakeUniqueFile(ZRAM_SYSFS "mem_used_total", "r");
...
{
    UniqueFile fp = MakeUniqueFile(BINDER_STATS, "r");
    if (fp == nullptr) {
        return -1;
    }
...
static void dumpNativeHeap(FILE* fp)
{
    ...
    UniqueFile in = MakeUniqueFile(maps, "r");
    if (in == nullptr) {
