...
static jlong android_os_Debug_getGpuPrivateMemoryKb(JNIEnv* env, jobject clazz) {
    ...
    ssize_t gpu_private_mem = memtrack_proc_gl_pss(p);

    if (gpu_private_mem < 0) {
        LOG(ERROR) << "getGpuPrivateMemoryKb: Failed to get total GPU-private (GL) memory";
        memtrack_proc_destroy(p);
        return -1;
    }

    memtrack_proc_destroy(p);
    return gpu_private_mem / 1024;
}
