...
public:
    ...
    std::optional<std::pair<char*, char*>> readLine(FailFn fail_fn) {
        ...
        while (true) {
            if (mNext == mEnd) {
                ...
                ssize_t nread =
                        TEMP_FAILURE_RETRY(read(mFd, mBuffer + mEnd, MAX_COMMAND_BYTES - mEnd));
                if (nread <= 0) {
                    if (nread == 0) {
                        return {};
                    }
    ...
    bool isSimpleForkCommand(int minUid, FailFn fail_fn) {
        ...
        while (mLinesLeft > 0) {
            ...
            if (arg_end - arg_start >= NN_LENGTH && strncmp(arg_start, NICE_NAME, NN_LENGTH) == 0) {
            ...
            if (arg_end - arg_start >= SU_LENGTH && strncmp(arg_start, SETUID, SU_LENGTH) == 0) {
            ...
            if (arg_end - arg_start >= SG_LENGTH && strncmp(arg_start, SETGID, SG_LENGTH) == 0) {
    ...
    void setFd(int new_fd) { mFd = new_fd; }

    int getFd() const { return mFd; }

    const char* niceNameAddr() const { return mNiceName; }

    // Debug only:
    void logState() const {
        ALOGD("mbuffer starts with %c%c, nice name is %s, "
              "mEnd = %u, mNext = %u, mLinesLeft = %d, mFd = %d",
              mBuffer[0], (mBuffer[1] == '\n' ? ' ' : mBuffer[1]), niceNameAddr(),
...
jboolean com_android_internal_os_ZygoteCommandBuffer_nativeForkRepeatedly(
        JNIEnv* env, jclass, jlong j_buffer, jint zygote_socket_fd, jint expected_uid, jint minUid,
    ...
    do {
        ...
        for (;;) {
            ...
            if ((fd_structs[SESSION_IDX].revents & POLLIN) != 0) {
                ...
                fail_fn_z(CREATE_ERROR("Poll returned with no descriptors ready! Poll returned %d",
                                       poll_res));
            }
            ...
            if (new_fd != session_socket) {
                // Move new_fd back to the old value, so that we don't have to change Java-level
                // data structures to reflect a change. This implicitly closes the old one.
                if (dup2(new_fd, session_socket) != session_socket) {
                    fail_fn_z(CREATE_ERROR("Failed to move fd %d to %d: %s", new_fd, session_socket,
                                           strerror(errno)));
                }
                close(new_fd);
            }
            ...
            if (getsockopt(session_socket, SOL_SOCKET, SO_PEERCRED, &credentials, &cred_size) ==
                -1) {
                fail_fn_z(CREATE_ERROR("ForkMany failed to get credentials: %s", strerror(errno)));
            }
            if (cred_size != sizeof credentials) {
                fail_fn_z(CREATE_ERROR("ForkMany credential size = %d, should be %d", cred_size,
                                       static_cast<int>(sizeof credentials)));
            }
        }
        first_time = false;
