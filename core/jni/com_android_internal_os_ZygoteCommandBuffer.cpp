  ...
  std::optional<std::pair<char*, char *>> readLine(FailFn fail_fn) {
    ...
    for (;;) {
      ...
      while (mNext < mEnd) {
        ...
        if (mBuffer[mNext++] == '\n') {
          if (--mLinesLeft < 0) {
            fail_fn("ZygoteCommandBuffer.readLine attempted to read pas mEnd of command");
          }
...
ALOGW("isSimpleForkCommand returning %d", saw_runtime_args && saw_setuid && saw_setgid);
  ...
  // Debug only:
  void logState() const {
    ALOGD("mbuffer starts with %c%c, nice name is %s mEnd = %u, mNext = %u, mLinesLeft = %d, mFd = %d",
          mBuffer[0], (mBuffer[1] == '\n' ? ' ' : mBuffer[1]),
          niceNameAddr(),
...
jint com_android_internal_os_ZygoteCommandBuffer_nativeGetCount(JNIEnv* env, jclass,
  ...
  int result = n_buffer->getCount(fail_fn);
  return result;
}
...
ALOGW("nativeNextArg returned %s", cresult);
  *endp = '\n';
  return result;
}
...
// least minUid.
NO_PAC_FUNC
jint com_android_internal_os_ZygoteCommandBuffer_nativeForkRepeatedly(
            JNIEnv* env,
            jclass,
            jlong j_buffer,
  ...
  do {
    ...
    for (;;) {
      ...
      if (new_fd == -1) {
        fail_fn_z("Accept(%d) failed %d: %s", zygote_socket_fd, errno, strerror(errno));
      }
      if (new_fd != session_socket) {
          ...
          if (dup2(new_fd, session_socket) != session_socket) {
            fail_fn_z(CREATE_ERROR("Failed to move fd %d to %d, errno = %d",
                                   new_fd, session_socket, errno));
          }
          close(new_fd);
      }
      ...
      if (setsockopt(session_socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, timeout_size) != 0) {
        fail_fn_z(CREATE_ERROR("Failed to set receive timeout for socket %d", session_socket));
      }
      if (setsockopt(session_socket, SOL_SOCKET, SO_SNDTIMEO, &timeout, timeout_size) != 0) {
        fail_fn_z(CREATE_ERROR("Failed to set send timeout for socket %d", session_socket));
      }
      if (getsockopt(session_socket, SOL_SOCKET, SO_PEERCRED, &credentials, &cred_size) == -1
          || cred_size != sizeof credentials) {
        fail_fn_z(CREATE_ERROR("ForkMany failed to get credentials, errno = %d", errno));
      }
    }
    first_time = false;
...
static const JNINativeMethod gMethods[] = {
        ...
        {"nativeForkRepeatedly", "(JIIILjava/lang/String;)I", (void *) METHOD_NAME(nativeForkRepeatedly)},
};
