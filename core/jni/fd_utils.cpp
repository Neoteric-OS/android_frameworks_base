...
FileDescriptorInfo* FileDescriptorInfo::CreateFromFd(int fd, fail_fn_t fail_fn) {
    ...
    if (!S_ISCHR(f_stat.st_mode) && !S_ISREG(f_stat.st_mode)) {
        ...
        fail_fn(android::base::StringPrintf("Unsupported st_mode for FD %d:  %s", fd,
                                            mode.c_str()));
    }
    ...
    if (!android::base::Readlink(fd_path, &file_path)) {
        fail_fn(android::base::StringPrintf("Could not read fd link %s: %s", fd_path.c_str(),
                                            strerror(errno)));
    }
    ...
    if (fd_flags == -1) {
        fail_fn(android::base::StringPrintf("Failed fcntl(%d, F_GETFD) (%s): %s", fd,
    ...
    if (fs_flags == -1) {
        fail_fn(android::base::StringPrintf("Failed fcntl(%d, F_GETFL) (%s): %s", fd,
                                            file_path.c_str(), strerror(errno)));
    }
...
void FileDescriptorInfo::ReopenOrDetach(fail_fn_t fail_fn) const {
    ...
    if (new_fd == -1) {
        fail_fn(android::base::StringPrintf("Failed open(%s, %i): %s", file_path.c_str(),
                                            open_flags, strerror(errno)));
    }

    if (TEMP_FAILURE_RETRY(fcntl(new_fd, F_SETFD, fd_flags)) == -1) {
        close(new_fd);
        fail_fn(android::base::StringPrintf("Failed fcntl(%d, F_SETFD, %d) (%s): %s", new_fd,
                                            fd_flags, file_path.c_str(), strerror(errno)));
    }

    if (TEMP_FAILURE_RETRY(fcntl(new_fd, F_SETFL, fs_flags)) == -1) {
        close(new_fd);
        fail_fn(android::base::StringPrintf("Failed fcntl(%d, F_SETFL, %d) (%s): %s", new_fd,
                                            fs_flags, file_path.c_str(), strerror(errno)));
    }

    if (offset != -1 && TEMP_FAILURE_RETRY(lseek64(new_fd, offset, SEEK_SET)) == -1) {
        close(new_fd);
        fail_fn(android::base::StringPrintf("Failed lseek64(%d, SEEK_SET) (%s): %s", new_fd,
                                            file_path.c_str(), strerror(errno)));
    }
    ...
    if (TEMP_FAILURE_RETRY(dup3(new_fd, fd, dup_flags)) == -1) {
        close(new_fd);
        fail_fn(android::base::StringPrintf("Failed dup3(%d, %d, %d) (%s): %s", fd, new_fd,
                                            dup_flags, file_path.c_str(), strerror(errno)));
    }
...
FileDescriptorInfo::FileDescriptorInfo(int fd)
      : fd(fd), stat(), open_flags(0), fd_flags(0), fs_flags(0), offset(0), is_sock(true) {}

FileDescriptorInfo::FileDescriptorInfo(struct stat stat, const std::string& file_path, int fd,
                                       int open_flags, int fd_flags, int fs_flags, off_t offset)
      : fd(fd),
        stat(stat),
        file_path(file_path),
...
void FileDescriptorInfo::DetachSocket(fail_fn_t fail_fn) const {
    ...
    if (dup3(dev_null_fd, fd, O_CLOEXEC) == -1) {
        fail_fn(android::base::StringPrintf("Failed dup3 on socket descriptor %d: %s", fd,
                                            strerror(errno)));
    }
...
void FileDescriptorTable::Restat(const std::vector<int>& fds_to_ignore, fail_fn_t fail_fn) {
    ...
    DIR* proc_fd_dir = opendir(kFdPath);
    if (proc_fd_dir == nullptr) {
        fail_fn(android::base::StringPrintf("Unable to open directory %s: %s", kFdPath,
                                            strerror(errno)));
    }
...
FileDescriptorTable::FileDescriptorTable(const std::unordered_map<int, FileDescriptorInfo*>& map)
      : open_fd_map_(map) {}
