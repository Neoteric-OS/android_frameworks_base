...
protected:
        ...
        uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags = 0) override
    {
        ...
        if (env->ExceptionCheck()) {
            ...
            String8 msg;
            msg.appendFormat("*** Uncaught remote exception! Exceptions are not yet supported across "
                             "processes. Client PID %d UID %d.", state->getCallingPid(), state->getCallingUid());
