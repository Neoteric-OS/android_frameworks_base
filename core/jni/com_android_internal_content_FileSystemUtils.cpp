...
bool punchHoles(const char *filePath, const uint64_t offset,
    ...
    for (size_t index = 0; programHeaders.size() >= 2 && index < programHeaders.size() - 1;
        ...
        IF_ALOGD() {
            ALOGD("Punching holes for length:%" PRIu64 "content which should be zero: %s", punchLen,
                  HexString(buffer.data(), buffer.size()).c_str());
        }

        bool isZero = std::all_of(buffer.begin(), buffer.end(), [](uint8_t i) { return i == 0; });
        if (!isZero) {
            ALOGD("Found non zero content while trying to punch hole.Skipping operation");
            return false;
        }

        // if we have a uncompressed file which is being opened from APK, use the offset to punch
...
bool punchHolesInElf64(const char *filePath, const uint64_t offset) {
    ...
    inputStream.seekg(offset + programHeaderOffset);
