...
bool punchHoles(const char *filePath, const uint64_t offset,
    ...
    // read in chunks of 60KB
    constexpr uint64_t kChunkSize = 60 * 1024;
    ...
    for (size_t index = 0; programHeaders.size() >= 2 && index < programHeaders.size() - 1;
        ...
        // Read and punch content in kChunkSize
        while (position <= endPosition) {
            ...
            readLength = std::min(readLength, kChunkSize);
            if (!ReadFullyAtOffset(fd, buffer, readLength, position)) {
            ...
            IF_ALOGD() {
                ALOGD("Punching holes for length:%" PRIu64 " content which should be zero: %s",
                      readLength, HexString(buffer, readLength).c_str());
            }

            bool isZero = std::all_of(buffer, buffer + readLength,
            ...
            int result =
                    fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, position, readLength);
            if (result < 0) {
                ALOGW("fallocate failed to punch hole, error:%d", errno);
            }
