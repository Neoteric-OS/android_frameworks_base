...
#define CREATE_ERROR(...) \
    StringPrintf("%s:%d: ", __FILE__, __LINE__).append(StringPrintf(__VA_ARGS__))

namespace android {
namespace zygote {

pid_t ForkCommon(JNIEnv* env, bool is_system_server, const std::vector<int>& fds_to_close,
                 const std::vector<int>& fds_to_ignore, bool is_priority_fork, bool purge = true);
 ...
 */
int forkApp(JNIEnv* env, int read_pipe_fd, int write_pipe_fd,
            const std::vector<int>& session_socket_fds, bool args_known, bool is_priority_fork,
            bool purge);

[[noreturn]] void ZygoteFailure(JNIEnv* env, const char* process_name, jstring managed_process_name,
