...
// KeyEventData field bundles — the cxx shared struct declared in
// rust/android_runtime/src/input_event_ffi.rs, whose generated header
// (input_event_ffi.rs.h) defines both sides of the struct.
...
rs_keyEvent_toJava(JNIEnv* env, const android_runtime::KeyEventData* data);
extern "C" __attribute__((visibility("hidden"))) void
rs_keyEvent_fromJava(JNIEnv* env, jobject eventObj, android_runtime::KeyEventData* outData);
extern "C" __attribute__((visibility("hidden"))) int32_t
rs_keyEvent_recycle(JNIEnv* env, jobject eventObj);
...
ScopedLocalRef<jobject> android_view_KeyEvent_obtainAsCopy(JNIEnv* env, const KeyEvent& event) {
    ...
    // KeyEventData::hmac is a std::array<uint8_t, 32>, the same type getHmac()
    // returns, so the field copies across directly.
    data.hmac = event.getHmac();
...
KeyEvent android_view_KeyEvent_obtainAsCopy(JNIEnv* env, jobject eventObj) {
    ...
    event.initialize(data.id, data.device_id, static_cast<uint32_t>(data.source),
                     ui::LogicalDisplayId{data.display_id}, data.hmac, data.action, data.flags,
