...
impl EventLog {
    ...
    pub(crate) fn new(buffer: LogBuffer, construct_event: ConstructEvent) -> EventLog {
        EventLog { buffer, construct_event }
    }
    ...
    pub(crate) fn write_array(
        ...
        for index in 0..count.min(255) {
            ...
            if is_instance::<IntegerBox>(env, &item) {
                ...
                event.append_i32(boxed.value(env).expect("GetIntField(Integer.value) cannot fail"));
                ...
                event.append_i64(boxed.value(env).expect("GetLongField(Long.value) cannot fail"));
                ...
                event.append_f32(boxed.value(env).expect("GetFloatField(Float.value) cannot fail"));
            } else {
...
impl PendingEvent {
    ...
    /// transmission result.
    fn write(self) -> i32 {
        finish_event(self.status, self.ctx, |ctx| ctx.write().unwrap_or_else(LogContextError::code))
    }
}
...
mod tests {
    ...
    fn append_error_wins_over_write_error() {
        assert_eq!(finish_event(-libc::E2BIG, Some(()), |_| -libc::EIO), -libc::E2BIG);
    }
    ...
    fn missing_context_skips_append_and_write() {
        ...
        assert_eq!(finish_event::<()>(status, None, |_| panic!("write ran")), -libc::EBADF);
    }
