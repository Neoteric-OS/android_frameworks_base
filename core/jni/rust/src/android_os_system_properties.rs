...
fn new_java_string<'local>(
    ...
    env.new_string(value).unwrap_or_else(|_| jni::objects::JString::null())
}
...
pub mod system_properties {
    ...
    fn native_get_string<'local>(
        ...
        let value =
            props::read_with(&key.to_str(), |value| (!value.is_empty()).then(|| value.to_owned()))
                .flatten();
    ...
    fn native_get_int(_env: &mut jni::Env<'_>, _class: JClass, key: &JNIStr, def: i32) -> i32 {
        props::read_with(&key.to_str(), libbase_parse::parse_int).flatten().unwrap_or(def)
    }
    ...
    fn native_get_long(_env: &mut jni::Env<'_>, _class: JClass, key: &JNIStr, def: i64) -> i64 {
        props::read_with(&key.to_str(), libbase_parse::parse_long).flatten().unwrap_or(def)
    }
    ...
    fn native_get_boolean(
        ...
        props::read_with(&key.to_str(), libbase_parse::parse_bool).flatten().unwrap_or(def)
    }
