...
fn key_event_data_from_java(env: &mut jni::Env<'_>, event: &KeyEvent<'_>) -> KeyEventData {
    ...
    KeyEventData {
        down_time: event
            .down_time(env)
            .expect("GetLongField(mDownTime) cannot fail"),
        event_time: event
            .event_time(env)
            .expect("GetLongField(mEventTime) cannot fail"),
        id: event.id(env).expect("GetIntField(mId) cannot fail"),
        device_id: event
            .device_id(env)
            .expect("GetIntField(mDeviceId) cannot fail"),
        source: event.source(env).expect("GetIntField(mSource) cannot fail"),
        display_id: event
            .display_id(env)
            .expect("GetIntField(mDisplayId) cannot fail"),
        ...
        key_code: event
            .key_code(env)
            .expect("GetIntField(mKeyCode) cannot fail"),
        scan_code: event
            .scan_code(env)
            .expect("GetIntField(mScanCode) cannot fail"),
        meta_state: event
            .meta_state(env)
            .expect("GetIntField(mMetaState) cannot fail"),
        repeat_count: event
            .repeat_count(env)
            .expect("GetIntField(mRepeatCount) cannot fail"),
