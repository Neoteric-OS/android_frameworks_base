...
//! Bug-for-bug Rust ports of the libbase value parsers that
//! `android.os.SystemProperties` relies on: `ParseInt<T>` (parseint.h) and
//! `ParseBool` (parsebool.cpp). `None` means the C++ parser reported failure,
//! in which case the JNI getters fall back to the caller's default.
//!
//! `ParseInt` semantics, faithfully reproduced: leading C whitespace is
