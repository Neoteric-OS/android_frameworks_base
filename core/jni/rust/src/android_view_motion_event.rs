...
//! across JNI (with `GetPrimitiveArrayCritical` access to the packed array,
//! released with `JNI_ABORT` when it was only read).
...
fn validate_pointer_count(pointer_count: i32) -> Result<(), JniError> {
    if pointer_count < 1 {
        return Err(JniError::IllegalArgument(
            "pointerCount must be at least 1".to_string(),
        ));
    }
    Ok(())
}
...
fn validate_pointer_properties_array<'array, 'local>(
    ...
    let length = env
        ...
        .expect("GetArrayLength cannot fail");
...
fn validate_pointer_coords_obj_array<'array, 'local>(
    ...
    let Some(array) = array else {
        return Err(JniError::IllegalArgument(
            "pointerCoords array must not be null".to_string(),
        ));
    };
    let length = env
        .get_array_length(array)
...
/// axes, the resampled flag, then the packed remainder (read with a critical
/// array section, released with `JNI_ABORT` since it is read-only).
fn pointer_coords_to_native(env: &mut jni::Env<'_>, coords: &JPointerCoords<'_>) -> PointerCoords {
    ...
    out.is_resampled = coords
        .is_resampled(env)
        .expect("GetBooleanField on a cached field ID cannot fail");

    let bits = coords
        .packed_axis_bits(env)
        .expect("GetLongField on a cached field ID cannot fail") as u64;
    ...
    if !packed.is_empty() {
        ...
        if !values_array.is_null() {
            ...
            if env
                .get_float_array_region(&*values_array, 0, &mut values)
                .is_ok()
            {
...
fn obtain_packed_axis_values_array<'local>(
    ...
    if !existing.is_null() {
        ...
        let size = env
            .get_array_length(&*existing)
            .expect("GetArrayLength cannot fail") as u32;
        if min_size <= size {
...
fn pointer_coords_from_native(
    ...
    if !axes_to_copy.is_empty() {
        ...
        if env
            .set_float_array_region(&*out_values_array, 0, &values)
            .is_err()
        {
            return;
        }
...
fn pointer_properties_to_native(
    ...
    PointerProperties {
        id: props
            .id(env)
            .expect("GetIntField on a cached field ID cannot fail"),
        tool_type: props
            .tool_type(env)
            .expect("GetIntField on a cached field ID cannot fail"),
    }
...
fn pointer_properties_from_native(
    ...
    props
        .set_id(env, pointer_properties.id)
...
fn get_native_ptr(env: &mut jni::Env<'_>, event_obj: &JObject<'_>) -> *mut MotionEvent {
    ...
    JLongHandle::from_jlong(
        event
            .native_ptr(env)
            .expect("GetLongField(mNativePtr) cannot fail"),
    )
    .as_ptr()
}
...
/// ownership transfers to the returned Java object.
#[no_mangle]
pub unsafe extern "C" fn rs_motionEvent_obtainFromNative(
    ...
    // SAFETY: Per this function's contract, `event` is live and owned by the
    // caller until this call, which transfers it to the Java object.
    unsafe { obtain_from_native(env, event) }
}
...
pub mod motion_event {
    ...
    fn nativeReadFromParcel(
        ...
        if status != 0 {
            ...
            return Err(JniError::Runtime(
                "Failed to read MotionEvent parcel.".to_string(),
            ));
        }
    ...
    fn nativeWriteToParcel(
        ...
        if status != 0 {
            return Err(JniError::Runtime(
                "Failed to write MotionEvent parcel.".to_string(),
            ));
        }
        Ok(())
    }
