...
/// # fn demo(env: &mut jni::JNIEnv<'_>, name: jni::sys::jstring) {
/// // SAFETY: `name` came from the JVM as a parameter of this native method.
/// let Some(mut copied) = (unsafe { BoundedUtfChars::<3073>::copy_from(env, name) }) else {
...
impl<const CAP: usize> BoundedUtfChars<CAP> {
    ...
    pub const MAX_UTF16_UNITS: usize = (CAP - 1) / 3;

    /// Copies the first [`MAX_UTF16_UNITS`](Self::MAX_UTF16_UNITS) UTF-16
    /// code units of `string` into a new buffer, or returns `None` if
    /// `string` is null.
    ///
    ...
    /// length, so it is always in bounds.
    ///
    /// # Safety
    ///
    /// `string` must be null or a valid JNI reference to a `java.lang.String`
    /// that stays live for the duration of the call (e.g. a parameter the JVM
    /// passed to the enclosing native method).
    pub unsafe fn copy_from(
        env: &mut jni::JNIEnv<'_>,
        string: jni::sys::jstring,
    ) -> Option<BoundedUtfChars<CAP>> {
        if string.is_null() {
            return None;
        }

        // The jni crate does not wrap GetStringUTFRegion, so call it through
        // the raw JNI function table, the same way the crate's internal
        // `jni_method!` macro invokes every JNI function: dereference the
        // env pointer twice to reach the `JNINativeInterface_` table and
        // unwrap the `Option` function pointer. Both functions are JNI 1.1
        // entries, present in every conforming table.
        let raw_env = env.get_raw();
        // SAFETY: `raw_env` comes from a live `JNIEnv`, which guarantees a
        // non-null pointer to a valid function table.
        let get_string_length = unsafe { (**raw_env).GetStringLength }
            .expect("JNIEnv function table is missing GetStringLength");
        // SAFETY: As above.
        let get_string_utf_region = unsafe { (**raw_env).GetStringUTFRegion }
            .expect("JNIEnv function table is missing GetStringUTFRegion");

        // SAFETY: `raw_env` is the current thread's env and the caller
        // guarantees `string` is a valid `java.lang.String` reference
        // (non-null was checked above). GetStringLength only reads.
        ...
        Some(Self::from_zero_padded(buf))
    }
