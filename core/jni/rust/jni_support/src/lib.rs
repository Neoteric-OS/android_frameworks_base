...
//! - [`throw_unless_pending`] — Error path used by generated `Result` shims
...
//! `jni_macros`) aborts. This matches the C++ `LOG_ALWAYS_FATAL`
//! convention: a native method that cannot uphold its contract takes down the
//! process loudly rather than continuing in a corrupt state. Use [`jni_call`],
//! [`throw!`](crate::throw), or `Result` returns for recoverable errors, and
//! reserve panics for genuinely fatal conditions.
...
pub use error::{jni_call, throw_unless_pending, JniError};
