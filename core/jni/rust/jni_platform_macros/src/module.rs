...
//! the Rust-friendly bridging (primitives and `bool`) before and after calling
//! the user's function. `native_method!` wraps that
...
///
/// At this layer every return is an ABI-identical primitive (or `bool`, or
/// void), so the shim returns the value unchanged.
enum ReturnBridge {
...
fn return_sig_token(ty: &str) -> Result<TokenStream, String> {
    ...
    let token = match ty {
        "()" | "" | "void" => quote! { void },
        // `bool` is the friendly return form; the wrapper type `ReturnBridge`
        // produces is `jboolean`.
        "bool" => quote! { jboolean },
...
mod tests {
    ...
    fn regular_method_emits_inner_impl_and_native_method_const() {
        let s =
            gen_regular(quote! { fn nativeGetValue(env: &mut jni::Env<'_>, clazz: jclass, ptr: jlong) -> jint { 0 } });
    ...
    fn expand_registers_verbatim_and_overridden_names() {
        ...
        assert!(out.contains("name = \"println_native\"") && !out.contains("printlnNative"), "{out}");
