...
ALOGW("In ForkCommon: at fork");
  pid_t pid = fork();
ALOGW("In ForkCommon: pid = %d", (int)pid);
...
static jint com_android_internal_os_Zygote_nativeForkApp(JNIEnv* env,
  ...
  // ??? FIX ME: Split this into an internal version with a purge parameter and
  // the JNI function.
ALOGW("In nativeForkApp");
  std::vector<int> fds_to_close(MakeUsapPipeReadFDVector()),
                   fds_to_ignore(fds_to_close);
...
ALOGW("In nativeForkApp:3");
  return zygote::ForkCommon(env, /* is_system_server= */ false, fds_to_close,
                            fds_to_ignore, is_priority_fork == JNI_TRUE, /*purge=*/true);
}
...
static const JNINativeMethod gMethods[] = {
        ...
        // @CriticalNative
        {"nativeAddUsapTableEntry", "(II)V", (void*)com_android_internal_os_Zygote_nativeAddUsapTableEntry},
