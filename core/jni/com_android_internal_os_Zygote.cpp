...
static pid_t ForkAndSpecializeCommon(JNIEnv* env, uid_t uid, gid_t gid, jintArray javaGids,
  ...
  if (pid == 0) {
    ...
    if (use_native_bridge && dataDir == nullptr) {
      // This should not happen but log a warning just in case (i.e. dataDir should never be null
      // if we need to use a native bridge).
      // In general, dataDir will never be null for normal applications. It can only happen in
      // special cases (for isolated processes which are not associated with any app). These are
      // launched by the framework and should probably not be emulated anyway.
      use_native_bridge = false;
      ALOGW("Native bridge will not be used because of missing data directory.");
    }
