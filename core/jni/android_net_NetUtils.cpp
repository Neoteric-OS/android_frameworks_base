...
static jobject android_net_utils_getDnsNetwork(JNIEnv *env, jobject thiz) {
    const int dnsNetId = getNetworkForDns();
    bool privateDnsBypass = dnsNetId & NETID_USE_LOCAL_NAMESERVERS;

    if (!privateDnsBypass && dnsNetId < 0) {
        throwErrnoException(env, "getDnsNetId", -dnsNetId);
    }

    jclass class_Network = env->FindClass("android/net/Network");
    jmethodID ctor = env->GetMethodID(class_Network, "<init>", "([IZ)V");
