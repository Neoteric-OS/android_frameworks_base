...
int register_com_android_internal_os_ZygoteInit(JNIEnv* env) {
    int stack_cache_max = 128;
    if (int rc = pthread_set_stack_cache_max_count_np(stack_cache_max); rc) {
        ALOGW("pthread_set_stack_cache_max_count_np(%d) failed: %d", stack_cache_max, rc);
    } else {
        ALOGI("pthread_set_stack_cache_max_count_np(%d) succeeded", stack_cache_max);
    }

    if (int rc = pthread_precache_thread_stacks_np(stack_cache_max); rc) {
        ALOGW("pthread_precache_thread_stacks_np(%d) failed: %d", stack_cache_max, rc);
    } else {
        ALOGW("pthread_precache_thread_stacks_np(%d) succeeded", stack_cache_max);
    }
