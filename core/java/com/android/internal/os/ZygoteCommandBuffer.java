...
class ZygoteCommandBuffer {
    ...
    /**
     * Clear the current buffer, read at least the first line of the next command into the buffer,
     * return the argument count from that line. Leave the buffer positioned at the beginning of
     * the second command line, i.e. the first argument. If the buffer has no associated file
     * descriptor, we just reposition to the beginning of the buffer, and reread existing contents.
     * Returns zero if we started out at EOF.
     */
    int clearAndGetCount() throws EOFException {
        try {
            return clearAndRetrieveCount(mNativeBuffer);
        } finally {
    ...
    private static native int clearAndRetrieveCount(long /* NativeCommandBuffer* */ nbuffer)
            throws EOFException;
     ...
     * @return Writable status pipe fd in each child, null in the parent.
     */
    public FileDescriptor forkRepeatedly(int[] managedFdsToClose, int[] managedFdsToIgnore
                              /* more arguments ??? */) {
        try {
            int intResult = forkMany(mNativeBuffer, managedFdsToClose, managedFdsToIgnore);
            if (intResult == -1) {
                return null;
            } else /* child */ {
    ...
    private static native int forkMany(long /* NativeCommandBuffer* */ nbuffer,
                                       int[] managedFdsToClose,
                                       int[] managedFdsToIgnore /*, more??? */);
