...
public final class SystemServerClassLoaderFactory {
     ...
     * `getOrMaybeCreateClassLoader` instead.
     *
    ...
    /* package */ static PathClassLoader createClassLoader(String path, ClassLoader parent) {
        if (sLoadedPaths.containsKey(path)) {
            throw new RuntimeException("A ClassLoader for " + path + " already exists");
        }
     ...
     * @hide for internal use only
     */
    public static PathClassLoader getOrMaybeCreateClassLoader(String path, ClassLoader parent) {
     ...
     */
    private static boolean allowClassLoaderCreation(String path) {
        ...
        // `test_com.android.server.apex` is for testing only and is okay to ignore.
        if (path.startsWith("/apex/test_com.android.server.apex")) {
            return true;
        }
        return false;
    }
