...
class ZygoteArguments {
     ...
     * null if the ZygoteCommandBuffer was positioned at EOF.
     */
    public static ZygoteArguments getInstance(ZygoteCommandBuffer args)
            throws IllegalArgumentException, EOFException {
        int argCount = args.clearAndGetCount();
        // ???? REMOVE ME
        Log.w("Zygote", "Number of arguments is " + argCount);
        if (argCount < 0) {
            throw new AssertionError("Error reading argument count");  // ??? revisit
        }
