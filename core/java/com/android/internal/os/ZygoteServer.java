 ...
 */
class ZygoteServer {
    // TODO (chriswailes): Change this so it is set with Zygote or ZygoteSecondary as appropraite
    ...
    /**
     * Listening socket that accepts new Blastula connections.
     */
    private LocalServerSocket mBlastulaSocket;
    ...
    Runnable runSelectLoop(String abiList) {
        ...
        Log.i(TAG, "Server socket FD: " + mZygoteSocket.getFileDescriptor().getInt$());
        ...
        while (true) {
            ...
            for (int index = 0; index < pollFDs.length; ++index) {
                int fd = pollFDs[index].fd.getInt$();

                try {
                    if (Os.readlink("/proc/self/fd/" + fd).equals("/dev/null")) {
                        if (index == 0) {
                            Log.e(TAG, "Zygote server socket (" + fd + ") mapped to /dev/null");
                        } else if (index < blastulaPoolEventFDIndex) {
                            Log.e(TAG, "Zygote peer socket (" + fd + ") mapped to /dev/null");
                        } else if (index == blastulaPoolEventFDIndex) {
                            Log.e(TAG, "Blastula pool event FD (" + fd + ") mapped to /dev/null");
                        } else {
                            Log.e(TAG, "Blastula reporting pipe (" + fd + ") mapped to /dev/null");
                        }
                    }
                } catch (ErrnoException ex) {
                    Log.e(TAG, "Unable to readlink for FD " + fd + ": " + ex.getMessage());
                }
            }
            ...
            while (--pollIndex >= 0) {
                ...
                Log.i(TAG,
                        String.format("FD %d ready to read at index %d",
                                      pollFDs[pollIndex].fd.getInt$(), pollIndex));
                ...
                if (pollIndex == 0) {
                    ...
                    Log.i(TAG, "New zygote peer FD: " + newPeer.getFileDescriptor().getInt$());
