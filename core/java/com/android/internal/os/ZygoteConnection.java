...
class ZygoteConnection {
    ...
    Runnable processCommand(ZygoteServer zygoteServer, boolean multipleOK) {
        ...
        try (ZygoteCommandBuffer argBuffer = ZygoteCommandBuffer.getInstance(mSocket)) {
            for (;;) {
                try {
                ...
                if (parsedArgs.mInvokeWith != null || parsedArgs.mStartChildZygote) {
                    ...
                    if (peer.getUid() == Process.SYSTEM_UID
                            /* ??? should do this only if zygote jit is done? */) {
                        Log.w("Zygote", "forkChildren");  // ??? REMOVE ME
                        ZygoteHooks.preFork();
                        Object result = Zygote.forkChildren(argBuffer,
                        ...
                        if (result instanceof Runnable) {
                            ...
                            // parent; we finished some number of forks.
                            // We already did the equivalent of handleParentProc().
                            ZygoteHooks.postForkCommon();
                            int r = (Integer) result;
                            if (r == -1) {
                                // argBuffer contains a command not understood by forkChildren.
                                continue;
                            } else {
                                // Couldn't read next command.
                                isEof = true;
                                return null;
                            }
                        }
                    } else {
                        Log.w("Zygote", "forkChild for non-usap");
                        ZygoteHooks.preFork();
                        Object result = Zygote.forkChild(argBuffer, null, fdsToClose,
                                                        /*isPriorityFork=*/ true);
                        if (!(result instanceof Zygote.PidFdPair)) {
                            // child; result is a Runnable.
                            zygoteServer.setForkChild();
                            Zygote.setAppProcessName(parsedArgs, TAG);  // ??? Necessary?
                            return (Runnable) result;
                            ...
                            // parent
                            ZygoteHooks.postForkCommon();
                            Zygote.PidFdPair p = (Zygote.PidFdPair) result;
                            handleParentProc(p.mPid, p.mFd);
                            return null;
                        }
                    }
                }
