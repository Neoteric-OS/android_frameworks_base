...
public final class DisplayCutout {
    ...
    public DisplayCutout getRotated(int startWidth, int startHeight,
        ...
        final int rotationDelta = getRotationToOverride(override, newBounds, defaultRotation);
        if (rotationDelta != ROTATION_0) {
            for (Rect bound : newBounds) {
                if (bound.isEmpty()) continue;
                RotationUtils.rotateCutoutBounds(bound, startWidth, startHeight, rotationDelta);
            }
            Collections.rotate(Arrays.asList(newBounds), rotationDelta);
        }
