...
public final class Display {
     ...
     *         <ul>
     *             <li>If requested from a non-activity context (for example, the application
     *                 context, where the WindowManager is accessed by
     *                 {@code getApplicationContext().getSystemService(Context.WINDOW_SERVICE)}),
     *                 metrics report the size of the entire display based on current rotation,
     *                 subtracting system decoration areas.
     *             <li>If requested from an activity (either using a WindowManager accessed by
     *                 {@code getWindowManager()} or
     *                 {@code getSystemService(Context.WINDOW_SERVICE)}), the resulting metrics
     *                 correspond to the current app window metrics. In multi-window mode, the size
     *                 can be smaller than the physical display size.
     *         </ul>
     * </ol>
     *
