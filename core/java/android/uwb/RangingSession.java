...
public final class RangingSession implements AutoCloseable {
     ...
     * being opened.
     * <p>If the session is already closed, calling {@link #close()} as no effect.
     *
     * {@link Callback#onClosed(int)} will be invoked using the same callback
     * object given to {@link UwbManager#openRangingSession(RangingParams, Executor, Callback)} when
     * the {@link RangingSession} was opened.
     *
     * The session may fail to close if the service is no longer reachable.
     *
     * @throw IOException if the session could not be closed
     */
    @Override
