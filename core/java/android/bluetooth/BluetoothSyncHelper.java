 ...
 * operations.
 * It uses Bluetooth GATT APIs to achieve Broadcast Scan assistance client operations.
 * Application should ensure BASS profile is connected with the given remote device before
 * performing the operations using {@link BleBroadcastAudioScanAssistManager}
 * interface operations
 *
 * <p>BluetoothSyncHelper is a proxy object for controlling the Bluetooth Scan Offloader
 * (BASS client) Service via IPC.
 *
...
public final class BluetoothSyncHelper implements BluetoothProfile {
   ...
   /**
    ...
    /*package*/ BluetoothSyncHelper(Context context, ServiceListener listener) {
        ...
        mAppCallbackWrappers = new IdentityHashMap<BleBroadcastAudioScanAssistCallback,
                                        IBleBroadcastAudioScanAssistCallback>();
        sBleAssistManagers = new IdentityHashMap<BluetoothDevice,
                                        BleBroadcastAudioScanAssistManager>();
        mContext = context;
        mAttributionSource = mBluetoothAdapter.getAttributionSource();
    }

    /**
     * Interface to get Broadcast Audio Scan assistance for LE Audio usecases.
     * This is instantiated per BluetoothDevice which is Scan delegator
     * Application will get an Instance of the  {@link BleBroadcastAudioScanAssistManager} for
     * the given scan delegator device
     *
     * @param BluetoothDevice Scan Delegator device for which BLE Broadcast SCAN Assistance
     * operations will be performed
     * @param {@link #BleBroadcastAudioScanAssistCallback} where callbacks related to BLE
     * Broadcast Scan assistance will be deliverd
     * @hide
     */
    ...
    public BleBroadcastAudioScanAssistManager getBleBroadcastAudioScanAssistManager(
        ...
        if (assistMgr == null) {
            ...
            //object already exists, just registers the callback and return the same object
    ...
    private IBleBroadcastAudioScanAssistCallback wrap(BleBroadcastAudioScanAssistCallback callback,
     ...
     boolean startScanOffload (BluetoothDevice device, boolean isGroupOp) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.startScanOffload(device, isGroupOp, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
     ...
     boolean stopScanOffload (BluetoothDevice device, boolean isGroupOp) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.stopScanOffload(device, isGroupOp, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
     ...
     boolean searchforLeAudioBroadcasters (BluetoothDevice device,
                                AttributionSource aAttributionSource) {
     ...
     boolean selectBroadcastSource (BluetoothDevice device,
                    ScanResult scanRes, boolean isGroupOp, AttributionSource aAttributionSource) {
     ...
     void registerAppCallback(BluetoothDevice device, BleBroadcastAudioScanAssistCallback
                    appCallback) {
     ...
     void unregisterAppCallback(BluetoothDevice device, BleBroadcastAudioScanAssistCallback
            appCallback) {
    ...
    boolean addBroadcastSource (BluetoothDevice sinkDevice,
        ...
        try {
            if (service != null && isEnabled()
                ...
                return service.addBroadcastSource(sinkDevice,
                               srcInfo, isGroupOp, aAttributionSource);
            }
            if (service == null)
            {
                Log.w(TAG, "Proxy not attached to service");
