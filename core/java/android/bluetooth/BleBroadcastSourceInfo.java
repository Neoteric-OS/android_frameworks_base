 ...
 *
 * <p>This is also used to pack all Broadcast source information as part of
 * {@link #ACTION_BROADCAST_RECEIVER_STATE} Intent.
 * User can retrieve the {@link BleBroadcastSourceInfo} using
 * {@link BleBroadcastSourceInfo#EXTRA_RECEIVER_STATE} extra field
 * @hide
 */
public final class BleBroadcastSourceInfo implements Parcelable {
   ...
   /**
     ...
     *
     * <p> Index of the Source Info object can be extracted using this extra field at Application
     * layer
     *
     * This is used to read the {@link BleBroadcastSourceInfo } parcelable object
     * @hide
     */
    public static final String EXTRA_SOURCE_INFO_INDEX =
                                "android.bluetooth.device.extra.SOURCE_INFO_INDEX";
    /**
     * Used as an int extra field in
     * {@link BleBroadcastAudioScanAssistManager#ACTION_BROADCAST_RECEIVER_STATE}
     * intent notifys the Broadcast Source Information to Application layer
     *
     * <p> Maximm number of the Broadcast Source Information that given broadcast receiver can hold,
     * can be extracted using this extra field at Application layer
     *
     * @hide
     */
    public static final String EXTRA_MAX_NUM_SOURCE_INFOS =
                                "android.bluetooth.device.extra.MAX_NUM_SOURCE_INFOS";
    ...
    private @BroadcastAssistEncryptionState int mEncryptionStatus;
    ...
    /**
     * Constructor to create an Empty object of {@link BleBroadcastSourceInfo } with
     * given source Id, which contains, Broadcast receiver state information for
     * Broadcast Assistant Usecases.
     *
    ...
    public BleBroadcastSourceInfo (byte sourceId) {
       ...
       mEncryptionStatus = BROADCAST_ASSIST_ENC_STATE_INVALID;
       mBroadcastCode = null;
     ...
     * Broadcast receiver state information for Broadcast Assistant Usecases.
     * This is mainly used for input purpose of
     * {@link BleBroadcastAudioScanAssistManager#addBroadcastSource} operation
     *
     *  @param audioSource BluetoothDevice object which is selected as Broadcast source
     *  @param advSid advertising Sid of the Broadcast source device for which receiver
     *          synchronized with.
     *  @param addressType type of address. This can be be one of
     *                                  {@link #BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC} or
     *                                  {@link #BROADCAST_ASSIST_ADDRESS_TYPE_RANDOM}
     *  @param metadataSyncState sync status of metadata at the receiver side from this
     *      Broadcast source. This can be one of
     *      {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IDLE},
     *      {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ},
     *      {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC},
     *      {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL} OR
     *      {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_NO_PAST}
     *  @param audioSyncState Audio sync status of metadata at the receiver side from
     *          this broadcast source. This can be one of
     *              {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED} OR
     *              {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED}
     *  @param audioBisIndex Audio BIS index for what Broadcast receiver synchronized with
     *  @param metadataLength Length of the  metadata field
     *  @prama metadata metadata information about the type Broadcast information being
     *      synchronized at receiver side
     *
     *
     *  @hide
     */
    /*package*/ BleBroadcastSourceInfo (BluetoothDevice audioSource,
                            ...
                            List<BleBroadcastSourceChannel> selectedBISIndices
                            ) {
       ...
       if (selectedBISIndices == null) {
           BASS_Debug(TAG, "selectedBISIndicesList is null");
       } else {
           for (int i=0; i<selectedBISIndices.size(); i++) {
             if (selectedBISIndices.get(i).getStatus() == true) {
                 ...
                 BASS_Debug(TAG, "index" + selectedBISIndices.get(i).getIndex() + "is set");
     ...
     * Broadcast receiver state information for Broadcast Assistant Usecases.
     *
     * This is mainly used for output purpose to create an object from the receiver state
     * information read from the remote BASS server. This will be packed and broadcasted as an
     * Intent using {@link #ACTION_BROADCAST_RECEIVER_STATE}
     *
     *  @param audioSource BluetoothDevice object which is selected as Broadcast source
     *  @param sourceId Source Id for this broadcast source info object
     *  @param advSid advertising Sid of the Broadcast source device for which receiver synchronized
     *         with
     *  @param addressType type of address. This can be be one of
     *          {@link #BLE_ASSIST_ADDRESS_TYPE_PUBLIC}
     *          or {@link #BLE_ASSIST_ADDRESS_TYPE_RANDOM}
     *  @param metadataSyncState sync status of metadata at the receiver side from this Broadcast
     *         source.
     *              This can be one of
     *                  {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IDLE},
     *                  {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ},
     ...
     *                  {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL} OR
     *                  {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_NO_PAST}
     *  @param audioSyncState Audio sync status of metadata at the receiver side from this broadcast
     *         source.
     *      This can be one of {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED} OR
     *                        {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED}
     *  @param encryptionStatus Encryotion state at Broadcast receiver.
     *          This can be one of {@link #BROADCAST_ASSIST_ENC_STATE_UNENCRYPTED},
                                {@link #BROADCAST_ASSIST_ENC_STATE_PIN_NEEDED} OR
     *                          {@link #BROADCAST_ASSIST_ENC_STATE_DECRYPTING}
     *  @param audioBisIndex Audio BIS index for what Broadcast receiver synchronized with
     *  @param metadataLength Length of the  metadata field
     *  @prama metadata metadata information about the type Broadcast information being
     *          synchronized at receiver side
    ...
    public BleBroadcastSourceInfo (BluetoothDevice audioSource,
                            ...
                            byte numSubGroups,
                            @BroadcastAssistAudioSyncState int audioSyncstate,
                            Map<Integer, List<BleBroadcastSourceChannel>> selectedBISIndicesList,
       ...
       int audioBisIndex = 0;
       if (selectedBISIndicesList != null) {
           for (Map.Entry<Integer, List<BleBroadcastSourceChannel>> entry :
                        selectedBISIndicesList.entrySet()) {
               List<BleBroadcastSourceChannel> selectedBISIndices = entry.getValue();
                  if (selectedBISIndices == null) {
                   //do nothing
                   BASS_Debug(TAG, "selectedBISIndicesList is null");
                  } else {
                      for (int i=0; i<selectedBISIndices.size(); i++) {
                          if (selectedBISIndices.get(i).getStatus() == true) {
                           audioBisIndex = audioBisIndex |
                                    (1<<selectedBISIndices.get(i).getIndex());
                              BASS_Debug(TAG, "index" +
                                    selectedBISIndices.get(i).getIndex() + "is set");
                          }
                      }
                  }
     ...
     *  @param metadataSyncState sync status of metadata at the receiver side from this Broadcast
     *          source.
     *          This can be one of {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IDLE},
     *                          {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ},
     *                          {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC},
     *                          {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL} OR
     *                          {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_NO_PAST}
     *  @param audioSyncState Audio sync status of metadata at the receiver side from this
     *          broadcast source. This can be one of
     *                      {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED} OR
     *                      {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED}
     *  @param encryptionStatus Encryotion state at Broadcast receiver. This can be one of
     ...
     *  @prama metadata metadata information about the type Broadcast information being
     *          synchronized at receiver side
     *  @param broadcastCode Numeric Character String maximum of 16 characters in length,
     *          which serves as broadcast PIN code
     *
     *  @hide
     */
    ...
    /*package*/ BleBroadcastSourceInfo (BluetoothDevice device,
                                ...
                                byte[] badCode,
                                byte numSubGroups,
                                Map<Integer, Integer> bisIndicesList,
                                Map<Integer, byte[]> metadataList
                                ) {
           mSourceId = sourceId;
           mMetaDataSyncState =  metadataSyncstate;
           ...
           mNumSubGroups = numSubGroups;
           mAudioBisIndexList = new HashMap<Integer, Integer> (bisIndicesList);
    ...
    public boolean isEmptyEntry()  {
        ...
        if (mMetaDataSyncState == (int)BROADCAST_ASSIST_PA_SYNC_STATE_INVALID &&
            ...
            mEncryptionStatus == (int)BROADCAST_ASSIST_ENC_STATE_INVALID
            ) {
                ret = true;
            }
    ...
    public int hashCode() {
        return Objects.hash(mSourceId, mMetaDataSyncState, mAudioSyncState,
                ...
                mEncryptionStatus, mBroadcastCode);
    }
    ...
    public String toString() {
        return "{BleBroadcastSourceInfo : mSourceId" + mSourceId
               ...
               + " mEncryptionStatus" + mEncryptionStatus
    ...
    /**
     * Sets the address type of the Broadcast source advertisement for the BleBroadcastSourceInfo
     *  Object
     *
     * @param byte addressType, this can be one of {@link #BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC} OR
     *          {@link #BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC}
     * @hide
     */
    ...
    /**
     * Gets the address type of the Broadcast source advertisement for the BleBroadcastSourceInfo
     *  Object
     *
     * @return byte addressType, this can be one of {@link #BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC} OR
     *          {@link #BROADCAST_ASSIST_ADDRESS_TYPE_PUBLIC}
     * @hide
     *
     * @deprecated
    ...
    /**
     * Sets the advertising Sid of the Broadcast source advertisement for the BleBroadcastSourceInfo
     *  Object
     *
    ...
    /**
     * Gets the advertising Sid of the Broadcast source advertisement for the BleBroadcastSourceInfo
     *  Object
     *
    ...
    /**
     * Sets the Metadata sync status at the Broadcast receiver side for the BleBroadcastSourceInfo
     * Object
     *
     * @param BroadcastAssistMetadataSyncState representing the state of Meta data sync status.
     *       this can be
     *       one of {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IDLE},
     *              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ},
     *              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC},
     *              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL} OR
     *              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_NO_PAST}
    ...
    /**
     * Gets the Metadata sync status at the Broadcast receiver side from the BleBroadcastSourceInfo
     * Object
     *
     * @return BroadcastAssistMetadataSyncState representing the state of Meta data sync status.
     *           this can be one of {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IDLE},
     *                              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNCINFO_REQ},
     *                              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_IN_SYNC},
     *                              {@link #BROADCAST_ASSIST_PA_SYNC_STATE_SYNC_FAIL} OR
    ...
    /**
     * Sets the Audio sync status at the Broadcast receiver side for the BleBroadcastSourceInfo
     * Object
     *
     * @param BroadcastAssistAudioSyncState representing the state of Meta data sync status.
     *      this can be one of
     *                {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED} OR
     *                {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED}
     *
    ...
    /**
     * Gets the Audio sync status at the Broadcast receiver side from the BleBroadcastSourceInfo
     * Object
     *
     * @return BroadcastAssistAudioSyncState representing the state of Meta data sync status.
     *          this can be one of {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_NOT_SYNCHRONIZED} OR
     *                           {@link #BROADCAST_ASSIST_AUDIO_SYNC_STATE_SYNCHRONIZED}
     * @hide
     */
    ...
    /**
     * Sets the Encryption status at the Broadcast receiver side for the BleBroadcastSourceInfo
     * Object
     *
     * @param BroadcastAssistEncryptionState  representing the state of Meta data sync status.
     *          This can be one of {@link #BROADCAST_ASSIST_ENC_STATE_UNENCRYPTED},
     *                           {@link #BROADCAST_ASSIST_ENC_STATE_PIN_NEEDED},
     *                           {@link #BROADCAST_ASSIST_ENC_STATE_DECRYPTING}
     ...
     * Object
     *
     * @return BroadcastAssistEncryptionState  representing the state of Meta data sync status.
     *          This can be one of {@link #BROADCAST_ASSIST_ENC_STATE_UNENCRYPTED},
     *                           {@link #BROADCAST_ASSIST_ENC_STATE_PIN_NEEDED} ,
     *                           {@link #BROADCAST_ASSIST_ENC_STATE_DECRYPTING}
     *                           Or {@link #BROADCAST_ASSIST_ENC_STATE_BADCODE}
     ...
     *
     *
     * @param int audioBis Index to which receiver need to be synchronized with
     * @hide
     */
    /*package*/ void setBroadcastChannelsSyncStatus(List<BleBroadcastSourceChannel>
                selectedBISIndices) {
        if (selectedBISIndices == null) {
            //set No preference
            BASS_Debug(TAG, "selectedBISIndicesList is null");
     ...
     * This maps the various broadcast source indices and sync status of them
     *
    ...
    public List<BleBroadcastSourceChannel> getBroadcastChannelsSyncStatus () {
        List<BleBroadcastSourceChannel> bcastIndices = new ArrayList<BleBroadcastSourceChannel>();
            ...
            while (bisIndexValue != 0) {
                if ((bisIndexValue&0x01) == 0x01) {
                    BleBroadcastSourceChannel bI =
                        new BleBroadcastSourceChannel(index, String.valueOf(index), true, i,
                                mMetadataList.get(i));
                    bcastIndices.add(bI);
                }
        ...
        BASS_Debug(TAG, "returning Bisindices:" + bcastIndices);
        return bcastIndices;
    }
    ...
    public static final @android.annotation.NonNull Parcelable.Creator<BleBroadcastSourceInfo>
                CREATOR = new Parcelable.Creator<BleBroadcastSourceInfo>() {
                public BleBroadcastSourceInfo createFromParcel(Parcel in) {
                    ...
                    BleBroadcastSourceInfo srcInfo = new BleBroadcastSourceInfo(sourceDevice,
                            sourceId, sourceAdvSid, broadcastId,
                            sourceAddressType, metaDataSyncState,audioSyncState,
                            encryptionStatus, broadcastCode, badBroadcastCode, numSubGroups,
                            bisIndexList, metadataList);
