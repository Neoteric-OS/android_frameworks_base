...
public final class BluetoothAdapter {
    ...
    @SystemApi
    @RequiresPermission(Manifest.permission.BLUETOOTH_ADMIN)
    public boolean connectAllEnabledProfilesWithRemoteDevice(@NonNull BluetoothDevice device) {
        try {
            mServiceLock.readLock().lock();
            if (mService != null) {
                return mService.connectAllEnabledProfilesWithRemoteDevice(device);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "", e);
        } finally {
    ...
    @SystemApi
    @RequiresPermission(Manifest.permission.BLUETOOTH_ADMIN)
    public boolean disconnectAllEnabledProfilesWithRemoteDevice(@NonNull BluetoothDevice device) {
        try {
            mServiceLock.readLock().lock();
            if (mService != null) {
                return mService.disconnectAllEnabledProfilesWithRemoteDevice(device);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "", e);
        } finally {
