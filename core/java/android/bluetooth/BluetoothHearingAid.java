...
public final class BluetoothHearingAid implements BluetoothProfile {
    ...
    public int getVolume(BluetoothDevice device) {
        if (VDBG) {
            log("getVolume(" + device + ")");
        }
        try {
            mServiceLock.readLock().lock();
            if (mService != null && isEnabled()
                    && isValidDevice(device)) {
                return mService.getVolume(device);
            }
            if (mService == null) Log.w(TAG, "Proxy not attached to service");
            return BluetoothProfile.PRIORITY_OFF;
        } catch (RemoteException e) {
     ...
     * @param device Paired bluetooth device
     * @param volume
     * @return true if volume is set, false on error
     * @hide
     */
    @RequiresPermission(Manifest.permission.BLUETOOTH)
    public boolean setVolume(BluetoothDevice device, int volume) {
        if (DBG) {
            log("setVolume(" + device + ", " + volume + ")");
        }
        try {
            mServiceLock.readLock().lock();
            if (mService != null && isEnabled()
                    && isValidDevice(device)) {
                if (volume != BluetoothProfile.PRIORITY_OFF
                        && volume != BluetoothProfile.PRIORITY_ON) {
                    return false;
                }
                return mService.setVolume(device, volume);
            }
            if (mService == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
     ...
     */
    @RequiresPermission(Manifest.permission.BLUETOOTH)
    public byte[] getCustomerId(BluetoothDevice device) {
        if (VDBG) {
        ...
        try {
            ...
            if (mService != null && isEnabled()
                    && isValidDevice(device)) {
                return mService.getCustomerId(device);
            }
            if (mService == null) Log.w(TAG, "Proxy not attached to service");
        } catch (RemoteException e) {
            Log.e(TAG, "Stack:" + Log.getStackTraceString(new Throwable()));
     ...
     *
     * <p> Can be LEFT or RIGHT.
     *
     * @param device Bluetooth device
     * @return side of the device
     * @hide
     */
    @RequiresPermission(Manifest.permission.BLUETOOTH)
