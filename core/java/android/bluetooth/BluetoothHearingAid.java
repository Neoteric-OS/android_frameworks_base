...
public final class BluetoothHearingAid implements BluetoothProfile {
    ...
    public static final int CUSTOMER_ID_INVALID = -1;
     ...
     */
    @RequiresPermission(Manifest.permission.BLUETOOTH)
    public int getClientId(BluetoothDevice device) {
        if (VDBG) {
        ...
        try {
            ...
            if (mService == null) {
                Log.w(TAG, "Proxy not attached to service");
                return CUSTOMER_ID_INVALID;
            }

            if (!isEnabled() || !isValidDevice(device)) return CUSTOMER_ID_INVALID;
