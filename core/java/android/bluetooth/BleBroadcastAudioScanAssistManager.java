...
public final class BleBroadcastAudioScanAssistManager {
    ...
    /**
     * Intent used to broadcast the "Broadcast receiver State" information of a Scan delegator
     * device.
     ...
     * <ul>
     * <li> {@link BluetoothDevice#EXTRA_DEVICE} - The remote device for which broadcast receiver
     ...
     * </ul>
     * <ul>
     * <li> {@link BleBroadcastSourceInfo#EXTRA_SOURCE_INFO_INDEX} - Index of the
     * BleBroadcastSourceInfo object broadcasted </li>
     * </ul>
     * <ul>
     * <li> {@link BleBroadcastSourceInfo#EXTRA_MAX_NUM_SOURCE_INFOS} - Maximum number of
     * source Informations that this Broadcast receiver can hold </li>
     * </ul>
     *
    ...
    /*package*/BleBroadcastAudioScanAssistManager(BluetoothSyncHelper scanOffloader,
     ...
     *
     * @return returns true if It is successfully initiated the Stopped the Search for
     * Audio broadcasters false otherwise
     *
     *@hide
     */
    @RequiresBluetoothLocationPermission
     ...
     * synchronization to the given Broadcast source device, upon acquision of Synchronization
     * information, It will be notified with available Broadcast source channels that can be
     * synchronized in the remote device.
     * Application should select set of Broadcast channels that need to be synchronized and
     * follow up with a call to {@link #addBroadcastSource} operation
     ...
     *
     * If this operation need to be performed over all the members of coordinated set members,
     * isGroupOp will be set to true. Select broadcast source operation will be performed on
     * behalf of all the Coordinated set devices
     *
     *
    ...
    private boolean isValidBroadcastSourceInfo(BleBroadcastSourceInfo srcInfo) {
        ...
        List<BleBroadcastSourceInfo> currentSourceInfos =
            mBluetoothSyncHelper.getAllBroadcastSourceInformation(
                        mBluetoothDevice, mAttributionSource);
    ...
    private void printSelectedIndices(List<BleBroadcastSourceChannel> selectedBISIndices) {
        if (selectedBISIndices == null) {
            log(TAG, "printSelectedIndices : no selected indices");
            return;
        }
        for (int i=0; i<selectedBISIndices.size(); i++) {
            log(TAG, selectedBISIndices.get(i).getDescription() + ": " +
                    selectedBISIndices.get(i).getStatus());
        }
    }
    /**
     * Adds a broadcast source information to the Scan delegator. This internally performs Periodic
     * synchronization to the given Broadcast source device, upon acquision of Synchronization
     * information, It will be written on to the "Scan delegators" Characteristics
     ...
     *
     * Successful addition Broadcast source will be indicated through Broadcast receiver state
     * information update intent through {@link #ACTION_BROADCAST_RECEIVER_STATE} intent
     *
     *
     * If this operation need to be performed over all the members of coordinated set members,
     * isGroupOp will be set to true. add broadcast source operation will be performed on behalf of
     * all the Coordinated set devices
     *
     * Same Broadcast source Information will be written on to all the members of Coordinated set
     * and PAST will be performed based on the request.
     *
     * In case of Group Operation, If there is any matching entry already present in any of
     * coordinated set members, Add Broadcast source opeation will be failed and result will
     * notified through {@link BleBroadcastAudioScanAssistCallback#onBleBroadcastAudioSourceAdded}
     *
     *  @param audioSource {@link #BluetoothDevice} object selected as Source which need to be
     *  synchronized with @param ScanResult {@link #ScanResult} result obtained from the
     *  {@link BleBroadcastAudioScanAssistCallback#onBleBroadcastSourceFound}
     *  @param syncState  can be one of {@link #SYNC_METADATA},
     *  {@link #SYNC_METADATA_AUDIO}
     *  @param selectedBroadcastChannels is a List of Broadcast channels that need to be
     *  synchronized with the given broadcast audio source from Available Broadcast indices.
     *  Available broadcast indices are notified application using
     *  {@link BleBroadcastAudioScanAssistCallback#onBleBroadcastSourceSelected}
     *  BroadcastSourceChannel.mStatus set to be TRUE or FALSE based on the need of synchronization.
     *
     *
     *  null value of selectedBroadcastChannels resulting in syncing to all available Broadcast
     *  channels.
     ...
     */
     ...
     public boolean addBroadcastSource (BluetoothDevice audioSource,
        ...
        if (mBluetoothSyncHelper == null) {
         ...
         printSelectedIndices(selectedBroadcastChannels);
         ...
         //all of these will be overridden at service layer later
     ...
     *
     *  However Successful updating of Broadcast source information will be indicated through
     *  Broadcast receiver state information update intent through
     *  {@link #ACTION_BROADCAST_RECEIVER_STATE} intent
     *
     * If this operation need to be performed over all the members of coordinated set members,
     * isGroupOp will be set to true. Update broadcast source operation will be performed on
     * behalf of all the Coordinated set devices
     *
     * Same Broadcast source Information change will be written on to all the members of
     * Coordinated set and PAST will be performed based on the request from remote.
     *
     * In case of Group Operation, If there are no matching source Information present in any of
     * coordinated set members, Update Broadcast source opeation will be failed and result will
     * notified through
     * {@link BleBroadcastAudioScanAssistCallback#onBleBroadcastAudioSourceUpdated}
     *
    ...
    /**
      ...
      * the synchronized audio at the receiver side
      *
      * It will be written on to the Scan delegator's Characteristics.
      ...
      *
      * If this operation need to be performed over all the members of coordinated set members,
      * isGroupOp will be set to true. set Broadcast PIN operation will be performed on all the
      * Coordinated set devices
      *
      * Same Broadcast PIN code will be written on to all the members of Coordinated set and
      * on the request from remote.
      *
      * In case of Group Operation, If there are no matching source Information(BD address,
      * adv instance)
      ...
      *
      *
      * However, Successful updating of Broadcast PIN code will be indicated through Broadcast
     ...
     *
     * If this operation need to be performed over all the members of coordinated set members,
     * isGroupOp will be set to true. remove broadcast operation will be performed on all the
     * Coordinated set devices
     *
     ...
     *
     * @return returns the List of Broadcast Source Information
     * {@link #BleBroadcastSourceInfo} stored in
