...
public final class BluetoothVolumeControl implements BluetoothProfile, AutoCloseable {
    ...
    /**
     * Intent used to broadcast the current volume state.
     *
     * <p>This intent will have:
     * <ul>
     * <li> {@link BluetoothDevice#EXTRA_DEVICE} - The remote device. It can
     * be null if no device is active. </li>
     * <li> {@link #EXTRA_VC_VOLUME} - The current volume state. </li>
     * <li> {@link #EXTRA_VC_MUTED} - The current mute state. </li>
     * </ul>
     *
     * <p>Requires {@link android.Manifest.permission#BLUETOOTH} permission to
     * receive.
     ...
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    public static final String ACTION_VC_VOLUME_STATE_CHANGED =
            "android.bluetooth.action.VC_VOLUME_STATE_CHANGED";

    public static final String EXTRA_VC_VOLUME =
            "android.bluetooth.extra.VC_VOLUME";
    public static final String EXTRA_VC_MUTED =
            "android.bluetooth.extra.VC_MUTED";

    /**
     * Intent used to broadcast new Volume Control Device availability.
     *
     * <p>This intent will have:
     * <ul>
     * <li> {@link BluetoothDevice#EXTRA_DEVICE} - The remote device. It can
     ...
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    public static final String ACTION_VC_DEVICE_AVAILABLE =
            "android.bluetooth.action.VC_DEVICE_AVAILABLE";

    private BluetoothAdapter mAdapter;
    private final BluetoothProfileConnector<IBluetoothVolumeControl> mProfileConnector =
            new BluetoothProfileConnector(this, BluetoothProfile.VOLUME_CONTROL,
                    TAG, IBluetoothVolumeControl.class.getName()) {
                @Override
    ...
    @Override
    @RequiresPermission(Manifest.permission.BLUETOOTH_PRIVILEGED)
    public @NonNull List<BluetoothDevice> getDevicesMatchingConnectionStates(@NonNull int[] states) {
        if (VDBG) log("getDevicesMatchingConnectionStates()");
        final IBluetoothVolumeControl service = getService();
        try {
            if (service != null && isEnabled()) {
    ...
    /**
     * Tells all the remote devices in the group to set an absolute volume.
     *
     * @param volume Absolute volume to be set on remote
     * @hide
     */
    ...
    public void setVolumeGroup(int groupId, int volume) {
        if (DBG) Log.d(TAG, "setVolumeGroup(vol: " + volume + ", group: " + groupId + " )");
        final IBluetoothVolumeControl service = getService();
        try {
            if (service != null && isEnabled()) {
