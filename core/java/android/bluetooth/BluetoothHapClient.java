...
public final class BluetoothHapClient implements BluetoothProfile, AutoCloseable {
    ...
    /**
     * Intent used to broadcast the change in connection state of the Hearing Acess Profile Client
    ...
    /**
     * Intent used to broadcast the device availability change and the availability of it's
    ...
    private final BluetoothProfileConnector<IBluetoothHapClient> mProfileConnector =
            new BluetoothProfileConnector(this, BluetoothProfile.HAP_CLIENT, "BluetoothHapClient",
                ...
                public IBluetoothHapClient getServiceInterface(IBinder service) {
                    return IBluetoothHapClient.Stub.asInterface(
     ...
     * the API.
     *
     * <p>User is expected to get group identifier for each of the connected
     * device to discover the device grouping. This allows him to make an informed
    ...
    public boolean selectActivePreset(@NonNull BluetoothDevice device, int presetIndex) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.selectActivePreset(device, presetIndex, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean groupSelectActivePreset(int groupId, int presetIndex) {
        ...
        try {
            if (service != null && isEnabled()) {
                // Note: Lower level stack verifies if group actually exists
                return service.groupSelectActivePreset(groupId, presetIndex, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean nextActivePreset(@NonNull BluetoothDevice device) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.nextActivePreset(device, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean groupNextActivePreset(int groupId) {
        ...
        try {
            if (service != null && isEnabled()) {
                // Note: Lower level stack verifies if group actually exists
                return service.groupNextActivePreset(groupId, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean previousActivePreset(@NonNull BluetoothDevice device) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.previousActivePreset(device, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean groupPreviousActivePreset(int groupId) {
        ...
        try {
            if (service != null && isEnabled()) {
                // Note: Lower level stack verifies if group actually exists
                return service.groupPreviousActivePreset(groupId, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean getAllPresetsInfo(@NonNull BluetoothDevice device) {
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.getAllPresetsInfo(device, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean setPresetName(@NonNull BluetoothDevice device, int presetIndex,
        ...
        try {
            if (service != null && isEnabled()
                    && isValidDevice(device)) {
                return service.setPresetName(device, presetIndex, name, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean groupSetPresetName(int groupId, int presetIndex, @NonNull String name) {
        ...
        try {
            if (service != null && isEnabled()) {
                return service.groupSetPresetName(groupId, presetIndex, name, mAttributionSource);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
