  ...
  public void registerSync(ScanResult scanResult, int skip, int timeout,
    ...
    try {
        ...
        callback.onSyncEstablished(0, scanResult.getDevice(), scanResult.getAdvertisingSid(),
                                   skip, timeout,
                                   PeriodicAdvertisingCallback.ADVERTISE_FAILED_INTERNAL_ERROR);
        return;
    }
      ...
      public void onSyncEstablished(int syncHandle, BluetoothDevice device,
          ...
          handler.post(new Runnable() {
              ...
              public void run() {
                  ...
                  if (status != PeriodicAdvertisingCallback.ADVERTISE_SUCCESS) {
                      // App can still unregister the sync until notified it failed. Remove callback after app was notifed.
      ...
      public void onSyncLost(int syncHandle) {
          handler.post(new Runnable() {
              ...
              public void run() {
                ...
                // App can still unregister the sync until notified it's lost. Remove callback after app was notifed.
                callbackWrappers.remove(callback);
              }
          });
      }
