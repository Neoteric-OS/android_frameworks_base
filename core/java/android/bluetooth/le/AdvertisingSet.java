...
public final class AdvertisingSet {
  ...
  /* package */ AdvertisingSet(IBluetoothGatt gatt,
    ...
    this.gatt = gatt;
    callbackWrapper = new AdvertisingSetCallbackWrapper(callback);

    try {
      // TODO(jpawlowski): periodicData and advertiseData should make it
      // possible to divide data
      // that should go to each packet, and order of data in packet
      gatt.registerAdvertisingSet(parameters, advertiseData, scanResponse,
                                  periodicParameters, periodicData,
                                  callbackWrapper);
