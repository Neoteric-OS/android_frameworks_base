...
public class WebSettings {
    ...
    /**
     * @deprecated Please use {@link #setLowBandwidthDisplayMode} instead. They are identical.
     */
    @Deprecated
    public synchronized void setUseDoubleTree(boolean use) {
        setLowBandwidthDisplayMode(use);
    }

    /**
     * @deprecated Please use {@link #getLowBandwidthDisplayMode} instead. They are identical.
     */
    @Deprecated
    ...
    /**
     * This is an optimization for high latency networks - enable
     * it if you will be displaying HTML that loads external
     * JavaScript or CSS files and can tolerate visible relayout.
     *
     * <p>Normally rendering engines like WebKit will block when they
     * encounter external references to JavaScript or CSS files. This
     * is because these files can alter what is displayed on the
     * page. On high latency connections such as typical 3G networks,
     * this causes noticable lag in page display and some users can
     * wrongly believe the page has stopped loading.</p>
     *
     * <p>Low bandwidth display mode makes WebKit do two rendering
     * passes.  In the first pass, rendering is done in parallel to
     * loading external JavaScript and CSS, so the results might be
     * wrong (but might not, depending on what the external files do).
     * Then when loading is complete layout is performed again, taking
     * the external files into account. The upside is that the user
     * sees an approximation of the web page faster. The downside is that
     * depending on the page being loaded, the result might be quite
     * different to the actual correct rendering.</p>
     *
     */
    public synchronized void setLowBandwidthDisplayMode(boolean use) {
    ...
    /**
     * @return True if low bandwidth display mode is active. See
     * {@link #setLowBandwidthDisplayMode} for an explanation of what
     * this does. The default is false.
     */
    public synchronized boolean getLowBandwidthDisplayMode() {
        return mUseDoubleTree;
    }
