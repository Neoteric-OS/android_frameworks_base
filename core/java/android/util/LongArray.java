...
public class LongArray implements Cloneable {
    ...
    /**
     * Changes the size of this LongArray. If the new size is smaller than the current size,
     * the unused section of the current backing array is filled with 0s. If the new size is larger
     * than backing array capacity, a new backing array is created from the current content of this
     * LongArray padded with 0s.
     */
    public void resize(int newSize) {
        // TODO throw on negative Size
        int oldSize = mSize;
        mSize = newSize;
        ensureCapacity(mSize);
        if (newSize < oldSize) {
            Arrays.fill(mValues, newSize, oldSize, 0);
        }
    }
