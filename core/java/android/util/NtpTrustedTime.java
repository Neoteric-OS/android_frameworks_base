...
public class NtpTrustedTime implements TrustedTime {
    ...
    public boolean forceRefresh() {
        synchronized (this) {
            ...
            if (client.requestTime(serverName, port, timeoutMillis, network)) {
                long ntpUncertainty = client.getRoundTripTime() / 2;
                mTimeResult = new TimeResult(
                        client.getNtpTime(), client.getNtpTimeReference(),
                        saturatedCast(ntpUncertainty));
                return true;
            } else {
                return false;
            }
    ...
    private int saturatedCast(long longValue) {
