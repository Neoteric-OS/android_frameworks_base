...
public final class VpnProfileState implements Parcelable {
     ...
     *
     * <p>This state represents the internal connection state of the VPN.
     * This state may diverge from the VPN Network's state during error and
     * recovery handling.
     */
     ...
     *
     * <p>The session key is an ephemeral key uniquely identifying the
     * session for a Platform VPN. The lifetime of this key is tied to the
     * lifetime of the VPN session. In other words, reprovisioning of the
     * VPN profile, restarting of the device, or manually restarting the
     * Platform VPN session will result in a new VPN session, and a new
     * key.
     *
     * @return the unique key for the platform VPN session, or null if it
     *         is not running.
     */
    @Nullable
     ...
     *
     * <p>If the PlatformVpnProfile is set to be running in always-on mode,
     * the system will ensure that the profile is always started, and
     * restarting it when necessary (e.g. after reboot).
     *
     * <p>Always-on can be set by an appropriately privileged user via the
     * Settings VPN menus, or by the Device Policy Manager app
     * programmatically.
     *
     ...
     *
     * <p>In lockdown mode, the system will ensure that apps are not
     * allowed to bypass the VPN, including during startup or failure of
     * the VPN.
     *
     * <p>Lockdown mode can be set by an appropriately privileged user via
     * the Settings VPN menus, or by the Device Policy Manager app
     * programmatically.
     *
     * {@see DevicePolicyManager#setAlwaysOnVpnPackage(ComponentName, String, boolean, Set)}
     */
