...
public class RoughtimeClient
{
    ...
    private Message createRequestMessage() {
        ...
        mRandom.nextBytes(nonce); // TODO: Chain nonces
    ...
    public boolean requestTime(InetAddress address, int port, byte[] key,
        ...
        if ((rightNow - mLastRequest) > timeout) {
            mResults.clear();
        }

        mLastRequest = rightNow;
    ...
    private boolean verifyNonce(byte[] root, byte[] path, byte[] nonce,
            int index) {
        mMessageDigest.update(new byte[]{ 0 });
        byte[] hash = mMessageDigest.digest(nonce);
        int pos = 0;
        byte[] one = new byte[]{ 1 };

        while (pos < path.length) {
            mMessageDigest.update(one);

            if ((index&1) != 0) {
                mMessageDigest.update(path, pos, 64);
                hash = mMessageDigest.digest(hash);
            } else {
                mMessageDigest.update(hash);
                mMessageDigest.update(path, pos, 64);
                hash = mMessageDigest.digest();
            }
