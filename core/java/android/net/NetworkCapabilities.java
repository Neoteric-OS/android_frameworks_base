...
public final class NetworkCapabilities implements Parcelable {
     ...
     * Test whether this UID is allowed on this network.
     *
     * This method is equivalent to allowsUid and provided for consistency of naming
     * across the NetworkCapabilities class.
     * nc is assumed nonnull.
     * @see #allowsUid
     * @hide
     */
    public boolean satisfiedByAllowedUids(NetworkCapabilities nc) {
        if (null == mAllowedUids) return true; // Everything is allowed
        if (null == nc.mAllowedUids) return false; // Not everything allowed but requires everything
        for (UidRange requiredRange : nc.mAllowedUids) {
            if (!allowsUidRange(requiredRange)) {
                return false;
            }
        }
        return true;
