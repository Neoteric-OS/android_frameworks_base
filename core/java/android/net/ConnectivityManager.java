...
public class ConnectivityManager {
    ...
    // Global singleton map used by all instances of ConnectivityManager for storying all
    // NetworkCallbacks registered by a process. Using global state for tracking all requests and
    // listens allows to support registering and unregistering a callback from different instances
    // of ConnectivityManager.
    ...
    public ConnectivityManager(Context context, IConnectivityManager service,
        ...
        mService = Preconditions.checkNotNull(service, "missing IConnectivityManager service");
        mCallbacks = Preconditions.checkNotNull(callbacks, "missing map of registered callbacks");
    ...
    private class CallbackHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            final NetworkCallback callback = getCallback(request);
            if (DBG) {
        ...
        private NetworkCallback getCallback(NetworkRequest request) {
            synchronized (mCallbacks) {
                return mCallbacks.get(request);
            }
        }
