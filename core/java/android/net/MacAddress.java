...
public final class MacAddress implements Parcelable {
    ...
    private static final long LOCALLY_ASSIGNED_MASK = longAddrFromByteAddr(addr(2, 0, 0, 0, 0, 0));
    private static final long MULTICAST_MASK = longAddrFromByteAddr(addr(1, 0, 0, 0, 0, 0));

    // Internal representation of the mac address as a single 8 bytes long.
    // The encoding scheme sets the two most significant bytes to 0. The 6 bytes of the
    // mac address are encoded in the 6 least significant bytes of the long, in big endian
    // order.
    ...
    public static long longAddrFromStringAddr(String addr) {
        ...
        int index = ETHER_ADDR_LEN;
        while (index-- > 0) {
            longAddr = Integer.valueOf(parts[index], 16) + (longAddr << 8);
        }
        return longAddr;
    }
    ...
    /** Returns a randomely generated mac address. */
    public static MacAddress getRandomAddress() {
        return getRandomAddress(new Random());
    }

    /** Returns a randomely generated mac address using the given Random object. */
    public static MacAddress getRandomAddress(Random r) {
        return new MacAddress(VALID_LONG_MASK & r.nextLong());
    }
