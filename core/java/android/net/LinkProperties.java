...
public final class LinkProperties implements Parcelable {
    ...
    public LinkProperties(LinkProperties source) {
        if (source != null) {
            ...
            mLinkAddresses.addAll(source.getLinkAddresses());
            mDnses.addAll(source.getDnsServers());
            mValidatedPrivateDnses.addAll(source.getValidatedPrivateDnsServers());
            mUsePrivateDns = source.mUsePrivateDns;
            mPrivateDnsServerName = source.mPrivateDnsServerName;
            mDomains = source.getDomains();
            mRoutes.addAll(source.getRoutes());
    ...
    public String toString() {
        ...
        final String linkAddresses = "LinkAddresses: ["
                + TextUtils.join(",", mLinkAddresses)
                + "] ";

        final String dns = "DnsAddresses: ["
                + TextUtils.join(",", mDnses)
                + "] ";

        final String usePrivateDns = "UsePrivateDns: " + mUsePrivateDns + " ";

        final String privateDnsServerName = mPrivateDnsServerName == null ? ""
                : "PrivateDnsServerName: " + mPrivateDnsServerName + " ";

        final String validatedPrivateDns = mValidatedPrivateDnses.isEmpty() ? ""
                : "ValidatedPrivateDnsAddresses: ["
                        + mValidatedPrivateDnses.stream().map(InetAddress::getHostAddress)
                                .collect(Collectors.joining(","))
                        + "] ";

        final String domainName = "Domains: " + mDomains;

        final String mtu = " MTU: " + mMtu;

        final String tcpBuffSizes = mTcpBufferSizes == null ? ""
                : " TcpBufferSizes: " + mTcpBufferSizes;

        final String routes = " Routes: ["
                + TextUtils.join(",", mRoutes)
                + "] ";

        final String proxy = mHttpProxy == null ? ""
                : " HttpProxy: " + mHttpProxy + " ";

        final String stacked = mStackedLinks.values().size() == 0 ? ""
                : " Stacked: ["
                + mStackedLinks.values().stream().map(link -> "[ " + link + " ]")
                        .collect(Collectors.joining(","))
                + "] ";
