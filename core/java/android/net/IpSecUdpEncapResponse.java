...
public final class IpSecUdpEncapResponse implements Parcelable {
    ...
    // There is a weird assymetry with FileDescriptor: you can write a FileDescriptor
    ...
    public IpSecUdpEncapResponse(int inStatus, int inResourceId, int inPort, FileDescriptor inFd)
        ...
        if (inStatus == IpSecManager.Status.OK && inFd == null) {
            throw new IllegalArgumentException("Valid status implied FD must be nonnull");
        }
        status = inStatus;
        resourceId = inResourceId;
        port = inPort;
        if (status == IpSecManager.Status.OK) {
            fileDescriptor = ParcelFileDescriptor.dup(inFd);
        } else {
            fileDescriptor = null;
        }
    }
