...
public final class IpSecManager {
    ...
    /**
     * Indicates that the requested system resource for IPsec, such as a socket or an SPI value is
     * unavailable.
     */
    public final class ResourceUnavailableException extends AndroidException {
    ...
    public static final class SecurityParameterIndex implements AutoCloseable {
        ...
        private SecurityParameterIndex(Context context, InetAddress remoteAddress, int spi)
         ...
         *
         * @param remoteAddr SPIs must be unique for each combination of SPI and remote address.
         *     Thus, the remoteAddress to which the SPI will communicate must be supplied.
     ...
     */
    public SecurityParameterIndex reserveOutboundSpi(InetAddress remoteAddr, int requestedSpi)
            throws ResourceUnavailableException {
        return new SecurityParameterIndex(mContext, remoteAddr, requestedSpi);
    }
     ...
     *     method will bind to the specified port or fail. If zero is provided, then the system will
     *     select an arbitrary port and bind to it. To retrieve the port number, call {@link
     *     android.system.Os#getsockname(FileDescriptor)}.
     * @return a {@link UdpEncapsulationSocket} that is bound (irrevocably) to the requested port
     */
     ...
     * that port.
     *
     * <p>is provided, then this method will bind to the specified port or fail. If zero is
     * provided, then the system will select an arbitrary port and bind to it. To retrieve the port
     * number, call {@link android.system.Os#getsockname(FileDescriptor)}.
     *
     * @return a {@link UdpEncapsulationSocket} that is bound (irrevocably) to the arbitrarily
     *     selected port
     */
    // Returning a socket in this fashion that has been created and bound by the system
    // is the only safe way to ensure that a socket is both accessible to the user and
