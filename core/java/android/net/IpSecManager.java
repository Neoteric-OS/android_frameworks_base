...
public class IpSecManager {
    ...
    public void addTransportModeTransform(IpSecTransform transform)
            throws TransformCollisionException, ErrnoException {
        int transformId;
        try {
            transformId =
                    mService.addTransportModeTransform(transform.getConfig(), new Binder());
            if (transformId >= 0) {
                transform.setTransformId(transformId);
            } else {
                throw new ErrnoException("addTransportModeTransform", -transformId);
            }
    ...
    public void addTransportModeTransform(IpSecTransform transform, int nattKeepalivePeriod)
            throws TransformCollisionException {}
     ...
     * @param transform a transport mode IpSecTransform object
     */
    public void removeTransform(IpSecTransform transform) {
        int transformId;
        synchronized (transform) {
            transformId = transform.getTransformId();
            Log.d(TAG, "Transform Id for this transform is " + transformId);
