 ...
 *
 * <p>Conversion to prefixes are deterministic, and will always return the same set of IP Prefixes.
 * IpPrefix instances will returned sorted based on {@link IpPrefix#lengthComparator()}.
 *
 * @hide
 */
public final class IpRange {
    ...
    public IpRange(@NonNull InetAddress startAddr, int prefixLen) {
        ...
        for (int bitIndex = prefixLen; bitIndex < 8 * endAddr.length; ++bitIndex) {
            endAddr[bitIndex / 8] |= (byte) (0x80 >> (bitIndex % 8));
        }
    }
    ...
    public List<IpPrefix> asIpPrefixes() throws UnknownHostException {
        final boolean isIpv6 = mStartAddr instanceof Inet6Address;
    ...
    public List<IpPrefix> getSubsetPrefixes(IpPrefix prefix) throws UnknownHostException {
        ...
        other[newPrefixLen / 8] = other[newPrefixLen / 8] ^ (0x80 >> newPrefixLen % 8);
