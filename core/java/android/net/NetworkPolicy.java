...
public class NetworkPolicy implements Parcelable, Comparable<NetworkPolicy> {
    ...
    /**
     * Version of the NetworkTemplate backup serializer that removes the subscriber Id match
     * rule.
     */
    private static final int TEMPLATE_BACKUP_VERSION_3_REMOVE_SUBSCRIBER_ID_MATCH_RULE = 3;
    /**
     * Latest Version of the NetworkTemplate Backup Serializer.
     */
    private static final int TEMPLATE_BACKUP_VERSION_LATEST =
            TEMPLATE_BACKUP_VERSION_3_REMOVE_SUBSCRIBER_ID_MATCH_RULE;
    ...
    private byte[] getNetworkTemplateBytesForBackup() throws IOException {
        ...
        out.writeInt(subscriberIdMatchRule);
    ...
    private static NetworkTemplate getNetworkTemplateFromBackup(DataInputStream in)
        ...
        if (version < TEMPLATE_BACKUP_VERSION_1_INIT || version > TEMPLATE_BACKUP_VERSION_LATEST
                || version == TEMPLATE_BACKUP_VERSION_2_SUPPORT_CARRIER_TEMPLATE) {
            // Version 2 of NetworkTemplate exposed separate subscriber match rules
            // which were persisted in backups, but not yet released to the public.
            // This was confusing and was removed in favor of using the list of subscriber IDs.
            // Discard it with existing exception that will be handled by the SettingsBackupAgent.
        ...
        final int metered;
        if (version >= TEMPLATE_BACKUP_VERSION_3_REMOVE_SUBSCRIBER_ID_MATCH_RULE) {
            metered = in.readInt();
        } else {
