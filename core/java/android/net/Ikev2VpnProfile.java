...
public final class Ikev2VpnProfile extends PlatformVpnProfile {
    ...
    @Nullable private final String mSessionName;
    private final int mType; // Defaults in builder
    ...
    private Ikev2VpnProfile(
        ...
        mAllowedAlgorithms = Collections.unmodifiableList(allowedAlgorithms);
    ...
    private void validate() {
        ...
        // Server Address not validated here beyond that it was provided to allow for hostnames
        // that are only reachable on certain networks.
        ...
        switch (mType) {
            case TYPE_IKEV2_IPSEC_USER_PASS:
                ...
                checkNotNull(mServerRootCaCert, MISSING_PARAM_MSG_TMPL, "Root CA cert");
                break;
    ...
    /** Retrieves the Session Name, if any was set, or null otherwise. */
    @Nullable
    ...
    /** Retrieves the maximum MTU set for this VPN profile, or the default if it was not set. */
    ...
    public int hashCode() {
        return Objects.hash(
                mSessionName, mType, mServerAddr, mUserIdentity, Arrays.hashCode(mPresharedKey),
                mServerRootCaCert, mUsername, mPassword, mRsaPrivateKey, mUserCert, mProxyInfo,
                mAllowedAlgorithms, mIsBypassable, mIsMetered, mMaxMtu);
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(mSessionName, other.mSessionName)
                && mType == other.mType
    ...
    public VpnProfile toVpnProfile() throws IOException, GeneralSecurityException {
        final VpnProfile profile = new VpnProfile("" /* Key unused */);
        profile.name = mSessionName;
        profile.type = mType;
        profile.server = mServerAddr;
        profile.ipsecIdentifier = mUserIdentity;
        profile.proxy = mProxyInfo;
        profile.allowedAlgorithms = mAllowedAlgorithms;
        ...
        switch (mType) {
            case TYPE_IKEV2_IPSEC_USER_PASS:
                ...
                profile.ipsecCaCert = certificateToPemString(mServerRootCaCert);
                break;
            case TYPE_IKEV2_IPSEC_PSK:
    ...
    public static Ikev2VpnProfile fromVpnProfile(@NonNull VpnProfile profile)
        ...
        builder.setSessionName(profile.name);
        builder.setProxy(profile.proxy);
        builder.setAllowedAlgorithms(profile.allowedAlgorithms);
        ...
        switch (profile.type) {
            ...
            case TYPE_IKEV2_IPSEC_RSA:
                ...
                PrivateKey key = getPrivateKey(profile.ipsecSecret);
    ...
    @NonNull
    @VisibleForTesting
    public static String certificateToPemString(@NonNull X509Certificate cert)
    ...
    @Nullable
    private static X509Certificate certificateFromPemString(@NonNull String certStr)
    ...
    public static String encodeForIpsecSecret(@NonNull byte[] secret) {
        checkNotNull(secret, "Required parameter was null: secret");
    ...
    private static byte[] decodeFromIpsecSecret(@NonNull String encoded) {
        checkNotNull(encoded, "Required parameter was null: encoded");
    ...
    public static final class Builder {
        @Nullable private String mSessionName;
        private int mType = -1;
        @NonNull private String mServerAddr;
        @NonNull private String mUserIdentity;
         ...
         *
         * <p>The session name will be displayed in system-managed dialogs and notifications. This
         ...
         */
        @NonNull
        public Builder setSessionName(@NonNull String name) {
        ...
        public Builder setAuthPsk(@NonNull byte[] psk) {
            checkNotNull(psk, "Required parameter was null: psk");
         ...
         * such as {@link ConnectivityManager#bindProcessToNetwork} to instead send/receive directly
         * over the underlying network or any other network they have permissions for.
         *
         ...
         * @return this {@link Builder} object to facilitate chaining of method calls.
         * @throws IllegalArgumentException if the value is not at least greater than the minimum
         *     IPv6 MTU.
         */
        @NonNull
         ...
         */
        @NonNull
        public Ikev2VpnProfile build() throws GeneralSecurityException {
