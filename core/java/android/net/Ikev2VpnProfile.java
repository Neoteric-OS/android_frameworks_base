...
public final class Ikev2VpnProfile extends PlatformVpnProfile {
    /** Prefix for when Private Key is an alias to look for in KeyStore @hide */
    public static final String KEYSTORE_ALIAS_PREFIX = "KEYSTORE_ALIAS:";
    ...
    public static Ikev2VpnProfile fromVpnProfile(
        ...
        switch (profile.type) {
            ...
            case TYPE_IKEV2_IPSEC_RSA:
                ...
                if (profile.ipsecSecret.startsWith(KEYSTORE_ALIAS_PREFIX)) {
                    Objects.requireNonNull(keyStore, "Missing Keystore for aliased PrivateKey");

                    final String alias =
                            profile.ipsecSecret.substring(KEYSTORE_ALIAS_PREFIX.length());
