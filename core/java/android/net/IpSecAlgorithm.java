...
public final class IpSecAlgorithm implements Parcelable {
     ...
     *
     * <p>This algorithm will be available on devices first-released with Android 12 and may be
     * available on devices launching prior. Callers MUST check if it is supported before using it.
     * {@link IllegalArgumentException} will be thrown if this algorithm is requested on an
     * unsupported device.
     *
    ...
    private static final Map<String, Integer> ALGO_TO_REQUIRED_FIRST_SDK = new HashMap<>();
    ...
    static {
        ...
        // STOPSHIP: Use Build.VERSION_CODES.S when it is defined
    ...
    public static @NonNull List<String> getSupportedAlgorithms() {
        final List<String> algoList = new ArrayList<>();

        for (String algo : ALGO_TO_REQUIRED_FIRST_SDK.keySet()) {
            if (Build.VERSION.FIRST_SDK_INT >= ALGO_TO_REQUIRED_FIRST_SDK.get(algo)) {
                algoList.add(algo);
            }
        }
    ...
    public static Set<String> loadOptionalAlgos(Resources systemResources) {
        ...
        final Set<String> optionalAlgoAllowSet = new HashSet<>();
        for (String algo : ALGO_TO_REQUIRED_FIRST_SDK.keySet()) {
            if (Build.VERSION.FIRST_SDK_INT < ALGO_TO_REQUIRED_FIRST_SDK.get(algo)) {
                optionalAlgoAllowSet.add(algo);
            }
        }

        // Validate resource.
        final Set<String> enabledOptionalAlgoSet = new HashSet<>();
