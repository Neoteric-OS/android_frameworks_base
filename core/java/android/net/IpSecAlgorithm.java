 ...
 * an IpSec Transform. Refer to RFC 4301.
 * @hide
 */
public class IpSecAlgorithm implements Parcelable {

    /** Refer to the ip-xfrm man pages for support-able key types;
     * however, only keys specified here will be supported.
     */
    ...
    private static final Set<String> ALL_ALGOS = new HashSet<String>(
        Arrays.asList(ALGO_AES_CBC, ALGO_HMAC_MD5, ALGO_HMAC_SHA1));
    ...
    public IpSecAlgorithm(String algorithm, byte[] key, int truncLenBits) {
        if(!ALL_ALGOS.contains(algorithm)) {
            throw new IllegalArgumentException("Unknown Algorithm "
                    + algorithm + " passed to IpSecAlgorithm");
        }
