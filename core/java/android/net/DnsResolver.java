...
public final class DnsResolver {
    ...
    /**
     * Base class for answer parsing.
     *
     * Users can provide concrete implementations of this class should override
     * {@link #parseAnswer} to parse raw DNS answers and convert them into the
     * desired type, then call {@link #onAnswer} with the converted types if the
     * conversion succeeded or {@link #onError} if the conversion failed.
     *
    ...
    public abstract static class AnswerCallback<T> {
         ...
         * to convert the raw response to <T> answer.
         * Then call onAnswer if the conversion succeeded, onError if it failed.
         *
         * @param result the raw answer to be converted
         */
        public abstract void onRawAnswer(@NonNull byte [] result);
         ...
         *
         * @param answer parsed answer of query.
         */
         ...
         * {@link android.net.DnsResolver#query query()}
         *
         * The system will call onError if an error happens when
         * issuing the DNS query or receiving the result.
         *
         * @param exception a {@link Exception} object with additional detail regarding the failure
         */
    ...
    public abstract static class RawAnswerCallback extends AnswerCallback<byte[]> {
        @Override
        public final void onRawAnswer(@NonNull byte[] result) {
            onAnswer(result);
        }
    }
    ...
    /**
     * Pass in a blob and corresponding flags, and poll the {@code FD_EVENTS}
     * on main looper. Get an answer back asynchronously through {@link AnswerCallback}.
     *
                    ...
                    executor.execute(
                            () -> {
                                ...
                                try {
                                    ...
                                    answerCallback.onError(e);
                                    return;
                                }

                                answerCallback.onRawAnswer(answerbuf);
                        });
