...
public final class VcnCellUnderlyingNetworkTemplate extends VcnUnderlyingNetworkTemplate {
    ...
    private static void validateCapabilitiesMatchCriteria(
        ...
        boolean requiredCapabilityeFound = false;
        for (Map.Entry<Integer, Integer> entry : capabilitiesMatchCriteria.entrySet()) {
            ...
            requiredCapabilityeFound |= (matchCriteria == MATCH_REQUIRED);
        }

        if (!requiredCapabilityeFound) {
            throw new IllegalArgumentException("Missing required capabilities");
        }
    }
    ...
    public static final class Builder {
        ...
        /**
         * Set the matching criteria for CBS networks.
         *
         * <p>A template where setCbs(MATCH_REQUIRED) will only match CBS networks (one with
         * NET_CAPABILITY_CBS). A template where setCbs(MATCH_FORBIDDEN) will only match a network
         * that does not support CBS (one without NET_CAPABILITY_CBS).
         *
        ...
        /**
         * Set the matching criteria for DUN networks.
         *
         * <p>A template where setDun(MATCH_REQUIRED) will only match DUN networks (one with
         * NET_CAPABILITY_DUN). A template where setDun(MATCH_FORBIDDEN) will only match a network
         * that does not support DUN (one without NET_CAPABILITY_DUN).
         *
         * @param matchCriteria the matching criteria for DUN networks. Defaults to {@link
         *     #MATCH_ANY}.
        ...
        /**
         * Set the matching criteria for IMS networks.
         *
         * <p>A template where setIms(MATCH_REQUIRED) will only match IMS networks (one with
         * NET_CAPABILITY_IMS). A template where setIms(MATCH_FORBIDDEN) will only match a network
         * that does not support IMS (one without NET_CAPABILITY_IMS).
         *
        ...
        /**
         * Set the matching criteria for INTERNET networks.
         *
         * <p>A template where setInternet(MATCH_REQUIRED) will only match INTERNET networks (one
         * with NET_CAPABILITY_INTERNET). A template where setInternet(MATCH_FORBIDDEN) will only
         * match a network that does not support INTERNET (one without NET_CAPABILITY_INTERNET).
         *
        ...
        /**
         * Set the matching criteria for MMS networks.
         *
         * <p>A template where setMms(MATCH_REQUIRED) will only match MMS networks (one with
         * NET_CAPABILITY_MMS). A template where setMms(MATCH_FORBIDDEN) will only match a network
         * that does not support MMS (one without NET_CAPABILITY_MMS).
         *
        ...
        /**
         * Set the matching criteria for RCS networks.
         *
         * <p>A template where setRcs(MATCH_REQUIRED) will only match RCS networks (one with
         * NET_CAPABILITY_RCS). A template where setRcs(MATCH_FORBIDDEN) will only match a network
         * that does not support RCS (one without NET_CAPABILITY_RCS).
         *
