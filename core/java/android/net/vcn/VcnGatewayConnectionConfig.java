 ...
 *
 * <p>A VCN connection based on this configuration will be brought up dynamically based on
 * NetworkRequests filed by apps, or as the system-wide default network switches.
 *
 * <p>Each gateway connection will expose a {@link Network} with the given Telephony-related
 * capabilities (and NET_CAPABILITY_INTERNET; see {@link Builder.addTunnelCapability}), and will
 * select the underlying network based on the required capabilities set in the configuration (see
 * {@link Builder.addRequiredUnderlyingCapability}). Allowed capabilities (as exposed by the VCN
