...
public class CameraDeviceImpl extends CameraDevice
    ...
    private boolean mRepeatingRequestIsZslStill = false;
     ...
     * @param isZslStill whether the lastFrameNumber is a Zsl still capture request.
     */
    private void checkEarlyTriggerSequenceComplete(
            final int requestId, final long lastFrameNumber,
            final boolean isZslStill) {
            ...
            if (repeating) {
                if (mRepeatingRequestId != REQUEST_ID_NONE) {
                    checkEarlyTriggerSequenceComplete(mRepeatingRequestId,
                            ...
                            mRepeatingRequestIsZslStill);
                }
                mRepeatingRequestId = requestInfo.getRequestId();
                int lastRequestType = requestArray[requestArray.length - 1].getRequestType();
                mRepeatingRequestIsZslStill =
                        lastRequestType == CaptureRequest.REQUEST_TYPE_ZSL_STILL;
            } else {
    ...
    public void stopRepeating() throws CameraAccessException {
        ...
        synchronized(mInterfaceLock) {
            ...
            if (mRepeatingRequestId != REQUEST_ID_NONE) {
                ...
                boolean isZslStill = mRepeatingRequestIsZslStill;
                mRepeatingRequestIsZslStill = false;
                ...
                checkEarlyTriggerSequenceComplete(requestId, lastFrameNumber, isZslStill);
            }
        }
    }
    ...
    static class RequestLastFrameNumbersHolder {
         ...
         * frame number.
         */
        public RequestLastFrameNumbersHolder(int requestId, long lastFrameNumber,
                boolean isZslStill) {
            if (isZslStill) {
                mLastRegularFrameNumber = CaptureCallback.NO_FRAMES_CAPTURED;
                mLastZslStillFrameNumber = lastFrameNumber;
            } else {
                mLastRegularFrameNumber = lastFrameNumber;
                mLastZslStillFrameNumber = CaptureCallback.NO_FRAMES_CAPTURED;
            }
    ...
    public class FrameNumberTracker {
        ...
        private void updateCompletedFrameNumber(long frameNumber,
            ...
            int type1 = (requestType + 1) % CaptureRequest.REQUEST_TYPE_COUNT;
            int type2 = (requestType + 2) % CaptureRequest.REQUEST_TYPE_COUNT;
            long maxFrameNumberSeen =
                    Math.max(mCompletedFrameNumber[type1], mCompletedFrameNumber[type2]);
            if (frameNumber < maxFrameNumberSeen) {
                ...
                if (!mSkippedFrameNumbers[requestType].isEmpty()) {
                    // frame number must be head of mSkippedRegularFrameNumbers if
                    // mSkippedRegularFrameNumbers isn't empty.
                    ...
                    int index1 = mSkippedOtherFrameNumbers[type1].indexOf(frameNumber);
                    int index2 = mSkippedOtherFrameNumbers[type2].indexOf(frameNumber);
                    ...
                    int index;
                    if (inSkippedOther1) {
                        srcList = mSkippedOtherFrameNumbers[type1];
                        dstList = mSkippedFrameNumbers[type2];
                        index = index1;
                    } else {
                        srcList = mSkippedOtherFrameNumbers[type2];
                        dstList = mSkippedFrameNumbers[type1];
                        index = index2;
                    }
                ...
                for (long i = Math.max(maxFrameNumberSeen, mCompletedFrameNumber[requestType]) + 1;
                        i < frameNumber; i++) {
