...
public class NdefMessageSignatureVerifier {
    private Set<X509Certificate> mTrustCertificates;

    public NdefMessageSignatureVerifier(Set<X509Certificate> trustCertificates) {
        mTrustCertificates = trustCertificates;
    }
     ...
     */
    private boolean verifySignature(byte[] message, String sigTypeAlg, String hashType,
     ...
     */
    private int readByte(ByteArrayInputStream buffer) throws FormatException {
     ...
     * @throws FormatException on out of data
     */
    private byte[] readMultiBytes(ByteArrayInputStream buffer, int n) throws FormatException {
     ...
     * @throws FormatException on out of data
     */
    private byte[] readCertificate(ByteArrayInputStream buffer) throws FormatException {
     ...
     */
    private byte[] readDataFromUri(byte[] uri) throws IOException {
    ...
    private X509Certificate findIssuer(final Certificate cert) throws IllegalArgumentException {
        // Find issuer name
        X500Principal issuerName = null;
        if (cert instanceof X509Certificate) {
            issuerName = ((X509Certificate)cert).getIssuerX500Principal();
        } else if (cert instanceof M2mCertificateLite) {
            issuerName = ((M2mCertificateLite)cert).getIssuerX500Principal();
        } else {
        ...
        for (X509Certificate issuerCert : mTrustCertificates) {
            if (!issuerName.equals(issuerCert.getIssuerX500Principal())) {
                continue;
            }
     ...
     */
    public int verify(NdefMessage msg)
                ...
                if (signatureId != NdefSignatureType.NO_SIGNATURE_PRESENT.getId()) {
                    ...
                    if (!certificateChain.isEmpty()) {
                        ...
                        try {
                            ...
                            parsedCertChain[certificateCount] = findIssuer(certForCA);
                            ...
                            for (j = (certificateCount - 1); j >= 0; j--) {
                                if (parsedCertChain[j] instanceof M2mCertificate) {
                                    ...
                                    if ((curPublicKey == null) &&
                                        (inheritedData.getPublicKey() != null)) {
                                        // Try to recreate it
                                        try {
                                            curPublicKey = curM2MCert.reconstructPublicKey(
                                                inheritedData.getPublicKey());
                                        } catch (Exception e) {
                                            throw new FormatException("M2M Certificate #" + j +
                                                ": exception on reconstructing PublicKey: " + e);
                                        }
                                    }
                                    if (curPublicKey == null) {
                                        throw new FormatException("M2M Certificate #" + j +
