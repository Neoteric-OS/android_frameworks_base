...
/**
 * CompatChangesFramework APIs - to be used by platform code only (including mainline
 * modules).
 *
 * @hide
 */
@SystemApi
public final class CompatChangesFramework {
    private static final String TAG = "CompatChangesFramework";

    private CompatChangesFramework() {}
     ...
     * @return {@code true} if the change is enabled for the current app.
     */
    public static boolean isChangeEnabled(long changeId, int uid) throws RemoteException {
        IPlatformCompat platformCompat = IPlatformCompat.Stub.asInterface(
                ServiceManager.getService(Context.PLATFORM_COMPAT_SERVICE));
        try {
