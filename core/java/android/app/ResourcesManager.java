...
public class ResourcesManager {
    ...
    */
    ...
    void applyConfigurationToResourcesImplLocked(@Nullable ResourcesKey key,
            @NonNull ResourcesImpl r, @NonNull Configuration config,
            @Nullable CompatibilityInfo compat) {
        if (!r.getConfiguration().isOtherSeqNewer(config)) {
            if (DEBUG || DEBUG_CONFIGURATION) Slog.v(TAG, "Skipping new config for impl: " + r
                    + " curSeq=" + r.getConfiguration().seq + ", newSeq=" + config.seq);
            return;
        }
