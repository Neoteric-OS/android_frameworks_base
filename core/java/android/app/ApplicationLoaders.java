...
public class ApplicationLoaders {
     ...
     * been cached by cacheNonBootclasspathSystemLibs.
     */
     ...
     */
    public void cacheNonBootclasspathSystemLibs(SharedLibraryInfo[] libs) {
        if (mSystemLibsPreloadMap != null) {
            Log.wtf(TAG, "already pre-loaded");
            return;
        }

        mSystemLibsPreloadMap = new HashMap<String, PreloadedSharedLibrary>();

        for (SharedLibraryInfo lib : libs) {
            cacheNonBootclasspathSystemLib(lib);
        }
    }

    /**
     * Preloads a single non-bootclasspath library.
     *
     * All of this library's dependencies must have previously been cached.
     */
    private void cacheNonBootclasspathSystemLib(SharedLibraryInfo lib) {
        ...
        if (dependencies != null) {
            ...
            for (SharedLibraryInfo dependency : dependencies) {
                ...
                PreloadedSharedLibrary preload = mSystemLibsPreloadMap.get(dependencyPath);

                if (preload == null) {
                    Log.e(TAG, "Failed to find dependency " + dependencyPath
                            + " of preload library " + path);
                    return;
                }

                sharedLibraries.add(preload.loader);
            }
        }
        ...
        if (classLoader == null) {
            Log.e(TAG, "Failed to preload " + path);
            return;
        }

        PreloadedSharedLibrary preload = new PreloadedSharedLibrary();
        preload.loader = classLoader;
        preload.sharedLibraries = sharedLibraries;

        Log.d(TAG, "Created zygote-cached class loader: " + path);
        mSystemLibsPreloadMap.put(path, preload);
    }
    ...
    /**
     * Returns lib cached with cacheNonBootclasspathSystemLib.
     *
    ...
    private ClassLoader getCachedNonBootclasspathSystemLib(String zip, ClassLoader parent,
        ...
        PreloadedSharedLibrary preload = mSystemLibsPreloadMap.get(zip);
        if (preload == null) {
            return null;
        }

        // cache must be built and loaded in the same environment
        if (!sharedLibrariesEquals(sharedLibraries, preload.sharedLibraries)) {
            Log.w(TAG, "Unexpected environment for preloaded library: (" + sharedLibraries + "|"
                    + preload.sharedLibraries + ")");
            return null;
        }
    ...
    private static class PreloadedSharedLibrary {
        ClassLoader loader;
     ...
     */
    private Map<String, PreloadedSharedLibrary> mSystemLibsPreloadMap = null;
}
