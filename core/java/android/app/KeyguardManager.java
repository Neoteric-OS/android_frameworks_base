...
public class KeyguardManager {
     ...
     * if switching to that user requires lock screen authentication.
     * <p>
     * In the case of a profile, when the device goes to the lock screen, up to two layers of
     * authentication may be required to regain access to the profile's apps: one to unlock the lock
     * screen, and one to unlock the profile (when a separate profile challenge is required). For a
     * profile, the device is considered to be locked as long as any challenge remains, either the
     * parent user's challenge (when applicable) or the profile's challenge (when applicable).
     * </div>
     *
