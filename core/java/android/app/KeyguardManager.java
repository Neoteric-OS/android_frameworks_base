...
public class KeyguardManager {
     ...
     * @deprecated see {@link BiometricPrompt.Builder#setAllowedAuthenticators(int)}
     */
    @Deprecated
     ...
     * </ul>
     * <p>
     * "Showing" refers to the logical state of the UI, regardless of whether the screen happens to
     * be on. When the power button is pressed on an unlocked device, the lock screen starts
     * "showing" immediately, even though the screen turns off.
     * <p>
     ...
     * {@code isKeyguardShowing()}. On many devices, the lock screen displays an <em>unlocked</em>
     ...
     * <p>
     * This returns the device locked state for the {@link Context}'s user. For users that can be
     * "switched" to (this usually means "full users"), the device is considered "locked" if the
     * lock screen is showing (i.e. {@link #isKeyguardLocked()} returns {@code true}) and is not
     * trivially dismissible, or if the user is not the current user.
     * <p>
     * Note, cases where the lock screen is trivially dismissible include: the lock screen is set to
     * swipe (as opposed to PIN, pattern, password, or none), face authentication has just
     * completed, or a trust agent is keeping the device unlocked.
     * <p>
     * For a profile with a unified challenge, the device is considered locked if and only if the
     * device is locked for the parent user.
     *
