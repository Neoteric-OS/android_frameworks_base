...
public class UpdateEngine {
    ...
    public static final class AllocateSpaceResult {
        ...
        /**
         * Estimated total space required on userdata partition to apply the payload.
         *
         * <p>
        ...
        public long totalSpaceRequired() {
            ...
            if (mErrorCode == ErrorCodeConstants.NOT_ENOUGH_SPACE) {
                return mSpaceRequired;
            }
            throw new IllegalStateException(String.format(
                    "totalSpaceRequired() is not available when error code is %d", mErrorCode));
        }
    ...
    public @NonNull AllocateSpaceResult allocateSpace(
        ...
        try {
            result.mSpaceRequired = mUpdateEngine.allocateSpaceForPayload(
                    payloadMetadataFilename,
                    headerKeyValuePairs);
            result.mErrorCode = result.mSpaceRequired == 0
                    ? ErrorCodeConstants.SUCCESS
                    : ErrorCodeConstants.NOT_ENOUGH_SPACE;
            return result;
        } catch (ServiceSpecificException e) {
            result.mErrorCode = e.errorCode;
            result.mSpaceRequired = 0;
            return result;
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
