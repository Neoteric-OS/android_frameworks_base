...
public class BatteryManager {
     ...
     * be (a) 0 if {@code targetSdkVersion <= 27} (O-MR1) or (b) Integer.MIN_VALUE
     *    if {@code targetSdkVersion > 27}.
     *
     * @param id identifier of the requested property
     *
     * @return the property value. If property is not supported or any other error,
     *    return (a) 0 if {@code targetSdkVersion <= 27} (O-MR1) or (b) Integer.MIN_VALUE
     *    if {@code targetSdkVersion > 27}.
     */
    public int getIntProperty(int id) {
        long value = queryProperty(id);
        if (mContext == null || mContext.getApplicationInfo().targetSdkVersion <=
                android.os.Build.VERSION_CODES.O_MR1) {
            return (int)value;
        } else {
            return value == Long.MIN_VALUE ? Integer.MIN_VALUE : (int)value;
        }
    }
