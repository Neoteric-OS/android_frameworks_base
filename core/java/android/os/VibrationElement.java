...
public class VibrationElement implements Parcelable {
    ...
    public static VibrationElement scale(VibrationElement element, float gamma, int maxAmplitude) {
        return new VibrationElement(element.duration, IntStream.of(element.channels)
                .map(a -> (int) (MathUtils.pow(a / (float) VibrationEffect.MAX_AMPLITUDE, gamma)
                * maxAmplitude)).toArray());
    }
    ...
    public int hashCode() {
        int result = 17;
        result += 37 * Long.hashCode(duration);
    ...
    public String toString() {
        return "VibrationElement" + Arrays.toString(channels);
    }
