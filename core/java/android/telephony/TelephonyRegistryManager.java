...
public class TelephonyRegistryManager {
    ...
    /**
     * Informs the system of an intentional upcoming carrier network change by a carrier app with
     * the subscription. This call only used to allow the system to provide alternative UI while
     * telephony is performing an action that may result in intentional, temporary network lack of
     * connectivity.
     *
     ...
     * @see TelephonyManager#hasCarrierPrivileges
     */
    public void notifyCarrierNetworkChange(int subId, boolean active) {
        try {
            sRegistry.notifyCarrierNetworkChangeWithSubId(subId, active);
        } catch (RemoteException ex) {
            // system server crash
        }
    }
