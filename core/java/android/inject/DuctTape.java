 ...
 * as and injection target by implementing the {@link android.inject.InjectionTarget}
 * interface. Once that is in place all fields of the class can be marked as
 * injection targets by adding the {@link android.inject.Inject} annotation.
 * </p>
 *
 * <pre>
 ...
 * its fields marked with {@link android.inject.Inject}. However, by adding the
 * {@link #OPTIONAL} tag to the {@link android.inject.Inject} annotation the
 * framework is told that it is OK not to inject anything into that field.
 * </p>
 *
 * <pre>
 ...
 * {@link android.inject.Inject} annotation, but you can also use custom tags.
 * These tags are used to select only those objects which have previously been
 * tagged with at least on of the given tags, e.g.
 * </p>
 *
 * <pre>
 ...
 * {@link android.inject.Inject} annotation. These can be a mix of built-in tags
 * and custom tags.
 * </p>
 *
 * <pre>
 ...
 * {@link android.inject.InjectionException}. However, by declaring the field as
 * an array it will instead receive all of the matching objects.
 * </p>
 *
 * <pre>
 ...
 * {@link android.inject.InjectionException} informing about the situation.
 * </p>
 *
 * <pre>
 ...
 * {@link adroid.inject.Singleton} interface. That tells the framework that the
 * same object instance should be used for all injections.
 * </p>
 ...
 * {@link android.net.ConnectivityManager}, {@link android.net.wifi.WifiManager},
 * {@link android.os.DropBoxManager}, {@link android.os.Looper},
 ...
 * </p>
 * <p>
 * <img src="../../../images/ducttape_graph.png"
...
public class DuctTape {
     ...
     */
    // This field cannot be static since that would cause trouble when
    ...
    /**
     * Indicates whether this instance has experienced more than one call to apply(). This can
     * typically be used to skip some steps during the first injection.
     */
     ...
     * {@link android.inject.Inject}. Note that when this tag is present on an
     * injection there will be no attempts made to use added classes to
     * instantiate objects for this field.
     */
    // The values of the constants OPTIONAL and ORDERED have been chosen
     ...
     * only be used together with {@link android.inject.Inject} and makes sure
     * that objects that other objects depend on are always present before the
     * objects that are dependent on them. Note that it does only affect fields
     * of with an array type.
     */
    ...
    private static class Tagged {
        ...
        public final void addTags(int[] tags, int len) {
            ...
            // Possible duplicates in the array argument are ignored since
    ...
    private static final class Node extends Tagged {
        ...
        // Preparation for when objects can originate from different contexts, and will need
        // their resources to be fetched from the right place.
        public Context mContext;
        ...
        public final void addChildren(Node[] children, int length) {
            ...
            // Quick bail out when addition would make no difference
            // This optimization is here since this method is a
            // performance bottleneck in very large graphs.
            Node[] thisChildren = mChildren;
            int childrenLen = mChildrenLen;
            boolean match = true;
        ...
        resolve: while (true) {
            switch (f.mStatus) {
                ...
                case Factory.UNRESOLVABLE:
                    return false;
                default:
                    // Set factory into pending state during validation
                    // This is done to be able to resolve circular
                    // dependencies when calculating the validity.
                    ...
                    validation: while (InjectionTarget.class.isAssignableFrom(current)) {
                        Field[] fields;
                        int fieldIdx;
                        if (cont == null) {
                        ...
                        fieldscan: while (fieldIdx < fieldsLen) {
                            ...
                            if (injection != null || cont != null) {
                                ...
                                if (cont == null) {
                                    ...
                                    // We need to know how many references
                                    ...
                                    while (node != null) {
                                        ...
                                        if (clazz.isInstance(candidate)) {
                                            ...
                                            if (numTags == 0 || node.hasTag(tags)) {
                                                ...
                                                if (found) {
                                                    if (array == null) {
                                                        // Too many possible injections means that
                                                        // the instance that this factory produces
                                                        // would cause a taping exception if used,
                                                        // i.e. we can conclude that this factory
                                                        // is invalid already here.
                                                        f.mStatus = Factory.UNRESOLVABLE;
                                                        break validation;
                                                    } else {
                                                        break;
                                                    ...
                                                    // Break early when we know that we can handle
                                                    // more than one instance, and already
                                                    // found one. Note however that this
                                                    // optimization is not safe for for
                                                    // non-singletons since they might create
                                                    // other non-singletons that in the end want to
                                                    // create an instance of this type again, thus
                                                    // causing an infinite instantiation loop.
                                    ...
                                    if ((id & 0xff000000) != 0) {
                                        ...
                                        if (obj != null) {
                                            ...
                                            if (res != null) {
                                             ...
                                             // See if there is any matching views
                                            if (View.class.isAssignableFrom(clazz)) {
                                                ...
                                                // Check activity for view when possible
                                ...
                                // This is run as a last step to make it
                                ...
                                while (true) {
                                    ...
                                    // This is done first in the loop to
                                    ...
                                    if ((numTags == 0 || candidate.hasTag(tags))
                                        ...
                                        // See if it matches what the current factory wants and
                                        // make sure we are not walking in a circle.
                                        // When we encounter a circular dependency we add ourselves
                                        // as dependable on that factory and assume it is valid.
                                        // If it turns out not to be (because not all of its
                                        // injected field has been validated yet) it will
                                        ...
                                        switch (candidate.mStatus) {
                                            ...
                                            case Factory.PROCESSING: {
                                                ...
                                                // We can not allow non-singletons to be part of
                                                // circular dependency loops since it would cause
                                                ...
                                                // Set circle as dependants to first factory.
                                            ...
                                            default: {
                                                ...
                                                // This is used to put no load what so ever on the
                                                // call stack when resolving factories, since the
                                                // depth would in the worst case be equal to the
                                                ...
                                                // Restart processing with candidate factory
                                                f.mCont = c;
                                                pathIdx++;
                                                f = candidate;
                                                continue resolve;
                                ...
                                // Abort in case we could not find anything to inject
                                if (!resolvable && vital) {
                                    f.mStatus = Factory.UNRESOLVABLE;
                                    break validation;
                                }
    ...
    private static Inject getInjection(final Field f) {
    ...
    private static String commonPrefix(String prefix, String text) {
        if (text.startsWith("java.lang.") || text.startsWith("int") || text.startsWith("boolean")
                ...
                || text.startsWith("char") || text.startsWith("short")
                || text.startsWith("byte")) {
            return prefix;
        } else if (prefix == null) {
            return text;
        } else {
    ...
    private void removeFactory(Factory factory) {
        ...
        // Queue for removal
        // This is done so that in case the factory is re-added before apply() is run the product
        // is not recreated.
     ...
     *
     * @throws InjectionException when an injection fails either because of
     ...
     * @see #isRobust()
     */
    public final void apply() throws InjectionException {

        // Do nothing unless tape is needed
        int nodesLen = mNumNodes;
        if (!mRobust && nodesLen > 0) {

            // Take height for objects that might be constructed by the factories
            ...
            // Clearing the head of this releases the rest of it in one sweep.
            mPendingRemoval = null;
            ...
            HashMap<TaggedClass, Implementors> implCache =
                    new HashMap<TaggedClass, Implementors>();
            ...
            while (node != null) {
                ...
                if (instance instanceof InjectionTarget) {

                    // Visit all classes that are injection targets
                    Class<?> current = instance.getClass();
                    while (InjectionTarget.class.isAssignableFrom(current)) {
                        ...
                        for (int j = 0; j < fieldsLen; j++) {
                            ...
                            Inject injection = getInjection(field);
                            if (injection != null) {
                                ...
                                if (cached == null) {
                                        ...
                                        if ((n.mFlags & Node.DO_NOT_REUSE) == 0) {
                                            ...
                                            if (include) {
                                                if (cacheLen == childrenLen) {
                                                    ...
                                                    System.arraycopy(children, 0, temp, 0,
                                                            cacheLen);
                                                    children = temp;
                                                }
                                                children[cacheLen++] = n;
                                    ...
                                    // Create implementing objects using all factories
                                    boolean generic = true;
                                    if (vital) {
                                        Factory factory = factories;
                                        while (factory != null) {
                                            if (factory.mStatus != Factory.UNRESOLVABLE
                                                    && (factory.mNonSingleton
                                                            || factory.mStatus !=
                                                                Factory.EXECUTED)) {

                                                // Check that the product is applicable
                                                Class<?> factoryClazz = factory.mClazz;
                                                if (clazz.isAssignableFrom(factoryClazz)
                                                        && (numTags == 0
                                                            || factory.hasTag(tags))) {

                                                    // Find the context of this factory
                                                    Context prodCtxt = factory.mContext;

                                                    // Make sure the class has access to all the
                                                    // types it needs.
                                                    if (factory.mStatus == Factory.EXECUTED
                                                        ...
                                                        try {
                                                            ...
                                                            // Skip creation in case this is not a
                                                            // singleton and there is already an
                                                            // instance of the product in the
                                                            // field.
                                                            // Since we are comparing class-types
                                                            // and different context sensitive
                                                            // classes are created using different
                                                            // class-loaders, it is safe to assume
                                                            // that on a match we have an instance
                                                            // having the expected origin.
                                                            if (mProcessed &&
                                                                    factory.mNonSingleton) {
                                                                try {
                                                                    Object reuse = null;
                                                            ...
                                                            // Only create product when one is not
                                                            // already available.
                                                            if (product == null) {
                                                            ...
                                                            // Mark factory as used at least once
                                                            factory.mStatus = Factory.EXECUTED;

                                                            // Append to end of node list to ensure
                                    ...
                                    // Look for compatible view in case such are available
                                    Object obj = null;
                                    if (View.class.isAssignableFrom(clazz)
                                            && (id & 0xff000000) != 0) {
                                        ...
                                        if (clazz.isInstance(o)) {
                                            ...
                                            // Add view as node in when it is compatible with the
                                    ...
                                    // See if we can find something in the context that can be
                                    // injected.
                                    ...
                                    // Add as child
                                    // The resource is not added as a node since it was not
                                    // included by the user, and we would also get into some
                                    // trouble figuring out if an object was already added.
                                    if (obj != null) {
                                    ...
                                    if (array != null && array.getComponentType().isPrimitive()
                                        ...
                                        if (array == int[].class) {
                                            ...
                                            for (int i = 0; i < cacheLen; i++) {
                                                temp[i] = ((Integer)children[i].mObject)
                                                        .intValue();
                                            }
                                            target = temp;
                                            ...
                                            for (int i = 0; i < cacheLen; i++) {
                                                temp[i] = ((Float)children[i].mObject)
                                                        .floatValue();
                                            }
                                            target = temp;
                                            ...
                                            for (int i = 0; i < cacheLen; i++) {
                                                temp[i] = ((Short)children[i].mObject)
                                                        .shortValue();
                                            }
                                            target = temp;
                                    ...
                                    // Convert cache
                                    // This is the actual array that will be injected later
                                    if (cacheLen > 0) {
                                        final Object[] typed;
                                        if (directArray) {
                                            clazz = array;
                                    ...
                                    if (cached != null) {
                                        ...
                                        if (array != null) {
                                            if (cached.mDirectArray) {
                                                ...
                                                // When we just cached the array there is no need
                                                // to do substitutions of non-singletons in there.
                                                if (cached.mGeneric || justCached) {
                                                    ...
                                                    try {
                                                        // Must create new instances of
                                                        // non-singletons in an array.
                                                        ...
                                                        // Inject the unique reference list
                                        ...
                                        throw new InjectionException("Nothing to inject into "
            ...
            node = firstNode;
            while (node != null) {
                if ((node.mFlags & (Node.WILL_BE_SORTED | Node.MERGED | Node.VISITED))
                        == Node.WILL_BE_SORTED) {
                    int pathLen = 0;
                    start: while (true) {
                        if ((node.mFlags & Node.MERGED) == 0) {
                            ...
                            for (int j = node.mLastVisit; j < node.mChildrenLen; j++) {
                                ...
                                // When this part of the graph has been detected as a cycle, we
                                // simply merge all the cyclic nodes into one single node. This new
                                // node contains references to all instances in the previous nodes,
                                // and all of their child references. We do not bother to remove
                                // child references to nodes that are removed since those will be
                                // skipped in all further processing anyway.
                                Node child = node.mChildren[j];
                                if (child != node && (child.mFlags & Node.MERGED) == 0) {
                                    for (int k = 0; k < pathLen; k++) {
                                        if (path[k] == child) {

                                            // Merge all cyclic nodes with this node
                                            for (int l = k; l < pathLen; l++) {
                                                Node segment = path[l];
                                                if ((segment.mFlags & Node.MERGED) == 0) {
                                                    segment.mFlags = (segment.mFlags
                                                            & ~Node.MERGED_HEAD)
                                                            | Node.MERGED;
                        ...
                        // Nodes with several instances starts those instances in random order,
                        // since they have circular dependencies and it is impossible to start them
                        // in a "correct" order.
                        if ((node.mFlags & Node.MERGED) == 0) {
            ...
            if (implCache != null) {
                for (Object[] injected : toBeSorted) {
                    ...
                    // The array is sorted by iterating the calculated dependency sequence, copying
                    // only entries also present in the injected array to a temporary array.
                    idx = 0;
                    ...
                    // The sorted array is copied back to the injected array to automatically apply
                    // the changes across all affected objects (since they already reference that
                    // array).
            ...
            // Since the factories have references to their product, they will not be lost.
     ...
     * is described using the <a href="http://en.wikipedia.org/wiki/DOT_language">DOT language</a>.
     * <p>
        ...
        for (int pass = 0; pass < 2; pass++) {
            for (int i = 0; i < all.size(); i++) {
                ...
                if (obj instanceof InjectionTarget) {
                    ...
                    while (InjectionTarget.class.isAssignableFrom(clazz)) {
                                        ...
                                        if (fieldVal != null) {
                                            ...
                                            if (fieldClazz.isArray()) {
                                                ...
                                                if (fieldClazz.isPrimitive()) {
                                                    // Find out if it is a temporary array.
                                                    Node o = mFirstNode;
