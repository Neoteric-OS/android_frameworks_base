...
public class WifiNetworkScoreCacheTest {
  ...
  private ScoredNetwork validScoredNetwork;
  private WifiNetworkScoreCache scoreCache =
  ...
  private ScoredNetwork buildScoredNetwork(WifiKey key, RssiCurve curve) {
    return new ScoredNetwork(new NetworkKey(key), curve);
  }

  // Called from setup
  private void initializeCacheWithValidScoredNetwork() {
    scoreCache.updateScores(ImmutableList.of(validScoredNetwork));
  }

  @Before
  public void setUp() {
    MockitoAnnotations.initMocks(this);
    validScoredNetwork = buildScoredNetwork(VALID_KEY, mockRssiCurve);
    scoreCache = new WifiNetworkScoreCache(mockContext);
  ...
  public void isScoredNetworkShouldReturnTrueAfterUpdateScoresIsCalled() {
    assertTrue(scoreCache.isScoredNetwork(VALID_SCAN_RESULT));
  }

  @Test
  public void isScoredNetworkShouldReturnFalseAfterClearScoresIsCalled() {
    scoreCache.clearScores();
    assertFalse(scoreCache.isScoredNetwork(VALID_SCAN_RESULT));
  }
  ...
  public void updateScoresShouldAddNewNetwork() {
    ...
    scoreCache.updateScores(ImmutableList.of(network2));

    assertTrue(scoreCache.isScoredNetwork(VALID_SCAN_RESULT));
    assertTrue(scoreCache.isScoredNetwork(result2));
  }

  @Test
  public void hasScoreCurveShouldReturnTrue() {
    assertTrue(scoreCache.hasScoreCurve(VALID_SCAN_RESULT));
  }

  @Test
  public void hasScoreCurveShouldReturnFalseWhenNoCachedNetwork() {
    ScanResult unscored = buildScanResult("fake", BSSID);
    assertFalse(scoreCache.hasScoreCurve(unscored));
  }
  ...
  public void getNetworkScoreShouldReturnScore() {
    ...
    assertEquals(score, scoreCache.getNetworkScore(result));
  }

  @Test
  public void getMeteredHintShouldReturnFalse() {
    assertFalse(scoreCache.getMeteredHint(VALID_SCAN_RESULT));
  }
  ...
  public void getMeteredHintShouldReturnTrue() {
    ...
    scoreCache.updateScores(ImmutableList.of(network));

    assertTrue(scoreCache.getMeteredHint(VALID_SCAN_RESULT));
  }
}
