...
import com.android.time.client.sntp.impl.Duration64;
...
public class NewSntpClientTest {
    ...
    private NewSntpClient mClient;
    private Network mNetwork;
    private InstantSource mInstantSource;
    ...
    public void setUp() throws Exception {
        ...
        mInstantSource = mock(InstantSource.class);
        ...
        mClient = new NewSntpClient(mInstantSource, mRandom);
    }
    ...
    public void testRequestTime_era0ClientEra0RServer() throws Exception {
        when(mInstantSource.instant()).thenReturn(LATE_ERA0_REQUEST_TIME);

        mServer.setServerReply(HexEncoding.decode(LATE_ERA_RESPONSE.toCharArray(), false));
        assertTrue(mClient.requestTime(mServer.getAddress().getHostAddress(), mServer.getPort(), 500, mNetwork));
    ...
    public void testRequestTime_era1ClientEra0Server() throws Exception {
        when(mInstantSource.instant()).thenReturn(EARLY_ERA1_REQUEST_TIME);
    ...
    private static void checkRequestTimeCalcs(
            Instant clientTime, Instant serverTime, NewSntpClient client) {
    ...
    private void checkCalculateClockOffset(Instant clientTime, Instant serverTime) {
        ...
        for (long totalElapsedTimeMillis : Arrays.asList(0, 20, 200, 2000, 20000)) {
            ...
            Timestamp64 clientRequestTimestamp = Timestamp64.fromInstant(
                    Instant.ofJavaTimeInstant(clientTime.toJavaTime().minusMillis(simulatedClientElapsedTimeMillis / 2)));
            Timestamp64 clientResponseTimestamp = Timestamp64.fromInstant(
                    Instant.ofJavaTimeInstant(clientTime.toJavaTime().plusMillis(simulatedClientElapsedTimeMillis / 2)));
            Timestamp64 serverReceiveTimestamp = Timestamp64.fromInstant(
                    Instant.ofJavaTimeInstant(serverTime.toJavaTime().minusMillis(simulatedServerElapsedTimeMillis / 2)));
            Timestamp64 serverTransmitTimestamp = Timestamp64.fromInstant(
                    Instant.ofJavaTimeInstant(serverTime.toJavaTime().plusMillis(simulatedServerElapsedTimeMillis / 2)));

            Duration actualOffset = SntpClientEngine.calculateClientOffset(
    ...
    private static Instant utcInstant(
        ...
        return Instant.ofJavaTimeInstant(
                LocalDateTime.of(year, monthOfYear, day, hour, minute, second, nanos)
    ...
    public void testTimeoutFailure() throws Exception {
        ...
        assertFalse(mClient.requestTime(mServer.getAddress().getHostAddress(), mServer.getPort(), 500, mNetwork));
        assertEquals(1, mServer.numRequestsReceived());
    ...
    public void testAcceptOnlyServerAndBroadcastModes() throws Exception {
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            reply[0] |= (byte) i;
            mServer.setServerReply(reply);
            final boolean rval = mClient.requestTime(mServer.getAddress().getHostAddress(), mServer.getPort(), 500,
                    mNetwork);
            switch (i) {
    ...
    private static Instant calculateIdealServerTime(String receiveTimestampString,
        ...
        return Instant.ofJavaTimeInstant(receiveTimestamp.toInstant(era).toJavaTime()
