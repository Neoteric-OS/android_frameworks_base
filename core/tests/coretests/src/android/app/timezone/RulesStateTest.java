 ...
 */
// TODO Move to CTS once this class is part of the SystemApi.
public class RulesStateTest {
    ...
    public void parcelable() {
        RulesState version = new RulesState(
                "2016a", formatVersion(1, 1), true,
    ...
    @Test
    public void isInstalledDistroWorseThan() {
        RulesState operationInProgress = new RulesState(
                "2016b", formatVersion(1, 1), true /* operationInProgress */,
                RulesState.STAGED_OPERATION_NONE, null /* stagedDistroRulesVersion */,
                RulesState.DISTRO_STATUS_NONE, null /* installedDistroRulesVersion */);
        try {
            operationInProgress.isInstalledDistroWorseThan(rulesVersion("2016b", 1));
            fail();
        ...
        RulesState nothingInstalled = new RulesState(
                ...
                RulesState.DISTRO_STATUS_NONE, null /* installedDistroRulesVersion */);
        try {
            nothingInstalled.isInstalledDistroWorseThan(rulesVersion("2016b", 1));
            fail();
        } catch (IllegalStateException expected ) {}
        ...
        assertEquals(installedVersion.isWorseThan(olderRules),
                rulesStateWithInstalledVersion.isInstalledDistroWorseThan(olderRules));

        DistroRulesVersion sameRules = rulesVersion("2016b", 1);
        assertEquals(installedVersion.isWorseThan(sameRules),
                rulesStateWithInstalledVersion.isInstalledDistroWorseThan(sameRules));

        DistroRulesVersion newerRules = rulesVersion("2016c", 1);
        assertEquals(installedVersion.isWorseThan(newerRules),
                rulesStateWithInstalledVersion.isInstalledDistroWorseThan(newerRules));
    }
