...
public class NetworkTimeSuggestionTest {
    ...
    private static final int ARBITRARY_CERTAINTY_MILLIS = 3333;

    @Test
    public void testEquals() {
        NetworkTimeSuggestion one = new NetworkTimeSuggestion(
                ARBITRARY_TIME, ARBITRARY_CERTAINTY_MILLIS);
        assertEquals(one, one);

        NetworkTimeSuggestion two =
                new NetworkTimeSuggestion(ARBITRARY_TIME, ARBITRARY_CERTAINTY_MILLIS);
        ...
        NetworkTimeSuggestion three = new NetworkTimeSuggestion(
                differentTime, ARBITRARY_CERTAINTY_MILLIS);
        assertNotEquals(one, three);
        assertNotEquals(three, one);

        int differentCertainty = ARBITRARY_CERTAINTY_MILLIS + 1;
        NetworkTimeSuggestion four = new NetworkTimeSuggestion(
                ARBITRARY_TIME, differentCertainty);
    ...
    public void testParseCommandLineArg_noReferenceTime() {
        ShellCommand testShellCommand = createShellCommandWithArgsAndOptions(
                "--unix_epoch_time 12345 --certainty_millis 111");
    ...
    public void testParseCommandLineArg_noUnixEpochTime() {
        ShellCommand testShellCommand = createShellCommandWithArgsAndOptions(
                "--reference_time 54321 --certainty_millis 111");
        NetworkTimeSuggestion.parseCommandLineArg(testShellCommand);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testParseCommandLineArg_noCertaintyMillis() {
        // TODO Finish this
        ShellCommand testShellCommand = createShellCommandWithArgsAndOptions(
    ...
    public void testParseCommandLineArg_validSuggestion() {
        ShellCommand testShellCommand = createShellCommandWithArgsAndOptions(
                "--reference_time 54321 --unix_epoch_time 12345 --certainty_millis 111");
