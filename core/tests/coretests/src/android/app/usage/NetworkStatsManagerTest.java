...
public class NetworkStatsManagerTest {
    ...
    public void testQueryDetails() throws RemoteException {
        ...
        android.net.NetworkStats.Entry uid1Entry1 = new android.net.NetworkStats.Entry("if1", uid1,
                android.net.NetworkStats.SET_DEFAULT, android.net.NetworkStats.TAG_NONE,
                100, 10, 200, 20, 0);

        android.net.NetworkStats.Entry uid1Entry2 = new android.net.NetworkStats.Entry(
                "if2", uid1,
                android.net.NetworkStats.SET_DEFAULT, android.net.NetworkStats.TAG_NONE,
                100, 10, 200, 20, 0);

        android.net.NetworkStats.Entry uid2Entry1 = new android.net.NetworkStats.Entry("if1", uid2,
                android.net.NetworkStats.SET_DEFAULT, android.net.NetworkStats.TAG_NONE,
                150, 10, 250, 20, 0);

        android.net.NetworkStats.Entry uid2Entry2 = new android.net.NetworkStats.Entry(
                "if2", uid2,
                android.net.NetworkStats.SET_DEFAULT, android.net.NetworkStats.TAG_NONE,
                150, 10, 250, 20, 0);
        ...
        when(mStatsSession.getHistoryIntervalForUid(any(NetworkTemplate.class),
                ...
                .then((InvocationOnMock inv) -> {
                    ...
                    assertEquals(NetworkTemplate.MATCH_MOBILE_ALL, template.getMatchRule());
    ...
    public void testQueryDetails_NoSubscriberId() throws RemoteException {
        ...
        NetworkStatsHistory emptyHistory = new NetworkStatsHistory(10, 0);

        when(mService.openSessionForUsageStats(anyInt(), anyString())).thenReturn(mStatsSession);
        when(mStatsSession.getRelevantUids()).thenReturn(new int[] { uid1, uid2 });

        when(mStatsSession.getHistoryIntervalForUid(any(NetworkTemplate.class),
                anyInt(), eq(android.net.NetworkStats.SET_ALL),
                eq(android.net.NetworkStats.TAG_NONE),
                eq(NetworkStatsHistory.FIELD_ALL), eq(startTime), eq(endTime)))
                .then((InvocationOnMock inv) -> {
                    NetworkTemplate template = inv.getArgument(0);
                    assertEquals(NetworkTemplate.MATCH_MOBILE_WILDCARD, template.getMatchRule());
                    return emptyHistory;
                });
        ...
        // Empty history: no data. Only testing that getHistoryIntervalForUid is called with a
        // MATCH_MOBILE_WILDCARD template (no subscriberId).
        assertFalse(stats.hasNextBucket());
    }

    private void assertBucketMatches(android.net.NetworkStats.Entry expected,
