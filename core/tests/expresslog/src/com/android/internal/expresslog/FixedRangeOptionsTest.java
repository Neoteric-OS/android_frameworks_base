...
public class FixedRangeOptionsTest {
    ...
    public void testBinIndexForRangeEqual10() {
        CounterHistogram.FixedRangeOptions options = new CounterHistogram.FixedRangeOptions(10, 100,
                1000);
        Assert.assertEquals(0, options.getBinForSample(99));
        Assert.assertEquals(options.getBinsCount() - 1, options.getBinForSample(1000));
        for (int i = 1, bins = options.getBinsCount() - 1; i < bins; i++) {
            Assert.assertEquals(100 + i * 90, options.getBinForSample(i));
        }
    }
