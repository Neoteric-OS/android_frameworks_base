...
Result<ResourceId> WARN_UNUSED ParseResReference(const AssetManager2& am, const std::string& res,
  ...
  auto resid_result = am.GetResourceId(res, "", fallback_package);
  if (resid_result.has_value()) {
    return *resid_result;
  }
...
void PrintValue(AssetManager2* const am, const AssetManager2::SelectedValue& value,
  ...
  switch (value.type) {
    ...
    case Res_value::TYPE_STRING: {
      ...
      out->append("\"");
      if(auto str = pool->string8ObjectAt(value.data); str.has_value()) {
        out->append(*str);
      }
    } break;
    default:
...
Result<std::string> WARN_UNUSED GetValue(AssetManager2* const am, ResourceId resid) {
  auto value = am->GetResource(resid); // TODO fix this
  if (!value.has_value()) {
