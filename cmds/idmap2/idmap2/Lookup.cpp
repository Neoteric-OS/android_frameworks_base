...
std::pair<bool, ResourceId> WARN_UNUSED ParseResId(const AssetManager2& am,
  ...
  // next, append the target as "package:" and try again
  resid = am.GetResourceId(package_name + ':' + raw);
  if (resid != 0) {
    return std::make_pair(true, resid);
  }
...
bool Lookup(const std::vector<std::string>& args, std::ostream& out_error) {
  ...
  ResourceId resid;
  bool lookup_ok;
  std::tie(lookup_ok, resid) = ParseResId(am, target_package_name, resid_str);
  if (!lookup_ok) {
