...
Result<Unit> Scan(const std::vector<std::string>& args) {
  ...
  for (const std::string& path : **apk_paths) {
    ...
    if (!overlay_info->requiredSystemPropertyName.empty() && !overlay_info->requiredSystemPropertyName.empty()) {
        // if property set & equal to value, then include overlay - otherwise skip
        if (android::base::GetProperty(overlay_info->requiredSystemPropertyName, "") != overlay_info->requiredSystemPropertyValue) {
            continue;
        }
    }
