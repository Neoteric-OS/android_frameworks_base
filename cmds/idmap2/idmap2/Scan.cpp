...
bool Scan(const std::vector<std::string>& args, std::ostream& out_error) {
  ...
  std::string target_package_name, target_package_path, output_directory;
  ...
  const CommandLineOptions opts =
      CommandLineOptions("idmap2 scan")
          ...
          .MandatoryOption("--target-package-name", "package name of target package",
                           &target_package_name)
          .MandatoryOption("--target-package-path", "path to target apk", &target_package_path)
  ...
  if (!apk_paths) {
    out_error << "error: failed to find apk files" << std::endl;
    return false;
  }
  ...
  for (auto iter = interesting_apks.cbegin(); iter != interesting_apks.cend(); ++iter) {
    ...
    if (!Create(std::vector<std::string>({
                    "--target-apk-path",
                    target_package_path,
                    "--overlay-apk-path",
                    *iter,
