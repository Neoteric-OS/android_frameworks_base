...
BLUEPRINT_FILES="$(echo "$ALL_FILES" | grep -e '\.bp')"
...
# bpfmt (caveat: will always check version in working directory)
if [ "$BLUEPRINT_FILES" ]; then
    bpfmt_output="$(bpfmt -d "$BLUEPRINT_FILES")"
    if [ "$bpfmt_output" ]; then
        echo -e "error: blueprint formatting (use 'bpfmt -w' to fix):\n$bpfmt_output"
        exit 1
    fi
fi
