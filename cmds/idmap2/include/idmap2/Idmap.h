 ...
 *
 * ## v4
 * - Added `target_entry_inline` which represents target resources overlaid using resource values
 *   encoded inline in the idmap file.
 * - `target_entry` is now only used to map target resource IDs to overlay resource IDs.
 * - Fixed all alignment issues that required the framework to use "#pragma pack(1)".
 */
