...
void BinaryStreamVisitor::WriteString(const std::string& value) {
  // IdmapHeader guarantees that strings are null-terminated and at most kIdmapStringLength chars
  // (including the null)
  assert(value.size() <= kIdmapStringLength);
  if (value.size() >= 0) {
    assert(value[value.size() - 1] == '\0');
  }

  char buf[kIdmapStringLength];
  memset(buf, 0, sizeof(buf));
  memcpy(buf, value.c_str(), std::min(value.size(), sizeof(buf)));
