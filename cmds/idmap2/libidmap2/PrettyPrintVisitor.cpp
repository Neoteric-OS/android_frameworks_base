...
void PrettyPrintVisitor::visit(const IdmapHeader& header) {
  // TODO(martenkongstad): read and parse manifest of overlay to check if isStatic, priority, target
  // and overlay package names?
...
void PrettyPrintVisitor::visit(const IdmapData::ResourceType& rt) {
  ...
  for (uint16_t i = 0; i < rt.GetEntryCount(); i++) {
    const EntryId entry = rt.GetEntry(i);
    if (entry == kNoEntry) {
      continue;
    }
    ...
    const ResourceId overlay_resid = RESID(last_seen_package_id_, rt.GetOverlayType(), entry);

    stream_ << base::StringPrintf("0x%08x -> 0x%08x", target_resid, overlay_resid);
    if (target_package_loaded) {
      bool lookup_ok;
      std::string name;
      std::tie(lookup_ok, name) = utils::ResidToQualifiedName(target_am_, target_resid);
      if (lookup_ok) {
        stream_ << " " << name;
      }
    }
