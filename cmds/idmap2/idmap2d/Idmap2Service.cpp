...
Status error(const std::string& msg) {
  PLOG(ERROR) << msg;
  return Status::fromServiceSpecificError(errno, android::String8(msg.c_str()));
}
...
Status Idmap2Service::getIdmapPath(const std::string& overlay_apk_path,
  ...
  assert(_aidl_return);
  *_aidl_return = Idmap::CanonicalIdmapPathFor("/data/resource-cache", overlay_apk_path);
  return ok();
}
...
Status Idmap2Service::removeIdmap(const std::string& overlay_apk_path,
  ...
  assert(_aidl_return);
  const std::string idmap_path =
      Idmap::CanonicalIdmapPathFor("/data/resource-cache", overlay_apk_path);
  if (unlink(idmap_path.c_str()) == 0) {
    *_aidl_return = true;
    return ok();
  } else {
    *_aidl_return = false;
    return error("failed to unlink " + idmap_path);
  }
}
...
Status Idmap2Service::createIdmap(const std::string& target_apk_path,
  ...
  // TODO(martenkongstad): replace hardcoded string with constant (everywhere in idmap2)
  const std::string idmap_path =
      Idmap::CanonicalIdmapPathFor("/data/resource-cache", overlay_apk_path);
  umask(0122);
