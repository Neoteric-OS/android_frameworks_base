...
static void run_idmap(const char *orig_apk, const char *overlay_apk, int idmap_fd)
{
    ...
    execl(IDMAP_BIN, IDMAP_BIN, "--fd", orig_apk, overlay_apk, idmap_str, (char*)NULL);
...
int idmap(const char *orig_apk, const char *overlay_apk, size_t priority, uid_t uid)
{
    ...
    if (pid == 0) {
        ...
        run_idmap(orig_apk, overlay_apk, idmap_fd);
