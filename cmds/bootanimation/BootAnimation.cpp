        ...
        do {
            ...
            for (size_t i = 0; i < static_cast<size_t>(numEvents); i++) {
                ...
                if (event.header.type == DisplayEventReceiver::DISPLAY_EVENT_HOTPLUG) {
                    ...
                    const status_t error = SurfaceComposerClient::getDisplayInfo(
                        mBootAnimation->mDisplayToken, &displayInfo);
                    if (error != NO_ERROR) {
                        SLOGE("Can't get active display configuration.");
                    }
                    mBootAnimation->resizeSurface(displayInfo.w, displayInfo.h);
                }
            }
        } while (numEvents > 0);
...
status_t BootAnimation::readyToRun() {
    ...
    ui::Size resolution = limitSurfaceSize(dinfo.w, dinfo.h);

    // create the native surface
    sp<SurfaceControl> control = session()->createSurface(String8("BootAnimation"),
            resolution.width, resolution.height, PIXEL_FORMAT_RGB_565);
    ...
    status = mDisplayEventReceiver->initCheck();
