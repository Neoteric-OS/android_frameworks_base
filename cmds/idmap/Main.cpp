...
namespace {
    ...
    bool is_idmap_stale_fd(const char* orig_apk_path, const char* overlay_apk_path, int idmap_fd)
    {
        ...
        uint32_t actual_orig_crc, actual_overlay_crc;
        if (get_zip_entry_crc(orig_apk_path, RESOURCES_FILENAME, &actual_orig_crc) == -1) {
            return true;
        }
        if (get_zip_entry_crc(overlay_apk_path, RESOURCES_FILENAME, &actual_overlay_crc) == -1) {
            return true;
        }

        return cached_orig_crc != actual_orig_crc || cached_overlay_crc != actual_overlay_crc;
    }

    bool is_idmap_stale_path(const char* orig_apk_path, const char* overlay_apk_path,
            const char* idmap_path)
    {
        ...
        bool is_stale = is_idmap_stale_fd(orig_apk_path, overlay_apk_path, idmap_fd);
        close(idmap_fd);
        return is_stale;
    }

    int create_idmap(const char* orig_apk_path, const char* overlay_apk_path,
            uint32_t** data, size_t* size)
    {
        uint32_t orig_crc, overlay_crc;
        if (get_zip_entry_crc(orig_apk_path, RESOURCES_FILENAME, &orig_crc) == -1) {
            return -1;
        }
        if (get_zip_entry_crc(overlay_apk_path, RESOURCES_FILENAME, &overlay_crc) == -1) {
            return -1;
        }

        // Assets are only accessible via AssetManager, so offload logic to the latter
        AssetManager am;
        bool b = am.createIdmap(orig_apk_path, overlay_apk_path, orig_crc, overlay_crc, data, size);
        return b ? 0 : -1;
    }
    ...
    int write_idmap_fd(int idmap_fd, const uint32_t* data, size_t size)
    {
        ...
        while (bytesLeft > 0) {
            ssize_t w = TEMP_FAILURE_RETRY(write(idmap_fd, data + size - bytesLeft, bytesLeft));
            if (w < 0) {
                fprintf(stderr, "Error: failed to write idmap: %s\n", strerror(errno));
                return -1;
            }
            bytesLeft -= w;
        }
...
/*
 * Expected arguments:
 *   idmap --path orig_apk_path overlay_apk_path idmap_path
 *   idmap --fd   orig_apk_path overlay_apk_path idmap_fd
 * where
 *   idmap            : binary name
 *   orig_apk_path    : path, original apk
 *   overlay_apk_path : path, overlay apk
...
int main(int argc, char* argv[])
{
    ...
    overlay_apk_path = argv[3];
    if (verify_apk_readable(overlay_apk_path) == -1) {
        fprintf(stderr, "Error: failed to read overlay apk %s\n", orig_apk_path);
        retval = 4;
        goto fail;
    }

#if 0
    printf("idmap mode=%s orig_apk_path=%s overlay_apk_path=%s argv[4]=%s\n",
            mode, orig_apk_path, overlay_apk_path, argv[4]);
#endif

    if (!strcmp(mode, "--path")) {
        if (getuid() != AID_SYSTEM || getgid() != AID_SYSTEM) {
            fprintf(stderr, "Error: running as user system required for --path (uid=%d, gid=%d)\n",
                    getuid(), getgid());
            retval = 5;
            goto fail;
        }
        ...
        if (is_idmap_stale_path(orig_apk_path, overlay_apk_path, idmap_path)) {
            if (create_idmap(orig_apk_path, overlay_apk_path, &data, &size) == -1) {
        ...
        if (is_idmap_stale_fd(orig_apk_path, overlay_apk_path, idmap_fd)) {
            if (create_idmap(orig_apk_path, overlay_apk_path, &data, &size) == -1) {
