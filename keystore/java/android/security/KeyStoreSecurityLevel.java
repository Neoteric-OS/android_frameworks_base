...
import android.security.keystore2.KeyStoreBackendBusyException;
...
public class KeyStoreSecurityLevel {
        ...
        while (true) {
            try {
                ...
                if (createOperationResponse.operationChallenge != null) {
                    challenge = new Long(createOperationResponse.operationChallenge.challenge);
                }
                ...
                switch (e.errorCode) {
                    case ResponseCode.BACKEND_BUSY: {
                        if (CompatChanges.isChangeEnabled(
                            ...
                            // backend being busy.
                            throw new KeyStoreBackendBusyException();
                        } else {
    ...
    public KeyMetadata generateKey(@NonNull KeyDescriptor descriptor, KeyDescriptor attestationKey,
            Collection<KeyParameter> args, @KeyFlags int flags, byte[] entropy)
            throws KeyStoreException {
    ...
    public KeyMetadata importKey(KeyDescriptor descriptor, KeyDescriptor attestationKey,
            Collection<KeyParameter> args, @KeyFlags int flags, byte[] keyData)
            throws KeyStoreException {
