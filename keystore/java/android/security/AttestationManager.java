...
public final class AttestationManager {
     ...
     *          // default = KeyProperties.TRUSTED_ENVIRONMENT
     *          .setSecurityLevel(KeyProperties.SECURITY_LEVEL_STRONGBOX)
     *          .build();
    ...
    private KeyParameter[] createKeyParameters(@NonNull Set<Integer> deviceIdentifiers)
        ...
        for (final int deviceIdentifier : deviceIdentifiers) {
            // TODO: replace '0' on the following lines with the correct value of
            // android.hardware.keymint.Tag when it becomes available.
            switch (deviceIdentifier) {
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_BRAND:
                    result.add(createKeyParameter(0, Build.BRAND));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_DEVICE:
                    result.add(createKeyParameter(0, Build.DEVICE));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_PRODUCT:
                    result.add(createKeyParameter(0, Build.PRODUCT));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_MANUFACTURER:
                    result.add(createKeyParameter(0, Build.MANUFACTURER));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_MODEL:
                    result.add(createKeyParameter(0, Build.MODEL));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_IMEI:
                    result.add(createKeyParameter(0, telephonyManager.getImei()));
                    break;
                case DeviceAttestationRequest.DEVICE_IDENTIFIER_MEID:
                    result.add(createKeyParameter(0, telephonyManager.getMeid()));
                    break;
                default:
