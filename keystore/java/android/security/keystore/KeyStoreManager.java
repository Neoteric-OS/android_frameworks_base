...
public class KeyStoreManager {
    ...
    /**
     * Grants access to the key in the app's namespace stored under the specified {@code alias}
     ...
     * @throws KeyStoreException if the specified key does not exist, or if an error is encountered
     *                           when attempting to grant access to the key
     * @see #getGrantedKeyFromId(long)
     */
    public long grantKeyAccess(@NonNull String alias, int uid) throws KeyStoreException {
        ...
        KeyDescriptor result = mKeyStore2.grant(keyDescriptor, uid, grantAccessVector);
        if (result == null) {
            Log.w(TAG, "Received a null KeyDescriptor from grant");
            ...
            Log.w(TAG, "Received a result outside the grant domain: " + result.domain);
            throw new KeyStoreException(KeyStoreException.ERROR_INTERNAL_SYSTEM_ERROR,
     ...
     * @throws KeyStoreException if the specified key does not exist, or if an error is
     *                           encountered when attempting to revoke access to the key
     */
    public void revokeKeyAccess(@NonNull String alias, int uid) throws KeyStoreException {
        KeyDescriptor keyDescriptor = createKeyDescriptorFromAlias(alias);
     ...
     * @return the {@link Key} that was shared with the app or null if no shared key exists with
     * the provided {@code id}
     ...
     */
    public @Nullable Key getGrantedKeyFromId(long id)
            throws UnrecoverableKeyException, KeyPermanentlyInvalidatedException {
        return AndroidKeyStoreProvider.loadAndroidKeyStoreKeyFromKeystore(mKeyStore2, null, id,
                Domain.GRANT);
    }
    ...
    public @NonNull List<X509Certificate> getGrantedCertificateChainFromId(long id)
        ...
        if (keyStoreKey instanceof AndroidKeyStorePublicKey) {
            ...
            Log.w(TAG, "keyStoreKey is not of the expected type: " + keyStoreKey);
        }
        return Collections.emptyList();
    }
    ...
    private static X509Certificate getCertificate(byte[] certificate) {
        ...
        if (certificate != null) {
            try {
                ...
                Log.w(TAG, "Caught an exception parsing the certificate: ", e);
            }
        }
        return result;
    }
        ...
        if (certificateChain != null) {
            try {
                ...
                if (certificates == null) {
                    Log.w(TAG, "Received null certificates from a non-null certificateChain");
                    return Collections.emptyList();
                }
                return certificates;
            } catch (Exception e) {
                Log.d(TAG, "Caught an exception parsing the certs: ", e);
            }
        }
        return Collections.emptyList();
    }
