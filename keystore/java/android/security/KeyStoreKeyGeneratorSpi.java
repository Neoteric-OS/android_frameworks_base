...
//            }
        ...
        if ((errorCode != KeyStore.NO_ERROR) && (errorCode != KeymasterDefs.KM_ERROR_OK)) {
            switch (errorCode) {
                case KeymasterDefs.KM_ERROR_UNSUPPORTED_KEY_SIZE:
                    throw new CryptoOperationException(
                            "Failed to generate key -- unsupported key size: " + keySizeBits);
                case KeymasterDefs.KM_ERROR_UNSUPPORTED_ALGORITHM:
                    throw new CryptoOperationException(
                            "Failed to generate key -- unsupported algorithm: "
                                    + KeyStoreKeyConstraints.Algorithm.toString(mAlgorithm));
                case KeymasterDefs.KM_ERROR_UNSUPPORTED_DIGEST:
                    throw new CryptoOperationException(
                            "Failed to generate key -- unsupported digest: "
                                    + KeyStoreKeyConstraints.Digest.toString(mDigest));
                case KeymasterDefs.KM_ERROR_INCOMPATIBLE_DIGEST:
                    throw new CryptoOperationException(
                            "Failed to generate key -- incompatible digest: "
                                    + KeyStoreKeyConstraints.Digest.toString(mDigest));
                case KeymasterDefs.KM_ERROR_UNSUPPORTED_BLOCK_MODE:
                    throw new CryptoOperationException(
                            "Failed to generate key -- unsupported block mode: "
                                    + KeyStoreKeyConstraints.BlockMode.toString(
                                            spec.getBlockMode()));
                case KeymasterDefs.KM_ERROR_INCOMPATIBLE_BLOCK_MODE:
                    throw new CryptoOperationException(
                            "Failed to generate key -- incompatible block mode: "
                                    + KeyStoreKeyConstraints.BlockMode.toString(
                                            spec.getBlockMode()));
                case KeymasterDefs.KM_ERROR_UNSUPPORTED_PADDING_MODE:
                    throw new CryptoOperationException(
                            "Failed to generate key -- unsupported padding: "
                                    + KeyStoreKeyConstraints.Padding.toString(spec.getPadding()));
                case KeymasterDefs.KM_ERROR_INCOMPATIBLE_PADDING_MODE:
                    throw new CryptoOperationException(
                            "Failed to generate key -- incompatible padding: "
                                    + KeyStoreKeyConstraints.Padding.toString(spec.getPadding()));
                case KeymasterDefs.KM_ERROR_INVALID_EXPIRATION_TIME:
                    throw new CryptoOperationException(
                            "Failed to generate key -- invalid expiration time."
                                    + " validity start: " + spec.getKeyValidityStart()
                                    + ", origination validity end: "
                                    + spec.getKeyValidityForOriginationEnd()
                                    + ", consumption validity end: "
                                    + spec.getKeyValidityForConsumptionEnd());
                case KeymasterDefs.KM_ERROR_INVALID_AUTHORIZATION_TIMEOUT:
                    throw new CryptoOperationException(
                            "Failed to generate key -- invalid user authentication validity"
                                    + " duration: "
                                    + spec.getUserAuthenticationValidityDurationSeconds());
                default:
                    throw new CryptoOperationException(
                            "Failed to generate key. Error code: " + errorCode);
            }
        }
