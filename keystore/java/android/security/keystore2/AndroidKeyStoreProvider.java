...
public class AndroidKeyStoreProvider extends Provider {
    ...
    public static void install() {
        ...
        Provider legacyWorkaroundProvider =
                new android.security.keystore.AndroidKeyStoreBCWorkaroundProvider(
                        "AndroidKeyStoreBCWorkaroundLagacy");
     ...
     * which implements both {@link PublicKey}.
     *
     * @param descriptor The oritinal key descriptor that was used to load the key.
     *
     * @param metadata The key metadata which includes the public key material
     *                 a reference to the stored private key material, the key characteristics.
     * @param iSecurityLevel A binder interface that allows using the privat key.
     * @param algorithm Must indicate EC or RSA.
     * @return AndroidKeyStorePuplicKey
    ...
    public static AndroidKeyStoreKey loadAndroidKeyStoreKeyFromKeystore(
        ...
        if (namespace == KeyStore.UID_SELF) {
    ...
    /**
     * Returns an {@code AndroidKeyStore} {@link java.security.KeyStore}} with the given designated
     * target namespaces. A keystore object will operate on keys in that namespace.
     * A namespace can be configured in Android's SEPolicy using the keystore2_key_contexts files.
     * This feature is available only to system and select vendor components and is the preferred
     * way to exert granular access control to keys and share key namespaces between system and
     * vendor components.
     *
     * <p>Note: the returned {@code KeyStore} is already initialized/loaded. Thus, there is
     * no need to invoke {@code load} on it.
     *
     * @param namespace Uid for which the keystore provider is requested.
    ...
    @SystemApi
    @NonNull
    public static java.security.KeyStore getKeyStoreForNamespace(int namespace)
            throws KeyStoreException, NoSuchProviderException {
        java.security.KeyStore result =
                java.security.KeyStore.getInstance("AndroidKeyStore", PROVIDER_NAME);
        try {
            result.load(new AndroidKeyStoreLoadStoreParameter(namespace));
        } catch (NoSuchAlgorithmException | CertificateException | IOException e) {
            throw new KeyStoreException(
                    "Failed to load AndroidKeyStore KeyStore for namespace " + namespace, e);
        }
        return result;
    }
}
