...
/**
 * @hide This should not be made public in its present form because it assumes that private and
 *     secret key bytes are available and would preclude the use of hardware crypto.
 */
public class KeyStore2 {
     ...
     *
     * <p>Keystore used to work under the assumption that the creation of cryptographic operations
     * always succeeds. However, the KeyMint backend has only a limited number of operation slots.
     * In order to keep up the appearance of "infinite" operation slots, the Keystore daemon would
     * prune least recently used operations if there is no available operation slot. As a result,
     * good operations could be terminated prematurely.
     *
     * <p>This opens AndroidKeystore up to denial-of-service and unintended livelock situations.
     ...
     *
     * <p>To break out of livelocks and to discourage DoS attempts we have changed the pruning
     * strategy such that it prefers clients that use few operation slots and only briefly. As a
     * result we can, almost, guarantee that single operations that don't linger inactive for more
     * than 5 seconds will conclude unhampered by the pruning strategy. "Almost", because there are
     * operations related to file system encryption that can prune even these operations, but those
     * are extremely rare.
     *
     * <p>As a side effect of this new pruning strategy operation creation can now fail if the
     * client has a lower pruning power than all of the existing operations.
     *
     * <p>Pruning strategy
     *
     * <p>To find a suitable candidate we compute the malus for the caller and each existing
     * operation. The malus is the inverse of the pruning power (caller) or pruning resistance
     * (existing operation). For the caller to be able to prune an operation it must find an
     * operation with a malus higher than its own.
     *
     * <p>For more detail on the pruning strategy consult the implementation at
     * https://android.googlesource.com/platform/system/security/+/refs/heads/master/keystore2/src/operation.rs
     *
     * <p>For older SDK version, KeyStore2 will poll the Keystore daemon for a free operation slot.
     * So to applications, targeting earlier SDK versions, it will still look like cipher and
     * signature object initialization always succeeds, however, it may take longer to get an
     * operation.
     *
     * <p>All SDK version benefit from fairer operation slot scheduling and a better chance to
     * successfully conclude an operation.
     */
    @ChangeId @Disabled // See b/180133780
        ...
        while (true) {
            try {
                ...
                if (firstTry) {
                    Log.w(
                            TAG,
                            "Looks like we may have lost connection to the Keystore " + "daemon.");
                    Log.w(
                            TAG,
                            "Retrying after giving Keystore "
    ...
    private synchronized IKeystoreService getService(boolean retryLookup) {
        if (mBinder == null || retryLookup) {
            mBinder =
                    IKeystoreService.Stub.asInterface(
                            ServiceManager.getService(KEYSTORE2_SERVICE_NAME));
    ...
    /**
     * Grant string prefix as used by the keystore boringssl engine. Must be kept in sync with
     * system/security/keystore-engine. Note: The prefix here includes the 0x which
     * std::stringstream used in keystore-engine needs to identify the number as hex represented.
     * Here we include it in the prefix, because Long#parseUnsignedLong does not understand it and
     * gets the radix as explicit argument.
     *
     * @hide
     */
     ...
     * to allow certain system components like racoon or vendor components like WPA supplicant to
     * use keystore keys with boring ssl.
     *
     * @param grantId the grant id as returned by {@link #grant} in the {@code nspace} filed of the
     *     resulting {@code KeyDescriptor}.
    ...
    /**
     * Convenience function to turn a keystore engine grant string as returned by {@link
     * #makeKeystoreEngineGrantString(long)} back into a grant KeyDescriptor.
     *
    ...
    /**
     * Create a grant that allows the grantee identified by {@code granteeUid} to use the key
     * specified by {@code descriptor} withint the restrictions given by {@code accessVectore}.
     *
    ...
    public KeyStoreSecurityLevel getSecurityLevel(int securityLevel) throws KeyStoreException {
        return handleRemoteExceptionWithRetry(
                (service) -> new KeyStoreSecurityLevel(service.getSecurityLevel(securityLevel)));
    }
    ...
    public void updateSubcomponents(
            @NonNull KeyDescriptor key, byte[] publicCert, byte[] publicCertChain)
            throws KeyStoreException {
        handleRemoteExceptionWithRetry(
                (service) -> {
    ...
    public int getNumberOfEntries(int domain, long namespace) throws KeyStoreException {
        return handleRemoteExceptionWithRetry(
                (service) -> service.getNumberOfEntries(domain, namespace));
    }
            ...
            switch (errorCode) {
                case ResponseCode.LOCKED:
                    return new KeyStoreException(
                            errorCode, "User authentication required", serviceErrorMessage);
                case ResponseCode.UNINITIALIZED:
                    return new KeyStoreException(
                            errorCode, "Keystore not initialized", serviceErrorMessage);
                case ResponseCode.SYSTEM_ERROR:
                    return new KeyStoreException(errorCode, "System error", serviceErrorMessage);
                case ResponseCode.PERMISSION_DENIED:
                    return new KeyStoreException(
                            errorCode, "Permission denied", serviceErrorMessage);
                case ResponseCode.KEY_NOT_FOUND:
                    return new KeyStoreException(errorCode, "Key not found", serviceErrorMessage);
                case ResponseCode.VALUE_CORRUPTED:
                    return new KeyStoreException(
                            errorCode, "Key blob corrupted", serviceErrorMessage);
                case ResponseCode.KEY_PERMANENTLY_INVALIDATED:
                    return new KeyStoreException(
                            errorCode, "Key permanently invalidated", serviceErrorMessage);
                ...
                default:
                    return new KeyStoreException(
                            errorCode, String.valueOf(errorCode), serviceErrorMessage);
            }
        } else {
