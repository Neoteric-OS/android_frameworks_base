 ...
 * PPS-MO (PerProviderSubscription Management Object) XML tree to an
...
public final class PPSMOParser {
        ...
        for (XMLNode child : node.getChildren()) {
            switch (child.getTag()) {
                ...
                case TAG_NODE:
                    ...
                    // Convert PPS XML tree to PPS tree.
     ...
     * @throws ParsingException
     */
    static private HashMap<String, Long> parseNetworkIds(PPSNode node)
            throws ParsingException {
        if (node.isLeaf())
    ...
    static private Pair<String, Long> parseNetworkIdInstance(PPSNode node)
        ...
        if (hessid == null)
            throw new ParsingException("NetworkID instance missing HESSID");
        ...
        for (PPSNode child : node.getChildren()) {
            switch(child.getName()) {
                case NODE_EAP_TYPE:
                    ...
                    break;
                case NODE_VENDOR_ID:
                    userCred.eapVendorId = parseInteger(getPpsNodeValue(child));
                    break;
                case NODE_VENDOR_TYPE:
                    userCred.eapVendorType = parseInteger(getPpsNodeValue(child));
                    break;
                case NODE_INNER_EAP_TYPE:
                    userCred.innerEapType = parseInteger(getPpsNodeValue(child));
                    break;
                case NODE_INNER_VENDOR_ID:
                    userCred.innerEapVendorId = parseInteger(getPpsNodeValue(child));
                    break;
                case NODE_INNER_VENDOR_TYPE:
                    userCred.innerEapVendorType = parseInteger(getPpsNodeValue(child));
                    break;
     ...
     * @return number of milliseconds or -1 (Credential.UNSET_TIMESTAMP)
     * @throws ParsingException
     */
