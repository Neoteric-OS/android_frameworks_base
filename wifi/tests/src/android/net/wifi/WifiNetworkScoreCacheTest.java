...
public class WifiNetworkScoreCacheTest {
    ...
    @Mock private CacheListener mockCacheListener;
    @Mock private Context mockContext; // isn't used, can be null
    @Mock private RssiCurve mockRssiCurve;

    @Captor private ArgumentCaptor<List<ScoredNetwork>> mUpdatedNetworksCaptor;
    ...
    @Test
    public void updateScoresShouldInvokeWifiListener() {
        mScoreCache = new WifiNetworkScoreCache(mockContext, mockCacheListener, mHandler);
        initializeCacheWithValidScoredNetwork();

        verify(mockCacheListener, timeout(100 /* timeoutMillis */))
                .networkCacheUpdated(mUpdatedNetworksCaptor.capture());
        List<ScoredNetwork> updatedNetworks = mUpdatedNetworksCaptor.getValue();
        assertEquals("One network should be updated", 1, updatedNetworks.size());
        assertEquals(mValidScoredNetwork, updatedNetworks.get(0));
    }
}
