...
public class PPSMOParserTest {
    ...
    private PasspointConfiguration generateConfigurationFromPPSMOTree() {
        ...
        config.homeSp.iconUrl = "http://test.com";
        config.homeSp.roamingConsortiumOIs = new long[] {0x112233L, 0x445566L};
        config.homeSp.matchAllOIs = new long[] {0x112233L};
        config.homeSp.matchAnyOIs = new long[] {0x445566L};
        config.homeSp.homeNetworkIds = new HashMap<>();
        config.homeSp.homeNetworkIds.put("TestSSID1", 0x112233445566L);
        config.homeSp.homeNetworkIds.put("TestSSID2", 0x778899aabbccL);
        config.homeSp.otherHomePartners = new String[] {"mi7.co.uk", "mi8.co.uk"};
        ...
        config.credential.userCredential.eapVendorId = 1;
        config.credential.userCredential.eapVendorType = 2;
        config.credential.userCredential.innerEapType = 23;
        config.credential.userCredential.innerEapVendorId = 3;
        config.credential.userCredential.innerEapVendorType = 4;
     ...
     */
    @Test
    public void parseFullPPSMOTree() throws Exception {
