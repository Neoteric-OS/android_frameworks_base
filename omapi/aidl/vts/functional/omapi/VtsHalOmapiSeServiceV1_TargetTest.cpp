...
class OMAPISEServiceHalTest : public TestWithParam<std::string> {
   protected:
    ...
    void testSelectableAid(
        ...
        auto mSEListener = ndk::SharedRefBase::make<::OMAPISEServiceHalTest::SEListener>();
...
TEST_P(OMAPISEServiceHalTest, TestGetReaders) {
    std::vector<std::shared_ptr<aidl::android::se::omapi::ISecureElementReader>> uiccReaders =
        {};
    std::vector<std::shared_ptr<aidl::android::se::omapi::ISecureElementReader>> eseReaders =
        {};
    std::vector<std::shared_ptr<aidl::android::se::omapi::ISecureElementReader>> sdReaders = {};
    ...
    for (const auto& [name, reader] : mVSReaders) {
        ...
        if (!(name.find(UICC_READER_PREFIX) != std::string::npos ||
              name.find(ESE_READER_PREFIX) != std::string::npos ||
              name.find(SD_READER_PREFIX) != std::string::npos)) {
            LOG(ERROR) << "Incorrect Reader name";
            FAIL();
        }

        if (name.find(UICC_READER_PREFIX, 0) != std::string::npos) {
            uiccReaders.push_back(reader);
        } else if (name.find(ESE_READER_PREFIX, 0) != std::string::npos) {
            eseReaders.push_back(reader);
        } else if (name.find(SD_READER_PREFIX, 0) != std::string::npos) {
    ...
    if (deviceSupportsFeature(FEATURE_SE_OMAPI_UICC.c_str())) {
        ...
        for (int slotNumber = 1; slotNumber <= uiccReaders.size(); slotNumber++) {
            std::shared_ptr<aidl::android::se::omapi::ISecureElementReader> uiccReader;
