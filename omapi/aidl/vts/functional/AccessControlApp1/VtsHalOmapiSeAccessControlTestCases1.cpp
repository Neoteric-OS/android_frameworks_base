...
class OMAPISEAccessControlTest1 : public TestWithParam<std::string> {
   protected:
    ...
    void testSelectableAid(std::vector<uint8_t> aid) {
        ...
        auto mSEListener = ndk::SharedRefBase::make<::OMAPISEAccessControlTest1::SEListener>();
...
TEST_P(OMAPISEAccessControlTest1, TestAuthorizedAID) {
    for (auto aid : AUTHORIZED_AID) {
        testSelectableAid(aid);
    }
}

TEST_P(OMAPISEAccessControlTest1, TestUnauthorizedAID) {
    for (auto aid : UNAUTHORIZED_AID) {
        testUnauthorisedAid(aid);
    }
}

TEST_P(OMAPISEAccessControlTest1, TestAuthorizedAPDUAID40) {
    for (auto apdu : AUTHORIZED_APDU_AID_40) {
        testTransmitAPDU(AID_40, apdu);
    }
}

TEST_P(OMAPISEAccessControlTest1, TestUnauthorisedAPDUAID40) {
    for (auto apdu : UNAUTHORIZED_APDU_AID_40) {
        testUnauthorisedAPDU(AID_40, apdu);
    }
}

TEST_P(OMAPISEAccessControlTest1, TestAuthorizedAPDUAID41) {
    for (auto apdu : AUTHORIZED_APDU_AID_41) {
        testTransmitAPDU(AID_41, apdu);
    }
}

TEST_P(OMAPISEAccessControlTest1, TestUnauthorisedAPDUAID41) {
    for (auto apdu : UNAUTHORIZED_APDU_AID_41) {
        testUnauthorisedAPDU(AID_41, apdu);
    }
}

INSTANTIATE_TEST_SUITE_P(PerInstance, OMAPISEAccessControlTest1,
                         testing::ValuesIn(::android::getAidlHalInstanceNames(
                             aidl::android::se::omapi::ISecureElementService::descriptor)),
                         android::hardware::PrintInstanceNameToString);
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(OMAPISEAccessControlTest1);
