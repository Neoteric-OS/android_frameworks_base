        ...
        int* outMaxValue, int* outFlat, int* outFuzz) const
{
    ...
    if(ioctl(device->id, EVIOCGABS(axis), &info)) {
        LOGE("Error reading absolute controller %d for device %s fd %d\n",
             axis, device->name.string(), device->id);
        return -1;
    }
...
int EventHub::getSwitchState(int32_t deviceId, int sw) const
{
    ...
    if (sw >= 0 && sw <= SW_MAX) {
        ...
        if (ioctl(device->id, EVIOCGSW(sizeof(sw_bitmask)), sw_bitmask) >= 0) {
...
int EventHub::getScancodeState(int32_t deviceId, int code) const
{
    ...
    if (code >= 0 && code <= KEY_MAX) {
        ...
        if (ioctl(device->id, EVIOCGKEY(sizeof(key_bitmask)), key_bitmask) >= 0) {
