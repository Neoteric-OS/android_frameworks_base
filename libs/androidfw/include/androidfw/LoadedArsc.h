...
struct TypeSpec {
  ...
  util::Result<uint32_t> GetFlagsForEntryIndex(uint16_t entry_index) const {
...
class LoadedPackage {
 public:
  ...
  util::OptionalResult<uint32_t> FindEntryByName(const std::u16string& type_name,
                                                 const std::u16string& entry_name) const;

  static util::OptionalResult<incfs::map_ptr<ResTable_entry>> GetEntry(
      incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index);

  static util::OptionalResult<uint32_t> GetEntryOffset(
      incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index);

  static util::OptionalResult<incfs::map_ptr<ResTable_entry>> GetEntryFromOffset(
  ...
  // the mipmap type.
  util::Result<std::monostate> CollectConfigurations(bool exclude_mipmap,
