...
class AssetManager2 {
 ...
 public:
  ...
  util::Result<std::set<ResTable_config>> GetResourceConfigurations(bool exclude_system = false,
  ...
  // Returns the resource name of the specified resource ID, or a null result if the resource was
  // not found or the name was missing/corrupt.
  //
  // Utf8 strings are preferred, and only if they are unavailable are the Utf16 variants populated.
  util::OptionalResult<ResourceName> GetResourceName(uint32_t resid) const;

  // Finds the resource ID assigned to `resource_name`, or a null result if no resource by that name
  // was found.
  //
  ...
  util::OptionalResult<uint32_t> GetResourceId(const std::string& resource_name,
  ...
  // Retrieves the best matching resource value with ID `resid`, or a null result if the resource
  // was not found.
  //
  ...
  util::OptionalResult<SelectedValue> GetResource(uint32_t resid, bool may_be_bag = false,
                                                  uint16_t density_override = 0U) const;

  // Resolves the resource referenced in `value` if the type is Res_value::TYPE_REFERENCE, or
  // returns a null result if the resolved resource could not be found.
  //
  ...
  util::OptionalResult<std::monostate> ResolveReference(SelectedValue& value) const;

  // Retrieves the best matching bag/map resource with ID `resid`, or a null result if one could not
  // be found.
  //
  // This method will resolve all parent references for this bag and merge keys with the child.
  // To iterate over the keys, use the following idiom:
  //
  //  util::OptionalResult<const ResolvedBag*> bag = asset_manager->GetBag(id);
  //  if (bag.has_value()) {
  //    for (auto iter = begin(*bag); iter != end(*bag); ++iter) {
  //      ...
  //    }
  //  }
  util::OptionalResult<const ResolvedBag*> GetBag(uint32_t resid) const;
  ...
  util::OptionalResult<const ResolvedBag*> ResolveBag(SelectedValue& value) const;
 ...
 private:
  ...
  struct FilteredConfigGroup {
      std::vector<std::pair<incfs::verified_map_ptr<ResTable_type>, ResTable_config>> type_configs;
  };
  ...
  util::OptionalResult<FindEntryResult> FindEntry(uint32_t resid, uint16_t density_override,
                                                  bool stop_at_first_match,
                                                  bool ignore_configuration) const;

  util::OptionalResult<FindEntryResult> FindEntryInternal(const PackageGroup& package_group,
  ...
  util::OptionalResult<const ResolvedBag*> GetBag(uint32_t resid,
                                                  std::vector<uint32_t>& child_resids) const;
...
class Theme {
 ...
 public:
  ...
  //
  // Returns a null result if the style failed to apply.
  util::OptionalResult<std::monostate> ApplyStyle(uint32_t resid, bool force = false);
  ...
  util::Result<std::monostate> SetTo(const Theme& other);
  ...
  util::OptionalResult<std::monostate> ResolveAttributeReference(
