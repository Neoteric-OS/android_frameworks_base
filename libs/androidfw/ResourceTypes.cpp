...
struct OverlayEntry {
    OverlayEntry(uint32_t t, uint32_t o, uint32_t leadingPadding) :
        targetResid(t), overlayResid(o), leadingPadding(leadingPadding) {}

    friend bool operator<(const OverlayEntry& lhs, const OverlayEntry& rhs) {
        return lhs.targetResid != rhs.targetResid ?
            lhs.targetResid < rhs.targetResid :
            lhs.overlayResid < rhs.overlayResid;
    }

    uint32_t targetResid;
    uint32_t overlayResid;
    uint32_t leadingPadding;
};

struct MatchingResources {
    void add(uint32_t targetResid, uint32_t overlayResid) {
        uint16_t targetTypeid = Res_GETTYPE(targetResid);
        if (map.find(targetTypeid) == map.end()) {
            map.emplace(targetTypeid, std::set<OverlayEntry>());
        }
        auto& entries = map[targetTypeid];
        uint32_t padding = 0;
        auto last = entries.crbegin();
        if (last != entries.crend()) {
            padding = targetResid - last->targetResid - 1;
        }
        entries.insert(OverlayEntry(targetResid, overlayResid, padding));
    }

    size_t NumberOfEntriesIncludingPadding(uint16_t type) {
        size_t n = map[type].size();
        auto entriesEnd = map[type].cend();
        for (auto ei = map[type].cbegin(); ei != entriesEnd; ++ei) {
            n += ei->leadingPadding;
        }
        return n;
    }
    ...
    std::map<uint16_t, std::set<OverlayEntry>> map;
};
        ...
        void** outData, size_t* outSize) const
{
    ...
    if (matchingResources.map.empty()) {
    ...
    *outSize += 2 * sizeof(uint16_t); // target package id, type count
    const auto typesEnd = matchingResources.map.cend();
    for (auto ti = matchingResources.map.cbegin(); ti != typesEnd; ++ti) {
    ...
    *typeData++ = htods(static_cast<uint16_t>(matchingResources.map.size())); // write: type count

    // write idmap data
    for (auto ti = matchingResources.map.cbegin(); ti != typesEnd; ++ti) {
        const size_t entryCount = matchingResources.NumberOfEntriesIncludingPadding(ti->first);
        auto ei = ti->second.cbegin();
        *typeData++ = htods(Res_GETTYPE(ei->targetResid) + 1); // write: target type id
        *typeData++ = htods(Res_GETTYPE(ei->overlayResid) + 1); // write: overlay type id
        *typeData++ = htods(entryCount); // write: entry count
        *typeData++ = htods(Res_GETENTRY(ei->targetResid)); // write: (target) entry offset
        uint32_t *entryData = reinterpret_cast<uint32_t*>(typeData);
        for (; ei != ti->second.cend(); ++ei) {
            for (size_t i = 0; i < ei->leadingPadding; ++i) {
                *entryData++ = htodl(0xffffffff); // write: padding
            }
            *entryData++ = htodl(Res_GETENTRY(ei->overlayResid)); // write: (overlay) entry
        }
