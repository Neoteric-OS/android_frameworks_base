...
status_t ResStringPool::setTo(incfs::map_ptr<void> data, size_t size, bool copyData)
{
    ...
    if (copyData || notDeviceEndian) {
        ...
        memcpy(mOwnedData, data.unsafe(), size);
        data = mOwnedData;
    }
    ...
    if (notDeviceEndian) {
        ResStringPool_header* h = const_cast<ResStringPool_header*>(mHeader.unsafe());
    ...
    mEntries = data.offset<uint32_t>(mHeader->header.headerSize);
    ...
    if (mHeader->stringCount > 0) {
        ...
        mStrings = data.offset<void>(mHeader->stringsStart);
        ...
        if (notDeviceEndian) {
            size_t i;
            uint32_t* e = const_cast<uint32_t*>(mEntries.unsafe());
            for (i=0; i<mHeader->stringCount; i++) {
                e[i] = dtohl(e[i]);
            }
            if (!(mHeader->flags&ResStringPool_header::UTF8_FLAG)) {
                uint16_t* s = const_cast<uint16_t*>(mStrings.convert<uint16_t>().unsafe());
    ...
    if (mHeader->styleCount > 0) {
        ...
        mStyles = data.offset<uint32_t>(mHeader->stylesStart);
        ...
        if (notDeviceEndian) {
            size_t i;
            uint32_t* e = const_cast<uint32_t*>(mEntryStyles.unsafe());
            for (i=0; i<mHeader->styleCount; i++) {
                e[i] = dtohl(e[i]);
            }
            uint32_t* s = const_cast<uint32_t*>(mStyles.unsafe());
        ...
        if (!stylesEnd || memcmp(stylesEnd.unsafe(), &endSpan, sizeof(endSpan)) != 0) {
...
{
    mError = NO_INIT;
    if (!mHeader.is_null() && mCache != NULL) {
 ...
 */
static inline util::Result<size_t> decodeLength(incfs::map_ptr<uint16_t>* str)
{
    if (UNLIKELY(!*str)) {
 ...
 * add it together with the next character.
 */
static inline util::Result<size_t> decodeLength(incfs::map_ptr<uint8_t>* str)
{
    if (UNLIKELY(!*str)) {
        return base::unexpected(kResourcesTableIncomplete);
    }
...
util::OptionalResult<StringPiece16> ResStringPool::stringAt(size_t idx) const
{
    if (mError == NO_ERROR && idx < mHeader->stringCount) {
        ...
        if (off < (mStringPoolSize-1)) {
            if (!isUTF8) {
                ...
                util::Result<size_t> u16len = decodeLength(&str);
                if (UNLIKELY(!u16len)) {
                ...
                if ((uint32_t)(str+*u16len-strings) < mStringPoolSize) {
                    ...
                    return StringPiece16(reinterpret_cast<const char16_t*>(str.unsafe()), *u16len);
                } else {
                ...
                util::Result<size_t> u16len = decodeLength(&u8str);
                if (UNLIKELY(!u16len)) {
                    return base::unexpected(u16len.error());
                }

                util::Result<size_t> u8len = decodeLength(&u8str);
                if (UNLIKELY(!u8len)) {
                ...
                if ((uint32_t)(u8str+*u8len-strings) < mStringPoolSize) {
                    ...
                    if (!decodedString.has_value()) {
                        return base::unexpected(decodedString.null_or_error());
                    }
                    ...
                    if (kDebugStringPoolNoisy) {
                      ALOGI("Caching UTF8 string: %s", u8str.unsafe());
                    }
...
util::OptionalResult<StringPiece> ResStringPool::string8At(size_t idx) const
{
    if (mError == NO_ERROR && idx < mHeader->stringCount) {
        ...
        if (off < (mStringPoolSize-1)) {
            ...
            util::Result<size_t> u8len = decodeLength(&str);
            if (UNLIKELY(!u8len)) {
                return base::unexpected(u8len.error());
            }
 ...
 **/
util::OptionalResult<StringPiece> ResStringPool::stringDecodeAt(size_t idx,
                                                                incfs::map_ptr<uint8_t> str,
                                                                size_t encLen) const {
    ...
    while ((uint32_t)(str+end-strings) < mStringPoolSize) {
        ...
        if (nullAddress.value() == 0x00) {
            ...
            return StringPiece((const char*) str.unsafe(), end);
        }
...
util::OptionalResult<String8> ResStringPool::string8ObjectAt(size_t idx) const
{
    if (util::OptionalResult<StringPiece> str = string8At(idx); str.has_value()) {
        return String8(str->data(), str->size());
    } else if (UNLIKELY(str.has_error())) {
        return base::unexpected(str.error());
    }

    if (util::OptionalResult<StringPiece16> str16 = stringAt(idx); str16.has_value()) {
        return String8(str16->data(), str16->size());
    } else if (UNLIKELY(str16.has_error())) {
        return base::unexpected(str16.error());
    }

    return String8();
}

util::OptionalResult<incfs::map_ptr<ResStringPool_span>> ResStringPool::styleAt(
    const ResStringPool_ref& ref) const
{
    return styleAt(ref.index);
}

util::OptionalResult<incfs::map_ptr<ResStringPool_span>> ResStringPool::styleAt(size_t idx) const
{
...
util::OptionalResult<size_t> ResStringPool::indexOfString(const char16_t* str, size_t strLen) const
{
    if (mError != NO_ERROR) {
        return base::unexpected(std::nullopt);
    }
    ...
    if ((mHeader->flags&ResStringPool_header::UTF8_FLAG) != 0) {
        ...
        if (mHeader->flags&ResStringPool_header::SORTED_FLAG) {
            ...
            while (l <= h) {
                mid = l + (h - l)/2;
                util::OptionalResult<StringPiece> s = string8At(mid);
                if (UNLIKELY(s.has_error())) {
            ...
            for (int i=mHeader->stringCount-1; i>=0; i--) {
                util::OptionalResult<StringPiece> s = string8At(i);
                if (UNLIKELY(s.has_error())) {
                    return base::unexpected(s.error());
                }
                if (kDebugStringPoolNoisy) {
        ...
        if (mHeader->flags&ResStringPool_header::SORTED_FLAG) {
            ...
            while (l <= h) {
                mid = l + (h - l)/2;
                util::OptionalResult<StringPiece16> s = stringAt(mid);
                 if (UNLIKELY(s.has_error())) {
                    return base::unexpected(s.error());
                }
            ...
            // block, start searching at the back.
            for (int i=mHeader->stringCount-1; i>=0; i--) {
                util::OptionalResult<StringPiece16> s = stringAt(i);
                 if (UNLIKELY(s.has_error())) {
                    return base::unexpected(s.error());
                }
...
struct ResTable::PackageGroup
{
    ...
    ssize_t findType16(const char16_t* type, size_t len) const {
        const size_t N = packages.size();
        for (size_t i = 0; i < N; i++) {
            util::OptionalResult<size_t> index = packages[i]->typeStrings.indexOfString(type, len);
            if (index.has_value()) {
...
nope:
    ...
    for (size_t ig=0; ig<NG; ig++) {
        ...
        for (size_t pi = 0; pi < packageCount; pi++) {
            ...
            do {
                util::OptionalResult<size_t> ti = group->packages[pi]->typeStrings.indexOfString(
...
uint32_t ResTable::findEntry(const PackageGroup* group, ssize_t typeIndex, const char16_t* name,
    ...
    for (size_t i = 0; i < typeCount; i++) {
        ...
        util::OptionalResult<size_t> ei = t->package->keyStrings.indexOfString(name, nameLen);
        if (!ei.has_value()) {
            continue;
        }
    ...
    for (size_t i = 0; i < packageCount; i++) {
        ...
        for (size_t j = 0; j < typeCount; j++) {
            ...
            for (size_t k = 0; k < numTypes; k++) {
                ...
                util::OptionalResult<String8> typeStr = typeStrings.string8ObjectAt(
                    type->typeSpec->id - 1);
...
util::OptionalResult<StringPiece> StringPoolRef::string8() const {
...
util::OptionalResult<StringPiece16> StringPoolRef::string16() const {
    if (LIKELY(mPool != NULL)) {
