...
struct FindEntryResult {
  ...
  std::variant<Res_value, incfs::verified_map_ptr<ResTable_map_entry>> entry;
...
util::Result<std::set<ResTable_config>> AssetManager2::GetResourceConfigurations(
...
util::OptionalResult<FindEntryResult> AssetManager2::FindEntry(uint32_t resid,
                                                               uint16_t density_override,
                                                               bool stop_at_first_match,
                                                               bool ignore_configuration) const {
  if (UNLIKELY(resource_resolution_logging_enabled_)) {
  ...
  if (!result.has_value()) {
    return base::unexpected(result.null_or_error());
  }
  ...
  if (!stop_at_first_match && !ignore_configuration && !apk_assets_[result->cookie]->IsLoader()) {
    for (const auto& id_map : package_group.overlays_) {
      ...
      if (overlay_result.has_nothing()) {
        continue;
      } else if (UNLIKELY(overlay_result.has_error())) {
...
util::OptionalResult<FindEntryResult> AssetManager2::FindEntryInternal(
  ...
  for (size_t pi = 0; pi < package_count; pi++) {
    ...
    if (use_fast_path) {
      ...
      for (const auto& type_config : filtered_group.type_configs) {
        const ResTable_config& this_config = type_config.second;
        ...
        const incfs::verified_map_ptr<ResTable_type>& type = type_config.first;
        util::OptionalResult<uint32_t> offset = LoadedPackage::GetEntryOffset(type, entry_idx);
        if (UNLIKELY(offset.has_error())) {
          return base::unexpected(offset.error());
        }

        if (offset.has_nothing()) {
          if (UNLIKELY(resource_resolution_logging_enabled_)) {
  ...
  auto best_entry = LoadedPackage::GetEntryFromOffset(best_type, best_offset);
  if (!best_entry.has_value()) {
    return base::unexpected(best_entry.null_or_error());
  }

  decltype(FindEntryResult::entry) entry;
  ...
  if (entry_size >= sizeof(ResTable_map_entry) &&
    ...
    if (auto map_entry = best_entry->convert<ResTable_map_entry>()) {
      entry = map_entry.verified();
    } else {
      return base::unexpected(kResourcesTableIncomplete);
    }
  } else {
    // The entry represents a value.
    if (auto entry_value = best_entry->offset<Res_value>(entry_size)) {
      Res_value value;
...
util::OptionalResult<AssetManager2::ResourceName> AssetManager2::GetResourceName(
...
util::OptionalResult<AssetManager2::SelectedValue> AssetManager2::GetResource(
...
util::OptionalResult<const ResolvedBag*> AssetManager2::ResolveBag(
    AssetManager2::SelectedValue& value) const {
  ...
  if (LIKELY(bag.has_value())) {
    value.flags |= (*bag)->type_spec_flags;
  }
  return bag;
}

util::OptionalResult<const ResolvedBag*> AssetManager2::GetBag(uint32_t resid) const {
  std::vector<uint32_t> found_resids;
...
util::OptionalResult<const ResolvedBag*> AssetManager2::GetBag(
  ...
  auto entry = FindEntry(resid, 0u /* density_override */, false /* stop_at_first_match */,
                         false /* ignore_configuration */);
  if (!entry.has_value()) {
    return base::unexpected(entry.null_or_error());
  }
  ...
  auto map = *entry_map;
  auto map_entry = map.offset<ResTable_map>(dtohs(map->size));
  ...
  if (UNLIKELY(!parent_bag.has_value())) {
    ...
    LOG(ERROR) << base::StringPrintf("Failed to find parent 0x%08x of bag 0x%08x.", parent_resid,
                                     resid);
    return base::unexpected(parent_bag.null_or_error());
  }
...
util::OptionalResult<uint32_t> AssetManager2::GetResourceId(const std::string& resource_name,
    const std::string& fallback_type, const std::string& fallback_package) const {
  ...
  for (const PackageGroup& package_group : package_groups_) {
    for (const ConfiguredPackage& package_impl : package_group.packages_) {
      ...
      util::OptionalResult<uint32_t> resid = package->FindEntryByName(type16, entry16);
      if (UNLIKELY(resid.has_error())) {
        return base::unexpected(resid.error());
      }

      if (resid.has_nothing() && kAttr16 == type16) {
      ...
      impl.loaded_package_->ForEachTypeSpec([&](const TypeSpec* spec, uint8_t type_index) {
        ...
        for (auto iter = spec->types; iter != iter_end; ++iter) {
          ...
          if (!filter_incompatible_configs || this_config.match(configuration_)) {
            group.type_configs.emplace_back(*iter, this_config);
          }
        }
      });
    }
...
util::OptionalResult<std::monostate> Theme::ApplyStyle(uint32_t resid, bool force) {
  ATRACE_NAME("Theme::ApplyStyle");

  auto bag = asset_manager_->GetBag(resid);
  if (!bag.has_value()) {
    return base::unexpected(bag.null_or_error());
  }
...
util::OptionalResult<std::monostate> Theme::ResolveAttributeReference(
      AssetManager2::SelectedValue& value) const {
...
util::Result<std::monostate> Theme::SetTo(const Theme& o) {
  if (this == &o) {
    return {};
  }
      ...
      for (size_t t = 0; t < package->types.size(); t++) {
        ...
        for (size_t e = 0; e < type->entry_count; e++) {
          ...
          if (attribute_dest_package_id != 0x01) {
            ...
            auto attribute_entry_result =
                o.asset_manager_->FindEntry(make_resid(p, t, e), 0 /* density_override */ ,
                                            true /* stop_at_first_match */,
                                            true /* ignore_configuration */);

            if (attribute_entry_result.has_nothing()) {
              continue;
            } else if (UNLIKELY(attribute_entry_result.has_error())) {
              return base::unexpected(attribute_entry_result.error());
            }
