...
bool AssetManager::createIdmap(const char* origApkPath, const char* overlayApkPath,
        uint32_t origCrc, uint32_t overlayCrc,
        uint32_t** outData, uint32_t* outSize)
{
    AutoMutex _l(mLock);
    const String8 paths[2] = { String8(origApkPath), String8(overlayApkPath) };
    ResTable tables[2];
