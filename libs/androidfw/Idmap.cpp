...
util::OptionalResult<StringPiece16> OverlayStringPool::stringAt(size_t idx) const {
...
util::OptionalResult<StringPiece> OverlayStringPool::string8At(size_t idx) const {
  const size_t offset = dtohl(data_header_->string_pool_index_offset);
  if (idmap_string_pool_ != nullptr && idx >= ResStringPool::size() && idx >= offset) {
