...
util::OptionalResult<AssetManager2::ResourceName> ToResourceName(
  ...
  if (auto type_str = type_string_ref.string8(); type_str.has_value()) {
    name.type = type_str->data();
    name.type_len = type_str->size();
  } else if (UNLIKELY(type_str.has_error())) {
    return base::unexpected(type_str.error());
  }

  if (name.type == nullptr) {
    if (auto type16_str = type_string_ref.string16(); type16_str.has_value()) {
      name.type16 = type16_str->data();
      name.type_len = type16_str->size();
    } else if (UNLIKELY(!type16_str.has_value())) {
      return base::unexpected(type16_str.null_or_error());
    }
  }

  if (auto entry_str = entry_string_ref.string8(); entry_str.has_value()) {
    name.entry = entry_str->data();
    name.entry_len = entry_str->size();
  } else if (UNLIKELY(entry_str.has_error())) {
    return base::unexpected(entry_str.error());
  }

  if (name.entry == nullptr) {
    if (auto entry16_str = entry_string_ref.string16(); entry16_str.has_value()) {
      name.entry16 = entry16_str->data();
      name.entry_len = entry16_str->size();
    } else if (UNLIKELY(!entry16_str.has_value())) {
      return base::unexpected(entry16_str.null_or_error());
    }
  }
