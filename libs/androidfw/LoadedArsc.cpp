...
static util::Result<bool> VerifyResTableEntry(incfs::verified_map_ptr<ResTable_type> type,
  ...
  auto entry = type.offset<ResTable_entry>(entry_offset);
  if (UNLIKELY(!entry)) {
    return base::unexpected(kResourcesTableIncomplete);
  }
  ...
  if (entry_size < sizeof(ResTable_map_entry)) {
    ...
    auto value = entry.offset<Res_value>(entry_size);
    if (UNLIKELY(!value)) {
       return base::unexpected(kResourcesTableIncomplete);
    }
...
util::OptionalResult<incfs::map_ptr<ResTable_entry>> LoadedPackage::GetEntry(
    incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index) {
  auto entry_offset = GetEntryOffset(type_chunk, entry_index);
  if (UNLIKELY(!entry_offset.has_value())) {
    return base::unexpected(entry_offset.null_or_error());
  }
  return GetEntryFromOffset(type_chunk, entry_offset.value());
}

util::OptionalResult<uint32_t> LoadedPackage::GetEntryOffset(
    incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index) {
  ...
  if (type_chunk->flags & ResTable_type::FLAG_SPARSE) {
    ...
    auto sparse_indices = type_chunk.offset<ResTable_sparseTypeEntry>(offsets_offset);
    auto sparse_indices_end = (sparse_indices + entry_count).iterator();
    bool error = false;
    auto result = std::lower_bound(sparse_indices.iterator(), sparse_indices_end, entry_index,
    ...
    const incfs::map_ptr<ResTable_sparseTypeEntry> entry = *result;
    if (error || !entry) {
      return base::unexpected(kResourcesTableIncomplete);
    }

    if (result == sparse_indices_end|| dtohs(entry->idx) != entry_index) {
      // No entry found.
  ...
  const auto entry_offset_ptr = type_chunk.offset<uint32_t>(offsets_offset) + entry_index;
...
util::OptionalResult<incfs::map_ptr<ResTable_entry>> LoadedPackage::GetEntryFromOffset(
    incfs::verified_map_ptr<ResTable_type> type_chunk,
    uint32_t offset) {
  util::Result<bool> validation = VerifyResTableEntry(type_chunk, offset);
  if (UNLIKELY(!validation)) {
    return base::unexpected(validation.error());
  }

  if (UNLIKELY(!validation.value())) {
    return base::unexpected(std::nullopt);
  }

  return type_chunk.offset<ResTable_entry>(offset + dtohl(type_chunk->entriesStart));
}

util::Result<std::monostate> LoadedPackage::CollectConfigurations(
  ...
  for (size_t i = 0; i < type_count; i++) {
    ...
    if (type_spec != nullptr) {
      if (exclude_mipmap) {
        ...
        util::OptionalResult<StringPiece16> type_name16 = type_string_pool_.stringAt(type_idx);
        if (UNLIKELY(type_name16.has_error())) {
          return base::unexpected(type_name16.error());
        }
        if (type_name16.has_value()) {
          if (kMipMap.compare(0, std::u16string::npos, type_name16->data(),
                              type_name16->size()) == 0) {
        ...
        util::OptionalResult<StringPiece> type_name = type_string_pool_.string8At(type_idx);
        if (UNLIKELY(type_name.has_error())) {
          return base::unexpected(type_name.error());
        }
...
util::OptionalResult<uint32_t> LoadedPackage::FindEntryByName(
    const std::u16string& type_name, const std::u16string& entry_name) const {
  util::OptionalResult<size_t> type_idx = type_string_pool_.indexOfString(type_name.data(),
                                                                          type_name.size());
  if (!type_idx.has_value()) {
    return base::unexpected(type_idx.null_or_error());
  }

  util::OptionalResult<size_t> key_idx = key_string_pool_.indexOfString(entry_name.data(),
                                                                        entry_name.size());
  if (!key_idx.has_value()) {
    return base::unexpected(key_idx.null_or_error());
  }
  ...
  for (auto iter = type_spec->types; iter != iter_end; ++iter) {
    ...
    for (size_t entry_idx = 0; entry_idx < entry_count; entry_idx++) {
      auto entry_offset_ptr = type.offset<uint32_t>(dtohs(type->header.headerSize)) + entry_idx;
      if (!entry_offset_ptr) {
        return base::unexpected(kResourcesTableIncomplete);
      }
      ...
      if (offset != ResTable_type::NO_ENTRY) {
        auto entry = type.offset<ResTable_entry>(dtohl(type->entriesStart) + offset);
        if (!entry) {
          return base::unexpected(kResourcesTableIncomplete);
        }
  ...
  while (iter.HasNext()) {
    ...
    switch (child_chunk.type()) {
      case RES_STRING_POOL_TYPE: {
        ...
        if (pool_address == header.offset<ResChunk_header>(dtohl(header->typeStrings))) {
        ...
        } else if (pool_address == header.offset<ResChunk_header>(dtohl(header->keyStrings))) {
      ...
      case RES_TABLE_LIBRARY_TYPE: {
        ...
        for (auto entry_iter = entry_begin; entry_iter != entry_end; ++entry_iter) {
          if (!entry_iter) {
            LOG(ERROR) << "RES_TABLE_LIBRARY_TYPE is incomplete due to incremental installation.";
            return {};
          }
          ...
          switch (overlayable_child_chunk.type()) {
            case RES_TABLE_OVERLAYABLE_POLICY_TYPE: {
              ...
              for (auto id_iter = ids_begin; id_iter != ids_end; ++id_iter) {
                if (!id_iter) {
                  LOG(ERROR) << "RES_TABLE_OVERLAYABLE_POLICY_TYPE is incomplete due to incremental"
                             << " installation.";
                  return {};
                }
