...
static status_t idmapLookup(const uint32_t* map, size_t mapSize, uint32_t key, uint32_t* outValue)
{
...
{
    // This function performs no validation of input data. On the other
    // hand, at this stage, it is not possible to distinguish a correct
    // idmap from random garbage.
    (void)mapSize;
    const uint32_t* p = map + 1;
    while (*p == 0) {
        ++p;
    }
    *outId = (map[*p + 2] >> 24) & 0x000000ff;
    return NO_ERROR;
}
...
status_t ResTable::add(const void* data, size_t size, void* cookie, bool copyData, Asset* idmap)
{
    return add(data, size, cookie, NULL, copyData, idmap);
}

status_t ResTable::add(Asset* asset, void* cookie, bool copyData, Asset* idmap)
{
    ...
    return add(data, size, cookie, asset, copyData, idmap);
}
                       ...
                       Asset* asset, bool copyData, Asset* idmap)
{
    ...
    if (idmap != NULL) {
        ...
        const void* idmap_data = idmap->getBuffer(true);
        ...
        uint32_t* outTypeSpecFlags) const
{
    ...
    while (ip > 0) {
        ...
        // If an array has been defined in one package, let it completely
        // shadow any definitions in other packages. Otherwise, items from
        // arrays of different size will be merged in what may be non-apparent
        // to the user.
        break;
    }
...
status_t ResTable::createIdmap(const ResTable& overlay, void** outData, size_t* outSize) const
{
    // see README for details on the format of map
