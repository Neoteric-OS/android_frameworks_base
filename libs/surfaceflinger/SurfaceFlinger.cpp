...
void SurfaceFlinger::postFramebuffer()
{
    if (!mInvalidRegion.isEmpty()) {
        ...
        handleScreenGrabs(mInvalidRegion);
...
void SurfaceFlinger::handleScreenGrabs(const Region& dirty)
{
    int32_t i;
    ...
    for(i=mGrabbers.size(); i>0; i--) {
        ...
        sp<BClient> client = mGrabbers[i-1];
        ...
        if(mSecureFrameBuffer) {
            ...
            Region::const_iterator it = dirty.begin();
            Region::const_iterator const end = dirty.end();
            const DisplayHardware& hw(graphicPlane(0).displayHardware());
            status_t err = 0;
