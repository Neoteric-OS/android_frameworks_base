...
private:
    sp<RenderNode> createCard(int x, int y, int width, int height) {
        return TestUtils::createNode(x, y, x + width, y + height,
            ...
            // this benchmark is only concerned with shader perf
            props.mutableOutline().setRoundRect(0, 0, width, height, 0, 1);
            props.mutableOutline().setShouldClip(false);

            // don't draw anything in card - we just want the shadow
        });
    }
};
