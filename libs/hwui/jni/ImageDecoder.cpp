...
void ImageDecoderPlugin::clearImageDecoderPlugin() {
    ...
    typedef void (*DestroyImageDecoderPluginFunc)(ImageDecoderPluginBase *);
    DestroyImageDecoderPluginFunc destroyImageDecoderPlugin =
        (DestroyImageDecoderPluginFunc) dlsym(mVendorLibHandle, "destroyImageDecoderPlugin");

    if (destroyImageDecoderPlugin != NULL) {
        (*destroyImageDecoderPlugin)(mPluginBase);
        mPluginBase = NULL;
    }
}
...
int ImageDecoderPlugin::decode(
    ...
    ALOGV("[%s:%d]", __FUNCTION__, __LINE__);
    if (mPluginBase != NULL) {
        return mPluginBase->decode(
                targetWidth, targetHeight, rect, colorType, std::move(colorSpace));
    }
    ALOGE("[%s:%d] vendor decode not init !", __FUNCTION__, __LINE__);
    return 0;
}
...
bool ImageDecoderPlugin::getHdrInfo(
        int* colorStandard, int* colorRange, int* colorTransfer, void *data) {
    ALOGV("[%s:%d]", __FUNCTION__, __LINE__);
    if (mPluginBase != NULL) {
...
void PostHdrColorInfo(JNIEnv* env, jobject jimageDecoder, jlong nativePtr) {
    ...
    if (nativePlugin->getHdrInfo(&colorStandard, &colorRange, &colorTransfer, (void*)data)) {
        ...
        jobject jcolorInfo = env->NewObject(
                gColorInfo_class,
                gColorInfo_constructorMethodID,
                colorStandard,
                colorRange,
                colorTransfer,
                byteBuffer);
...
static jobject ImageDecoder_nGetSampledSize(JNIEnv* env, jobject /*clazz*/, jlong nativePtr,
    ...
    auto* decoder = reinterpret_cast<ImageDecoder*>(nativePtr);
    if (decoder->mNativePlugin) {
        ImageDecoderPlugin* nativePlugin =
                reinterpret_cast<ImageDecoderPlugin*>(decoder->mNativePlugin);
...
int register_android_graphics_ImageDecoder(JNIEnv* env) {
    ...
    gImageDecoder_postColorInfoMethodID = GetMethodIDOrDie(
            env, gImageDecoder_class, "PostColorInfo", "(Landroid/graphics/ImageColorInfo;)V");

    gColorInfo_class = MakeGlobalRefOrDie(
            env, FindClassOrDie(env, "android/graphics/ImageColorInfo"));
