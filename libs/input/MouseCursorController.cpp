...
MouseCursorController::MouseCursorController(sp<SpriteController> mSpriteController,
...
void MouseCursorController::setPositionLocked(float x, float y,
                                              PointerController* mPointerController) {
...
void MouseCursorController::fade(PointerController::Transition transition,
                                 PointerController* mPointerController) {
    AutoMutex _l(mLock);
...
void MouseCursorController::unfade(PointerController::Transition transition,
                                   PointerController* mPointerController) {
    AutoMutex _l(mLock);
...
void MouseCursorController::setDisplayViewport(const DisplayViewport& viewport,
    ...
    if (oldViewport.displayId != viewport.displayId || oldDisplayWidth != newDisplayWidth ||
        ...
        if (getBoundsLocked(&minX, &minY, &maxX, &maxY)) {
            ...
            loadResourcesLocked(mPointerController->mPolicy,
                                mPointerController->mLocked.presentation);
        } else {
            mLocked.pointerX = 0;
            mLocked.pointerY = 0;
        }
...
void MouseCursorController::updatePointerLocked(PointerController* mPointerController)
        REQUIRES(mLock) {
    if (!mLocked.viewport.isValid()) {
        return;
    }
    ...
    if (mLocked.pointerIconChanged || mPointerController->mLocked.presentationChanged) {
        if (mPointerController->mLocked.presentation == PointerController::Presentation::POINTER) {
            if (mLocked.requestedPointerType ==
                mPointerController->mPolicy->getDefaultPointerIconId()) {
...
void MouseCursorController::loadResourcesLocked(sp<PointerControllerPolicyInterface> mPolicy,
