...
void PointerControllerContext::addCallback(int32_t displayId,
                                           std::function<bool(nsecs_t)> callback) {
    mAnimator.addCallback(displayId, callback);
}

void PointerControllerContext::removeCallback(int32_t displayId) {
...
void PointerControllerContext::PointerAnimator::addCallback(int32_t displayId,
                                                            std::function<bool(nsecs_t)> callback) {
    std::scoped_lock lock(mLock);
    mLocked.callbacks.push_back(std::make_pair(displayId, callback));
    startAnimationLocked();
}

void PointerControllerContext::PointerAnimator::removeCallback(int32_t displayId) {
    std::scoped_lock lock(mLock);
    for (auto it = mLocked.callbacks.begin(); it != mLocked.callbacks.end(); it++) {
        int32_t dispId = (*it).first;
        if (dispId == displayId) {
