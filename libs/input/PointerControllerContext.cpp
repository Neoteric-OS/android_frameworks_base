...
void PointerControllerContext::registerCallback(std::function<bool(nsecs_t)> callback) {
    mAnimator.addCallback(callback);
}
...
void PointerControllerContext::PointerAnimator::addCallback(std::function<bool(nsecs_t)> callback) {
    std::scoped_lock lock(mLock);
    mLocked.callbacks.push_back(callback);
...
void PointerControllerContext::PointerAnimator::handleCallbacksLocked(nsecs_t timestamp)
    ...
    for (auto it = mLocked.callbacks.begin(); it != mLocked.callbacks.end();) {
        bool keepCallback = (*it)(timestamp);
        if (!keepCallback) {
