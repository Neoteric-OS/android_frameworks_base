 ...
 * {@code IdentityCredential} was created. Use {@link #decryptMessageFromReader} to decrypt it.
...
public class IdentityCredential {
    ...
    public GetEntryResult getEntries(@NonNull Collection<EntryNamespace> entryNamespaces,
            ...
            PublicKey readerEphemeralPublicKey) {
        return null;
    }
    ...
    /**
     * Sets the number of dynamic authentication keys the {@code IdentityCredential} will maintain,
     * for direct access, and the number of times each should be used. Direct access authentication
     * keys must be stored in the security hardware's limited storage space, so the count should be
     * small.
     *
     * {@code IdentityCredential}s will select the least-used dynamic authentication key each time
     * {@link #getEntries getEntries} is called.
     ...
     *             if the {@code keyCount} is negative or {@code maxUsesPerKey} is less than 1.
     */
    public void setAvailableDirectAccessAuthenticationKeys(int keyCount, int maxUsesPerKey)
            throws IllegalArgumentException {}
    ...
    /**
     * Gets a collection of direct access dynamic authentication keys that need certification.
     *
     * When there aren't enough certified dynamic authentication keys stored in the security
     * hardware for direct access use, either because the key count has been increased or because
     * one or more keys have reached their usage count, this method will generate replacement keys
     * and certificates and return them for issuer certification. The issuer certificates and
     * associated static authentication data must then be provided back to the
     * {@code IdentityCredential} with {@link #storeAuthenticationData}.
     *
     * @return A collection of certificates for dynamic authentication keys that need issuer
     *         certification.
     */
    public @NonNull Collection<X509Certificate> getDirectAccessAuthKeysNeedingCertification() {
        return null;
    }
     ...
     *            p use. May be an X.509 certificate or some other format, as specified by the
     ...
     */
    public void storeAuthenticationData(X509Certificate authenticationKey, byte[] issuerCert,
     ...
     *
     * @return Collections of dynamic authentication key usage counts.
     */
    public @NonNull Collection<Integer> getAuthenticationDataUsageCount() {
        return Collections.emptyList();
    }

    /**
     * Get the number of times the direct access dynamic authentication keys have been used.
     *
     * @return Collections of dynamic authentication key usage counts.
     */
    public @NonNull Collection<Integer> getDirectAccessAuthenticationDataUsageCount() {
        return Collections.emptyList();
    }
}
