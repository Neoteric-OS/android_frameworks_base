...
public abstract class IdentityCredential {
     ...
     */
    public @NonNull abstract KeyPair createEphemeralKeyPair() throws IdentityCredentialException;
     ...
     *     provisioned,if {@link #setAvailableAuthenticationKeys(int, int)} was called with
     *     {@code keyCount} set to 0 or {@link #setAllowUsingExhaustedKeys(boolean)}
     *     hasn't been called with {@code true} and all available authentication keys have
     *     been exhausted.
     */
    public abstract @NonNull CryptoObject getCryptoObject() throws IdentityCredentialException;
     ...
     * and use {@link #getCryptoObject()} together with {@link android.biometric.BiometricPrompt})
     ...
     * detached payload is the ReaderAuthentication CBOR described below. The certificate
     * containing the Reader public key shall be included as a 'x5chain' element as described
     * in draft-ietf-cose-x509-04. It shall be included as an unprotected header element.
     * The 'x5chain' shall include at least one certificate and may contain more.</p>
     *
     * <p>The CDDL schema for ReaderAuthencation is:</p>
     * <pre>
     ...
     * {@code readerCertificateChain}.</p>
     *
     * <p>Note that only items referenced in {@code entriesToRequest} are returned - the
     * {@code requestMessage} parameter is only used to for enforcing reader authentication.</p>
     *
     * TODO: mention flexibility about how deviceEngagement is laid out just as we do in
     ...
     * the same namespaces and entries as the passed-in {@code entriesToRequestMessage}.
    ...
    public static class GetEntryResult {
         ...
         *     ]
         *
         *     DocType = tstr           ; DocType as used in Documents structure
         *
         *     DeviceEngagement = bstr  ; The bytes of the device engagement structure
         *     ReaderKey = bstr         ; The bytes of the COSE_Key structure containing the
         *                              ; reader ephemeral public key
         *
         *     DeviceNameSpaces = bstr  ; The bytes of DeviceNameSpaces
         * </pre>
         *
         * where
         *
