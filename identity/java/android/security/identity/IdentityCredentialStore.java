 ...
 * 
 * <p>
 * For secure hardware that supports it, IdentityCredentials can also be presented to readers via
 * "direct access" to allow use of IdentityCredentials when the mobile device battery's state of
 * charge is too low for normal operation, that is when Android cannot be booted but there is enough
 * power available to run the secure hardware and a low-power communication channel, such as NFC.
 * For direct access mode to function, the secure hardware must have support for the protocols used
 * for the specific type of credential, and the app must have requested direct access support for
 * each on data element that should be made directly available. See
 * {@link WritableIdentityCredential#personalize} and {@link EntryNamespace}.
 * </p>
 * 
 * <p>
 * Creation and provisioning of an IdentityCredential is a multi-step process. The complexity is
 * necessary to ensure that the issuing authority's server can:
 *
 * <ul>
 ...
 * 
 * <ol>
 * <li>(Optionally) Use {@link #getSecureHardwareType} and {@link #getSecureHardwareCertification}
 * to verify that the available secure hardware appears to satisfy the issuing authority
 * requirements.</li>
 ...
 * <li>Call {@link IdentityCredential#generateAuthenticationKeyPair} one or more times, to generate
 * a set of signing keys that are used to authenticate the credential data to readers. The return
 * value is an X.509 public key certificate with an Android Keystore extension, signed by the
 * CredentialKey. This certificate should be sent to the issuing authority server, so it can issue
 * its own certificate for the public key, to prove that the signing key is authorized by the
 * issuing authority.</li>
 * <li>For each signing key pair, call {@link IdentityCredential#storeAuthenticationData
 * IdentityCredential.storeAuthenticationData}, to store the certificate provided by the issuing
...
public class IdentityCredentialStore {
    /** @hide */
    @IntDef(prefix = {"SECURITY_HARDWARE"}, value = {SECURITY_HARDWARE_SOFTWARE_ONLY,
            SECURITY_HARDWARE_TRUSTED_EXECUTION_ENVIRONMENT, SECURITY_HARDWARE_EMBEDDED_SECURE_CPU,
            SECURITY_HARDWARE_DISCRETE_SECURE_CPU})
     ...
     * package and includes its own internal. Discrete secure CPUs are designed to resist physical
     * hardware attacks. Barring some egregious implementation error, this sort of implementation is
     * strictly more secure than one that runs in a
     * {@link #SECURITY_HARDWARE_TRUSTED_EXECUTION_ENVIRONMENT}, and has some security advantages
     * and some security disadvantages as compared to a
     * {@link #SECURITY_HARDWARE_EMBEDDED_SECURE_CPU}. This sort of implementation may be capable of
     * supporting direct access. Use
     */
    public static final int SECURITY_HARDWARE_DISCRETE_SECURE_CPU = 3;

    /** @hide */
    @IntDef(prefix = {"SECURITY_CERTIFICATION"}, value = {SECURITY_CERTIFICATION_UNCERTIFIED,
            SECURITY_CERTIFICATION_CC_EAL_4_PLUS, SECURITY_CERTIFICATION_CC_EAL_5_PLUS})
    @Retention(RetentionPolicy.SOURCE)
    ...
    /** @hide */
    @IntDef(prefix = {"CipherSuite"}, value = {
     ...
     * 
     * <p>
     * At present this is the only ciphersuite offered by <code>IdentityCredential</code>s.
     */
     ...
     */
    public static @SecurityHardwareType int getSecureHardwareType() {
        // TODO(swillden): Implement
        return SECURITY_HARDWARE_SOFTWARE_ONLY;
    }
     ...
     */
    public static @SecurityCertification int getSecureHardwareCertification() {
        // TODO(swillden): Implement
        return SECURITY_CERTIFICATION_UNCERTIFIED;
    }

    private static IdentityCredentialStore instance = new IdentityCredentialStore();
     ...
     */
    public Collection<String> getDirectAccessDocTypes() {
        // TODO(swillden): Implement
    ...
    public WritableIdentityCredential createCredential(@NonNull String credentialName,
            Collection<String> docTypes, short cipherSuite)
     ...
     * 
     * @return A collection of documents of the specified type. Will be empty if no matching
     *         credentials are found.
     */
    public @NonNull Collection<IdentityCredential> getCredentialsByDocType(
